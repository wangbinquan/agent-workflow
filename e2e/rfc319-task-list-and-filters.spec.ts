...
test('RFC-319 TASK-19：全新安装的空列表教用户开工，而过滤过窄的空列表给的是退回全量的出口', async ({
  page,
}) => {
...
test('RFC-319 TASK-16：四个视图页签的计数与行数一致、搜索去抖只打一次、状态过滤经 URL 双向往返', async ({
  page,
}) => {
...
test('RFC-319 TASK-X5：筛选弹窗的范围三档与类别三档各自独立收敛，默认档不写进 URL', async ({
  page,
}) => {
  await openTasks(page)
  await expect(
...
test('RFC-319 TASK-17：根层「加载更多」与子分支「加载更多子任务」各自独立翻页，互不清空对方', async ({
  page,
}) => {
...
test('RFC-319 TASK-21：别处新建的任务通过 WS 推出「已更新」横幅，点刷新才把新行取回来', async ({
  page,
}) => {
  await openTasks(page)
  await expect(
...
test('RFC-319 TASK-24：详情页取消运行中的任务需要两击，第一击只换文案、一个请求都不发', async ({
  page,
}) => {
...
test('RFC-319 TASK-45：工作区从 pruning 走到 pruned，详情页自己收敛并关掉节点重试', async ({
  page,
}) => {
