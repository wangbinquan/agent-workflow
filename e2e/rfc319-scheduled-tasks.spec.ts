...
test('EVENT-39 新建定时任务：四种频率逐字段落成服务端的 scheduleSpec，时区随创建者，下次触发时刻与弹窗预告的第一条指向同一时刻 @nightly', async ({
  page,
}) => {
...
test('EVENT-40 立即运行：下次触发时刻、上次结果与失败水位一格不动，只多出一个任务；已暂停的定时任务在详情页也照样跑得动 @nightly', async ({
  page,
}) => {
  const suffix = tag()
...
test('EVENT-41 启停开关：列表里关掉就清空下次触发时刻，详情页重新打开则按频率重算时刻并把失败水位清零 @nightly', async ({
  page,
}) => {
  const suffix = tag()
...
test('EVENT-44 详情页运行历史：从「还没跑过」变成两行，每行对得上服务端的任务清单，点进去就是那个任务 @nightly', async ({
  page,
}) => {
  const suffix = tag()
...
test('EVENT-45 定时任务列表：搜索只留下命中的行，「已暂停」视图只留下关掉的那条，行高保持稠密且整页不横向溢出 @nightly', async ({
  page,
}) => {
  const suffix = tag()
...
test('EVENT-47 配置已经读不出来的定时任务：列表禁掉「立即运行」并说明原因，详情页给出具体的解析理由，同列表其它行照常渲染 @nightly', async ({
  page,
}) => {
  const suffix = tag()
...
test('EVENT-X3 删除定时任务：第一次点击只是待确认（服务端仍在），第二次才真的删掉并回到列表 @nightly', async ({
  page,
}) => {
  const suffix = tag()
