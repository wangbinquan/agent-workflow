...
test('RFC-319 INTENT-50: 未选记忆 / 未选技能时，发起融合被本地拦下——不发请求、不清空已写的意图 @nightly', async ({
  page,
}) => {
...
test('RFC-319 INTENT-51 / INTENT-X9: 没有待审融合时是空态且无徽章；列表拉取失败时给得出重试，且绝不冒充空态 @nightly', async ({
  page,
}) => {
  expect(
...
test('RFC-319 INTENT-52: 融合运行中把人指向澄清收件箱，答完之后详情页自己翻到待审批（不用刷新） @nightly', async ({
  page,
}) => {
  fusionAId = (
...
test('RFC-319 INTENT-51 / INTENT-X9: 待审融合出现在记忆页「融合」页签，徽章与服务端计数一致，点行进得去详情 @nightly', async ({
  page,
}) => {
...
test('RFC-319 INTENT-55: 驳回并写下反馈 → 融合带着反馈重跑下一轮（iteration+1），上一轮提案不再摆在审批位上 @nightly', async ({
  page,
}) => {
...
test('RFC-319 INTENT-56: 取消要点两次——单击只是预备；确认之后连引擎任务一起停，服务端也不再接受审批 @nightly', async ({
  page,
}) => {
...
test('RFC-319 INTENT-57: 融合失败时把失败原因摆在页面上，且不再提供任何审批入口 @nightly', async ({
  page,
}) => {
