...
test('pipeline failure types expand into equal-width required nodes and only show compatible tools', async ({
  ...
  await firstCategory.getByLabel('Type key').fill('compile-error')
  await firstCategory.getByLabel('Name').fill('Compile error')
  await firstCategory
  ...
  await secondCategory.getByLabel('Type key').fill('test-failure')
  await secondCategory.getByLabel('Name').fill('Test failure')
  await secondCategory.getByLabel('Description', { exact: true }).fill('One or more tests fail')
  await toolDialog
    .locator('label.form-field')
    .filter({ hasText: 'Fallback category' })
    .getByRole('combobox')
    .click()
