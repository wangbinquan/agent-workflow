...
test('pipeline failure types expand into equal-width required nodes and only show compatible tools', async ({
  ...
  await expect(classifierPrimaryInput).toContainText('Failed checks')
  await expect(classifierPrimaryInput).toContainText('Categories')
  await expect(classifierPrimaryInput).toContainText('Fallback type')
  ...
  await expect(classifierPrimaryOutput).toContainText('Groups')
  await expect(classifierPrimaryOutput).not.toContainText('contextPatches')
  ...
  const firstCategory = toolDialog.locator('.tool-dispatch-definition').nth(0)
  await firstCategory.getByLabel('Type key', { exact: true }).fill('compile-error')
  await firstCategory.getByLabel('Name', { exact: true }).fill('Compile error')
  await firstCategory
    .getByLabel('Description', { exact: true })
    .fill('Compilation or type checking fails')
  await toolDialog.getByRole('button', { name: 'Add problem type', exact: true }).click()
  const secondCategory = toolDialog.locator('.tool-dispatch-definition').nth(1)
  await secondCategory.getByLabel('Type key', { exact: true }).fill('test-failure')
  await secondCategory.getByLabel('Name', { exact: true }).fill('Test failure')
  await secondCategory.getByLabel('Description', { exact: true }).fill('One or more tests fail')
  await toolDialog.getByRole('combobox', { name: 'Fallback category' }).click()
  await page.getByRole('option', { name: 'Test failure', exact: true }).click()
  await toolDialog
  ...
  await expect(repairPrimaryInput).toContainText('Failure type')
  await expect(repairPrimaryInput).toContainText('Problems')
