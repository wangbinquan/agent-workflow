...
async function runComplexCameraScenario(
  ...
  if (profile === 'desktop') {
    ...
    // Select through the same physical card-header target a user clicks. A synthetic click on
    // the React Flow wrapper does not traverse WebKit's pointer selection path reliably.
    await focusTarget.locator('.canvas-node__header').click()
