...
| RFC-304 code capability platform  | Real eight-stage review engine → production code-host adapter/client → stateful GitLab/GitHub API → published-comment read-back for both providers; real pushes synchronize REST and special refs, while issue label/comment webhooks and CI job logs cover requirement and CI-fix ingress.                                                         |
