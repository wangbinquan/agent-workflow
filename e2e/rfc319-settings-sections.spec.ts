...
test('RFC-319 CFG-01：?tab= 是设置分区的唯一事实源——深链接直达、导航改 URL、脏值回落默认分区', async ({
  page,
}) => {
...
test('RFC-319 CFG-02：非管理员直接敲 /settings URL 被弹回首页，管理员照常进入', async ({
  page,
  browser,
}) => {
...
test('RFC-319 CFG-05：服务端明确拒绝时，页面说清原因、原样保留草稿、且不进入写屏障', async ({
  page,
}) => {
...
test('RFC-319 CFG-26：越界的提交排除规则前端拦、服务端也拦；合法规则原样落库并归一化', async ({
  page,
}) => {
...
test('RFC-319 CFG-27：GC 三组旋钮整体落库——嵌套对象不被拆散，且只写本分区自己的键', async ({
  page,
}) => {
  await seedConfig({
...
test('RFC-319 CFG-23：并发/配额六项一次保存全部落地，同一个进程立刻按新值应答，且不要求重启', async ({
  page,
}) => {
...
test('RFC-319 CFG-07：他人改了同一项时给出提示、保住我的草稿，「用服务端的值」落到对方的新值', async ({
  page,
}) => {
  await seedConfig({
...
test('RFC-319 CFG-09：带着未保存改动离开设置页会被拦下；没有改动时不拦，同页换分区也不拦', async ({
  page,
}) => {
...
test('RFC-319 CFG-35：主题保存后当场换肤、刷新仍在，选「跟随系统」则真的跟随系统', async ({
  page,
}) => {
...
test('RFC-319 CFG-36：语言保存后整页当场切换，刷新仍在，且守护进程的值压过浏览器本地猜测', async ({
  page,
}) => {
