...
test('WG-07 / WG-13 配置自动保存：版本只在回执落地后前进，七个字段逐个落库并在刷新后回读', async ({
  page,
}) => {
...
test('WG-10 / WG-14 成员增删与 leader 指派：就绪性提示逐条消失、启动按钮随之解禁、移除后焦点交给邻卡', async ({
  page,
}) => {
...
test('WG-11 成员校验：重复别名 / @ / 逗号 / 空白当场拦住且一次保存都不发；human 必须活跃、agent 必须真实存在（服务端同样拦）', async ({
  page,
}) => {
  const group = await seedWorkgroup({
...
test('WG-12 模式切换的 UI 后果：free_collab 把三开关显示成强制 ON 却不改存储、dynamic_workflow 抹掉开关段与「加人类成员」', async ({
  page,
}) => {
  const group = await seedWorkgroup({
...
test('WG-23 未保存离开守卫：干净时不打扰，成员面板的半截草稿拦住侧栏跳转与刷新，「留下」保住草稿、「放弃」才放行', async ({
  page,
}) => {
  const group = await seedWorkgroup({
