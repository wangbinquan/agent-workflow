...
// network-free (fixtures/stub-opencode-workgroup-matrix.ts).
...
test('leader-worker: clarify → same-member fan-out → protocol retry → gate reject/revise/approve', async () => {
  ...
  const secondGate = await waitForRoom(
    ...
    (value) =>
      ...
      value.assignments.some(
        (assignment) => assignment.title === 'implementation-v2' && assignment.status === 'done',
      ),
  )
  ...
  expect(
    beforeApproval.runs.filter((run) => run.status === 'failed' || run.status === 'interrupted'),
  ).toEqual([])

  expect((await readWorktree(task.id, 'showcase/app.txt')).content).toBe(
    'implementation v2 after gate rejection\n',
  )
  expect((await readWorktree(task.id, 'showcase/tests.txt')).content).toBe('independent tests v1\n')
...
test('free-collab: parallel planning, normalized dedup, dual message/task tracks, batch merge and gate', async () => {
  ...
  expect(gated.messages.some((message) => message.bodyMd.includes('FC_PRIVATE_SIGNAL'))).toBe(true)
  expect(gated.messages.some((message) => message.bodyMd.includes('FC_PUBLIC_SIGNAL'))).toBe(true)
  ...
  expect(alphaRuns.some((run) => run.promptText?.includes('## Initial planning turn'))).toBe(true)
  expect(betaRuns.some((run) => run.promptText?.includes('## Initial planning turn'))).toBe(true)
  expect(
  ...
  expect(batchRuns.every((run) => run.promptText?.includes('FC_PUBLIC_SIGNAL') === true)).toBe(true)
...
test('dynamic-workflow: reject generated graph, regenerate a two-agent DAG, then preserve literal downstream input', async () => {
  ...
  expect(orchestratorRuns[0]?.promptText).toContain('DW_MATRIX_GOAL literal {{dw_goal_literal}}')
  ...
  expect(reviewer?.promptText?.match(/draft-v2 literal \{\{must_stay_literal\}\}/g)).toHaveLength(1)
  expect(
