...
test('RFC-319 MEM-X7: 空库时「已批准」与「已归档」各说各的话，不是同一句套话，也不是错误态假扮的空 @nightly', async ({
  page,
}) => {
  expect(
...
test('RFC-319 MEM-23: /memory 的分区由 URL 决定——默认落在 All、深链直达、未知 tab 静默回落且保住 hash @nightly', async ({
  page,
}) => {
...
test('RFC-319 MEM-02: 四个非 global 档都能从下拉选中真实目标，服务端记下的 scope 与所选一致 @nightly', async ({
  page,
}) => {
  await openApp(page, '/memory')
...
test('RFC-319 MEM-03: 空/超长标题、空/超长正文、非 global 缺目标、超长标签——都在本地拦下且不发请求 @nightly', async ({
  page,
}) => {
...
test('RFC-319 MEM-20: 按 scope 浏览的五档计数与服务端逐档一致，空档给空态，行内编辑改得动 @nightly', async ({
  page,
}) => {
...
test('RFC-319 MEM-06: 编辑弹窗的详情拉不到时只给错误与重试，绝不摆出一个能写空值的保存按钮 @nightly', async ({
  page,
}) => {
...
test('RFC-319 MEM-07: 编辑弹窗里什么都没改就保存 —— 不发 PATCH、version 不动，改一个字才发 @nightly', async ({
  page,
}) => {
...
test('RFC-319 MEM-11: 冲突候选能并排对比，「批准并取代」之后旧条目退出可用面并接上 supersede 链 @nightly', async ({
  page,
}) => {
...
test('RFC-319 MEM-12: 批准失败时页面把失败摆出来，候选留在原地，服务端状态一动不动 @nightly', async ({
  page,
}) => {
...
test('RFC-319 MEM-X6: 五个记忆面板拉取失败时都给错误 + 重试，绝不冒充空态；重试成功后恢复原样 @nightly', async ({
  page,
}) => {
...
test('RFC-319 MEM-X6: 已经加载出来的行不会被一次失败的后台刷新清空——错误横幅与列表同时在场 @nightly', async ({
  page,
}) => {
