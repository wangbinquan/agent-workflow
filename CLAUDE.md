...
- **Agent management** — virtual agent names. **DB is source of truth** (frontmatter fields + body markdown stored in DB columns). RFC-224/227 production execution builds a complete controlled config, passes it through private `OPENCODE_CONFIG_CONTENT`, then verifies the same byte-frozen server's final `/config` + ordered `/agent` inventory; inline merge priority and the reported OpenCode version are not trust boundaries.
...
- **Concurrent config/resource collision** — RFC-224 resolves this by exclusion plus same-instance attestation, not by assuming inline precedence:
  ...
  - Only selected MCP config is admitted. Local MCP and the optional shell enter the selected provider's no-network child boundary; enabled plugins and `dependsOn` are unsupported in verified v1.
