...
- **Agent management** — virtual agent names. **DB is source of truth** (frontmatter fields + body markdown stored in DB columns). RFC-224/227 production execution builds a complete controlled config and passes it through private `OPENCODE_CONFIG_CONTENT`. **RFC-251 removed the post-boot attestation** (the second `/agent` read and the byte comparison against the sealed config) — the config is sent and executed, and residual merge surfaces are no longer proven absent. The reported OpenCode version is not a trust boundary; process containment (RFC-227) remains the enforced boundary.
...
- **MCP management** — only the node's selected MCP closure enters the controlled config. Remote MCP config is closed-schema; local MCP runs through the selected containment provider's sealed no-network child launcher with sanitized env. Repo/global MCP is not inherited by the verified path. **RFC-251**: the agent's selected plugin closure and its `dependsOn` closure ARE supported — plugins are encoded into `config.plugin` by the shared `buildPluginSpecArray` (and `OPENCODE_PURE` is derived from that selection, since it would otherwise empty `plugin_origins` silently), and closure members are registered as `mode:'subagent'` entries with `task` allowed only when the closure is non-empty.
- **Runtime management** — OpenCode has no minimum, maximum or exact-version gate. The administrator-selected executable is copied into a private seal and re-hashed before execution; SHA-256 freezes bytes for that run and is not a vendor signature. Compatibility is qualified from the direct API behavior codec. Linux bwrap and macOS Seatbelt are built-in containment providers; the core consumes open provider capabilities so a future Windows Job Object/AppContainer implementation does not require an OpenCode version or OS gate.
...
- **Process isolation** — RFC-205/227/233 select containment by provider capability, not OS name. One daemon-scoped `ContainmentCoordinator` performs fresh exact qualification per spawn and emits the sole immutable admission plan consumed by runner/OpenCode/FFF/status; lightweight discovery is diagnostic only. Linux uses one canonical root-owned bwrap for outer/child plus the FFF proof. macOS avoids nested `sandbox-exec`: Bash/local-MCP plans use one child Seatbelt profile, while plans without model-controlled children retain runner outer. `enforce` blocks before runtime side effects, `warn` atomically degrades outer+child to none with an alert, and `off` does no qualification. Settings mode changes hot-apply to subsequent admissions; configured/effective mismatch is explicit.
...
- **Concurrent config/resource collision** — RFC-224 resolved this by exclusion plus same-instance attestation; **RFC-251 kept the exclusion and dropped the attestation**:
  - `OPENCODE_CONFIG_CONTENT` carries one complete controlled config. The launcher still reads `/config/providers`, `/agent` and `/skill` once each — enough to catch a genuinely missing provider or pinned skill — but no longer re-reads `/config` or compares identity, so it does not prove the effective config was untampered.
  ...
  - Only selected MCP config is admitted. Local MCP and the optional shell enter the selected provider's no-network child boundary. Selected plugins and `dependsOn` are supported again per RFC-251.
  - Verified inventory is derived from the single `/agent` read plus frozen skill/MCP manifest metadata (RFC-251; there is no second response to compare against). It does not re-enable the legacy inventory plugin or query `/mcp/status`.
  - Production uses one administrator-selected, byte-frozen binary and one loopback server. Linux additionally proves the FFF boundary before server start. It creates/resumes the exact session through the behavior-qualified direct API after attestation; it does not execute a separate `opencode run`.
- **Session identity and resume ownership** — each business chain has a private persistent OpenCode store and owner row that freezes session/project/task/node, runtime-binary/config/session digests, protocol codec and store key. Reported version is nullable telemetry and is never compared during resume. launcher ↔ runner nonce/lease acknowledgement must succeed before SSE/POST. System agents use fresh ephemeral stores and cannot resume.
...
The full current contract and repository code anchors are in `docs/OPENCODE_CONFIG.md`,
`design/RFC-224-opencode-execution-identity/`, and the superseding
`design/RFC-227-opencode-version-neutral-cross-platform-admission/`. Re-run the behavior and
containment qualification suites before changing any of these mechanisms.
