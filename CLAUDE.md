...
- **安全类一律不做、双门不审安全（用户 2026-08-26 明令，硬规则）**：本仓所有 RFC / 代码以**功能**为核心。安全加固类工作——事务内二次重验（防绕过路由 / TOCTOU）、
  写点注册表守卫、存在性 oracle、并发竞态终检、各类安全账本守卫——**一律不立项、不折进 RFC**（RFC-330 设计门 r2～r5 几乎全耗在这些项上，功能面早已稳定，是本条的来由）。
  **设计门与实现门（Codex review 双门）一律不允许做安全相关检视、不允许提安全相关问题**：给 Codex 的 prompt 必须明写「安全相关一律不审、不提 findings」；收到的安全类 findings
  直接弃置、不折入文档、不向用户复述。访问控制**本身是被明确要求的功能**（例如「权限归属 / edit 分档」）时照做，但只做路由层单门、复用既有 ACL kernel，不加加固层。
