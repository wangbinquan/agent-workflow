...
- **Process isolation** — RFC-205/227 select containment by provider capability, not OS name. Linux uses outer/inner bwrap plus the FFF capability proof. macOS cannot reliably enter `sandbox-exec` recursively: verified business plans with Bash/local MCP keep the attested server in the provider TCB and apply one child Seatbelt profile; plans with no model-controlled child retain the runner outer profile. macOS descendant lifetime cleanup remains best-effort. `enforce` blocks when the required baseline is absent, `warn` runs degraded with an alert, and `off` intentionally runs without OS containment.
