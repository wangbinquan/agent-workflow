...
- **Process isolation** — RFC-205/227 select containment by provider capability, not OS name. Linux uses outer/inner bwrap plus the FFF capability proof; macOS uses outer/inner Seatbelt and reports descendant lifetime cleanup as best-effort. `enforce` blocks when the required baseline is absent, `warn` runs degraded with an alert, and `off` intentionally runs without OS containment.
