...
- 新纯函数 `deriveWorkgroupRunHistory(members, leaderMemberId, hostRuns, assignments, messages)`——三类归属规则与 `deriveMemberCurrentRuns` 现有逻辑同源：leader 轮（`__wg_leader__`、排除 `wg-gate`）→ leaderMemberId；assignment 轮 shardKey=assignment.id 反查 assignee；message-turn `msg:${memberId}:*` 前缀解析**仍封装在后端**。
...
`workgroupRunner.ts` 三处 mint 点（leader / assignment / message-turn）在 `mintNodeRun` 返回**之后**（事务外）补发：
...
- message-turn 的 `adoptedRunId` 分支（行已存在）**不发帧**——只在真 mint 后发。
...
- `deriveMemberPresence(memberId, assignments, currentRun) → 'working' | 'awaiting' | 'queued' | 'idle'`
  - `working`：`currentRun?.status==='running'` 或该成员有 running assignment；
  - `awaiting`：`currentRun?.status==='awaiting_human'` 或有 awaiting_human assignment（clarify park / 待人工交付；RFC-181 A2 遣散后自然回落）；
  - `queued`：`currentRun?.status==='pending'` 或有 dispatched assignment；
  - `idle`：其余。优先级 working＞awaiting＞queued＞idle。
...
- `buildRoomTimeline(messages, standaloneTurns)` 扩展：新 entry 形态 `{type:'turn', entry}`，按 ULID（`nodeRunId` vs message id 字典序）插入时间线；round 分隔逻辑不变。
...
- **TurnCard**（`WorkgroupRoom.tsx` 内、与 `DispatchCard` 同族；样式复用 `.workgroup-room__card` 家族 + `--turn` 修饰，禁自造 chrome）：
  - 内容：`@displayName` + 轮类标签（被 @ 轮 / 领导轮）+ 状态 chip + 耗时 + `note==='clarify-suppressed'` 时「反问已压制」辅 chip（D11）+「查看会话」按钮（`.btn .btn--xs`，`onClick={() => setDrawerRunId(entry.nodeRunId)}`）。
...
- **drawer 历轮标签 wg 化（P1-3，可裁）**：`shared/schemas/task.ts` `NodeRun` 补 `rerunCause`（DB 列已存在，`routes/tasks.ts` node-runs select 加列，零 migration）；`lib/node-history.ts` 对 `__wg_leader__ / __wg_member__` 节点按 rerunCause 显示「领导轮 / 派发轮 / 被 @ 轮」，不再露 shardKey 原串。
- **滚动锚定（P1-1）**：log 容器 scroll 监听维护 `atBottom`（阈值 ~48px，初始 true）；贴底效果改为"timeline 长度变化 **且** atBottom 时贴底"（现状 `:183-188` 无条件贴底废除）；`!atBottom` 时渲染「回到最新」浮标（`.btn .btn--sm`，绝对定位 log 右下），点击贴底并复位。
...
| `routes/tasks.ts` | node-runs select + rerunCause（P1-3） |
| `services/workgroupRunner.ts` | 3 mint 点后 pending 广播（唯一后端行为面改动） |
...
1. **后端 `deriveWorkgroupRunHistory` table**：三类 kind 归属（leader / assignment / message-turn）、升序、wg-gate 排除、triggerMessageId / assignmentId 回填、note 派生（`clarify-suppressed` 前缀——与 RFC-181 契约测试互链注释）、`deriveMemberCurrentRuns` 投影等价（对拍 rfc179 选取规则，原测试继续绿）。
2. **后端 pending 广播**：三 mint 点各断言一帧 `{type:'node.status', status:'pending'}`；message-turn adopted 分支不发帧。
3. **前端 lib table**：`deriveMemberPresence` 四态 × 优先级全分支（含 awaiting_human assignment / currentRun 双来源）；`turnCardsForMessage` / `standaloneTurnEntries`（降级路径）；timeline interleave（ULID 时序、round 分隔不动）；`formatRoomTimestamp` 同日 / 跨日。
4. **组件集成（workgroup-room.test.tsx）**：message-turn 卡挂触发消息下、pending→running→done 文案流转、终态定格后仍渲染（"跑完不消失"回归锁）；leader 卡独立行；assignment 轮不出 TurnCard（防双卡）；「查看会话」/ pill / 花名册 chip / 执行记录行点击各自打开 drawer（`setDrawerRunId` 断言）；执行记录倒序 + 空态；drawer runs 成员作用域（跨成员串台回归断言）；「反问已压制」辅 chip（note 驱动）；滚动锚定（贴底跟随 / 上翻不动 + 浮标出现与点击）。
...
8. **i18n 对称** + **prompt 隔离源级断言** + P1-3 时 `NodeRun.rerunCause` wire 解析测试。
