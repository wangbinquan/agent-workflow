...
- **G7 零溢出（一处已披露的例外）**：非工作组 clarify（普通节点 / 跨节点 / 问题板）行为零变化；`requireTaskMember` 答题权边界不动；prompt 隔离不破（RFC-099——归属信息绝不进 agent prompt，人类成员在 prompt 里恒以 `displayName` 别名出现）。
  **唯一例外**：G9 给「停止反问」加 shard 维度后，**普通任务里 fan-out 分片节点**的 stop 从「停该节点全部分片」收窄为「只停这一片」（`design.md` §3.7.4）。这本质是 bug 修复（停 shard 3 的问题不该顺带静音 shard 7），但确属行为变化——显式披露并配测试，不藏在实现里。

### 2.1 加人之后不能「反问永不停」（用户 2026-07-20 第二轮追加）

删掉 `autonomous` 会**同时删掉工作组里唯一能喊停反问的手段**。盘完现存天花板后确认这不是理论风险（详见 `design.md` §3.6 的取证表）：

- **lw 模式下 worker 的反问 ping-pong 零预算**：asking run 与 clarify-answer 续跑都是 `__wg_member__` 行，而 `countRoundsUsed` 在 `leader_worker` 只数 `__wg_leader__`（`workgroupRunner.ts:673`）；更糟的是 parked worker 会让 leader 不被唤醒（`workgroupWake.ts:150-152`），**整个任务的轮数就此冻结**。真·无限。
- **全仓不存在任何反问代际上限**（`MAX_CLARIFY*` / `CLARIFY_*_LIMIT` 零命中；`createClarifySession` 与代际计数器都是纯计数）。
- **`maxDurationMs` 不是天花板**：park 的任务对 `enforceLimits` 不可见（只扫 `running`，`limits.ts:34`），且 `startedAt` 从不重置 ⇒ 不但拦不住久拖，还会在**人答完后的第一个 tick 把任务秒杀**（`tests/scheduler-audit-gap1-limits-resume-startedat.test.ts:183-217` 锁住了这个行为）。
- **「停止反问」在工作组里是 no-op**：directive 行写得进去，但 host 路径写死 `clarifyChannel.directive='delegated'`（`scheduler.ts:862`）且从不读那张表。

因此本 RFC 追加三个目标：

- **G8 反问预算（D14）**：给每个**提问方**一个反问次数上限——leader 一个（任务级累计）、每张派单一个（派单级累计，新派单自动重置）。到限即转入 RFC-181 的硬压制通道（驳回 + 重提示「自行决断」+ 耗尽丢弃），**绝不 park**。数值进组定义、可配置（D16），默认 3。
- **G9 打通「停止反问」（D15）**：把工作组的 clarify 通道接进普通任务早已具备的 `directive:'stop'` 机制，让人在回答界面点「提交并停止反问」后后续反问被硬驳回。**复用既有机制与 UI，不新增任何开关。**
- **G10 修复 park 期间的计时错账（D17）**：任务在 `awaiting_human` 期间不应把等待时长计入 `maxDurationMs`。反问等得越久、恢复后被秒杀的概率越高——与本 RFC 直接相关，用户拍板顺手修。
...
- **不修 RFC-181 缺口 B（D14）**：member 的反问续跑仍不计入 `leader_worker` 的 `max_rounds`（`countRoundsUsed` 的「rounds = leader 轮」语义不动）。反问的天花板由 G8 的专门预算负责，不去搅动轮数账本。
- **不给普通（非工作组）任务加反问预算**：G8 只作用于工作组 host run。普通节点的反问同样无代际上限（研究已确认），但那是独立问题、独立范围。
- **不做 build-version 偏移强制刷新**：见 `design.md` §5 的升级窗口讨论，属独立 RFC。
...
**第二轮（2026-07-20，针对「加了人工之后反问不能永远不停」）：**

- **D14 硬上限 = 专门的反问预算**，不走「把 member 反问计入 `max_rounds`」那条路（缺口 B 仍不修）。理由：`maxRounds` 默认 1000，用它当反问天花板等于没有天花板；专门预算数字小、语义清晰，且只掐「原地打转」——每张派单独立计数，干完一张换一张自动重置，不惩罚「活干得多顺带多问几次」。
- **D15 人的止血阀 = 打通既有「停止反问」**，不新增开关。普通任务已有全套机制（回答界面按钮 + 画布节点开关 + 后端硬驳回），工作组只是把这条线写死成 `delegated` 而接不上；接通它即可，粒度天然落在「谁在问」。
- **D16 反问上限可配置**：进组定义（像 `maxRounds` 那样给一个数字），默认 3。
- **D17 顺手修 park 计时错账**：`awaiting_human` 期间的等待不计入 `maxDurationMs`。
...
8. **反问预算（G8）**：同一提问方问满预算后，再发 `<workflow-clarify>` 被驳回并被要求自行决断，任务**不** park；leader 与派单各自独立计数；新派单从 0 重新计；预算值来自组定义、可在组表单调整。
9. **停止反问（G9）**：工作组的反问轮里点「提交并停止反问」后，该提问方后续的 `<workflow-clarify>` 被硬驳回、不建 session、不 park；普通任务的同一按钮行为零变化。
10. **park 计时（G10）**：任务在 `awaiting_human` 停留任意时长后被恢复，不会因为等待时长而触发 `task-time-limit-exceeded`；真正的运行时长仍被正确累计并仍能触发上限。
