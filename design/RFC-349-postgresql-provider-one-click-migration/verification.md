...
**688.5ms 的归因仍在做**：本机同一份代码在 10 核上是 221.4ms、托管 2 核上是 688.5ms，
约 3 倍——与 CPU 规模一致，但也可能是一次大 GC。为此给 daemon 的事件循环采样器加了两件事：
门槛可用 `AGENT_WORKFLOW_EVENT_LOOP_STALL_LOG_MS` 临时调低（默认仍是 1s，取值夹在
[采样周期×2, 60s]），以及每条 stall 记录**堆用量与增量**——阻塞式计算与一次大 GC 在延迟上
长得一样，只有堆的走向能把它们分开。归因结论落定前不动 `EVENT_LOOP_GAP_MS = 500` 这条门槛。
