...
2. **9 条回归 lane 整批删除**（第一版只是把它们从 crash job 后面改挂到 compiled smoke 上，
   实测整条仍要 41.9 分钟——瓶颈换成了最慢的 `e2e-3` 27 分钟，于是 2026-09-04 用户裁决删除）。
   它们**不产出任何 provider 证据**：跑的是 SQLite 上的普通套件，守卫一直断言那个 job 里不含
   任何 `RFC349_*` 连接串。逐条对完确认全是别处的重复，且都是**更弱**的一份——

   | 删掉的 lane | 归属方                 | 归属方为何更强                                                                                                          |
   | ----------- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------- |
   | backend×4   | `ci.yml`               | 同命令同 env，但跑 ubuntu + macOS 各 4 片；种子每轮按 run number 变，删掉的那条写死 `--seed=349001`，永远只探同一个顺序 |
   | frontend    | `ci.yml`               | ubuntu + macOS + Windows 各 3 片；删掉的那条只有 ubuntu                                                                 |
   | e2e×4       | `e2e-full-nightly.yml` | 逐字相同的 `--shard=N/4 --workers=1` 全量档，同样不加 `--grep-invert`                                                   |

   它们还被 `needs:` 挂在取证 job 后面（取证红了整批 skip），历史上几乎没真跑过：backend 那条
   第一次执行是 `b3883154e`，一跑就暴露自身拓扑缺陷。删除的代价清单落在
   `RFC349_T10_REGRESSION_COVERAGE_OWNERS`，守卫逐行回查归属方**仍在跑那条命令、且仍是更强的
   那一份**——归属方哪天自己降级（Main CI 钉死种子 / 掉一个 OS / 全量 e2e 腿重新加上
   `--grep-invert`），这条当场红，而不是安静地少掉一块覆盖。
