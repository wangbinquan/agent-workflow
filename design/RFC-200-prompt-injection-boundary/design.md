...
- `0c4ebcce`：T4 `promptFencing` 围栏、闭合 token 剥离、行首锚点中和与 legacy 空 nonce 退化。
- `e149bd40`：T1 `node_runs.envelope_nonce` 迁移、生成/持久化与 run 复用基座。
- `cc25c302`：T2/T3/T5/T6/T7 完整 emit/parse/runner/followup 接线、所有已知 prompt 输入面、workgroup 协议、PromptPreview、golden/源码锁/集成/e2e fixture 收口。

### 10.2 与设计稿的实现差异

- nonce 的 mint/persist 由既有 `nodeRunMint` 边界负责，runner 读取同一持久化值后贯穿 render、emit、followup 与 parse；没有在 runner 内再生成第二份 nonce。该实现仍满足「每个 run 单一事实源」与 resume/followup 复用不变量。
- missed-issues pass 发现初始 §4.2 表未覆盖两条不经通用 `renderUserPrompt` 的内部 agent 路径：`commitPush.ts` 的 commit-message 生成和 `memoryDistiller.ts` 的记忆蒸馏。两者已一并改为 nonced protocol + fenced raw diff/stat/push error/source context，解析只接受同 run nonce。
- fan-in/fusion 的结构化 map 保持为数据容器，由共享 renderer 在最终 prompt 边界统一围栏，未额外套一层重复围栏；workgroup 与内置 orchestrator/merge/fusion 的协议则在各自实际 emit 点显式传入 nonce。
- 预览路径固定使用 `PREVIEW`，因此 UI 展示与运行时协议同构且快照确定。

### 10.3 验证结果

- RFC-200 自有门禁：backend 聚焦回归 `117 pass / 0 fail`、shared `24 pass / 0 fail`、frontend PromptPreview `6 pass / 0 fail`；workspace typecheck、backend lint、变更文件 format 与 `git diff --check` 全绿。
- 真实 runner fixture 会先输出正确 nonced value、再输出 forged bare envelope；commit-push Chromium e2e `1 pass / 0 fail`，持久化结果只采用正确 nonce 的信封。
- 在旁路补齐前的完整 backend 基线为 `5841 pass / 22 skip / 0 fail`；完整 Playwright 基线为 `118 pass / 22 skip`。补齐后的最新共享树全量受到并行 RFC-199 未收口改动影响：backend 剩 1 个稳定 route source-lock 红项、shared 剩 1 个 wrapper-git 契约红项、Playwright 剩 4 个 RFC-199 失败；均不在 RFC-200 文件范围，未越界修复。
- 尚未 push，故未执行远端 exact-SHA CI；plan 的最终全仓/push 门保持未勾选。
