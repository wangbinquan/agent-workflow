...
+ 任务详情 9 个 `?tab=` + 评审详情 + 工作组房间页 + 工作组聊天室 + 3 个弹窗
+ **≤720px 窄视口重扫**。

**仍未覆盖（如实登记，勿当作「已验证干净」）**：`clarify.detail`、`skills.detail`、
`mcps.detail`、`plugins.detail`、`scheduled.$id`、`fusions.detail` 等详情页尚未进
`ROUTES`（各需自己的 fixture）。测试名叫 *not clipped anywhere*，实际语义是
「**在已审计面上**没有裁剪」——这条边界必须写明，否则就是下一个「看起来全绿」的
陷阱。补面的做法与本批相同：加 fixture → 进 ROUTES → 报出的违规按判据清零。
...
**7 条采纳并修复，1 条视觉部分经实测反驳。** 其中 4 条是守卫自身的假阴——正是
它最该被审的地方：

| # | 发现 | 处置 |
| --- | --- | --- |
| P0 | 标记后、测量前若 React 重建节点，`measure()` 静默返回空，而 coverage 仍按 `tag()` 的数量算「已测」 | **修**：`measure()` 回报 `alive`，覆盖数改由**实际测到**的节点累计；并让 workgroup fixture 先跑到终态（聊天室是 WS 驱动的活页面，正是最容易churn 的那个面） |
| P1 | 画在**非可聚焦祖先**上的 `:focus-within` 环永远测不到——而本 RFC 改的 `.user-picker .chips-input__row:focus-within` 恰是 `<div>` | **修**：强制聚焦后向上遍历祖先，凡自身在画环的一并测量；补 self-check 用纯 `<div>` 包装器锁死 |
| P1 | `room < 0` 直接跳过，把**裁得最狠**的那侧藏起来；且跳过用硬 `< 0`、判违规用 `< ink-0.5`，阈值不对称 | **修**：跳过阈值改 `< -0.5`，与违规阈值对齐（亚像素是布局舍入，不是刻意外挑） |
| P1 | **视觉基线没刷新**，本 RFC 加的 padding 会移动像素 | **修**（最重的一条）：实测证实改动前 25/25 全过、改动后 **9 个场景红**（含 `/auth`——它的 token 输入框自动聚焦）。先按本 RFC 判据把 `.react-flow__controls-button` 改成环内移消掉无谓 churn，其余据实刷新 darwin 基线；ubuntu 基线按 README Option A 走 `workflow_dispatch` 产物回填（两轮，因同一 test 两张 snapshot 需迭代），**最终 ubuntu 零差异通过** |
| P1 | coverage 门用**全文档**控件数，而 shell 导航每条路由都渲染 ⇒ 几乎恒真 | **修**：只统计主区域（`.content` 内、`.sidebar` 外）的控件 |
| P2 | 扫描面 ≪「anywhere」；`.clarify-question` 的 3px 外扩光晕**两个守卫都覆盖不到** | **部分修**：`.clarify-question` 光晕改 inset 并纳入静态表；补 `/workgroups/{name}`；其余详情页仍未覆盖，如实登记为缺口 |
| P2 | 静态守卫可被长手写法 / 不同 selector key 绕开 | **修**：识别 `outline-style`/`outline-width` 长手（变异验证会红）；后代选择器（`.form-field input:focus`）这类无类名可匹配的写法是该表的固有局限，已写进注释并交由几何层兜底 |
| P2 | `.tabs__tab` 内移环与激活下划线视觉合并 | **反驳**：放大截图对比「激活+聚焦」与「非激活+聚焦」，焦点（方框）与选中（下划线+蓝字）清晰可辨，未合并。其「底边永久不可判」一半属实，但补偿控制在于静态守卫会拦住任何给 `.tabs__tab` 恢复外扩环的改动 |

另有一处与审查者结论不一致：UA `outline-style: auto` 的实际外溢，审查者测得 ≤1px、
我放大截图测得需 2px（padding 0/1 可见被切、2 才干净）。保留 2px 下限属**保守**方向
（只可能多报、不会漏报），测量方法已写进 design.md §6.2.2 供复核。
