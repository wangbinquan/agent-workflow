...
**逐条落实情况（收口核对）**：审计空跑 → CDP 强制 + 4 条引擎自检 + AC2 变异测试 ✅；假阳性 → 四类排除，实施中另修掉伪元素装饰投影误报 ✅；e2e 时长 → 单 spec、全量 sweep ~34s ✅；白名单永久挂账 → **已清空并转硬失败** ✅；选择器不稳 → key 带实例序号 ✅；**只测默认视口 → 曾漏做，收口自查时补上 ≤720px 重扫** ✅。另在实施中新增三条表外防线：**覆盖门**（每个面必须实测到非零控件数，见 §6.2.1/§6.2）、**逐边判定**、**UA `auto` 环按实际画幅测量**（§6.2.2）。
...
- 2026-07-20 v3 —— **实施收口**。T0–T8 全交付，累计清零 **322 条**真实裁剪（37 基线 + 274 编辑器/任务详情 + 11 评审/聊天室），白名单清空、审计转硬失败。实施期间自查/实测又补了三条**表外防线**，每一条都源于「不相信绿色、去验证一条具体断言」：
  - **覆盖门**（§6.2.1）—— 第二批 fixture 因 API 契约用错静默失败，两个面一个控件都没量却报「0 违规」。现在每个面必须实测到非零控件数，弹窗触发按钮找不到直接失败。
  - **逐边判定**（§6.2.1）—— 整体「是否完全在视口内」的门会因 `.tabs__tab` 的 `margin-bottom:-1px` 把整个 `.tabs` 跳过，隐藏 15 条真实裁剪。
  - **UA `auto` 环按实际画幅测量**（§6.2.2）—— `getComputedStyle` 报 1px、实际画 2px，1px 余量的容器会被误判为放得下。
  另外 **T6 的两条静态推测被实测证伪**（`.review-detail__layout`、`.page--editor > .form-grid` 本就无需改，真凶是外层 `.page--review-detail`），这正是本 RFC「让几何说话、不照静态清单改代码」的价值实证。
  ⚠️ 唯一未完成：**实现门 Codex review 因配额耗尽未跑**（Jul 25 恢复后按 plan.md T8 补），期间以独立对抗审查作为不完全替代。
