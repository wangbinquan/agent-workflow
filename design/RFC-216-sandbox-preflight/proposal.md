...
- **AC-3**：Linux **`Bun.which('bwrap')`→路径但探测非零（装了但起不来）** → 报告注明
  「已安装但试跑失败（exit N；stderr 摘要）」+ 打印 **强 hedge 的** sysctl 指引 + 安全
  提示（明说是启发式推断、非确证）；**不**打印安装命令；`mode≠off` 时退出 1。
- **AC-4**：任一不可用且 `mode≠off` → 报告含「装完 / 改完后需**重启 daemon** 生效
  （探测开机缓存）」。
- **AC-5**：`sandboxMode=off` → 报告注明「config 已关闭沙箱」，**默认**退出 0。
- **AC-5b（决策 A）**：`--require-available` 严格档 → 机制可用退 0；否则（含 off、含
  不可用）非零。默认档不加此 flag、off 仍退 0。
- **AC-6（只读断言，P1-1 + P1-2）**：命令全程**不写任何文件**（含缺 config 时不建目录/
  不落默认配置）、**不执行任何包管理器 / sysctl / 写命令**。守卫用**两层**：①静态——
  `guidance.ts`/`cli/sandbox.ts` 源码禁 `node:child_process`/`Bun.$`/`execSync`/`spawnSync`/
  fs 写 API，`Bun.spawn` 至多一次且在 `boundedSpawn`；②子进程 sentinel——全新 HOME +
  PATH 放会写 sentinel 的假 apt/bwrap/sysctl，断言零执行 + 零文件/目录写入（含无
  config.json）。**不用**会匹配打印字符串的文本 grep。
...
- **AC-10（P2-2）**：机制探测有界——`boundedSpawn` 命中 deadline 即 `kill`+回收 → 报告
  「探测超时」+ 退出 1；永不退出的假机制 fixture 证明命令不挂起。
