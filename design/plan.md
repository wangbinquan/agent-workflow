...
| [RFC-035](./RFC-035-ux-consistency/proposal.md)                  | UX 一致性专项（**stub，待 RFC-032 完工后展开**）：背景见 `design/ux-audit.md` 全量审计；引入 4 类设计 token（间距 / 字号 / 圆角 / 阴影 + 语义色 success/warn/info）+ 补 `.btn--ghost` `.btn--xs` 静默缺失的 CSS + 收敛 4 套并行状态指示系统到 `<StatusChip>` + 收敛 4 套 tab 实现到 `.tabs` modifier + 推广 `<Form>` helper 到全部表单路由（35%→100%）+ 推广 `.data-table` 到 repos/agent-import/reviews + 抽出 `<Dialog>` / `<EmptyState>` / `<LoadingState>` / `<DetailLayout>` shared components + retrofit 已有 page；0 backend / 0 shared / 0 DB；依赖 RFC-032 PR1 落地 | Draft       |
