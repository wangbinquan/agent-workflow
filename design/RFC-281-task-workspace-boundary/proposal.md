...
- **B4 · claude 声明 permission 的节点：多仓 mounts 变为可读**+**可写**。今天 `dontAsk` 下 cwd 之外即拒，多仓任务的其他成员 mount 本就够不着——本 RFC 顺带修复（属能力恢复）。**实现门 P1-1 补齐**：`additionalDirectories` 只解决读（T0 §5-5 实测 dontAsk 下写它仍报 “Write tool access not available in current mode”），写另需 `permissions.allow` 的 `Edit(//<dir>/**)` 规则——按官方文档，`//` 前缀才是文件系统根，且只发 `Edit(...)`（它覆盖 Write/NotebookEdit；单独的 `Write(...)` 规则 claude 接受却从不查询）。
...
- **B9 · opencode：机器级 skill 根一并 re-allow（实现门 P1-2 修正，非收缩）**。opencode 的默认 `external_directory` 白名单含 `skill.dirs()`（`agent/agent.ts:108-113` @1.18.4），来源是 `~/.claude/skills`、`~/.agents/skills` 与配置目录下的 `{skill,skills}`（`skill/index.ts:185-195`）。平台 deny 基线合并在 defaults 之后会一起遮蔽它们——SKILL.md 仍进 prompt（配置层读取不过权限），但模型按其指示读同目录脚本会被拒，形成「技能一半能用一半报错」。现按 opencode 同一口径把这些根放行回来（`machineSkillRoots()`），**不新增可达面**：这些目录在本 RFC 之前本就是 allow。
- **B10 · claude：不下发 `allowUnsandboxedCommands`（实现门 P1-3 修正）**。claude schema 原文：该键为 `false` 时 `dangerouslyDisableSandbox` 参数**被完全忽略、所有命令必须沙箱化**。那是 headless 下模型撞到 cwd 外写（`bun install` 写 `~/.bun/cache`、`npm ci` 写 `~/.npm/_cacache`、`cargo`/`pip` 同理）时**唯一**的自救路径；焊死它等于让典型 build 节点烂在 EPERM 上无人可救。防误入不需要这一层（写边界由 `filesystem` 默认 cwd+tmp+allowWrite 承担），故沿用上游默认 `true`（§0）。
