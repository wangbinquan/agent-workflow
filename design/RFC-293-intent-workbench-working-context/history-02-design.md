...
> 2026-08-12 修订：已吸收第一轮 Codex source-backed 设计门的 12 个 P1、1 个 P2，以及第二轮的 6 个 P1；
...
- **interrupt**：将 effectful change 与 exact 当前 turn cancel 原子持久化；旧 runtime取得 whole-tree exit
  proof后才应用。
...
2. 一个 session 同时至多一个 `inFlightTurnId`；exact `turnId+claimId` 同时至多一个 runtime owner。
   `main exited`、`drainTimedOut` 或 `unreaped` 都不等于 runtime 已退出；successor 只能在 predecessor 的
   whole-process-tree exit proof 持久收敛后 spawn。
...
6. 每个 requested addition（包括 already-current-root）都在最终事务内复验 canonical row 存在与
   `canViewResourceInTx`；通过后，只有把 non-root/new identity 提升为 root 才 effectful。remove 只允许
   exact current root handle。no-op 不能绕过授权或 exact causal turn fence。
...
新增 `usePinnedScroll` 纯 DOM hook。`pinnedRef`只由 scroll事件（含显式“回到最新”产生的 programmatic
scroll）按当前 DOM更新；每次 render读取的是上一个 content signature留下的 ref。signature变化后的
`useLayoutEffect`先消费该旧值完成 follow/unseen判定，再允许 ResizeObserver更新新高度 baseline；
ResizeObserver不得在 effect前用新增高度覆写 `wasPinned`，避免用结果反推原因：
...
const PostIntentWorkingSetChangeSchema = z
  .object({
    ...
    supersedesChangeStateVersion: z.number().int().min(1).nullable(),
    mode: z.enum(['after-current', 'interrupt']),
    delta: IntentWorkingSetDeltaSchema,
  })
  .strict()
  .superRefine(/* supersedes id/version must be both null or both present */)

const PostIntentWorkingSetTransitionSchema = z
...
type IntentWorkingSetChangeDto = {
  ...
  stateVersion: number
  interruptCommittedAt: number | null
...
- 服务端先按 `(createdAt DESC,id DESC)` 的不可变创建序列取 latest row；只有它仍是
  `queued|failed` 时才投影
...
- `resourceId` 只供按现有 owner/admin-audit detail授权的 machine client重建 picker identity；不得进入
  可见 DOM/accessibility tree、analytics、WS、turn model text或 ordinary log。actor-safe
  名称/type/handle才可展示；audit只读客户端不实例化可编辑 picker。
...
current-action wire先按 question/request item key canonical排序，以从 `secret.key` 派生的独立
`intent-current-action-v1` key做 HMAC-SHA-256；answer正文只作为 HMAC输入，不以明文出现在 digest、
log或 HTTP receipt。
...
interrupt_committed_at   INTEGER NULL
...
Rows immutable except state/stateVersion/interruptCommittedAt/receipt/error/resolution/updatedAt。replace transaction 可把 exact
latest `queued|failed` row按 id+stateVersion标 `superseded` 并回填 `resolvedByChangeId`，再 insert 新 row；cancel 只做
...
generation_policy_max_rounds   INTEGER NULL
generation_policy_admitted_at  INTEGER NULL
dispatch_claim_id              TEXT NULL
dispatch_daemon_id             TEXT NULL
dispatch_claimed_at            INTEGER NULL
dispatch_phase                 TEXT NULL CHECK legacy-unfenced|claimed|waiting|launching|spawned|reap-pending
cancel_requested_at            INTEGER NULL
runtime_lease_token            TEXT NULL
runtime_lease_started_at       INTEGER NULL
runtime_supervisor_pid         INTEGER NULL
runtime_tree_identity          TEXT NULL
runtime_lease_endpoint         TEXT NULL
...
- generation policy只持久化 authoritative `maxGenerateRounds` 与 keyed HMAC fingerprint；key从现有
  `secret.key` 以独立 domain `intent-generation-policy-v1` 派生，不复用 mutation/SecretBox nonce。HMAC输入是
  fresh resolved config的 canonical完整执行语义（含 extra instructions），DB不保存其正文、credential、
  env或 secret-derived明文，ordinary log/HTTP/WS也不输出 fingerprint。dispatcher fresh resolve并以同一
  daemon secret计算后必须 timing-safe exact match。context apply后配置再变化会 typed start-fail，不得
  偷换 policy；
- dispatcher 以 exact session/turn/kind/claim-null/cancel-null CAS 写 claim；claim 后每个 await 与 spawn
  前都复验 exact cancellation；
- spawn不能留下“子进程已活、DB还没有 identity”的窗口：spawn前先 CAS写 `phase='launching'` 与随机
  `runtimeLeaseToken/runtimeLeaseStartedAt`，由 managed-process supervisor完成 §7.4 handshake；model exec/release前，exact
  supervisor identity `{pid,treeId,leaseEndpoint}` 必须已持久化。POSIX Unix socket / Windows named-pipe
  challenge-response将 token绑定到仍存活的原 supervisor，不依赖可复用 PID；无法验证时 boot fail closed；
- runtime主进程退出或 legacy pump deadline 到达后，只有 §7.4 whole-tree exit proof 才能进入 terminal
  wrapper；尚未证明时 CAS为 `reap-pending` 并保留 claim、lease、session slot与 semaphore占用。daemon
  crash可丢弃尚未安装的 draft candidate，但不能据此启动 successor；boot先 fence，再以 typed restart
  terminal收口；
- migration 将升级瞬间已有的 legacy `running` row 标为 `legacy-unfenced`。旧版本未持久化 runtime
  identity，不能证明 detached child已死，因此 boot **不得**自动 settle/resume并启动 HTTP；正常升级必须
  先在旧版等 session idle。历史 crash row只允许 daemon停止时由专用 doctor取得 daemon lock，要求操作员
  对 exact turn输入 `--confirm-no-live-process` 后 typed settle；该动作有审计且不自动 drain/start successor；
...
5. 仅对新 mutation做 fresh transaction admission：验证 owner active/current permission、session writable、
   无 unsettled apply、context revision与 exact supersedes id+stateVersion；随后在**分类 no-op之前**完成 causal
   turn gate：running时要求 current agent `running` turn exact属于本 session且
   `(id,seq)==(expectedInFlightTurnId,expectedTurnSeq)`；idle时要求 `expectedTurnSeq===session.turnSeq`，且
   `expectedInFlightTurnId` 要么为 null，要么是本 session该 seq的 latest terminal agent turn且其后没有任何
   user/agent turn。古老 id返回 stale，跨 session id同形404；任一路径失败都不插 ledger row。latest row仍为
   mutable queued/failed时，新 request必须携它的 exact supersedes id+stateVersion；latest已 terminal-resolved
   时两字段必须为 null，不得跳过 unresolved row另起 change；
6. 同一 admission transaction对**每个** requested addition查询 canonical resource row并执行
   `canViewResourceInTx`，包括 manifest里已经是 root的 identity；删除/不可见同形404。通过后才针对 current
   manifest运行纯函数。invalid remove整批失败；全 add-satisfied且无 remove在该 transaction直接插
   `no-op` receipt，即使 session仍 running也不解析 policy、不 queue、不 interrupt。若它是合法 replacement，
   同 transaction先把 exact mutable old row标 superseded再插 no-op；任一 admission失败则 old row保持原状；
7. effectful + inFlight + `after-current`：在 admission transaction insert普通 queued row；effectful +
   inFlight + `interrupt`：同一 transaction insert/replace queued row时直接设置
   `interruptCommittedAt`，并以同一个 exact turn gate写 old turn `cancelRequestedAt`。journal与 durable
   cancel要么一起 commit、要么都不存在；policy留到 terminal drain按届时 authoritative配置解析；
8. effectful + idle admission不写 row，只产出预检结果；随后才在 §6.3 policy fence内 fresh resolve
   authoritative policy，resolve失败零 row/零 context。final transaction重做步骤5–6与 idle/exact-terminal
   判断、重算 delta，再原子插 journal、apply context与 reserve turn；中间若出现新 turn/context/pending
   change则 stale零副作用；
9. apply/queue row绑定 `runAsUserId=session.ownerUserId` 与 current-session-owner policy，不持久化
   cookie、PAT id/secret/scopes。

cancel/dismiss/GET receipt 都先执行同一个 owner-scope授权，再按 exact change id 查询；transition body
携 `expectedStateVersion` 且只允许 latest row变化。cancel already canceled 与 dismiss already dismissed
幂等返回原 receipt，其他 terminal state返回 typed current receipt，不把“已经不是 queued/failed”伪装成丢失。
`interruptCommittedAt!==null` 的 queued row不是可变 target；只能等待 exact old turn terminal并 drain。

### 6.2 interrupt

effectful interrupt 的 journal insert/replace、`interruptCommittedAt` 与 exact old turn
`cancelRequestedAt` 已由 §6.1 step 7 在一个 transaction 内提交。route 在 commit 后只发 non-throwing 的
best-effort 内存 abort/wake，不再承担 durable cancel：

```ts
dispatcher.wakeInterrupt({
  sessionId,
  changeId,
...
- 同一 transaction的 exact
  `(sessionId,latest supersedes id/stateVersion,turnId,turnSeq)` admission决定赢家。如果另一标签的
  replace/cancel先赢，interrupt整批不提交且绝不取消 turn；如果 interrupt先赢，之后 replace/cancel返回
  `intent-interrupt-committed` current receipt，stop不可逆。响应丢失后的 same-body replay能直接看到已经
  committed的 interrupt receipt，不会退化成 after-current；
- unclaimed reservation看到 cancel flag后绝不再 claim/spawn。统一 convergence worker先锁定 exact
  `claim NULL + cancelRequestedAt` 快照，事务外准备 typed fresh policy result，再在第二个可重入 transaction
  复验同一快照、settle old turn并 drain queued；dispatcher、reconciler、boot都调用这一流程。若 daemon在
  cancel commit、policy prepare或 terminal transaction之间崩溃，下次仍从 durable flag重入，不得把它当
  普通 unclaimed dispatch；
...
- `wakeInterrupt`先读取/复验 DB 中 exact committed change+cancel tuple，再对 live registry 的 exact owner
  abort；它的失败只影响唤醒延迟，不改变已提交事实；
...
仅驻内存的 resolved config、persistable HMAC fingerprint/max或 typed failure。所有 terminal path都先
准备该结果，避免“先看没有 queue、随后才插 queue”的竞态；然后
`drainQueuedWorkingSetChangeInTx` 在 session idle 的事务内调用：

policy prepare到 terminal transaction结束必须整体运行在现有
`withRuntimeProbeConfigFence(configPath)` 内，与 `/api/config` 的 invalidate-before-write/atomic replace
串行；最终 DB transaction还要重读 resolved runtime row的 execution-profile fingerprint/version。该
transaction是 generation policy的线性化点：设置/Runtime更新若先完成，必须被新 policy/budget看到；
若后完成，则 context reservation在线性顺序上先发生，后续 dispatcher mismatch会 typed start-fail。
在进程 fence内再以固定锁序取得 `config.json.intent-policy.lock`：实现复用 `util/lock.ts` 的
O_EXCL PID-file语义并在进程内先经 `KeyedSerialQueue`，bounded busy返回 typed retry。`/api/config`、
config CLI与 policy prepare都走该 cross-process lock；因此 CLI仍可等待后热更新，不会绕过 daemon。
直接手改 live `config.json` 不属于支持的 mutation面；atomic replace若绕过 lock，dispatcher HMAC mismatch
仍 fail closed但不承诺零副作用。
...
4. 对每个 requested addition（含 already-root）在同一 tx按 `ACL_TABLES + canViewResourceInTx`复验
   canonical row存在与可见；通过后 already-root只记 satisfied且不读取正文。removals允许移除已失效 root；
...
8. effectful：写 manifest/watermark、contextRevision +1；插一条 user `context-change` turn；插 agent
   `running` turn（claim NULL、new nonce、run-as+policy max/fingerprint）；更新 session
...
所有清理 `inFlightTurnId` 的路径必须调用同一 async terminal wrapper。对曾进入 `launching|spawned|
reap-pending` 的 turn，wrapper 的硬前置条件是携带 §7.4 exact whole-process-tree exit proof；单纯
`main exited`、legacy `drainTimedOut`、`unreaped/child-unkillable`、AbortSignal已触发或 supervisor结果 promise
已返回都不构成证明。前置满足后才准备 fresh generation policy result，再在一个 terminal transaction
中调用 helper：
...
若模型主进程已给出结果，但 output drain或 process-tree empty proof尚未完成，dispatcher把 exact claim CAS为
`reap-pending`，保留 `inFlightTurnId`、runtime lease/identity、semaphore slot与内存中的 validated terminal
candidate，持续 fence/reap且不调用上述 wrapper。正常运行中证明 tree empty后才安装 candidate；daemon在此
期间崩溃可放弃未持久化 candidate，boot仍须先证明旧 tree消失，再以 typed restart error调用 wrapper。
安全优先于保留一份尚未 terminal commit的草稿。

### 6.5 Combined current action reservation

`POST /current-action` 与 working-set change 使用同一安全顺序，不另造较弱路径：

1. coarse auth/wire parse 后先取得 owner-only scope；以 strict wire body（含 sourceTurnId）canonicalize/
   hash并立即查 generation ledger。same body返回 current receipt且不受后来 archive/turn推进、source
   row损坏或删除影响，changed body 409；
2. 只有新 mutation且 scope成功，才能按 `sourceTurnId + scope.sessionId` 读取/safe-parse私有
   questions/mount requests，并验证完整 answers/decisions；
3. 新 mutation在事务外成功解析 authoritative generation policy；事务内 fresh检查 owner
...
6. 先把 phase写 `waiting`并进入 abortable semaphore；等待期间不 dump、不持有 resource正文；
7. 获取 slot后重验 cancel/claim，并在 generation-policy fence下 fresh resolve + timing-safe重验绑定
   fingerprint，以 exact claim CAS写 `generationPolicyAdmittedAt`；随后才构造 §11.1 frozen disclosure snapshot与 held seed，紧邻 launch前做 final
   disclosure admission。只有 admission CAS成功才通过 §7.4 launch handshake，在 DB持久化 exact process
   identity后 release supervisor执行模型；
8. 所有 config/claim/admission/spawn错误走统一 exact typed settlement；若 launch已经开始，必须先取得
   whole-tree exit proof。`finally` 只有在未 launch或 proof已持久收敛后才能 compare claim删除 registry并
   wake；`reap-pending` owner继续占 registry/semaphore并重试 fence，不能用 finally制造 successor窗口。

route wake + boot scan + terminal wake + reconciler poll都走一个入口。禁止 route自己
`void runIntentTurn`，禁止 catch后只 log不 settle。

slot后 policy recheck transaction是执行配置的 final admission线性化点；在它 commit前完成的 config/
runtime变化必须使 fingerprint失配，commit后的变化不追溯取消已经 admitted 的 run，与 disclosure
admission采用同一时间边界。后续 runner只消费这次 recheck返回并仍驻内存的 resolved config，不再
第三次 load产生 body/fingerprint分叉。
...
- current running、claim NULL且 `cancelRequestedAt!=NULL`：不等 grace，走 cancel terminal wrapper +
  queued drain，spawn count保持0；
- claimed row连续两个 grace scan不在当前 daemon exact live registry时必须按 phase收敛：只有
  `claimed|waiting` 且 lease token/identity/endpoint全空，才可原位 typed
  `intent-runner-owner-lost` settlement并drain；
- `launching` 即使 identity仍 NULL也一律视为“runtime存在性未证明”。reconciler按
  `runtimeLeaseToken+runtimeLeaseStartedAt` 关闭仍在 registry的 initial control pipe，尝试 endpoint
  challenge；endpoint尚未出现则等待 handshake deadline，直到证明 supervisor从未 release且已退出。不能
  因 PID/identity为空直接 settle；
- `spawned|reap-pending` 必须经 persisted token+endpoint+tree identity做完整 process fence；任何阶段只要
  不能证明 tree empty，就保留 claim/slot并继续报告，不得 drain或启动 successor；
...
1. exact claim先在 DB写 `phase='launching'`、`runtimeLeaseToken/runtimeLeaseStartedAt`。POSIX endpoint是
   0700目录下的0600 Unix socket；Windows endpoint是只允许当前 service SID/user SID的 named pipe。名称只由
   `{appHome identity,turnId,claimId}` 派生，token不进入 path/argv/env/log；
2. daemon spawn supervisor并保留初始 control pipe。POSIX supervisor建立自己的 process group；Windows
   supervisor建立并持有 `KILL_ON_JOB_CLOSE` Job Object、把 model child纳入 job。两端都建立 lease server，
   以随机 challenge证明持有同一 token后阻塞，**尚未 spawn模型**；
3. daemon通过 endpoint challenge校验 token，以 exact claim transaction写
   `{pid,treeId,leaseEndpoint}` 与 `phase='spawned'`；commit成功后才发送 authenticated release，supervisor
   才 spawn模型 child并在整个模型生命周期保持 lease server，由它转发 TERM/KILL、wait并 reap；
4. daemon在 DB commit前死亡会关闭初始 pipe；supervisor在 EOF或短 handshake deadline时自杀并清
   socket，因而不存在无 DB identity的模型 survivor；daemon在 commit后/release前死亡时，boot能以
   DB token向 endpoint challenge并终止尚未 spawn模型的 supervisor；release后死亡时同一 endpoint仍由包裹
   模型的 supervisor持有；
5. cancel在 launching阶段先写 durable flag、关闭 pipe/向 lease endpoint发 authenticated terminate并
   验证 supervisor退出；旧 claim不得 release。PID复用但无匹配 challenge只会 fail closed，不会 signal
   无关进程。

supervisor不改变 runtime wire：它 fork model child而不是用模型替换自己，byte-for-byte relay stdin/
stdout/stderr、保留 EOF/backpressure/line truncation/pump-error语义并转发 signal。它把主进程真实 exit
code/signal、drain outcome与**独立的 whole-tree proof**分开上报：POSIX必须 wait/reap direct child并证明该
lease的 process group除仍在报告结果的 supervisor外已无成员；Windows必须 wait handle并证明不可
breakaway的 Job Object active-process count为0。然后 supervisor关闭 lease endpoint并退出，daemon再
wait/reap supervisor；只有
`{challengeMatched,treeEmpty,endpointClosed,supervisorReaped}` 全真才形成 exit proof。legacy
`drainTimedOut`只是 capture evidence，`unreaped/child-unkillable`明确是否定证明；二者都先进入
`reap-pending`，重复 TERM/KILL/empty probe，不能直接返回 terminal wrapper。

生产 Intent path必须注入这个 launcher，测试 `runFn` 只能经显式 fake-launch-lease adapter模拟
identity/exit proof，不能以“无进程 test seam”绕过 DB phase不变量。既有 system-agent capture golden与
managed-process pump/reap tests必须回归；普通 system-agent调用者可保留旧 outcome适配，但 Intent不得把
旧 `managedProcess` outcome本身当 proof。
...
3. 对 `launching` token-only与已 spawn identity分别完成 §7.3 phase fence；让 supervisor按 POSIX process
   group / Windows Job Object TERM-or-terminate，预算尾部强杀，取得 whole-tree proof；
4. 只有 proof成立的 exact row才 typed settle/drain；超预算保留 claimed+lease/identity与
   `reap-pending`给下次 boot fence，不能清 row后假装进程已死；
...
`runtimeLeaseToken + DB pid/treeId/endpoint`：launching且只有 token时等待 handshake deadline并证明
supervisor已自杀；endpoint challenge匹配才向 supervisor发 authenticated terminate/reap，并再次证明
endpoint关闭且 POSIX group / Windows Job live count归零；DB/endpoint不一致或无法安全验证（含 PID reuse
歧义）则 daemon boot fail closed并给运维诊断，不启动 successor，绝不直接 signal仅 PID相同的进程。

### 7.5 Boot

启动顺序：

1. migrate并 inventory `legacy-unfenced`，但尚不启动任何 listener/dispatcher；
2. `fenceClaimedIntentProcessesOnBoot`：先收口所有带 RFC-293可验证 lease的 row；对 `launching`
   token-only执行与 daemon-alive reconciler相同的
   handshake-deadline fence，对 `spawned|reap-pending`按 token+lease endpoint证明/终止/reap旧 process
   trees；任一无法形成 whole-tree proof则 boot fail closed；
3. 若 inventory含 `legacy-unfenced` current row，输出 exact session/turn与 recovery指引并 boot fail closed；
   它们没有可验证 identity，不能混入 step 2的“无 runtime”集合；
4. converge apply journals；
5. `recoverClaimedIntentTurnsOnBoot`：只 settle已证明无 runtime的 RFC-293 claimed current rows，并在同 tx
   drain queued；每个 drain先准备 fresh authoritative generation policy；
6. start dispatcher + short reconciler；它 dispatch所有 unclaimed current rows，包括 boot drain新建 turn；
7. 最后启动 HTTP listener；hourly maintenance只报告长期异常，不代替 short reconciler。
...
| POST   | `/api/intent-sessions/:id/working-set-changes/:changeId/cancel`  | body含 expectedStateVersion；latest queued→canceled；重复返回原 receipt  |
| POST   | `/api/intent-sessions/:id/working-set-changes/:changeId/dismiss` | body含 expectedStateVersion；latest failed→dismissed；重复返回原 receipt |
...
route registry先登记 static `by-mutation` 再登记 `:changeId`，避免把字面量吞成 change id；mutation id
放 path segment前按 shared length/charset校验。
...
- `working-set-interrupting`：kind仍 `generating`，正文“正在停止本轮，停止请求不可撤销”；
...
```ts
{
  ...
  reason: 'audit' | 'archived' | 'apply-running' | 'turn-running' |
          'working-set-queued' | 'working-set-interrupting' | 'working-set-failed' | 'draft-stale' |
          'validation' | 'ready'
}
```

工作上下文允许在普通 `turn-running` 时打开与 queue；`working-set-interrupting` 与 `apply-running`
只读。commit、message、current action均消费同一投影，不再各写一套 boolean。
...
- claim 后 `prepareIntentDisclosureSnapshotInTx` 用 exact turn/claim重读 current user/session、mounts、
  ...
  visible资源为 `{type,id,owner,visibility,aclRevision,contentFence}`；snapshot开始时已经不可用的 manifest
  entry冻结 `{type,id,handle,availability:'unavailable'}` tombstone；identity只在内存/digest输入，不能投影；
...
- disable user、role变化、ACL grant撤销、owner transfer、visibility/content fence变化或任一 mounted root
  的 visible/unavailable分类变化，都丢弃 held seed、typed settle、spawn=0；从 snapshot开始就 unavailable
  的 root继续按 RFC-291省略正文并只输出 type+handle Access note，不使整轮失败。admission commit后的变化
  不追溯终止已启动 run。
...
- log：sessionId/changeId/state/errorCode/counts，不记 additions/removals JSON、policy HMAC、lease token/
  socket或 disclosure digest；
...
| causal/add admission | stale/cross-session turn或任一 addition不可见 | no-op分类前失败、零 row                       | refetch，保留可修正 staged     |
...
| no-op                | causal+all-add ACL通过后 empty/all roots      | no-op receipt                                 | 关闭 Dialog，不生成            |
...
| current turn         | interrupt commit/response loss                | journal+durable cancel同 tx；可 replay        | “正在停止”，不退化为默认模式   |
| daemon alive         | wake丢失/token-only launch/owner丢失          | phase-aware fence；无 proof不drain            | 最终自动继续或typed error      |
| runtime drain        | main exited但drainTimedOut/unreaped           | reap-pending；保留 claim/slot                 | “正在收尾”，不启动 successor   |
| graceful shutdown    | live detached model process tree              | whole-tree proof后才settle                    | 重启后不双跑                   |
| hard restart         | claimed row + matching old process            | boot先fence/prove empty，再settle/drain       | successor只在旧进程死后启动    |
...
- gate：owner/status/permission/ACL/delete/OCC/unsettled/budget；每条失败断言 manifest bytes、epoch、
  watermark、turn count、draft pointer均未变；already-root addition在删除/撤权后也同形404且ledger零 row；
- exact turn gate：running→terminal race、同 session古老 turn、terminal后新 prompt、跨 session id；每种再与
  empty/all-root no-op交叉，失败均 ledger零 row；
- exact cancel：effectful journal+cancel同 tx的 commit/rollback/response-loss，unclaimed、claim committed/
  pre-controller、config await、abortable semaphore waiting、launching、spawned process、later-turn mismatch、
  conditional registry delete；cancel后旧 runtime spawn count=0或 whole-tree proof后才 terminal；
- settle handoff：questions/changeset/error/start-failure/cancel/boot五条 terminal path；所有 launched path缺
  proof时 helper拒绝；
...
- dispatcher：double wake single spawn、claim CAS、fresh session actor（非 daemon fallback）、disabled owner、
  lost wake、claim后owner注册前异常、unclaimed grace redispatch、claimed/waiting-without-lease可收口、
  launching-token-only必须等 deadline证明、spawned/reap-pending完整 fence、live owner不按墙钟回收、next
  refresh wake；
- shutdown/process fence：main exit+pipe holder触发 drainTimedOut、child-unkillable、graceful TERM/KILL、
  containment empty/endpoint close/supervisor reap四证明、lease challenge mismatch fail closed、daemon PID
  SIGKILL后 boot清旧 process tree再 successor；
- platform fence：POSIX Unix socket/process-group与 Windows current-SID named pipe/Job Object各跑真实 helper；
  Windows supervisor死亡关闭最后 Job handle后 active-process count为0，不能以 `taskkill` snapshot当证明；
...
- migration from live legacy running row必须 legacy-unfenced且 listener前阻断；offline doctor无显式 exact
  operator proof不得 settle；schema drift admission。
...
11. 只 SIGKILL daemon PID，记录 supervisor PID/tree identity/lease endpoint；boot后 endpoint关闭且
    tree live count为0，successor才
...
- DB additive migration；旧 sessions默认无 journal、claim fields NULL；live legacy row标
  `legacy-unfenced` 并阻止 boot，不能猜测进程已死；
...
- 若 dispatcher 启动自检发现 schema/partial index 缺失，daemon boot fail closed，不以“退回手工重跑”
  静默降级。
- managed-process adapter若不能提供可验证 lease challenge，带 claimed runtime identity的 boot fail closed，
