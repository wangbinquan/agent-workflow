...
- [ ] T0.3 按仓库 Codex 设计门要求做 source-backed adversarial review；逐条核实 finding，记录到
      `design-gate-YYYY-MM-DD.md`。第一轮 12 P1 + 1 P2 全部修入三件套；必须以修订版复审，无阻断项
      才再次请批。
...
- [ ] T1.1 在 `packages/shared/src/schemas/intentSession.ts` 增加：
  ...
  - `expectedTurnSeq`、完整 terminal state/receipt、cancel/dismiss receipt；
...
- [ ] T2.4 `intent_turns` 加 run-as、generation-policy、claim/cancel/phase、runtime lease/process identity与 disclosure
      admission columns；migration 把升级瞬间 legacy running row标 synthetic claimed，避免 boot重跑
      可能已 spawn 的 turn。nullable legacy row只能 terminal，不能 resume。
...
- [ ] T4.4 先取 latest row再决定是否投影 queued/failed；任意 terminal可按 change/mutation id查询，
      历史 failed不能复活。
- [ ] T4.5 failed→dismiss或supersede、queued→cancel/supersede exact stateVersion CAS；重复动作返回原
      receipt，其它 terminal返回 typed current state。
...
- [ ] T5.2 每条 terminal path在事务前 fresh resolve canonical generation policy；apply前以 current-session
      owner session actor复验 principal/session/permission/ACL/OCC/apply，并用绑定 max查 budget；失败
      row terminal、manifest/epoch/turn零变化。
...
- [ ] T5.6 questions/error/start-failure/unclaimed cancel/boot recovery五条 caller全部接同一 async
      policy-prepare + terminal transaction；覆盖“预读无 queue 后竞态插入 queue”。
...
- [ ] T6.1 DB增加 exact cancel request；registry按 `turnId+claimId`持 controller/phase，abort/delete compare
      两者。
- [ ] T6.2 `requestCancelExact` 区分 unclaimed、claimed-pre-controller、waiting、spawned；每个 claim后 await
      与 spawn前复验 durable flag。
- [ ] T6.3 semaphore实现 abortable acquire，abort时从 waiter queue移除；interrupt先 journal commit后
      persist cancel + best-effort abort。
- [ ] T6.4 claim/pre-controller、config await、semaphore wait、spawned process、later-turn mismatch、old
      finally delete-new-owner、双 click/response loss barrier tests；未 spawn路径启动次数为0。
...
- [ ] T7.3 reservation前持久化 generation policy fingerprint/max；dispatcher fresh resolve exact match，
      policy changed typed settle，不换配置运行。
- [ ] T7.4 实现 claim-bound `prepareIntentDisclosureSnapshotInTx` / `admitIntentDisclosureInTx`；held seed
      仅 final admission后交给模型。
- [ ] T7.5 短周期 fake-clock reconciler：lost wake、unclaimed grace、claimed-without-live-owner连续 scan；
      exact live owner不按墙钟回收，hourly maintenance只诊断。
- [ ] T7.6 managed process实现 launch-token + supervisor control-pipe handshake：DB identity commit前模型
      不 exec，parent EOF/timeout自杀；持久化防 PID reuse identity。graceful shutdown停止 claim、abort
      semaphore/controller、TERM/KILL/reap exact Intent process group。
- [ ] T7.7 boot在 listener前 fence旧 claimed runtime；无法证明死亡则 fail closed；只 recover已无 runtime的
      claimed/legacy turn，unclaimed留给 dispatcher，recovery drain queued。
...
- [ ] T7.9 tests：double wake、cross-session concurrency、same-session singleflight、lost wake、claim-owner
      窗口、runFn/config error、policy降额/改变、disclosure TOCTOU、graceful/hard kill、broadcast cursor。
...
- [ ] T8.3 idle/running/race-to-idle、expectedTurnSeq、same-session old/cross-session turn、replace/replay/
      cancel/dismiss/error route integration tests。
...
- [ ] T9.4 owner scope→ledger-before-freshness；same request replay/changed body/source superseded/candidate
      ACL changed/one invalid decision全 rollback。
...
- [ ] T14.1 `IntentWorkingContextBar` applied/queued/interrupt/refresh/failed/no mount/read-only状态。
...
- [ ] T17.2 runtime overlap probe记录每次启动/结束、PID/PGID/birth token与max concurrent；interrupt/boot
      必须max=1。
...
- [ ] T18.10 daemon-alive lost wake自愈；hard kill只杀 daemon PID，旧 child消失后才 successor。
