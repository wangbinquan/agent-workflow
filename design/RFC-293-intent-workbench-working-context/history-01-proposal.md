...
- 设计门：第一轮 source-backed review 的 12 个 P1 与 1 个 P2 已纳入本版；复审通过前仍不授权生产代码。
...
- 不把原始 resource id、凭据、secret 或隐藏 owner 信息投影到 UI、turn 文本或模型 prompt。
...
7. 已有 queued change 时打开的是其 staged 目标，可“更新排队变更”或“取消排队”；failed change
   可“调整并替换”或 exact “放弃”；
...
“停止当前轮并立即刷新”先持久化同一 queued request，再以 exact
`(sessionId,inFlightTurnId,turnSeq)` 在 DB 记录 cancel request，并 best-effort 中止同一 claim/controller。
...
terminal receipt。跨标签页看到不同 id/revision/turnSeq 必须 refetch，不覆盖别人的更新。
...
只有空 delta，或所有 additions 已经是 root 且没有 removals 时，request 收敛为 `no-op` receipt，不
bump epoch、不启动新轮。任一 remove 必须指向 exact current root；known non-root、unknown handle 与
valid+invalid mixed 都整批 stale/404，不得借 no-op 改写 legacy remove 语义。相同 request replay 只返回原 receipt。
...
detached runtime 通过 supervisor handshake 启动：DB 持久化防 PID 复用 identity 前不得 exec 模型，
parent 在 handshake 中死亡时 supervisor 自杀，关闭 spawn→DB 无身份窗口。
graceful shutdown 停止新 claim、终止并 reap exact Intent 进程组；硬崩溃后的 boot 在启动 successor 前
必须用防 PID 复用身份确认旧 runtime 已死。不能只把 DB row 标 terminal 就宣称没有双跑。
...
- **AC-8**：`interrupt` 绑定 exact old turn/seq/claim；claim 前、controller 注册前、semaphore 等待中都可取消，不能取消后来 turn，也不能让两轮 runtime 重叠。
...
- **AC-15**：空 delta或全 add-satisfied 且无 remove 才 no-op；不 bump、不生成、不消耗 budget且 receipt 可 replay。remove non-root/unknown 失败并锁住 legacy 404。
...
- **AC-20**：短周期 reconciler 在 daemon 不重启时收敛 lost wake、unclaimed past grace 与 claimed-without-owner；真实 live owner 不按墙钟误回收。
- **AC-21**：launch-token/supervisor handshake 在 identity commit前不 exec模型且 parent EOF自杀；
  graceful shutdown停止 claim、abort/kill/reap Intent进程组；硬崩溃 boot在旧 runtime身份未证明消失时
  fail closed，不启动 successor。
