...
## 15. 手动问题：自主新增 / 复制（用户 2026-06-28 加需求；语义=「人提问→指派 agent 处理」）

用户定：手动问题 = **人写一条问题/指令，指派给某 agent 节点，下发后该节点带这条指令重跑**（不走「人回答」环节）。这与 override 执行同一管道——手动问题就是「来源=人、内容=人写指令」的 `task_questions` 条目，**复用 T9 的 mint rerun + 注入**。故 **§15 依赖 §14（T9）**。

### 15.1 数据模型（最小扩展 `task_questions`）
- `origin_node_run_id` 改 **nullable**（手动问题无 clarify 轮来源；migration 0062）。`source_kind` 枚举加 `'manual'`。
- 新增 `manual_title TEXT` + `manual_body TEXT`（人写的问题标题 + 指令正文）；`role_kind` 对手动问题恒 `'designer'`（修订型、可改派/可下发）。
- 派生兼容：`listTaskQuestions` 的 reconcile 只管 clarify 轮；手动问题是**直接插入**的条目（不经 reconcile），但**复用同一 phase 派生 + 承接 run 解析**（条目级，§14.2 F2）。`questionTitle`=`manual_title`、`answerSummary`=`manual_body`（即「指令」当作注入内容）。
- 来源节点 `source_node_id`：手动问题无来源节点 → 记 `null` / 特殊值；看板「来源」列显「手动」。

### 15.2 生命周期 + 下发
> **已修订（2026-06-29 Codex impl-gate fold，用户拍板）**：原 §15.2-1 允许「`待指派`（未指派）」手动行——但**无承接节点的手动行不进 park gate（有效目标为 null）→ 不 park 任务 → 可被调度器越过完成 → 孤儿**。故**收紧为：手动问题创建时必选承接 agent 节点**（与 §15 主语义「指派给某 agent 节点」一致），恒落 `待下发`（已指派、可后续 `reassignTaskQuestion` 改派）。**弃 `待指派`-manual 状态**。另：① park gate 纳入手动行（`source_kind='manual'` 第二查询、无 clarify 轮 join、同 undispatched/in-flight 分类）；② 创建 + 下发**仅 deferred-dispatch 任务**（注入/下发是 deferred-only），且**终态任务（done/canceled）拒**（`assertTaskAcceptsQuestions`，insert/stamp/mint 前；`failed`/`interrupted`/`awaiting_*` 可恢复故放行）。详见实现 commit。
1. **新增/复制**：人在「新增问题」表单填 标题+指令、**必选**承接 agent 节点 → 插入一条 `source_kind='manual'` 条目，恒 `待下发`（已指派）。**复制**=把某「待指派」条目（任一来源）的标题/指令预填进同一表单、选承接节点后另存为新手动条目。
2. **指派**：复用 `reassignTaskQuestion`（手动问题的「有效目标」就是它的 `override_target_node_id`，无图默认）。
3. **下发**：复用 §14 `dispatchTaskQuestions` → mint 承接 agent 的 rerun，把 `manual_body` 作为 External Feedback / 指令注入（复用 `buildExternalFeedbackContext` 的注入面，但内容源自手动条目而非 cross_clarify_session——需让注入支持「手动指令」来源）。
4. **处理中 → 已处理待确认 → 完成**：与 override 条目同（条目级承接 run + 人工确认）。

### 15.3 复用「新增问题界面」
现仓**无**人创建问题的 UI（问题只由 agent 经 clarify 协议产生；`QuestionForm` 只回答）。故**新建** `QuestionAuthorForm`（公共组件：`<Field>`+`<TextInput>` 标题、`<TextArea>` 指令、`Select` 承接节点——全走 §前台统一组件，禁原生）。**新增**与**复制**都打开这一个表单（复制时预填）。放在任务中心看板顶部「+ 新增问题」按钮 + 卡片「复制」动作。

### 15.4 与 T9 的依赖 + 开放点（待 Codex 设计 gate）
- **强依赖 T9**：下发注入机制（mint rerun + 注入内容）是 §14 的产物；§15 让注入内容多一个「手动指令」来源。**T9 未落则手动问题的「下发→agent 重跑」无法端到端**（可先做新增/复制/指派的记录 + UI，下发待 T9）。
- **注入面归一**：`buildExternalFeedbackContext` 当前只读 cross_clarify_session；要支持手动条目，需把「注入内容解析」抽成「按承接条目取内容」（cross 来自 session、manual 来自 `manual_body`）——这是 §14 条目级消费重构的自然延伸。
- **source_node_id 可空 / ACL**：手动问题的归属（创建者）走 §RFC-099 资源 ACL；任务成员可创建。
- **phase 派生对无轮条目**：`deriveQuestionPhase` 的 `roundStatus` 对手动条目无意义 → 手动条目恒视作「已答」（指令即内容），phase 只由 staged / 承接 run / confirmation 决定。

**实现顺序**：§15 依赖 §14——**先 T9（§14）落地**，再做手动问题（§15）。两者共一次 Codex 设计 gate + 实现 gate。

## 16. Codex 设计 gate fold（2026-06-28，编码前——吸取 §13.1 教训）

对 §14+§15 跑了 Codex adversarial **设计** gate（base `51a4ac8`）= **needs-attention / no-ship，4 findings（1 critical + 3 high）**，全采纳、折进设计后再实现：

- **C1 [critical] 延迟下发缺合法的人工 gate 锚点（§14.1）**：现码 `submitCrossClarifyAnswers` 一旦答即把 cross-clarify node_run `awaiting_human→done` + `/clarify` best-effort resume 任务（crossClarify.ts:426-470 / routes/clarify.ts:270）；调度只认 `status='awaiting_human'` 的 clarify session 为人工 gate 证据（scheduler.ts:934-967），且 `lifecycleInvariants` 把「task=awaiting_human 但无 awaiting_human node_run」判**损坏**（:380-397）。⟹ 只靠 task_questions 的 gate 会被 resume/完成（无 handler 下发）或被修复器当非法 park 清掉。**Fold**：把「未下发 task_questions」做成**一等人工 gate**——改调度 gate 装载/frontier、stuck 检测/repair/invariants、route resume；释放走 RFC-097 CAS 且**先复检无未下发条目**。这是 §14 真正的 crux，编码前必须先定。
- **H2 [high] 可跑 override 目标可能拿不到注入（§14.1/.2）**：注入是**拓扑门控**——scheduler 仅当 `agentHasExternalFeedbackChannel`（有 `__external_feedback__` 入边）才建 crossClarifyContext（scheduler.ts:1776-1782/2326-2334；shared/clarify.ts:652-660）。改派候选是任意 agent 节点，无该边的可跑节点会**无注入重跑**（这正是我 revert 的 T7 CP5 拿 `isDesignerOverrideTarget` 打的补丁）。**Fold**：下发改成**run-scoped 注入信号**（按 `node_run_id`/`trigger_run_id`+`rerun_cause` 选反馈，脱离图拓扑），而非放宽闸门；或限制目标到有注入边的节点 + 测负例。
- **H3 [high] per-entry 老化非 run-scoped → 重复下发会重复注入（§14.2）**：`buildExternalFeedbackContext` 无当前 run id、scheduler 只按目标节点调用；批量下发重试/并发时条目仍 unconsumed → 注入进多个 rerun。旧 session 路径有 `designerRunTriggeredAt` 当粗粒 already-triggered 守卫，§14 删了 session 戳却没补行级认领。**Fold**：定义**下发 CAS**——选 `trigger_run_id IS NULL` 的 staged 行、与 mint 原子地 stamp `trigger_run_id`/dispatch_id，prompt 构建只取「被该 run 认领」的条目。（这同时落实了读侧 `resolveTriggerForEntry` 一直想要的持久化 `trigger_run_id`。）
- **H4 [high] 手动问题破坏 task_questions 身份与读侧（§15.1）**：`origin_node_run_id` 改 nullable 撞 `UNIQUE(origin_node_run_id, question_id, role_kind)`——SQLite nullable-unique 允许重复 → 手动行可重复身份；且 list 按 origin 映射 clarify_rounds、跳过无轮条目 → 手动行不可见/卡死（schema.ts:1692-1741；taskQuestions.ts:43-64/205-245/292-318）。**Fold**：给手动行**独立契约**——partial unique index 或非空合成身份；`source_kind='manual'` 的 DTO + `source_node_id` 可空；list/phase 派生改为「无 clarify_round 时由 staged + trigger_run_id + confirmation 决定」；补「重复创建手动问题」+「已下发手动行可见」测试。

**结论**：§14（model A 延迟下发）的真正难点是 **C1 的一等 gate 集成 + H3 的下发 CAS + H2 的 run-scoped 注入**——这三者是「正确实现 override/手动问题下发」的地基，必须在写 `dispatchTaskQuestions` 前先把调度/lifecycle/注入面设计补全。**下一步**：按 C1/H2/H3/H4 修订 §14/§15 的接口契约（调度 gate 集成、run-scoped 注入、下发 CAS、手动行 schema），再进实现。**已交付且在线**：读侧全链 + 改派 + 答案路径 + 卡片布局（问→答相邻）。

## 17. C1 gate 机制锁定（编码前核实现状，2026-06-28）

核实现有码（confirm 假设、避免又一次 F1）：
- **gate 锚点**：`loadOpenClarify`（scheduler.ts:934-967）按 `cross_clarify_sessions.status='awaiting_human'` 判任务是否 park。
- **submit 现行**：`submitCrossClarifyAnswers`（crossClarify.ts:426-470）把 session+round `status='answered'`、并 `transitionNodeRunStatus(resume-clarify)` 把 cross node_run `awaiting_human→done`，随后才按 readiness 触发 designer rerun + questioner cascade。

**关键发现**：不能简单「答后保持 session `awaiting_human` 当 gate」——因为 **questioner cascade（反问者续跑，阻塞-产出型、不可改派、必须立即跑）依赖 status='answered'**；保持 awaiting_human 会同时卡住 questioner cascade。故 **C1 必须是独立的新 gate 源**（Codex C1 正解），不是复用 session 状态。

**锁定机制**：
1. **submit 拆「答」与「designer 下发」**：designer 域轮答时——照常 `status='answered'` + node_run done + **questioner cascade 立即跑**（不变，golden-lock）；但 **designer rerun 不在此触发**（延迟）。questioner-only 轮：完全不变。
2. **新 gate 源**：任务有「**已答、designer 域、未下发**（无 `trigger_run_id`）的 task_questions 条目」即视为 park。`loadOpenClarify`（或并行 gate）加这一源；`lifecycleInvariants`（:380-397）放行「task awaiting_human 且有未下发 designer 条目、即使无 awaiting_human node_run」；stuck-repair 同步；route resume 释放前复检无未下发条目（RFC-097 CAS）。
3. **`dispatchTaskQuestions(taskId, entryIds)`**：H3 CAS 认领（`trigger_run_id IS NULL`→原子 stamp）+ 逐有效目标 mint rerun（cause `cross-clarify-answer`）+ H2 run-scoped 注入（按 `trigger_run_id`/run 选反馈，非拓扑边）+ 释放 gate（任务回 running）。
4. **golden-lock**：无「未下发 designer 条目」时，新 gate 源恒空 → 调度/invariants/repair/resume 逐字同现行；questioner-only 与现有自动下发路径完全不动。

**实现地基顺序**：①新 gate 源（scheduler+invariants+repair+resume，golden-lock，全 backend 套护栏）→ ②submit 拆答/延迟 designer → ③`dispatchTaskQuestions`（CAS+mint+run-scoped 注入）→ ④读侧 `resolveTriggerForEntry` 改用持久化 `trigger_run_id` → ⑤前端「批量下发」→ ⑥手动问题（§15 H4 schema + `QuestionAuthorForm`）。①是核心 lifecycle、最高风险，须最谨慎。

### 17.1 待编码前核实的「分支时序」难点（勿凭假设，避免又一个 F1）
混合轮（Q1 designer 域 + Q2 questioner 域）里两分支**下发时序不同**：Q2→questioner 续跑**立即**（阻塞-产出型不可改派）、Q1→designer **延迟**。但任务 `status` 单值，「questioner 在跑」与「designer 待人工下发」并存时该如何表达？初步设想：gate 不是「立即 park」而是「**完成门控**」——任务有「未下发 designer 条目」时**不得 finalize/done**；当无其它 ready 工作（questioner 跑完）且仍有未下发条目 → 落 `awaiting_human`。**但这要先核实 cross-clarify 真实拓扑**（questioner 是先于还是后于 designer 跑？RFC-056 里 questioner 审计 designer 产出、designer 经 External Feedback 修订、questioner 再审——若 questioner 等 designer，则延迟 designer 即 park 整链、无并存问题；若并存则需完成门控）。**编码前必须 read 调度 frontier/completion + RFC-056 拓扑确认**，再定 gate 落点。这条不核实就写 = 重蹈 at-submit T7 覆辙。**故 ①（核心 lifecycle gate）的实现须独立聚焦 session、先核拓扑再编码、全程 backend 套护栏。**

## 18. 一键下发 = 前沿 mint + 逐节点队列（2026-06-28，用户纠正并确认）

**前因**：①（run-scoped 注入 + override 执行，6 个未推送 commit）按「**给每个有效目标节点一次性 mint 重跑**」（A、B 同时 mint）实现，经 ~6 轮 Codex gate 反复爆出 double-execution / 顺序 / 消费错配——**根因是 mint-all-upfront 在和调度器抢活**。用户纠正了正确模型，并确认「**只 mint 上游前沿 + 靠新鲜度级联**」。

### 18.1 正确模型
1. **逐节点队列**：问题归属其**承接节点**（designer 题 = `override ?? 图设计者`；self/questioner = 提问节点）。每节点持有一条「已下发未处理」问题队列。问题不绑定到某条特定 run，只是节点队列里的一项。
2. **下发**：选中问题（部分/全部待下发）置 `dispatched_at`（已提交执行）；**只给「受影集合的上游前沿」节点** mint 重跑——前沿 = 受影承接节点里**在 DAG 中无受影祖先**的那些（A 是 B 上游且二者都受影 → 只 mint A；A、C 互不依赖都受影 → 都 mint）；kick 调度器。
3. **级联（复用调度器现有机制）**：前沿 A 跑出 fresh done → `recomputeFreshnessAndDemote`（RFC-074）把下游受影节点 demote + 重派 → B 对 **A 的新输出**重跑。**A、B 绝不同时 mint-to-run**；顺序由调度器新鲜度保证。
4. **注入（逐节点队列）**：节点 X **重跑时**（前沿触发或级联），`buildExternalFeedbackContext(X)` 取「承接节点==X 且 `dispatched_at` 已置、未消费」的问题 → 绑定到本 run（stamp `trigger_run_id`）→ 注入其 Q&A。**绑定发生在节点重跑时，不在 batch-dispatch 时**（级联节点的 run 在 dispatch 时还不存在）。
5. **消费（逐节点）**：X 的 run done+output → 本 run 绑定的问题消费（已处理）。X 再次重跑只取仍未消费的。
6. **read-side 相位**：pending（未 staged 未 dispatched）→ staged（`staged_at` 置、未 dispatched）→ **processing**（`dispatched_at` 置、承接节点将/在重跑）→ **awaiting_confirm**（承接节点 post-dispatch run done+output、问题已消费）→ done（人工确认）。
7. **park gate**：键改为 `dispatched_at IS NULL`（未下发的 designer 问题即 park）——而非旧的 `trigger_run_id IS NULL`。释放 = 选中问题全 `dispatched_at` 置 + resumeTask。
8. **golden-lock**：非 deferred 任务无 `dispatched_at`、无队列 → 即时流 + 图注入 + 消费**逐字不变**。

### 18.2 与现 6 个未推送 commit 的 rework 差异
- `dispatchTaskQuestions`：mint-per-target → **mint-前沿-only**（算受影集合的 DAG 上游前沿；下游交级联）。
- 注入绑定：batch-dispatch 时 stamp → **节点重跑时 stamp**（`buildExternalFeedbackContext` 取队列 + 绑定 + 注入）。
- 加 **`task_questions.dispatched_at`**（已提交执行标记；区别于「绑定到处理 run 的 `trigger_run_id`」）。
- gate / read-side 相位 / 防御 改键 `dispatched_at`。
- **复用不动**：deferred-flag、submit 拆分（答不即时下发）、dispatch-route、reassign-CAS、directive=stop 过滤、多源就绪 barrier（仍在前沿 mint 前校验）。
- 旧「per-target 消费 / C1 graph 排除 / run-scoped trigger_run_id 绑定 batch-dispatch」全部由逐节点队列模型取代——这些正是反复爆 bug 的 mint-all-upfront 残留。

### 18.3 为何这一模型消解了反复爆的 bug
double-execution（A 被图设计者+override 双跑）、顺序（B 抢在 A 前对旧输出跑）、消费错配（图 prompt 排除但消费仍盖）——全部是 mint-all-upfront 绕过调度器导致的。改成「只触发前沿、注入/消费按节点队列、顺序交调度器新鲜度」后，这些都由调度器现有 DAG-order + freshness 天然保证，不再需要 per-target 消费/排除的层层补丁。
