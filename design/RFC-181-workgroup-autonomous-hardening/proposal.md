...
- **D4a A2 翻 on 遣散在途 park（设计门 P0；二审修订）**：false→true 时若有 open clarify park，**单一加锁事务**遣散：clarify_sessions+clarify_rounds 双表 canceled（与答案提交串行化——陈旧答案 409，防竞态 mint 重跑 / crash 半态）+ 中介 park run canceled（勘误：asking run 已 done，park 载体是 `createClarifySession` 的中介 clarify run）+ assignment `awaiting_human→dispatched/open` requeue（**显式扩 `WORKGROUP_ASSIGNMENT_TRANSITIONS` 两条合法边**——现表该态仅可 →running/failed/canceled）+ resume；仅 false→true 触发、无 park 则 no-op、不碰 gate park。复用既有 canceled 语义原语、不新造 cancel。
- **D5 C 注入点 = runNode 收尾分类 + runner 重提示（二审修订）**：`WorkgroupHostRunRequest` 带 `clarifySuppressed?: () => Promise<boolean>` **即时判定器**（envelope 到达时重读任务最新 config——关死"run 起于 false、中途翻 on"的在途竞态窗）；压制在 `runNode` 收尾期分类并**持久化**（宿主 run 行落 `failed` + `errorMessage='clarify-suppressed:<n>'` + 终态广播；不建 session、不 mint 中介 run——若仅 hook 层内存改判，DB 行仍是 done，RFC-182 无从派生「反问已压制」且卡片误导）；**独立前缀 `clarify-suppressed`**（**不复用** `clarify-questions-`——后者 leader 耗尽 `throw` 杀任务，与 C 的 drop-and-continue 收场冲突，必须区分）；leader runner 加 `clarify-suppressed` 重试分支、耗尽 drop-and-continue（不 `throw`，滑入 idle→nudge）；worker runner 在 failed 分支加同前缀重试、耗尽标记 assignment failed；message-turn 靠现有 `!==done→return` 天然 drop。
...
- **A2（设计门 P0；二审修订）**：任务卡在 clarify park（leader/worker 各一）时 PATCH autonomous false→true → **单事务**（双表 canceled + 中介 run canceled + assignment 经新合法边 requeue）+ resume → agent 以压制态重跑推进、任务脱离 `awaiting_human`；并发陈旧答案提交 → 409/幂等拒绝；转移表新边 oracle 全表穷举锁；无 park no-op、true→true no-op、gate park 不被误遣散。
- **C**：`clarifyEnabled=false` 且 host run 发 clarify → `runHostNode` 不 `createClarifySession`、返回 `clarify-suppressed:*`（勘误 2026-07-14：旧文误写 `clarify-questions-suppressed:*`，会撞上 leader 既有 `clarify-questions-` 重试分支的前缀匹配——独立前缀是 design D5 的硬约束），且**宿主 run 行持久落 `failed` + 该前缀 errorMessage + 终态广播**（DB 断言——RFC-182 note 派生依据），run 起于 false、中途翻 on 的在途竞态由判定器即时重读关闭；leader/worker runner 重提示重发；耗尽后 leader drop-and-continue（滑入 idle→nudge，不 `throw`、不 park）、worker 标记 assignment failed（不 park）；压制判定为假（含非全自动）→ 现状不变（clarify 正常 park，不回归 RFC-172/RFC-023 round-trip）。
- **D（二审修订：默认仅 create 作用域）**：create schema 缺省 autonomous=true；新建组（form/API 缺省）autonomous=true；**update/PUT 省略 autonomous＝保留现值**（老 PUT 不得静默翻转已有组，双向回归锁——`workgroupConfigFields` 为 create/update 共享，不得在共享层挂默认）；新建表单 `Switch` 默认 ON、编辑老组显示存储值；**已有组行为不变**（回归锁）；无新 migration（`upgrade-rolling` 计数不变）。
