...
4. **统一指令（用户细化决策）**：cross-clarify 与新路径**共用同一套中性「更新或重新生成」指令**——不再保留 RFC-056 严格「Do NOT regenerate」（决策 D4）。两路径互斥不重复注入。
5. **零回归（行为面）**：首次运行、循环的下一迭代、同会话续跑（inline / followup）、强制反问态均**不**注入。
6. **零 DB migration、零前端、无 i18n**：仅复用既有 `node_runs` + `node_run_outputs` 表；指令是 agent-facing 英文协议文案。

### 非目标

- **不为文件型端口内联文件内容**：文件端口的产物本就是 worktree-相对路径，prior-output 逐字渲染该路径即可（用户决策：「输出是路径就只给路径」）；agent 文件在则读、不在则按指令重生成。**不做「回滚前捕获正文」**（决策 D8，与 RFC-014 sibling 同惯例）。
...
- **AC-1**：评审 reject/iterate 重跑某 agent 节点时，若该节点本轮有过 done 产出（即便旧 run 已被 supersede 成 `canceled`），新 prompt 含 `## Prior Output (to update or regenerate)` + `## Update Directive`（中性「更新或重新生成、吐完整结果」文案）。inline 端口回灌完整正文；文件端口回灌 worktree-相对路径（产物即路径，agent 读它）。**review-iterate 时只回灌迭代目标端口**（兄弟端口归 RFC-014，互补去重，D10）；review-reject 回灌全部端口。
- **AC-2**：手动重试 / 级联 / 恢复 / 反问重跑时，同样满足前提即注入；不满足前提（无上次产物）则不注入。文件端口逐字给路径（不做回滚前捕获，D8）。
...
- **AC-7**：**cross-clarify 触发的设计者重跑**改用统一中性指令（标题 `to update or regenerate`、中性 directive），prior-output **块本身**（`### <port>`+正文）逐字不变；既有 cross-clarify 测试更新到新措辞后全绿；且不与本 RFC 的泛化段落重复注入（互斥）。
...
- 共享：`packages/shared/src/prompt.ts`（`RenderPromptInput` 加字段 + 渲染分支）、`packages/shared/src/clarify.ts`（RFC-056 的 `CROSS_CLARIFY_*` prior-output 常量改名去前缀为中性、值统一，两路径共用，D4）。
