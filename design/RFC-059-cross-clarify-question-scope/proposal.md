...
> 状态：**Ready（2026-05-23，RFC-058 已落 main）**
> Owner：—
...
> **RFC-058 落地后差异**（本次刷新已在 plan.md / design.md 中应用，本 proposal 内容仅在 §2.1 第 3 项 + 关联文档表中体现）：
> - migration 编号 **0031 → 0032**（RFC-058 占了 0031）。
> - 双写目标表：`cross_clarify_sessions` + `clarify_rounds` 都加 `question_scopes_json TEXT NULLABLE` 列；submit dual-write 同步落两张表。
> - 反问者侧入口实际是 `packages/backend/src/services/clarifyRounds.ts:294` 的 `buildPromptContext({ consumerKind: 'cross-questioner', ... })`；遗留 `buildQuestionerCrossClarifyContext` 仅作 PR-A baseline 锚，两条入口都不读 `questionScopesJson`、C3 守门覆盖双入口 grep。
> - DTO `CrossClarifySession` → `ClarifyRound`（带 `kind: 'self'|'cross'` 判别符）；前端 scope UI 挂在 `s.kind === 'cross'` 分支。
...
3. **新列双写**（migration 0032）：`cross_clarify_sessions.question_scopes_json TEXT NULLABLE` + `clarify_rounds.question_scopes_json TEXT NULLABLE`。提交时把 scope 映射序列化为 JSON 同事务写两张表（RFC-058 dual-write 期协议）。NULL = 旧行 / 未指定 → 渲染时全部按 designer 处理（lossless replay）。
...
10. **错误码扩展（仅新增 1 条）**：`cross-clarify-question-scopes-malformed`（fail，HTTP 422 — 落地用 `ValidationError`，codebase 约定 → 422；草稿曾写 400，422 更准）—— submit body 的 questionScopes 字段不是合法 Record 形态 / scope 值不在 enum 内 / 引用了不存在的 questionId。
...
- **A9（schema 校验 fail）**：questionScopes 引用未知 questionId / scope 值非 enum → HTTP 422 + 错误码 `cross-clarify-question-scopes-malformed`（落地用 `ValidationError`；codebase 约定 ValidationError → 422，比 proposal 草稿 400 更准）。
- **A10（self-clarify 路径不受影响）**：RFC-023 self-clarify 节点的 POST answers 路径忽略 questionScopes 字段（不校验、不持久、不影响）；ClarifySession 测试套件零退化。
- **A11（migration 0032 上行可跑）**：drizzle migration test 通过 + 已有 cross_clarify_sessions / clarify_rounds 行新列默认 NULL（双表）。
...
- **B3** backend tests **≥ +12**：
  ...
  - migration 0032 3（上行可跑 + 已有 cross_clarify_sessions 行 NULL 默认 + 已有 clarify_rounds 行 NULL 默认）
