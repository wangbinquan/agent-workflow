...
**合同冻结门**：V1 新 collection/byte/field/ref 限制与现 schema 有差异，不能直接照抄类型再宣称等价。T1 必须输出 source→rendered→current admission→target codec 的字段矩阵，包含默认 scratch、Agent allowClarify、空 inputs 省略、description null、body/external-id、uploads=[]、当前 employee revision 解析时点。先证明或修订每一项预算与计数单位，再冻结 public ledger。超限输入不截断、不丢弃、不静默转新格式；合同修订需独立呈批。

已核实 DE `domain/runtimeModel.ts` 的 intake 下游还有 target value 1,000、body 2 MiB、externalId 500 的字符串长度限制，并非仅由 EC template 上限决定。T1 必须使用整条 admission 链的交集作为既有能力，不能把当前早已被下游拒绝的输入误报为新增收缩，也不能混淆 UTF-16 length、code points 与 UTF-8 bytes。

因此本 Draft 明确的是 ownership、持久化和切换协议；V1 的兼容定稿是 T2 的前置交付物，不是可留到上线后的债。不能在 T1 未结论时注册假的生产 liveness 或让 optional legacy fallback 永久混在两个 ports 中。

## 3. Durable origin / work intent

拟新增 EC-owned `event_automation_work_intents`，同一记录承载 origin 和 intent，避免另造通用 journal。字段：origin ref、delivery id、subscription id、rule id/revision/digest、owner ref、port id、已物化 target-specific payload/格式版本/digest、首次解析目标 ref、provider receipt、状态、claim identity 和时间。`delivery_id` 唯一，`origin + port` 唯一；新 migration 在实施时串行分配。

`attemptCount` 与现 claimedBy/claimExpiresAt 足以识别 delivery 当前尝试时优先复用，禁止另起第二套 worker epoch。EC 内部 claim-scope factory 由现 worker/store 创建不可变 claim ref；新 consumer 仅在该 scope 调用建 intent/写结果。必要的 consumer signature 变更要逐一迁移现 subscriber adapters，不能让它们伪造 claim。
...
provider 需要既有 source provenance 时，由 same-origin 的受约束 participant 在 live tx 中解析只属于该 source 的绑定；不解码 ref 成任意 ID，不另传 caller-controlled delivery 字段。该 participant 由 EC offered、IA/TE/DE adapters 按其 consumer 最小字段使用，精确登记 public surface；不能由 TE 直接读 EC 表。
...
rule `recordResult` 和 delivery settle 应在 EC-owned 事务中同时验证该 claim 与规则版本，避免当前 consumer 写完结果才发现 lease 丢失。涉及非 automation consumers 的通用 delivery 流程只做必要 claim-context 适配，不重写它们的业务结算。
