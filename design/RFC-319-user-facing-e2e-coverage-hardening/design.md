...
### 4.1 判据（实测后改过一次，见 §4.3）

分母派生自 `packages/frontend/src/routes/*.tsx` 的 `createRoute({ path })` 与 `router.tsx`
路由树——**60 条**（比落档时按文件名估的 58 多两条：`router.tsx` 里两条 redirect 路由）。

分子**也是运行期实测**，而且与 R1 共用同一份 journal，不需要额外机制：daemon 的请求日志
里除了 `/api/*`，还有浏览器对 SPA 文档的 GET（`GET /agents/01M0…`）。把这些非 `/api`、
非 `/assets/` 的 GET 归一回路由模式，就得到「哪些路由被真正加载过」。

判据一句话：**默认的 `bun run e2e` 全量跑里，这条路由被真实加载过吗**。

### 4.2 为什么不是「源码里提过」

落档时设想的 R2-static（路由字面量出现在 `e2e/**` 里）实测下来**会把一整族漂绿**：
`/code/*` 十条路由在 e2e 源码里到处都是，但那些出现**全部**落在视觉回归套件里，
而它要 `RUN_VISUAL_REGRESSION=1` 才跑——PR 腿一次都不会执行。这与本 RFC 审计抓到的
`e2e/rfc250-visual-states.spec.ts:530` 那条 skip 是同一件事：**看起来有覆盖，实际不跑**。

所以「提过」被降级为**纯诊断**（账本里的 `mentionedButNeverLoaded` 一栏），
不参与通过判定。它的用处是告诉接手的人该做什么：

| 诊断 | 含义 | 该做什么 |
| --- | --- | --- |
| 提都没提过 | 这个页面从未进过任何人的视野 | 从零写一条 spec |
| 提过却从未加载 | 多半只活在默认不跑的套件里 | 先确认它到底在哪跑，再决定是搬出来还是补新的 |

静态信号仍保留一处**通过判定**用途，但方向相反：`gate:local` 里有一条
「不在账本里的路由，e2e 源码里至少要提到它」——它挡的是**新加页面零痕迹**，
是一条便宜的即时拦截，不必等当晚的全量腿。

### 4.3 已知的保守方向

SPA 的客户端导航（点链接从 `/skills` 进 `/skills/$id`）**不产生服务端请求**，所以只靠
文档加载会把「只被点进去过」的路由报成缺口。这个方向是安全的（多报缺口不会漏掉回归），
而且修法本身有价值：补一次 `goto` 直达断言等于顺带锁住深链接，那本来就是真实用户场景。

精确化仍留给 T25（共享 `test` 入口 + `framenavigated` fixture），但**不再是 R2 成立的前提**
——落档时它是必需项，实测后降级为增强项，因为文档加载这一路已经把 60 条里的 42 条判绿了。

### 4.4 开工时的实测值

- 端点：**462 条声明，191 条被打到，271 条一次都没有**（58.7% 的 API 面零 e2e 流量）。
- 路由：**60 条，42 条被加载过，18 条从未**；18 条里 **14 条**属于「提过却从未加载」。
- 归一器的反方向检查：`/api` 请求里对不上注册表的 **0 条**——462 条声明全部能被具体路径对回。
