...
### 1.3 同族老化判据 `isTargetNodeConsumed` 也 shard 盲

`clarifyRerunLedger.ts:249`：`runs.some(r => r.nodeId === targetNodeId && r.iteration === iteration && ... && r.id >= sinceRunId)`——同样无 shard 过滤。被 `clarifyQueue.ts`（老化）+ `crossClarify.ts` 复用。

> 注意：RFC-172 R2-T3 已给**老化窗口的 `node_runs` 扫描**加了 shard 过滤（clarifyQueue.ts），但那是 aging **window**；`isTargetNodeConsumed` 这条**派生消费**判据仍 shard 盲。两者不是一回事——本 RFC 补的是后者 + `resolveHandlerRun` 家族。

### 1.4 分发门如何最终落到 shard 盲判据
...
### 2.2 `isTargetNodeConsumed` 加可选 `shardKey`

同款追加 `(shardKey === undefined || r.shardKey === shardKey)`。调用方（clarifyQueue aging / crossClarify）在 workgroup member 场景传本 run 的 shard，其余传 `undefined`。
...
### 2.5 S4 — self 回滚守卫 shardKey 透传

RFC-172 S4：retry 回滚把每个重试节点回滚到其 `pre_snapshot`（git stash）。当前走 node 级 `pickFreshestRun`（scheduler-audit-s13 G7 锁 `pickFreshestRun(scoped, {topLevelOnly:false})`）。多 member 并发回滚需按 shard 选 freshest。把 self 回滚路径的 `pickFreshestRun` 入参 `scoped` 加 shard 过滤（`undefined` golden-lock）。改动须同步更新 `scheduler-audit-s13-freshest-fork-source-guards.test.ts` 的 G7 锁断言。
...
- **宽（推荐）**：shard 化 `resolveHandlerRun` + `isTargetNodeConsumed` 家族 + 全调用方。一次可选参数，golden-lock 由 `undefined` 保。读侧一致。
...
- `isTargetNodeConsumed`：`shardKey='A'` 时 sibling B 的 done+output 不老化 A；`undefined` 复现今日。
...
**golden-lock 回归**：`rfc128-p5-bc` / `rfc131` / `rfc132` / `rfc133` / `rfc164` / `cross-clarify-*` 全绿；`scheduler-audit-s13` G7 锁按 S4 改动同步。

**源码锁**：`resolveHandlerRun` 的 `shardKey === undefined || r.shardKey` 追加行；`isTargetNodeConsumed` 同款；分发门 (target,shard) 跳过逻辑。

## 6. 风险

- `resolveHandlerRun` / `isTargetNodeConsumed` 是**读侧核心**，被渲染/老化/消费/门多处复用——改错会波及既有 golden-lock。缓解：可选参数 + `undefined` 默认 = 所有现有调用方零行为变化；逐调用方审计传 `undefined` vs 传 shard。
