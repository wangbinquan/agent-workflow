...
- [ ] `resolveHandlerRun` 传 shardKey 后 sibling 不掩盖消费判据。
- [ ] S4：member B（shard B）open ledger 不挡 member A（shard A）self 回滚。
- [ ] golden-lock：`shardKey===undefined` 既有断言全绿（`clarify-auto-dispatch-*` S4 门含在内）。
- [ ] 四门 + 单二进制 smoke + CI 全绿。
