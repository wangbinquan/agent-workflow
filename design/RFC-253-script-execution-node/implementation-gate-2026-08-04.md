...
## 2. 复核后不成立 —— ⚠ **这一节的两条结论后来都被第二轮推翻，见下方第二轮记录**

> **订正（同日第二轮）**：下面两条我判为「不成立 / 未复现」的结论**都是错的**，而且错因值得记：
> 「依赖安装隔离」那条我的复核**预设了 sandbox ctx 会被传入**，实际调用点一个参数都没传，
> 安装器一层边界都没有；「权限投影」那条我说「未复现、缺可复跑判据」，第二轮直接给出了三条
> 可复跑构造（嵌套 wrapper / exitCondition / 上游内容）。教训：**当评审给不出复现步骤时，
> 正确的动作是自己去把那条路走完，而不是判它未复现**——我当时只在脑子里试了几个构造就收手了。
...
# 第二轮实现门（双路并行，2026-08-04）

第一轮的 Codex 作业被 provider 的安全策略拒绝而失败，只抢救到部分结论。本轮改为**两条独立路径并行**，
交叉比对：

- **Codex**，措辞改为中性的契约一致性评审（核验实现是否满足文档声明的契约、找代码与注释不一致、
  检查不变量在所有路径成立），不再使用「攻击 / 构造绕过 / 沙箱逃逸」等触发安全过滤的词汇。**这次跑通了。**
- **独立子代理**，完整对抗式安全评审（RFC-240 在 Codex 故障当日采用过同样的替代方案）。

两者都钉在 `802741ec..ca11a4d6`、只读、并被告知范围内 `863f5178` 是他人的文档提交。

## 判定：**不通过**。两路各自独立给出不通过，且在核心问题上高度一致。

### 两路一致命中的（交叉验证，可信度最高）

| 缺陷                                                             | 性质                                                                                                                                                                                                                                                                                                      |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`readonly:true` 的边界在 `sandboxMode` off/warn 下根本不存在** | **P0**。第一轮 F2 **只修了渲染器、没修准入**：profile 选型只看 network，`runner-filesystem-v1` 没有 `failClosed`，于是 off/degraded 下 `sandboxActive()` 为 false、`readOnlyWorktrees` 一行都不渲染。净效果与修复前一致——**而这正是我自己在 F1 里已经解决过的同一类问题，却没把对称性用到旁边的开关上**。 |
| **依赖安装器从未进入任何 containment**                           | **P1**。`ensureScriptDepsEnv` 的唯一调用点没传 `sandbox`，pip/npm 以 daemon 身份零边界运行。这同时**推翻了第一轮的驳回理由**——那条「安装器不是 jail」的复核预设了 ctx 被传入。                                                                                                                            |
| **`ScriptNodeSchema` 零调用方**                                  | **P1**。正文/端口数/依赖数三条上限全是死的；依赖数尤其实际——执行器把 specs 全量摊进一条 pip/npm argv。                                                                                                                                                                                                    |
| **node 依赖 `libDir` 指向空目录**                                | **P1**。`npm --prefix` 写 `<dir>/node_modules`，而 `NODE_PATH` 指向 `<root>/lib`。`language:'node'` + `dependencies` 这条组合**从未能工作**（独立评审实跑 npm 证明）。                                                                                                                                    |
| **敏感投影只看直接容器**                                         | **P1**。wrapper 归属是传递的，把脚本的 1 次循环整个塞进 50 次循环即可改变执行次数而投影不变。                                                                                                                                                                                                             |

### Codex 独有命中（9 条，均已核实属实）

最重要的是**又有三条 AC 被写成「已交付」而从未实现**：**AC-27** env 值脱敏（详情/列表直接返回定义、
YAML 直接序列化、前端明文 `TextInput`——一行没写）、**AC-32** 存量 JSON/YAML 输入框迁移（全部仍是
`TextArea`）、**AC-35** 读投影（只写库、DTO 不读，而设计门 P1 要的恰是「光写库不算」）。另有
**AC-4 与 AC-11b 自相矛盾**（一条要 path 端口驱动 fanout、另一条要拒绝 path 端口），是我自己写的两条
验收标准打架。

其余：输出 `kind` 未形成端到端契约（落库不写 kind、前端编辑丢 kind、`markdown_file` 作为 path alias
绕过字面正则）；fanout 归属检查同样只看直接容器；`child-unkillable` 在父进程 exit 0 时被当成功；
取消发生在 retry 循环/准入/依赖安装阶段时的结算不正确；「精确钉版本」仍接受 `requests==2.*` /
`lodash@4`；manifest 不回写解析版本与产物摘要；事件 payload 与 agent 不同形；pid receipt 是
best-effort；AC-5c 的「逐字节」对非法 UTF-8 不成立。

## 处置

**已修 + 回归测试 + 突变实证**：readonly fail-closed 准入（新增 `outer-readonly-v1`）、安装器
containment（按 buildDir 构造专用 ctx，网络照常放行）、strict schema 上线校验器、node `libDir`
按语言分档、单端口截断改 fail-closed（新码 `script-output-truncated`）、投影补全**完整祖先链**与循环
退出项、fanout 归属改走祖先扫描、`child-unkillable` 不再算成功、精确版本正则收紧、timeout 不再覆盖
aborted + 定时器泄漏、解释器探测加超时、`LD_*`/`DYLD_*`/`BASH_ENV`/`ENV` 进运行期保留表、NUL 值检查复活。

**订正为事实**（不实声称的危害大于缺功能）：`containedSpawnRegistry.ts` 从未存在、
`collectScriptDepsEnvs` 零调用方、`CodeEditor` 注释里的迁移声称、以及上述四条 AC。

**未修、已登记 `docs/audit-backlog.md`**：AC-27 脱敏（含明确警告——落地前节点 env 不应放真实密钥）、
AC-32 迁移、AC-35 读投影、AC-33 事件同形、输出 kind 端到端契约、manifest 摘要、取消结算的几处分支、
以及执行分支（scheduler 540 行 + scriptDepsEnv 282 行）与前端组件的测试缺口。

## 对本 RFC 自身的结论

功能可用、主链路正确，但**「实现完成」这个判断此前下早了**。根因不在于漏了某条检查，而在于
**测试的落点系统性偏低**：大量停在纯函数与渲染器层，没有穿过准入与实际执行路径——所以 readonly 的四条
断言全绿，而删掉生产代码里设置该标志的整行也不会红。下一次给这类「开关式安全属性」写测试，判据必须是
**从调用方走到最终生效点**，而不是「渲染函数在给定入参下输出了什么」。
