...
| **T28** | 脱敏：`intentSecretSlots` 新增 `script-node-env` carrier，接进详情/列表/YAML 导出/校验器消息/诊断五个读面；执行期明文                                   | 五面 + 明文各一例                                                               |
...
| **T42** | `intentDoc.ts` supported node forms 补 script（设计门 P2）                                                                                                             | 一例                               |
