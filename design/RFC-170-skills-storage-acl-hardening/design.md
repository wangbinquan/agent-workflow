...
**通用不变量**：① 任何 FS 副作用前，`phase='intent'` 行**已单独 COMMIT**（先意图后 FS）；② 每步 FS 副作用后**单独 COMMIT** 推进 `phase`；③ `phase='db-committed'` 与对 `skills` 的最终 DB 写（swap/delete/version bump）**同一 DB 事务**；④ boot 顺序 = **operations recovery → snapshot verify（§3/G4-5）→ source reconcile → 开 HTTP**（`start.ts` 现 DB@170 / reconcile@263 / HTTP@341，序确可插）；⑤ 恢复完成置 `phase='done'`+`active=0`。

**恢复的完备性定理（G5-1 核心，取代旧「表外即 quarantine」的枚举脆弱性）**：因不变量①②规定每步 FS 副作用**夹在两次 phase-COMMIT 之间**，崩溃时刻只可能落在「某 phase 已 COMMIT、其后续 FS 步骤未开始 / 进行中 / 已完成但下一 phase 未 COMMIT」——故对**记录的 `phase=Pₖ`**，合法 FS 探测集**恰为** `{Pₖ 提交时的 FS 形态} ∪ {推进到 Pₖ₊₁ 过程中/完成的 FS 形态}` 两段，二者由 op-scoped 路径 + 期望指纹唯一消歧。**恢复动作是 phase 的纯函数**：`phase ≤ db-committed` 之前（含）的所有崩溃**一律回滚**（撤 intent/锁、按 op-scoped 备份还原、删 staging——因 DB 权威尚未推进）；`phase ≥ db-committed`（DB 权威已推进）的所有崩溃**一律前滚**（从 `S`/backup/token 快照补齐 live publish、置 done、释放 lease/锁——**绝不回滚已提交的 DB 权威**）。⑥ **仅当 FS 探测同时与 `Pₖ` 两段合法形态都不符**（既非「该 phase 提交态」也非「向下一 phase 推进态」，例如指纹不匹配、canonical 与 phase 声称的世代冲突）才是 impossible-state → 该 skill quarantine。下表按此定理逐 kind 展开为**示例窗**（非穷举清单——任何未列窗均由「phase≤db-committed→回滚 / ≥db-committed→前滚」定理裁决，故不存在「合法窗被漏列而误 quarantine」）：

**每 kind 的 (phase × FS 探测) → 唯一动作**（`C`=canonical `files` 存在，`S/B/K`=op-scoped staged/backup/candidate 存在，指纹须匹配列值否则 quarantine）：

- **reserve**（skillId 全新、无 occupier）：intent → fs-staged(`S` 建好) → fs-published(`S` rename→`C`) → db-committed(`reservation_state='ready'`, 同 DB 事务) → done。
  - intent + ¬C + ¬S：删 reservation 行、撤 intent（回滚）。
  - fs-staged + ¬C + S：删 S、撤 intent（回滚，尚未发布）。
  - fs-staged + C + ¬S：发布已成 phase 未进→**前滚**补 `ready`+done。
  - db-committed + C + ready：清理+done（幂等）。其余组合→quarantine。
- **replace**（G4-3/**G5-4 修正**：`replaceSourceConflict` 的 replacement **永远来自 source candidate**——reconcile 删 occupier 后插 `authority_kind='source-external'`+`externalPath=候选 source path`〔`skill-source.ts:335,520`〕。故**不存在「发布新 managed canonical」的 replace**；occupier 的 `authority_kind` 只决定**如何清理旧占用者**，replacement 一律是 external DB 行。三类 occupier 子机，**全在 intent 事务对 old+new skillId 各插 `skill_operation_locks` 行**〔§10，PK 双锁〕）：
  - **managed occupier**（有 `files` 目录）：intent(+双锁) → fs-staged(occupier `files` rename→`B` 备份、指纹入 `backup_fingerprint`) → db-committed(**同事务**：DELETE occupier row + INSERT external row〔`authority_kind='source-external'`, `externalPath=candidate`, `authority_owner_user_id=source.created_by`〕 + DELETE 双锁) → done(清 `B`)。**不建 `K`、不发布 managed `C`**（旧子机错误已删）。
    - intent/fs-staged + `B`(指纹匹配) + occupier row 仍在 + 无 external row：**回滚** rename `B`→occupier `files`、撤 intent+锁。
    - db-committed + external row 在 + `B` 在：清 `B`+journal（幂等）。其余→quarantine。
  - **source-backed external occupier**（无 `files`、occupier 是 `externalPath` DB 行）：纯 DB——intent(+双锁) → db-committed(**同事务** detach 旧 external 行 + 插新 external 行 + DELETE 双锁) → done。崩在 db-committed 前=无 FS 副作用、撤 intent+锁即回滚；崩后=幂等完成。
  - **hand-imported external occupier**（G5-4 补：无 source parent、`externalPath` 指向用户手选目录）：与 source-backed external 同为纯 DB 子机（detach 旧 hand-external 行 + 插新 external 行），**但**若新 replacement 来自 source candidate 则新行为 `source-external`、若来自另一手选目录则 `hand-external`+`authority_owner_user_id=actor`。旧 hand-external 目录**不删**（用户资产，非平台所建）。恢复同 source-backed external。
- **migrate**（legacy/分叉首采纳，仅 managed）：intent → fs-staged(live `files` rename→`K`、从 token 快照重建 `C`〔legacy 无旧快照者：此步改为「`K` 即权威候选、`C` 暂缺待决策」〕、隔离快照 copy) → db-committed(CAS 推进世代 + `version_state`/`migration_marker`) → done。
  - intent + K 存在 + ¬C：live 已 rename、重建未成→**从 token 快照重建 `C`**（legacy 无旧快照→保持 `K` 为候选、skill 置 pending-decision，不无中生有）。
  - fs-staged + K + C + DB 未推进：**前滚**比较隔离快照与基线→CAS 推进或（有新分叉）登记下一代候选保持 pending。
  - db-committed：幂等完成。其余→quarantine。
- **delete**（全类型，G3-3/G3-7）：intent → fs-staged(`skills/{name}` rename→`.trash/<skillId>-<op_id>`) → db-committed(DELETE `skills` 行，同事务) → done(清 trash)。
  - intent + trash 存在 + ¬C + DB 行在：**前滚**补 DB delete + 清 trash（快照在 trash、必要时先从 token 重建再删）。
  - db-committed + DB 行无 + trash 存在：清 trash+journal（幂等）。其余→quarantine。
  - （source-backed external DELETE 走 detach 非本机；hand-external DELETE 无 trash 目录、纯 DB。）
- **version-write（lease，G4-1 补全；G5-2 补 fusion）**：覆盖 **§2 全六条 managed version writer**——combined-save / file PUT·DELETE / restore / ZIP overwrite / **fusion approval（`fusion.ts:751` `commitSkillVersion`）**——的 staging→DB bump→publish 整周期。**六者由同一 funnel 在 staging 前取 lease、done/回滚后释放**（fusion 完成 DB bump 尚未 publish canonical 时若无 active lease，migrate/delete/replace intent 可移走/删除 skill root、fusion 仍 publish 成功而写入已消失的目录）。intent(建 lease) → fs-staged(从 token 快照建 `S`〔§2/G2-3 非 live〕、施 patch；fusion 的 `S`=融合结果树) → **fs-versioned**(把 `S` 内容 `cpSync` 物化进不可变 `versions/v<target>/files`、fsync 目录、指纹入 `candidate_fingerprint`——G5-1：**版本目录是 `skill_versions` INSERT 的前置物**，现实现顺序 `skillVersion.ts:318` 亦先建 versionDir 再 DB) → db-committed(bump `content_version`/`meta_revision` + 插 `skill_versions` 指向 `versions/v<target>`，同事务) → fs-published(`S` rename→`C`) → done(释放 lease)。
  - intent/fs-staged + DB 未 bump：删 `S`、撤 lease（回滚，提交前失败=可幂等重试，wire 409/422）。
  - **fs-versioned + `versions/v<target>` 已物化(指纹匹配) + DB 未 bump**：版本目录是幂等可重算物——**前滚**补 DB bump+INSERT（指向既有 versionDir）继续，或若 `S` 仍在亦可回滚删两者（皆一致，因 DB 权威未动）。
  - **db-committed + DB 已 bump 到 target_version + ¬C-matches-target**（真实高危窗：DB 提交后崩、live 未发布）：**前滚从 `versions/v<target>/files`（DB 已指向、指纹已录，最可靠源）或 `S`（若在）重建 `C`**、置 done、释放 lease（**绝不回滚已提交版本**，否则版本历史与 live 永久错配）。
  - fs-published + C(指纹=target) ：释放 lease+done（幂等）。**其余按完备性定理裁决**（phase≤db-committed 回滚 / ≥db-committed 前滚），仅指纹冲突→quarantine。
  - lease 期间同 skill 的 delete/replace intent 因 partial-unique(`active=1`)+`skill_operation_locks` 被拒（互斥）。
...
- `skills` ADD `version_state TEXT NOT NULL DEFAULT 'legacy-unbackfilled'`（G2-4/**G3-5 收紧**：`'legacy-unbackfilled'|'snapshot-unverified'|'snapshot-authoritative'|'quarantined'`；本 migration 对**已有 `skill_versions` 行的 managed skill** UPDATE 为 **`'snapshot-unverified'`**〔**不**直接标 authoritative——旧 funnel hash 跳过 symlink，EXISTS 不证明快照可信〕，其余 managed 留 `'legacy-unbackfilled'`；**boot 在开放 HTTP/runtime 前**逐 `'snapshot-unverified'` skill 验证〔content_version 对应行/目录存在 + hash 非 NULL 且匹配 + 整树无 symlink + SKILL.md 合法〕→ 通过才 CAS `'snapshot-authoritative'`，否则 `'quarantined'`/pending；external 行此列忽略。**成本预算（G5-P6，fail-closed 非 warn-continue）**：验证按 skill 逐个 CAS 推进（`version_state` 本身即 durable progress——已验的下次 boot 不重验），单 skill 设**文件数/字节数/wall 上限**〔超限该 skill 直接 `'quarantined'`、不阻塞其余〕；全量验证不设总 wall 门（逐 skill 幂等续跑），但**未验证完毕的 skill 在 `getSkill`/list/scheduler 一律不可见**（G2-5 reservation 过滤同款），故「HTTP 已开、个别巨树 skill 仍在后台验」安全——不会以未验状态注入 runtime）；
...
- `skills` ADD **`authority_owner_user_id TEXT`（G5-5 内容权威 provenance）**：external skill（`source-external`+`hand-external`）落谁**实际控制磁盘内容**（source-external=`skill_sources.created_by`；hand-external=import 时的 `ownerUserId`）。通用 ACL 允许 transfer `owner_user_id`，但 external 的内容永远由原 importer/registrar 的 `externalPath` 控制——`authority_owner_user_id` 与 `owner_user_id` 分离，使「A 控内容、B 控 ACL」的失配**可被检测**（§8 external 禁 owner transfer 是**预防**，此列是**升级前已发生失配的事后侦测锚**）。migration 回填：`source-external`←`skill_sources.created_by`、`hand-external`←现 `owner_user_id`（升级前无 provenance、只能取当前 owner——故失配侦测对 hand-external 是「无法证明一致即 degraded」，见下）；managed 此列 NULL（快照即权威、无外部控制者）；
- **CREATE TABLE `skill_operations`**（G2-2/**G3-4 收紧**两段提交状态机）：`op_id TEXT PK, skill_id TEXT NOT NULL, kind TEXT NOT NULL CHECK(kind IN('reserve','replace','migrate','delete','version-write')), phase TEXT NOT NULL, active INTEGER NOT NULL DEFAULT 1 CHECK(active IN(0,1)), staging_path/backup_path/next_skill_id/candidate_path/candidate_fingerprint/backup_fingerprint/target_version/generation/owner_user_id/created_at`（**G4-1：`version-write` 必须在枚举内否则插 lease 违反 CHECK；`staging_path`/`target_version`/`backup_fingerprint` 为 §6a 探测所需**）；**partial unique index `ON (skill_id) WHERE active=1`**（`skill_id`/`active` NOT NULL 才使 partial unique 成 DB 不变量）；**无 `skills` FK cascade**。**逐 kind 转移表（G3-4）见 §6a**：每 kind 的 phase 序列 + 每崩溃窗恢复动作明确定义；普通版本写「DB 提交后发布 live」期间须持**覆盖 staging→DB→publish 整周期的 lease（= 该 skill 一条 `kind='version-write'` active 行）**、非只在最终事务查一次。存量资源**不补** `skill_operations` 行。
- **CREATE TABLE `skill_operation_locks`（G5-3 双 skillId 互斥的 DB 不变量）**：`locked_skill_id TEXT PRIMARY KEY, op_id TEXT NOT NULL, created_at INTEGER NOT NULL`。**为何单独一张表**：`skill_operations` 每行只有一个 `skill_id`，其 partial-unique 无法锁 replace 需要的**第二个**（`next_skill_id`）——DB swap 后 replace op 仍以 old id 占锁，针对 new id 的 delete/version-write 可插第二条 active op，两者恢复器互相清 backup/发布 canonical 而覆盖。修复：**凡涉及两个 skillId 的 op（replace 的 old+new、reserve-into-occupied 的 name slot）在 intent 事务内对每个受影响 skillId `INSERT` 一行 lock**；PK 冲突（另一 op 已锁其一）→ 该 op 拒绝（409 busy）。锁随 `phase='done'`/回滚在**同事务** `DELETE`。这把「同事务锁 old+new」从**口头约定**变成 **DB 约束**（单表 partial-unique 做不到）。恢复器启动时先按 `op_id` 归属清理孤儿 lock 行（对应 op 已 done/不存在）。
