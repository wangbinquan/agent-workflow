...
- **migrate**（legacy/分叉首采纳，仅 managed；intent 事务插自身 skillId 锁）：intent → fs-staged(live `files` rename→`K` 备份原 live、从 token 快照物化 `C`〔legacy 无旧快照者：`K` 即权威候选、`C` 暂缺〕、隔离快照 copy) → db-committed(CAS 推进世代 + `version_state`/`migration_marker`) → done(释放锁)。
  - **phase < db-committed（intent/fs-staged，`K`/`C` 探测任意）→ rollback**：`rename K→files` 复原**升级前的原始 live**、删 `C`/隔离快照、撤 intent+锁——迁移未提交、原样退回等下次触发（G6-1：不在 pre-DB 投机重建/推进世代）。
  - **phase ≥ db-committed → rollForward**：世代已 CAS 推进 → 确保 `C` 存在（缺则从 token 快照重建）、清 `K`/隔离快照、done；legacy 无旧快照的 pending-decision 分叉在此登记候选保持 pending（幂等）。
...
- `skills` ADD `version_state TEXT NOT NULL DEFAULT 'legacy-unbackfilled'`（G2-4/**G3-5 收紧**：`'legacy-unbackfilled'|'snapshot-unverified'|'snapshot-authoritative'|'quarantined'`；本 migration 对**已有 `skill_versions` 行的 managed skill** UPDATE 为 **`'snapshot-unverified'`**〔**不**直接标 authoritative——旧 funnel hash 跳过 symlink，EXISTS 不证明快照可信〕，其余 managed 留 `'legacy-unbackfilled'`；**两层验证（G6-4 关键——durable 采纳态 ≠ 本 boot 完整性）**：
  ...
  - **每 boot 重验完整性（不可省，G6-4 反例：首验后 snapshot 离线损坏，永久 authoritative 会让下次 boot 仍签 token、用户保存把损坏内容铸成新权威版本）**：**每次 boot 在开放 HTTP/runtime 前，对所有 `snapshot-authoritative` 亦重新 hash 校验 current snapshot**〔目录存在 + hash 匹配 `content_hash` + 整树无 symlink + SKILL.md 合法〕，结果落**内存 `bootVerifiedSet`（boot epoch 作用域，不持久）**；**detail/list/runtime/token-writer 统一 gate 于 `bootVerifiedSet` 命中**——未在本 boot 通过者不可见/不可注入/不可签 token（即便 `version_state='snapshot-authoritative'`）；重验失败→本 boot 视为 quarantined 且 CAS `version_state='quarantined'`。`'snapshot-unverified'` 首采纳深验通过才 CAS `'snapshot-authoritative'`，否则 `'quarantined'`/pending；external 行此列忽略。
  - **成本预算（G5-P6/G6-4，fail-closed 非 warn-continue）**：单 skill 设**文件数/字节数/wall 上限**〔超限该 skill 本 boot 直接 quarantined、不阻塞其余〕；全量重验不设总 wall 门（逐 skill 续跑、命中即入 `bootVerifiedSet`），**durable progress 只表示「本次 boot 已完成该 skill」不跨 boot**；未入本 boot `bootVerifiedSet` 的 skill 在 `getSkill`/list/scheduler/runtime 一律不可见（G2-5 reservation 过滤同款），故「HTTP 已开、个别巨树仍在后台重验」安全——绝不以未验状态注入 runtime；
