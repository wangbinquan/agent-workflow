...
**通用不变量**：① 任何 FS 副作用前，`phase='intent'` 行**已单独 COMMIT**（先意图后 FS）；② 每步 FS 副作用后**单独 COMMIT** 推进 `phase`；③ `phase='db-committed'` 与对 `skills` 的最终 DB 写（swap/delete/version bump）**同一 DB 事务**；④ boot 顺序 = **operations recovery → snapshot verify（§3/G4-5）→ source reconcile → 开 HTTP**（`start.ts` 现 DB@170 / reconcile@263 / HTTP@341，序确可插）；⑤ 恢复完成置 `phase='done'`+`active=0`；⑥ **任何 (phase, FS 探测) 落在下表之外的组合 = impossible-state → 该 skill quarantine**（不猜、不放行）。
...
- **replace**（G4-3 按 occupier `authority_kind` 分两子机）：
  - **managed occupier**：intent → fs-staged(occupier `files` rename→`B`、新树建 `K`) → fs-published(`K` rename→`C`) → db-committed(DB 身份交换 old→new skillId，**同事务锁 old+new 两 skillId 的 active op**) → done(清 B)。
    - intent/fs-staged + B 存在(指纹匹配) + ¬C：**回滚** rename B→`C`、删 K、撤 intent。
    - fs-published + C(指纹=K) + B 存在 + DB 未交换：**前滚**补 DB 交换+清 B。
    - db-committed + C + DB 已交换：清 B+journal（幂等）。其余→quarantine。
  - **source-backed external occupier**：**无 managed 目录可 rename**——occupier 是 DB 行（`externalPath` 指向 source 目录）。子机=纯 DB：intent → db-committed(同事务 detach 旧 external 行 + 插新 external 行 `externalPath=候选 source path`) → done。崩在 db-committed 前=无 FS 副作用、撤 intent 即回滚；崩在后=幂等完成。**锁 old+new skillId**。
...
- **version-write（lease，G4-1 补全）**：覆盖 combined-save/file PUT·DELETE/restore/ZIP overwrite 的 staging→DB bump→publish 整周期。intent(建 lease) → fs-staged(从 token 快照建 `S`〔§2/G2-3 非 live〕、施 patch) → db-committed(bump `content_version`/`meta_revision` + 插 `skill_versions`，同事务) → fs-published(`S` rename→`C`) → done(释放 lease)。
  - intent/fs-staged + DB 未 bump：删 S、撤 lease（回滚，提交前失败=可幂等重试，wire 409/422）。
  - **db-committed + DB 已 bump 到 target_version + ¬C-matches-target**（真实高危窗：DB 提交后崩、live 未发布）：**前滚从 `S`（若在）或从 token 指向的新版本快照重建 `C`**、置 done、释放 lease（**绝不回滚已提交版本**，否则版本历史与 live 永久错配）。
  - fs-published + C(指纹=target) ：释放 lease+done（幂等）。其余→quarantine。
  - lease 期间同 skill 的 delete/replace intent 因 partial-unique(`active=1`) 被拒（互斥）。
...
- `skills` ADD `version_state TEXT NOT NULL DEFAULT 'legacy-unbackfilled'`（G2-4/**G3-5 收紧**：`'legacy-unbackfilled'|'snapshot-unverified'|'snapshot-authoritative'|'quarantined'`；本 migration 对**已有 `skill_versions` 行的 managed skill** UPDATE 为 **`'snapshot-unverified'`**〔**不**直接标 authoritative——旧 funnel hash 跳过 symlink，EXISTS 不证明快照可信〕，其余 managed 留 `'legacy-unbackfilled'`；**boot 在开放 HTTP/runtime 前**逐 `'snapshot-unverified'` skill 验证〔content_version 对应行/目录存在 + hash 非 NULL 且匹配 + 整树无 symlink + SKILL.md 合法〕→ 通过才 CAS `'snapshot-authoritative'`，否则 `'quarantined'`/pending；external 行此列忽略）；
...
撞 `upgrade-rolling` journal 计数锁 **89→90** bump（标题+断言+注释，[reference_migration_bumps_journal_count_test]）。**注**：token/snapshot/reservation/version_state/skill_operations 全套仅服务 managed skill（G2-1）；`acl_revision` 覆盖全六资源全类型。共 **≈15 ALTER + 2 CREATE/index**（六表 acl_revision + fusion token + skills 六新列 + skill_sources 两列 + skill_operations 表 + partial-unique index）。
