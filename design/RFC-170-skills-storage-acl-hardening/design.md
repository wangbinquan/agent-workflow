...
- **migration ×1**（**精确清单以 §10 为准：18 ALTER + 2 CREATE TABLE〔`skill_operations` 六-kind 状态机〔含 `precondition_json`〕 + `skill_operation_locks` 双锁〕 + partial-unique index + 3 backfill UPDATE**；勿再引旧「9/17 ALTER」）：六类 ACL 资源表 `acl_revision` + `fusions.precondition_token` + `skills` 八新列〔`meta_revision`/`migration_marker`/`reservation_state`/`version_state`/`authority_kind`/`source_state`/`origin_source_id`/`authority_owner_user_id`〕 + `skill_sources` 三列〔`lifecycle_state`/`deleted_at`/`source_revision`〕；撞 `upgrade-rolling` journal 计数锁 89→90 bump（[reference_migration_bumps_journal_count_test]）。**全套 token/snapshot/reservation/OCC 语义仅适用 managed skill**（G2-1 三类权威模型：source-backed external=外部 SKILL.md 权威、hand-imported external=DB metadata 权威，二者只受 §7/§7a/§8 保护）。
...
- **adopt-managed（G8-1/G9-1/G10-1，degraded external → managed 采纳，仅对 degraded external；子机全文见 §7a）**：**两阶段 capture→confirm→commit，linearize 到已捕获 candidate（不宣称源静止）**。阶段 A：intent(+自身 skillId 锁 + **`precondition_json` 持久化完整前态**〔skill 全前态+`source_revision`〕) → fs-captured(**descriptor-relative no-follow**〔§7b `openat(O_NOFOLLOW)`、symlink reject〕捕获 external 树进 op-scoped immutable candidate、算 `candidate_fingerprint`) → 返回 `opId`+指纹待用户确认。阶段 B（用户确认 `opId`+`candidateFingerprint` 一致）：fs-versioned(candidate 原子物化 `versions/v1`) → db-committed(重读 ACL + CAS **完整前态**〔含 externalPath/sourceId/originSourceId/`source_revision`〕→ **① INSERT `skill_versions(v1)`**〔`content_version=1`/`content_hash`/`files_path`〕 **② 字段**：`authority_kind:'managed'`/`source_kind:'managed'`/`source_id:NULL`/`external_path:NULL`/canonical `managed_path`/`version_state:'snapshot-authoritative'`/**`authority_owner_user_id:NULL`**/`migration_marker:'migrated'`/description 投影/**bump `meta_revision`**/清 `source_state`) → done(清 candidate+释放锁)。**绝不 rename 外部目录**；迟到 reconcile 由「reconcile 写回也推进 `source_revision` + 条件更新 affected=0 跳过」堵（§7a）。
...
- **adoption 出口（owner/admin 显式、带 `expectedSkillId` OCC）**，二选一——**G8-1：adoption 是新 FS+DB mutation，必须进封闭 inventory + 有崩溃恢复子机，不能只靠 `expectedSkillId`**（它只挡删除重建 ABA、挡不住同 ID 两个 adoption 并发、或 adoption × source reconcile〔`skill-source.ts:507` 可更新 externalPath/删行〕交错）：
  1. **转 managed（推荐默认）= 新 mutation kind `adopt-managed`**（`skill_operations` 第六 kind、§6a 子机、intent 事务插自身 skillId 锁）。**G10-1：linearize 到「已捕获的不可变 candidate」，不宣称源静止**——外部目录是用户可变资产，任何顺序遍历都可能被同步写者制造「旧 SKILL.md × 新支持文件」混合观察（三重 hash 只是 quiescence heuristic、非原子快照证明），故**提交对象是 candidate 副本、不是源的再读**：
     - **两阶段（capture → confirm → commit）**：**阶段 A capture**——intent(+锁 + **持久化完整 precondition 到 `skill_operations.precondition_json`**〔`skillId/sourceState/authorityKind/authorityOwner/externalPath/sourceId/originSourceId` + `skill_sources.source_revision`〕) → fs-captured(**descriptor-relative no-follow**〔§7b 同款 `openat(O_NOFOLLOW)`、遇 symlink reject〕把 `externalPath` 树捕获进 op-scoped immutable candidate、算 `candidate_fingerprint`) → 返回 `opId`+`candidateFingerprint` 给用户；**阶段 B commit**——用户确认「采纳此 candidate」（带 `opId`+`candidateFingerprint`，比对一致否则 409 让其重新 capture）→ fs-versioned(candidate 原子物化 `versions/v1`) → db-committed。**混合树问题消解**：即便 candidate 是某中间态，它是**确定的、用户已确认的**内容，提交对象与源后续变化无关（三重 hash 的「证明源静止」claim 撤销）。
     - **阶段 A→B 窗口（capture 完成、confirm 前）的崩溃/超时/放弃（G10-1 收尾）**：该 op 停在 `phase='fs-captured'`、`active=1`、持 skillId 锁。**pre-db-committed 故走 §6a 二分律 rollback**（删 candidate、释放锁、skill 保持 degraded）；**加 TTL**（`created_at` + 阈值）——boot recovery 与 hourly GC **清理超时未 confirm 的 captured adoption op**（rollback），避免悬挂 candidate 长期占锁挡重试 adoption。confirm 迟到于 TTL 回收后=`opId` 不存在→409 让其重新 capture。
     - **db-committed 同事务的完整原子提交（G10-1 补全 v1 权威行）**：**重读 ACL + CAS 完整前态**〔`expectedSkillId`+`source_state='degraded'`+`externalPath`+`sourceId`+`originSourceId`+`source_revision`〕→ **① INSERT `skill_versions(v1)`**（`content_version=1`、`content_hash`=candidate 指纹、`files_path`=v1 路径——否则得到 `snapshot-authoritative` 却无权威版本行）→ **② 同步全部字段**：`authority_kind:'managed'`、`source_kind:'managed'`、`source_id:NULL`、`external_path:NULL`、canonical `managed_path`、`version_state:'snapshot-authoritative'`、**`authority_owner_user_id:NULL`**（managed 无外部控制者）、`migration_marker:'migrated'`、description 投影、**bump `meta_revision`**、清 `source_state`；任一前态不符→409 → done(清 candidate+释放锁)。
     - **挡迟到 reconcile（G10-1 关键：`source_id=NULL` 只挡新扫描、挡不住已预读的写者）**：**source reconcile 的每个 child 更新/删除在其最终事务内检查 `skill_operation_locks` + 用 `WHERE id=? AND source_id=? AND authority_kind=? AND source_state=?` 条件更新、affected=0 即跳过/409**（现 `skill-source.ts:517` 只 `WHERE skills.id=?`→迟到 reconcile 可覆盖 adoption 结果）；并在 adoption 最终事务**同时 CAS `skill_sources.source_revision`**（推进使已预读旧 revision 的 reconcile 写失效）。
     - 崩溃恢复（§6a 二分律）：`phase<db-committed→rollback`（删 candidate/version、撤锁，external 仍 degraded）；`≥db-committed→rollForward`（确保 v1/canonical 在、done）。**只读捕获、绝不 rename 外部目录**。
...
**修复**：**external 不套 managed 式 boot hash，但 runtime 必须每次运行捕获「安全私有副本」**再交 CLI，而非 symlink mutable external root。**且捕获必须是 descriptor-relative no-follow（G9-2/G10-2 关键——`lstat` 判 regular 后再按 pathname `copyFile` 有经典 TOCTOU：`lstat 判 regular → 源换成指向秘密文件的 symlink → copyFile 按路径打开并跟随 → 副本得到秘密字节的普通文件 → 复扫无 symlink 错误通过`）**——

- **从锚定目录 fd 起、逐层 `openat(dirfd, name, O_NOFOLLOW|O_DIRECTORY)`**（每层组件都 no-follow，杜绝父目录分量被换成 symlink）；文件用 **`openat(dirfd, name, O_NOFOLLOW)` 打开后 `fstat` 确认 regular**，**再从该 fd 复制字节**（复制的是已打开的 inode、非 pathname 重新解析）；**遇 symlink（`O_NOFOLLOW` 触发 `ELOOP`）一律 reject**——该 skill 本次注入 fail-closed。**禁止「先校验 pathname、再按 pathname copy」**（那正是 TOCTOU 窗口）。
- 复制到**任务私有、进程只读**的 staging（每次运行新建）；把**不可变副本**交 CLI（CLI 读副本不读源，消除定稿后源替换）。
- **实现约束**：若 Bun/Node 层不能在 macOS/Linux 可靠提供 `openat`/`O_NOFOLLOW` 语义，用**受控 native helper**；**实现不了就 fail-closed，绝不退回 `lstat + copyFile`**。
- **两 runtime（opencode `driver.ts:118` + claude `config.ts:38` best-effort 路径，全仓仅此二注入 seam）统一走此捕获**；**策略拒绝用独立错误类型、在 Claude best-effort catch 之外重抛**（普通 IO/credential bridge 仍 best-effort，但安全拒绝不得被吞成「跳过 skill 继续」）。

**测试（确定性 seam）**：在「枚举/`lstat` 完成、`open` 前」分别把最终文件与父目录分量替换为逃逸 symlink，断言标记秘密字节**从未进入副本**、且 opencode/claude 都拒绝。代价：external 失去「symlink 实时可见」、改为「每运行捕获当次内容」——共享安全必要取舍（每运行重捕获仍反映最新内容）；managed 不受影响（快照权威 copyDir、本无外部 symlink）。
...
单个 migration 文件（多语句 `--> statement-breakpoint` 分隔，[reference_migration_statement_breakpoint]），**18 ALTER + 2 CREATE TABLE + partial-unique index + 3 backfill UPDATE**（设计门 1–10 轮累计扩定；精确清单见下，勿再引旧「9/17 ALTER」计数）：
...
- **CREATE TABLE `skill_operations`**（G2-2/**G3-4 收紧**两段提交状态机）：`op_id TEXT PK, skill_id TEXT NOT NULL, kind TEXT NOT NULL CHECK(kind IN('reserve','replace','migrate','delete','version-write','adopt-managed')), phase TEXT NOT NULL, active INTEGER NOT NULL DEFAULT 1 CHECK(active IN(0,1)), staging_path/backup_path/next_skill_id/candidate_path/candidate_fingerprint/backup_fingerprint/target_version/generation/owner_user_id/created_at/`**`precondition_json`**（**G10-1：adopt-managed 阶段 A 持久化完整 adoption precondition〔skill 全前态 + `source_revision`〕供阶段 B commit CAS 比对；其它 kind 可 NULL**）`（**G4-1：`version-write` 必须在枚举内否则插 lease 违反 CHECK；`staging_path`/`target_version`/`backup_fingerprint` 为 §6a 探测所需**）；**partial unique index `ON (skill_id) WHERE active=1`**（`skill_id`/`active` NOT NULL 才使 partial unique 成 DB 不变量）；**无 `skills` FK cascade**。**与 `skill_operation_locks` 的关系（G6-2 澄清，非冗余非冲突）**：此 partial-unique 只约束 op **自身 `skill_id`** 至多一条 active 行——它挡不住 replace 的**第二个** id（`next_skill_id`）被别的 op 占用；`skill_operation_locks` 才是**跨 op 的唯一互斥面**（锁全部 affected id）。二者对「op 自身单 id」确有重叠（都保证同 id 无并发 op），但**方向正交**：partial-unique 保证「一个 skill 至多一个 in-flight op 记录」（便于 boot 按 skill 找恢复目标），locks 保证「任一 op 触碰的所有 id 互斥」（含 replace 的 new id）。保留两者：locks 是权威互斥，partial-unique 是 per-skill 恢复索引 + 二次防御。**逐 kind 转移表（G3-4）见 §6a**：每 kind 的 phase 序列 + 每崩溃窗恢复动作明确定义；普通版本写「DB 提交后发布 live」期间须持**覆盖 staging→DB→publish 整周期的 lease（= 该 skill 一条 `kind='version-write'` active 行）**、非只在最终事务查一次。存量资源**不补** `skill_operations` 行。
...
- `skill_sources` ADD **`source_revision INTEGER NOT NULL DEFAULT 0`（G10-1 单调 CAS revision）**：reconcile 对某 child 写回（description/externalPath/updatedAt）**须在最终事务推进本 source 的 `source_revision`**；adoption/rebind 最终事务把发起时读取的 `source_revision` 作为 CAS 前态之一——**迟到 reconcile（已预读旧 revision）在 adoption 提交后写回时 revision 已变、affected=0 跳过**，堵死「adoption 成功后迟到 reconcile 覆盖」（现 `schema.ts:153` 的 `schemaVersion/updatedAt` 非定义化 CAS revision、不可依赖）。
...
撞 `upgrade-rolling` journal 计数锁 **89→90** bump（标题+断言+注释，[reference_migration_bumps_journal_count_test]）。**注**：token/snapshot/reservation/version_state/skill_operations 全套仅服务 managed skill（G2-1）；`acl_revision` 覆盖全六资源全类型。共 **18 ALTER + 2 CREATE TABLE + 1 partial-unique index + 3 backfill UPDATE**：ALTER = 六表 `acl_revision`(6) + `fusions.precondition_token`(1) + skills 八新列(`meta_revision`/`migration_marker`/`reservation_state`/`version_state`/`authority_kind`/`source_state`/`origin_source_id`/`authority_owner_user_id`=8) + skill_sources 三列(`lifecycle_state`/`deleted_at`/`source_revision`=3)；CREATE TABLE = `skill_operations`（含 `precondition_json` 列）+ `skill_operation_locks`；index = `skill_operations` 上 `(skill_id) WHERE active=1` partial-unique；UPDATE = `authority_kind`/`version_state`/`authority_owner_user_id` 三条回填（第三条为 CASE：`source-external`←`skill_sources.created_by`、`hand-external`←`NULL` 且顺带 `source_state='degraded'`，G6-3）。
...
## 23. 170 独立设计门——第 10 轮（Codex adversarial-review，2026-07-13）

verdict **NOT APPROVED：P0=0 / P1=2 / P2=0**。**趋势 10→11→13→13→12→10→6→4→4→2——总量续降至 2、P2 清零、零 P0 连续七轮**。**rebind CAS 含 external_path、adopt 显式写 managed/source/path 字段、CLI 只读副本消除定稿后 TOCTOU、两 runtime seam 覆盖、G6~G8 全核心均判 sound 不重开**。本轮 2 P1 是上轮两折在**最难子问题**上仍有确定反例（adoption 线性化 + capture-time symlink），已按 gate 精确折入：

- **G10-1（P1）adoption 未线性化**：① 三重 hash `H_before==H_staging==H_after` 非原子快照证明（同步写者可让三次遍历都观察同一「旧 SKILL.md × 新支持文件」混合）；② 「持久化完整 precondition」无存储面（`skill_operations` 无 `precondition_json`、`skill_sources` 无单调 revision）；③ 完整 CAS 只挡「reconcile 先提交」、挡不住**迟到 reconcile**（reconcile 预读旧行 → adoption CAS 成功清 source_id → reconcile 迟到 `WHERE id=?` 写回覆盖）；④ adopt 的 db-committed 未建权威 `skill_versions(v1)`（得 `snapshot-authoritative` 却无版本行），漏清 `authority_owner_user_id`、漏 `migration_marker`/desc/`meta_revision`。**折（§7a/§6a/§10）**：改**两阶段 capture→confirm→commit，提交对象是已捕获 candidate 非源再读**（撤「证明源静止」claim）；`skill_operations` 加 `precondition_json`、`skill_sources` 加单调 `source_revision`；**reconcile 写回改条件更新〔`WHERE id+source_id+authority_kind+source_state`〕+推进 source_revision+查 operation lock、affected=0 跳过**；db-committed **INSERT `skill_versions(v1)`** + 补全 `authority_owner_user_id=NULL`/`migration_marker='migrated'`/desc/`meta_revision`。migration 18 ALTER（+`source_revision`）。
- **G10-2（P1，安全）private copy 只闭合捕获后 TOCTOU、未闭合捕获时 symlink swap**：`lstat 判 regular → 源换成 symlink → copyFile 按路径跟随 → 副本得秘密字节 → 复扫无 symlink 错误通过`。**折（§7b/plan T9b）**：捕获契约钉死 **descriptor-relative no-follow**——锚定 fd 逐层 `openat(O_NOFOLLOW|O_DIRECTORY)`、文件 `openat(O_NOFOLLOW)`+`fstat` regular+**从该 fd 复制字节**（非按 pathname 重解析）；禁「先校验 pathname 再按 pathname copy」；Bun/Node 不能可靠提供则用 native helper、**实现不了 fail-closed 不退回 `lstat+copyFile`**；加「枚举/lstat 完成、open 前替换文件及父目录分量」确定性 seam 测试、策略拒绝独立错误类型在 Claude best-effort catch 外重抛。

**P2=0**：全文计数已 canonical 一致（本轮 P1-1 加 `source_revision` 使 17→18 ALTER 已同步 §0/§10/plan/proposal）。

**收敛判读**：**总量 10→2、P1 2、P2 0、零 P0 连续七轮、G6~G8 全核心 sound**——设计已高度成熟。本轮 2 P1 是两个最新机制（adoption / external 捕获）在**最难子问题**上的收口（linearize 到 candidate + descriptor-relative no-follow），均用**行业标准安全模式**（confirm-against-captured / `openat(O_NOFOLLOW)`）、未引入新概念。下轮验收这两收口是否清零 P1 → APPROVED。
