...
**通用不变量**：① 任何 FS 副作用前，`phase='intent'` 行**已单独 COMMIT**（先意图后 FS）；② 每步 FS 副作用后**单独 COMMIT** 推进 `phase`；③ `phase='db-committed'` 与对 `skills` 的最终 DB 写（swap/delete/version bump）**同一 DB 事务**；④ boot 顺序（**G7-P2 定为增量开放，非全量 barrier——否则合法巨树要么被 per-skill wall 上限误 quarantine、要么无限阻塞 HTTP**）：**operations recovery（快、必须先于 HTTP，崩溃恢复）→ source reconcile → 开 HTTP**；**snapshot 完整性重验在开 HTTP 之后后台逐 skill 进行**，初始 `bootVerifiedSet` 为空、随每 skill 验证通过而加入；**所有入口（detail/list/runtime/token-writer/scheduler）严格 gate 于本 boot `bootVerifiedSet` 命中**——未验 skill 一律不可见/不可注入/不可签 token（与未 ready reservation 同款过滤），故「HTTP 已开、巨树仍在后台验」安全、且巨树只是**延迟可用**而非 quarantine 或阻塞启动（`start.ts` 现 DB@170 / reconcile@263 / HTTP@341，ops-recovery 插在 DB 后、verify 挂在 HTTP 后）；⑤ 恢复完成置 `phase='done'`+`active=0`。
...
- **adoption 出口（owner/admin 显式、带 `expectedSkillId` OCC）**，二选一，**状态转换 + ACL 复核 + provenance 写入同一 `dbTxSync`**：
  1. **转 managed（推荐默认）**：捕获当前 external 树为**平台快照**（走 §3 snapshot ingestion：遇 symlink reject、hash、物化进 `versions/v1`）→ `authority_kind='managed'`、断开 `externalPath`/`origin_source_id`、`version_state='snapshot-authoritative'`、清 `source_state`——此后平台快照即权威，与外部目录彻底解耦（最安全、消除「外部可变 target」隐患）；
  2. **rebind 内容权威**：可靠 actor（当前 owner/admin）**显式认领**为内容控制者 → `authority_owner_user_id=actor`、清 `source_state`，保持 external + `externalPath` 不变（承认「我就是控制这个目录的人」）——适用于 owner 确实掌控该外部路径的场景。
...
- `skills` ADD `version_state TEXT NOT NULL DEFAULT 'legacy-unbackfilled'`（G2-4/**G3-5 收紧**：`'legacy-unbackfilled'|'snapshot-unverified'|'snapshot-authoritative'|'quarantined'`；本 migration 对**已有 `skill_versions` 行的 managed skill** UPDATE 为 **`'snapshot-unverified'`**〔**不**直接标 authoritative——旧 funnel hash 跳过 symlink，EXISTS 不证明快照可信〕，其余 managed 留 `'legacy-unbackfilled'`；**两层验证（G6-4 关键——durable 采纳态 ≠ 本 boot 完整性）**：
  ...
  - **每 boot 重验完整性（不可省，G6-4 反例：首验后 snapshot 离线损坏，永久 authoritative 会让下次 boot 仍签 token、用户保存把损坏内容铸成新权威版本）**：**开 HTTP 后后台**对所有 `snapshot-authoritative` 逐 skill 重 hash 校验 current snapshot〔目录存在 + hash 匹配 `content_hash` + 整树无 symlink + SKILL.md 合法〕，通过即加入**内存 `bootVerifiedSet`（boot epoch 作用域，不持久）**；**detail/list/runtime/token-writer/scheduler 统一 gate 于 `bootVerifiedSet` 命中**——未在本 boot 通过者不可见/不可注入/不可签 token（即便 `version_state='snapshot-authoritative'`）；重验**失败（hash 不符/含 symlink）**→CAS `version_state='quarantined'`（真损坏，fail-closed）。`'snapshot-unverified'` 首采纳深验通过才 CAS `'snapshot-authoritative'`，否则 `'quarantined'`/pending；external 行此列忽略。
