...
## 6a. `skill_operations` 逐 kind 转移表 + 崩溃恢复（G3-4）

**通用不变量**：① 任何 FS 副作用前，`skill_operations` 的 `phase='intent'` 行必须**已单独 COMMIT**（`dbTxSync` 不能让 FS 入 DB 事务，故先提交意图再动 FS）；② 每步 FS 副作用后**单独 COMMIT** 推进 `phase`；③ `phase='db-committed'` 与对 `skills` 的最终 DB 写（swap/delete/version bump）**同一 DB 事务**；④ boot **先**恢复全部 `active=1` 的 operations、**再**跑 source reconcile（`start.ts:263` 顺序前置）；⑤ 恢复靠 `op_id`+`kind`+`phase`+`skillId`+ FS 探测（canonical/backup/candidate/trash 是否存在 + 指纹）**逐 kind 判**前滚/回滚，完成置 `phase='done'`+`active=0`。

| kind                       | phase 序列                                                                                                                                                  | 每崩溃窗恢复                                                                                                                                                                                                                                                                             |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **reserve**                | intent → fs-staged（staging 建好）→ fs-published（staging rename→canonical）→ db-committed（`reservation_state='ready'`）→ done                             | intent 崩：删 staging + 撤 intent（name reservation 行一并回滚，因与 intent 同 phase-1 事务）。fs-staged 崩：canonical 不存在→删 staging 回滚。fs-published 崩：canonical 存在但 reservation 非 ready→前滚补 `ready`+完成。                                                              |
| **replace**                | intent → fs-staged（occupier rename→backup、候选 staging 建好）→ fs-published（候选 rename→canonical）→ db-committed（DB 身份交换）→ done（清 backup）      | intent 崩（occupier 已 rename backup 但 phase 仍 intent 的窗）：**探测 backup 存在**→回滚 rename 回 canonical + 撤 intent（故 intent≠「无副作用」，恢复器按 backup 探测而非仅信 phase）。fs-published 崩：canonical=新树、DB 未交换→前滚补 DB 交换。db-committed 崩：清 backup+journal。 |
| **migrate**                | intent → fs-staged（live rename→candidate、从 token 快照重建 live、隔离快照 copy）→ db-committed（CAS 推进世代 + `version_state`/`migration_marker`）→ done | intent 崩（live 已 rename candidate 的窗）：探测 candidate 存在 + live 缺→**从 token 快照重建 live**（legacy 首采纳无旧快照者→留 candidate 待人工，不无中生有）。fs-staged 崩：DB 未推进→前滚比较+CAS 或回滚。                                                                           |
| **delete**                 | intent → fs-staged（`skills/{name}` rename→`.trash/{skillId}-{ulid}`）→ db-committed（DELETE `skills` 行，同事务）→ done（清 trash+journal）                | intent 崩（根目录已进 trash 的窗）：探测 trash 存在 + canonical 缺 + DB 行在→**前滚补 DB delete + 清 trash**（快照在 trash 中、必要时可先从 token 重建再删）。db-committed 崩：清 trash+journal。                                                                                        |
| **version-write**（lease） | 覆盖 staging→DB bump→publish live 整周期的 `active` lease                                                                                                   | 版本写「DB 提交后发布 live」期间持 lease，杜绝并发 delete/replace intent 插入（它们的 phase-1 intent 会因该 skill 已有 active lease 被 partial-unique 拒）；发布失败即修即隔离（§3）后释放 lease。                                                                                       |
...
- `skills` ADD **`authority_kind TEXT NOT NULL`（G3-7 稳定 discriminator）**：`'managed'|'source-external'|'hand-external'`——**不再靠 `sourceId != null` 判**（FK `ON DELETE SET NULL` 会把 orphan 误转 hand-external）；migration 依现 `sourceKind`+`sourceId` 一次性回填（managed→'managed'、external 且 sourceId!=null→'source-external'、external 且 sourceId==null→'hand-external'）；
...
- **CREATE TABLE `skill_operations`**（G2-2/**G3-4 收紧**两段提交状态机）：`op_id TEXT PK, skill_id TEXT NOT NULL, kind TEXT NOT NULL CHECK(kind IN('reserve','replace','migrate','delete')), phase TEXT NOT NULL CHECK(phase IN('intent','fs-staged','fs-published','db-committed','done')), active INTEGER NOT NULL DEFAULT 1 CHECK(active IN(0,1)), backup_path/next_skill_id/candidate_path/candidate_fingerprint/generation/owner_user_id/created_at`；**partial unique index `ON (skill_id) WHERE active=1`**（`skill_id`/`active` NOT NULL 才使 partial unique 成 DB 不变量）；**无 `skills` FK cascade**。**逐 kind 转移表（G3-4）见 §6a**：每 kind 的 phase 序列 + 每崩溃窗恢复动作明确定义；普通版本写「DB 提交后发布 live」期间须持**覆盖 staging→DB→publish 整周期的 lease（= 该 skill 一条 `kind='version-write'` active 行）**、非只在最终事务查一次。存量资源**不补** `skill_operations` 行。
...
**范围再定界结论（3 轮设计门）**：3 轮共 34 findings、且 findings 递增（10→11→13）——证明 R5–R16「推演基线」显著低估了 external 安全/权威模型、source discriminator 稳定性、旧 endpoint 封闭、前端 ACL 管线四个维度。**建议**：① 独立安全漏洞（G3-1 symlink 越界读）**可即刻单修**（不依赖协议）；② 其余按新暴露的四维度**重新定界 scope 后再走后续设计门**，而非在原 scope 上继续折——否则每轮仍会挖出新面。**待用户就 scope 定界拍板后再续。**
