...
**通用不变量**：① 任何 FS 副作用前，`phase='intent'` 行**已单独 COMMIT**（先意图后 FS）；② 每步 FS 副作用后**单独 COMMIT** 推进 `phase`；③ `phase='db-committed'` 与对 `skills` 的最终 DB 写（swap/delete/version bump）**同一 DB 事务**；④ boot 顺序（**G7-P2 定为增量开放，非全量 barrier——否则合法巨树要么被 per-skill wall 上限误 quarantine、要么无限阻塞 HTTP**）：**operations recovery（快、必须先于 HTTP，崩溃恢复）→ source reconcile → 开 HTTP**；**managed snapshot 完整性重验在开 HTTP 之后后台逐 skill 进行**，初始 `bootVerifiedSet` 为空、随每 managed skill 验证通过而加入。**统一可用性判定 `isSkillAvailableThisBoot(skill)`（G8-2，所有 detail/list/runtime/token-writer/scheduler 入口共用此单一 predicate，按 `authority_kind` 分流——不是「一律要 `bootVerifiedSet`」，否则 external 永无生产者、rebind 出口永久不可用）**：
  ...
  - **external（source-external/hand-external，非 degraded）**：**不走 `bootVerifiedSet`**（外部目录非平台快照、不做启动 hash；其安全由**读/注入时的 realpath containment**〔G3-1〕逐次保证）——要求 `source_state` 正常（NULL）+ authority/provenance 合法；
...
- **adopt-managed（G8-1，degraded external → managed 采纳，仅对 degraded external；子机全文见 §7a）**：intent(+自身 skillId 锁) → fs-staged(**只读捕获**用户 external 树进 op-scoped immutable staging、symlink reject/hash/fsync；**绝不 rename 外部目录**——源是用户不可移动资产) → fs-versioned(原子物化 `versions/v1`，同 version-write 协议) → db-committed(重读 ACL + CAS 全前态〔`source_state='degraded'`+`expectedSkillId`〕：`authority_kind:'managed'`、断 `externalPath`/`origin_source_id`、`version_state:'snapshot-authoritative'`、清 `source_state`) → done(清 staging+释放锁)。
...
- **adoption 出口（owner/admin 显式、带 `expectedSkillId` OCC）**，二选一——**G8-1：adoption 是新 FS+DB mutation，必须进封闭 inventory + 有崩溃恢复子机，不能只靠 `expectedSkillId`**（它只挡删除重建 ABA、挡不住同 ID 两个 adoption 并发、或 adoption × source reconcile〔`skill-source.ts:507` 可更新 externalPath/删行〕交错）：
  1. **转 managed（推荐默认）= 新 mutation kind `adopt-managed`**（`skill_operations` 第六 kind、§6a 子机、intent 事务插自身 skillId 锁）：intent(+锁) → fs-staged(**把当前 external 树捕获进 op-scoped immutable staging**、走 §3 ingestion：**遇 symlink reject**、hash、fsync) → fs-versioned(原子物化进 `versions/v1`〔同 version-write 的 op-scoped staged→rename 协议〕) → db-committed(**最终事务重读 ACL + CAS 全部前态**：`authority_kind:'managed'`、断 `externalPath`/`origin_source_id`、`version_state:'snapshot-authoritative'`、清 `source_state`；**CAS 条件含 `source_state='degraded'` + `expectedSkillId`**，任一不符→409) → done(清 staging+释放锁)。崩溃恢复：`phase<db-committed→rollback`（删 staging/version、撤锁，external 仍 degraded）；`≥db-committed→rollForward`（确保 v1/canonical 在、done）。**注：源是用户不可移动的 external 目录，故只读捕获、绝不 rename 外部目录**（区别于 migrate 的 rename 本地 managed live）。
  2. **rebind 内容权威 = 纯 DB 单事务 one-shot CAS**（无 FS 副作用）：CAS 条件 = `expectedSkillId` + **`source_state='degraded'` + 现 `authority_kind`/`authority_owner_user_id` 全前态匹配**（防两 adoption 并发/与 reconcile 交错后覆盖）→ 单事务置 `authority_owner_user_id=actor`、清 `source_state`、保持 external + `externalPath` 不变；前态不符→409。**必须比全前态、非仅 skillId**。
