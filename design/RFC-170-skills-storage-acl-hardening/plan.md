...
- **RFC-170-T1**（同批地基）migration ×1（**精确清单以 design.md §10 为准：18 ALTER + 2 CREATE TABLE + partial-unique index + 3 backfill UPDATE**——六表 `acl_revision` + `fusions.precondition_token` + `skills` 八新列〔`meta_revision`/`migration_marker`/`reservation_state`/`version_state`/`authority_kind`/`source_state`/`origin_source_id`/`authority_owner_user_id`〕 + `skill_sources` 三列〔`lifecycle_state`/`deleted_at`/`source_revision`〕）**+ 专表 `skill_operations`**（**六 kind** reserve/replace/migrate/delete/version-write/**adopt-managed** 的 phase/路径/指纹/generation/owner/**precondition_json**，boot 幂等续跑）**+ 专表 `skill_operation_locks`**（双 skillId 互斥 PK 锁，G5-3）；journal 89→90 计数锁 bump；`--> statement-breakpoint` 分隔。**移入批次 B**——schema 状态与快照权威+reservation 保护同批生效，避免存量 schema 脱节。
...
- **RFC-170-T9b**（同批，G9-2/**G10-2** 安全，§7b）**external runtime 安全捕获**：`stageSkills` 对 external skill **停止直接 symlink mutable external root**（现 `stageSkills.ts:51,56`），改为**每次运行 descriptor-relative no-follow 捕获任务私有只读副本**——从锚定 fd 逐层 `openat(O_NOFOLLOW|O_DIRECTORY)`、文件 `openat(O_NOFOLLOW)`+`fstat` 确认 regular+**从该 fd 复制字节**（非按 pathname 重解析）、遇 symlink `ELOOP` reject；**禁止「lstat 判 regular 后按 pathname copyFile」**（capture-time TOCTOU）；Bun/Node 不能可靠提供则用**受控 native helper**、实现不了 **fail-closed 不退回 `lstat+copyFile`**。两 runtime（`driver.ts:118`/`config.ts:38`，全仓仅此二 seam）统一、策略拒绝**独立错误类型在 Claude best-effort catch 外重抛**。补**确定性 seam 测试**：在「枚举/lstat 完成、open 前」替换最终文件**及父目录分量**为逃逸 symlink，断言标记秘密字节从未进入副本 + opencode/claude 都拒绝。managed 走快照 copyDir 不受影响。**运行时安全必须与写协议同批**。
...
- **RFC-170-T-BOOT**（G3-5/**G7-P2 增量开放**）：boot = ops-recovery → source reconcile → **开 HTTP**，snapshot 重验**挂在 HTTP 之后后台**逐 managed skill 跑（content_version 行/目录 + hash 匹配 + 无 symlink + SKILL.md 合法）→ 通过入内存 `bootVerifiedSet`（仅 managed）、失败 CAS `'quarantined'`；**detail/list/runtime/token-writer/scheduler 统一 gate 于单一 `isSkillAvailableThisBoot` predicate（G8-2，按 authority_kind 分流：managed 要 bootVerifiedSet、external 走 source_state 合法不入 set、degraded 只元信息、post-boot reserve/adopt-managed 提交后即时入集）**；存量一律先 `'snapshot-unverified'`、首采纳深验通过才 `'snapshot-authoritative'`。**不设全量 barrier**（合法巨树 managed 延迟可用、external 立即可用，非 quarantine/阻塞启动）；补「首验后离线损坏→下次 boot 抓住不签 token」+「未验 managed 不可注入」+「正常 source-external/hand-external/rebind 后 external/post-boot create 四条可用性」测试。
...
- **RFC-170-T10b**（G7-2/**G8-1/G8-2**，§7a）**degraded external adoption 状态机**：legacy external（一切 hand-external + owner≠registrar 的 source-external）`source_state='degraded'` 期间从 list/detail/runtime/write **fail-closed**（仅露「待 adoption」元信息）。**adoption 是入册的新 mutation（§5 第 14/15 类）**，二选一：**⑭ `adopt-managed`=新 `skill_operations` kind、两阶段 capture→confirm→commit（G10-1 linearize 到已捕获 candidate、不宣称源静止）**：阶段 A intent(+skillId 锁+`precondition_json` 全前态)→descriptor-relative no-follow 捕获 external 树进 op-scoped immutable candidate〔§7b `openat(O_NOFOLLOW)`/symlink reject〕+算指纹→返 `opId`+指纹；阶段 B（用户确认 `opId`+指纹）→原子物化 `versions/v1`→最终事务重读 ACL+CAS 全前态〔含 `external_path`/`source_id`/`origin_source_id`/`source_revision`〕→**INSERT `skill_versions(v1)`**+同步全字段〔managed/`source_id=NULL`/`external_path=NULL`/`managed_path`/`snapshot-authoritative`/`authority_owner_user_id=NULL`/`migration_marker='migrated'`/desc/bump metaRev/清 source_state〕→done；崩溃走 §6a 二分律、**绝不 rename 外部目录**；**source reconcile 写回改条件更新〔`WHERE id+source_id+authority_kind+source_state`〕+推进 `source_revision`+查 operation lock**（堵迟到 reconcile 覆盖）。**⑮ `rebind`=纯 DB 单事务 CAS 完整前态**〔含 `external_path`〕置 `authority_owner_user_id=actor`。测试：`adopt-managed` 各 phase kill/restart、**managed×rebind 并发只一个提交另一个 409**、**adoption×source reconcile〔更新 externalPath/删行〕reconcile-先/后提交两序都不覆盖**、capture-time 换 symlink 必 reject、**转 managed 后 DB 有权威 `skill_versions(v1)` 行**、**A import→transfer B→upgrade→B 必须 adoption 才可用** 回归、**`isSkillAvailableThisBoot` 四条可用性**、**`resolve(normal)→标 degraded→stage` 两 runtime 交错**。**批次 B 只 gate（degraded fail-closed + predicate），adoption 出口 UI/服务在 C**。
...
- [ ] 6+6+13+14/15 封闭 inventory（含 lazy backfill + adopt-managed/rebind adoption）+ 全写入口 ACL 屏障
