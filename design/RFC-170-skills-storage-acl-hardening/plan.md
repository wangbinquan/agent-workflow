...
- **RFC-170-T1**（同批地基）migration ×1（**精确清单以 design.md §10 为准：17 ALTER + 2 CREATE TABLE + partial-unique index + 3 backfill UPDATE**——六表 `acl_revision` + `fusions.precondition_token` + `skills` 八新列〔`meta_revision`/`migration_marker`/`reservation_state`/`version_state`/`authority_kind`/`source_state`/`origin_source_id`/`authority_owner_user_id`〕 + `skill_sources` 两列〔`lifecycle_state`/`deleted_at`〕）**+ 专表 `skill_operations`**（**六 kind** reserve/replace/migrate/delete/version-write/**adopt-managed** 的 phase/路径/指纹/generation/owner，boot 幂等续跑）**+ 专表 `skill_operation_locks`**（双 skillId 互斥 PK 锁，G5-3）；journal 89→90 计数锁 bump；`--> statement-breakpoint` 分隔。**移入批次 B**——schema 状态与快照权威+reservation 保护同批生效，避免存量 schema 脱节。
...
- **RFC-170-T9b**（同批，G9-2 安全，§7b）**external runtime 安全捕获**：`stageSkills` 对 external skill **停止直接 symlink mutable external root**（现 `stageSkills.ts:51,56`），改为**每次运行 no-follow 遍历捕获任务私有只读副本**（遇 symlink reject/或复制 containment 验证字节 → 完成后再扫 → 交不可变副本给 CLI）；两 runtime 统一、策略拒绝置于 Claude best-effort catch 外。补**两 runtime 的 escaping `SKILL.md`（`{external}/x→/etc/shadow`）+ `resolve→替换 symlink→stage` TOCTOU** 拒注入测试。managed 走快照 copyDir 不受影响。**运行时安全必须与写协议同批**。
...
- **RFC-170-T10b**（G7-2/**G8-1/G8-2**，§7a）**degraded external adoption 状态机**：legacy external（一切 hand-external + owner≠registrar 的 source-external）`source_state='degraded'` 期间从 list/detail/runtime/write **fail-closed**（仅露「待 adoption」元信息）。**adoption 是入册的新 mutation（§5 第 14/15 类）**，二选一：**⑭ `adopt-managed`=新 `skill_operations` kind**（intent+skillId 锁 → 只读捕获 external 树进 op-scoped immutable staging〔symlink reject/hash/fsync〕→ 原子物化 `versions/v1` → 最终事务重读 ACL + CAS 全前态〔`source_state='degraded'`+`expectedSkillId`〕置 managed/断 externalPath/`snapshot-authoritative` → done；崩溃恢复走 §6a 二分律；**绝不 rename 外部目录**）；**⑮ `rebind`=纯 DB 单事务 CAS**（比全前态非仅 skillId，置 `authority_owner_user_id=actor`、保持 external）。测试：`adopt-managed` 各 phase kill/restart、**managed×rebind 并发只一个提交另一个 409**、**adoption×source reconcile(更新 externalPath/删行)交错**、含 symlink 的 external 树转 managed 必 reject、**A import→transfer B→upgrade→B 必须 adoption 才可用** 回归、**`isSkillAvailableThisBoot` 四条可用性**（正常 source-external / 正常 hand-external / rebind 后 external / post-boot adopt-managed）、**`resolve(normal)→标 degraded→stage` 两 runtime 交错**（终端 stage 与 pre-resolve 用同一 predicate、策略拒绝不被 Claude best-effort catch 吞成「跳过继续」）。**批次 B 只 gate（degraded fail-closed + predicate），adoption 出口 UI/服务在 C**。
