...
### RFC-185-T6 — fan-out opt-in 开关（修订，2026-07-14 用户验收反馈）

- 用户验收指出首版把 FAN-OUT 无条件注入改变了原有固定模式 → 按 design.md §D4 改 opt-in：
  migration 0094（`workgroups.fan_out` 默认 false）+ journal/计数锁 93→94 + shared create/update/runtime schema +
  backend CRUD（create `?? false`/update `?? existing`/rowTo）+ launch 冻结 + `renderWgProtocolBlock` 条件注入
  （else 分支=原版逐字节）+ `ConfigPatchSchema`/patch 落库/审计 + 前端 Form/ConfigDialog Switch（lw-only）+
  workgroup-form/workgroup-room lib 全链 + i18n zh/en。
- 测试：opt-in 三态锁（off 无 FAN-OUT 且原 port 收口逐字节/缺省字段=off/on 注入）+ CRUD roundtrip
  （默认 false/显式 true/omitted-preserve）+ launch 冻结 + 前端 draft↔payload 往返 + dialog PUT {fanOut:true} +
  既有 fixture 补 fanOut 字段。
- 依赖：T1-T5（对首版的行为修订）。
