...
**T6 实现门（Codex review，`review --base 015d417a^`）**：1 P1 + 1 P2，全折：

| # | Finding | 折入 |
|---|---|---|
| T6-P1 | opt-in 只 gate 了 prompt——off 组的 leader 硬发同成员多条时 `parseWgAssignmentsPort` 照收、runner 照插，OFF 不是硬保证（正中用户「不能改固定模式」要求；RFC-181 C 硬压制同判例） | 校验层强制：`parseWgAssignmentsPort` 加 `opts.allowSameMemberFanOut`（**default deny**——漏传 flag 的未来调用点 fail closed），off 时同成员重复整 port 拒绝 → 既有 malformed-retry 通道重提示；runner 传 `config.fanOut === true`。引擎级测试：off 组双条同成员 → 重试注入 `duplicate member` → 单条落库，绝不并发 |
| T6-P2 | `persistGate` 用引擎 pass-start 的 `rawConfig` 整 JSON 覆写——中途 per-task PATCH（fanOut/autonomous，后者是**既有**同类竞态）被静默丢弃 | `persistGate` 改 `dbTxSync` 内 reload-and-merge（fresh 行读不到时回退引擎快照）；route PATCH 写点对称改造（fresh-read + 仅覆盖本次 patch 键，同事务）。竞态复现测试：leader turn 在途时 PATCH 翻 fanOut → done 后 gate 落库且 fanOut 存活 |

### 6.2 端到端实测（2026-07-14，生产 daemon + 真实 opencode/glm-5.2）

用户要求实际启动工作组任务验证。三次任务迭代，暴露并修复两个**普适**产品缺陷（弱模型协议遵从问题，非 fan-out 特有），最终全链闭环：

| 任务 | 结果 | 发现 |
|---|---|---|
| `01KXFXVE…` | failed | leader 第一轮 fan-out 意图完美（3 条同成员+自包含 brief+continue），但 member 写成 `"@writer"`（照抄 roster 块的 `- @writer` 展示格式）→ unknown-member 拒 → 重试轮输出崩坏 → fatal。**修复①**：`WgMemberRefSchema` 管道剥 `@` 前缀（member/to 通用，roster/fan-out-dup 校验均在裸名上）+ 协议文案 bare-name 提示 |
| `01KXFYD5…` | failed | 修①后 member 裸名正确，但模型自创 `<wg_output><wg_assignments>` 标签结构 → envelope-missing → **一次就 fatal**（信封缺失不走协议重试，与端口内容错误的重试通道不对称）。**修复②**：`ENVELOPE_RULES` 加字面信封形状示例（反例点名 `<wg_output>`）；**修复③**：`WorkgroupHostRunResult` 透传结构化 `failureCode`（RFC-145 ratchet），leader/assignment 两个 turn driver 对 `envelope-missing` 走 malformed-retry 通道（一次全新 turn + 显式错误提示），不再一击杀任务 |
| `01KXFYSZ…` | **done（2m15s）** | **全链闭环**：leader 第 1 轮一次派 3 单同成员（round 1）→ 3 实例 **137ms 内同时启动**（started_at 互差 <137ms，shardKey 互异）→ 各自独立交付 result → 其中一实例 run envelope-missing 失败被修复③**实战救活**（同 shardKey 重试 run 完成，assignment 未失败）→ 全部终态后 leader 聚合轮唤醒 → 验收 3 文件宣告 done。worktree 产物 alpha/beta/gamma.txt 内容逐字节正确 |

实测同时确认：dispatch 消息逐单公示（免确认语义）、失败隔离、barrier 聚合、`autonomous` 全程零人工介入。
