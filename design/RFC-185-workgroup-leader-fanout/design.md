...
**数据**：零 wire 变更。`runHistory`（RFC-182 G5 单源）已含每成员全部 run 及 status，前端本地派生即可。

**纯函数**（`packages/frontend/src/lib/workgroup-room.ts` 新增）：

```ts
/** RFC-185 — 该成员当前在途（非终态）的 host run 数：fan-out 并发规模。 */
export function countMemberActiveRuns(
  ...
  return runHistory.filter(
    (e) =>
      e.memberId === memberId &&
      (e.status === 'pending' || e.status === 'running' || e.status === 'awaiting_human'),
  ).length
}
```

在途定义 = `pending | running | awaiting_human`（与 presence 四态的非 idle 输入一致；终态 done/failed/canceled/interrupted 不计）。clarify-park 的实例会被正确计入：RFC-182 impl-gate P1 已在 `deriveWorkgroupRunHistory` 投影层把 open-clarify 的 host run（DB 行假 `done`）修正为 `awaiting_human`（`workgroupRoom.ts:172-178,222`），本函数消费的正是该投影后的 status。
