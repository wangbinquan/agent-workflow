...
### B10 落地记录（T60–T64，findings G-06 / FE-01 / FE-02）

- **T60（G-06）**：前端的设计系统约束此前是**逐文件迁移白名单**而不是棘轮——`dialog-grep` 只点名三个文件、`data-table-callsite` 四个、`empty-loading-callsite` 三个路由，于是**新写一个自造的 overlay / 空状态 / 下拉框，一条都不会红**。补三条全前端 AST 规则：原生 `<select>` 禁令（CLAUDE.md 明令禁止，而实测全仓零命中不是因为大家守规矩，是因为**没人拦**）、modal chrome class 只归 Dialog、`.segmented` 家族只能由公共 `<Segmented>` 渲染。三个 allowlist 各带「必须承重」自证——该自证当场报出**三条空豁免**（`Select.tsx` / `MultiSelect.tsx` 是自绘 combobox，那两处 `<select` 在注释里；`Dialog.tsx` 的 overlay 提及也全在注释里），已全部删除，规则因此更严。`__panel` 未纳入 modal chrome 判据：实测它在本仓是通用布局词，纳入会一上来就需要六条豁免。
- **T61（FE-02，能力影响 C8）**：把 RFC-286 F1 的**三名单**换成**不变量**。F1 修的是三个具体名字，守卫也就硬编码了那三个，于是同类 bug 在别处继续存在而 F1 看不见。实测查出五族「有调用、零定义」——渲染出来是无样式裸文本：`page__subtitle`(5)、`form-section__hint`(9，而同族 `__title`/`__body` 都在，不是命名错觉)、`inspector-hint`(7)、**`inspector__readonly`(12，比 finding 点名的任何一个都大，两个修饰符还负责表达「这条只读值处于错误/警告态」)**、`dep-tree__empty`/`__error`(6，其中 `dep-tree__error` 带 `role="alert"`——一条错误提示被渲染成完全无样式的裸文本)。五族已补齐 CSS；判据换成「每个静态 className token 都必须在 CSS 有定义」+ 160 条只减不增的现状快照 + xyflow 库白名单。两个已知边界明写在负向 fixture 里：带插值的 className 整条跳过（拆开会得到 `chip--` 这种半截 token，第一版实测多出七十几个假违规），且只看静态字符串。
- **T62（R9.4）**：`.dialog__overlay:has(> .<特性面板>)` 四处——**方向是反的**：共享原语必须逐一认识每个用它的业务面板，加第五个就要回来改 styles.css，而写新面板的人不会想到。改为 Dialog 新增 `overlayClassName` + 四个通用变体（`--flush` / `--sheet-start` / `--sheet-end` / `--flush-narrow`），由特性自己声明要哪种。`script-code-editor` 那条原本只在 `max-width: 720px` 内生效，故单独给了窄屏变体而不是无条件 flush（否则会改掉桌面行为）。余下四条 `agent-import` 的主题配色覆盖入账：它们不是布局而是**在主题下把公共按钮改色**，收口需要判断这套配色该不该进 `.btn--primary` 本身，删掉会直接改外观。
- **T63（FE-01）**：`JoinModeField` 手搓 `.segmented` 容器 + 两个 `className={... 'is-active'}` 按钮——而 `.segmented .is-active` 在 styles.css 里**根本不存在**（公共原语用的是 `segmented__option--active`，36px 最小高度那条也键在 `segmented__option` 上），于是两个按钮在一个有样式的胶囊容器里完全没样式，还丢了 `role=radiogroup`/`radio`、`aria-checked`、roving tabIndex 与方向键导航。改用公共 `<Segmented>`，`testidPrefix` 正好产出原有的两个 testid，行为测试一行未改。
- **T64（G-06）**：给三个迁移白名单各加头注「本文件不是全前端棘轮，全前端判据在哪」+ coverage 自证（名单只能增不能减、每个条目必须指向真实文件），堵住「删条目消红」这条最容易的退路。`RETROFITTED_ROUTES` 的下限按**今天的真实条数 3** 写死（另六个资源页的空/载入态已搬进共享的 ResourceSplitPage / ResourceGalleryPage，由各自断言覆盖）——目的是「不许减」，不是「必须涨」。
- **T65（视觉基线）**：本批改动会否移动既有 9 个视觉场景待 CI 的 Playwright 任务判定。按仓规，若变红需**人工审 PNG** 后才接受新基线——不在本次自动提交范围内。
