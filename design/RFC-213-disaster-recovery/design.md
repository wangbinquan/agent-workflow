...
- **实现裁定（订正 v2 §9.3 早稿）——不做「首次 resume 跳过 clean」**：早稿曾拟 (a) restore 重建的 worktree 首次 resume 跳过 reset/clean 以保 untracked。落地时判定该改动**语义错误**：它让重跑节点在自己的部分产物上运行，正违背 `rollbackToSnapshot`「给节点干净的 pre-run 起点」的目的（WRITE 节点会双重应用）。**实际实现**：重建的 worktree 供**查看/手工救**；resume 照常正确回滚到 pre_snapshot；用户失活由已落的 **mismatch-protect**（restore 后 `auto_recovery_suspended=1`，挡 auto-resume 静默回滚）兜底。故 G4a = 捕获 + 重建缺失 worktree + suspend，**零 resume 路径改动**。
