...
- **`architecture/` canonical 重采：已完成**（`ca158aa7b`）。PR-4 落地时工作树装着并发 session
  的 RFC-358 在制品，census 走 `readdirSync` 读工作树，重采出来大半是他们的符号。等他们把
  RFC-358 提上去（`877c80e09`）之后，改用**在 HEAD 的只读导出上重采**的姿势做完：
  `git archive HEAD` 出一份干净副本（不是 worktree、不建分支、不动索引），在副本上跑生成器与
  全部架构守卫（446 pass / 0 fail），只把产物拷回来。这样产物只描述已提交的代码，与 CI 算的
  逐字相同，并且**顺带清掉了我在 PR-1 / PR-3 的重采里误带进去的 5 条**别人的符号/边。
  这条姿势本身沉淀进了 `docs/dev-gotchas.md`——本仓 2026-09-04 一天内四笔互相收红的
  `chore(architecture)`，根因都是在共享工作树上重采。
- **AC-3 的规模档：仍未做**，见 `proposal.md §6.1`。
