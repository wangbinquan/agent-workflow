...
| #   | 差异                                                                                                                             | 处置                                                                                                  | 断言                                                                                                                           |
| --- | -------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 1   | `LIKE`：SQLite 对 ASCII 不敏感，PostgreSQL 敏感                                                                                  | `dialect.userSearchLike()` → `LIKE` / `ILIKE`；`escapeLike` 与 `ESCAPE '\'` 两侧不变                  | 真库两侧各跑一次大小写混排搜索；前提侧在 `bun:sqlite` 里可执行钉住（沿用 `rfc349-provider-search-case-parity.test.ts` 的形状） |
| 2   | 标量子查询里的 `COUNT(*)`：PostgreSQL 回 int8 ⇒ 驱动交回字符串                                                                   | `dialect.asInt(expr)` → 恒等 / `(expr)::int`；投影层仍统一 `Number(...)`                              | 真库断言 `typeof row.open_alert_count === 'number'` 与 facets 四个数                                                           |
...
| 9   | 未标类型的参数在比较里可能需要显式 cast（PostgreSQL 的 `$n` 无类型推断兜底时）                                                   | 出现即在 `dialect` 里补 cast；**不预先猜**                                                            | 真库 lane 是唯一能发现它的地方（AC-6 的存在理由）                                                                              |
