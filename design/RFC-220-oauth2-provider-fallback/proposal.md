...
5. **身份字段来源可配置**(userinfo 的格式由 IdP 平台自定,标准字段名不一定存在):
   ...
   - `subjectClaim`:指定从 userinfo 读取 subject(登录身份键)的字段名;留空 =
     标准 `sub`(string-only,无隐式回退——D2 修订);仅作用于 userinfo 路径,
     id_token 路径永远用规范 `sub`(D6,用户 2026-07-22 二次追加)。
...
- **不做通用 per-provider claim 映射配置**。userinfo 按标准 OIDC claims 提取,仅有
  两个显式选择器 `usernameClaim`(D5)/ `subjectClaim`(D6);**没有任何隐式字段
  回退**(D2 修订:自定/数值 id 平台必须显式配 `subjectClaim: 'id'`);更复杂的
  映射需求(如 `email ← mail`)将来另立 RFC。
- **不支持 signed userinfo(application/jwt 响应)**、token introspection;只接受
  JSON userinfo 响应。(原「不支持 UserInfo POST 形态」已被 D8 收窄:支持且仅支持
  `post_json` 一种显式配置的 POST 形态,标准 form-post 等仍在范围外。)
...
| D2 | userinfo 按**标准 claims**提取:认 `sub/email/email_verified/name/preferred_username`;不做通用可配映射。**修订(四轮设计门,D6 落地后)**:原「`sub` 缺失容错取 `id`」的隐式回退**撤销**——逐用户逐次回退是命名空间混用(缺 sub 用户的 id 可撞他人 sub 串号,同一用户形态抖动则重复建号);默认 `sub` string-only 无回退,数值/自定 id 平台用 `subjectClaim: 'id'` 显式表达。 |
...
| D5 | (2026-07-22 中途追加)per-provider 可选 **`usernameClaim`**:指定从身份响应读取用户名的字段名,命中值写入 `claims.preferred_username`。**留空** = 标准 `preferred_username`(string-only 现状);**配置后字段缺失** = null 直接进既有用户名推导链(email 局部 / oidc-sub),**不回读**标准 `preferred_username`(admin 显式指定即权威,静默回读会掩盖配错字段名——五轮设计门统一口径,与 design §5.2 一致)。作用于 id_token 与 userinfo 两条路径。 |
| D6 | (2026-07-22 二次追加;二轮设计门升级为模式开关)per-provider 可选 **`subjectClaim`**:指定从 userinfo 读取 subject(登录身份键)的字段名。与 D5 有意不对称——配置后字段缺失**硬失败不回落**(身份键静默换源会造成账号分裂/串号);留空 = 标准 `sub`(string-only,无任何隐式回退——D2 修订)。**配置即把该 Provider 的身份源锁死为 userinfo**(id_token 即便可验签也不参与身份判定):两路径命名空间混用会让自定字段值命中他人 sub 而以他人身份登录,单命名空间必须由机制而非 UI 告警保证;id_token 路径的规范 `sub` 永不可被配置重定向。**已有身份的 Provider 禁改 subjectClaim**(三轮设计门:存量身份键留在旧命名空间,换源可致重复建号或身份串号;PATCH 变更在存在 user_identities 时 409;确需换源 = 删除并重建 Provider,数据后果明示于 design §2.3);在途 callback 由身份写入时的事务内重验兜底(四/五轮)。 |
...
| D8 | (2026-07-22 交付后追加)per-provider **`userinfoRequestStyle`** 枚举:`get_bearer`(默认,标准 GET + Authorization: Bearer,存量逐字节不变)/ `post_json`(非标准平台风格:POST + JSON body 恰好三成员 `{ client_id, access_token, scope }`,**不带**鉴权头;`scope` 取 Provider 配置的 scopes 原样——用户两拍板)。响应仍为 JSON userinfo 对象,提取语义(D2/D5/D6)不变;migration 0109。 |
| D9 | (2026-07-22 交付后追加)userinfo **200 错误对象识别**:合法 JSON 对象携带非空 `error` 或非零 `errorCode` 时,fetch 层直接 `userinfo-fetch-failed` 并携带 IdP 的错误串(附 `error_description`/`errorMessage`/`message`),不交给 claims 提取(否则塌成费解的 missing-subject)。零值/空串/null 视为成功包装(`{errorCode:0,...}` 惯例),照常提取。 |
...
- **AC-6 trustEmailVerified**:开启后 email 视为已验证(两路径),invite/allowlist
  可放行且 identity.emailVerified 落库为 true——覆盖**新建、绑定、存量登录**三形
  (存量身份在 login 分支比对同步,六轮设计门);默认关,行为与现状一致。
...
- **AC-9 身份字段选择器**:
  ...
  - subjectClaim:配置后 userinfo 路径 subject 取自该字段;字段缺失/类型不符 →
    确定性 400,**不回落**;留空 = 标准 `sub`(string-only,无 id 回退,D2 修订);
    id_token 路径不受影响;大数(非安全整数)值拒绝;在途 callback 撞上配置变更 →
    400 重登(写入时重验)。
  - 两者非法字段名在 create/patch 即被拒。
- **AC-11 userinfo 请求方式(D8)**:`post_json` 模式发 POST + JSON 三成员且无
  Authorization 头(路由级全链锁:假 IdP 仅接受该形态);默认 `get_bearer` 与现状
  逐字节一致;枚举外取值 create/patch 被拒;前端 Segmented 可切换并回填。
