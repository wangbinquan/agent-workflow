...
| D6 | (2026-07-22 二次追加)per-provider 可选 **`subjectClaim`**:指定从 userinfo 读取 subject(登录身份键)的字段名。与 D5 有意不对称——配置后字段缺失**硬失败不回落**(身份键静默换源会造成账号分裂/串号);留空走 D2 默认(sub→id);**仅 userinfo 路径**,id_token 路径永远用规范 `sub`(防身份键被重定向到可变字段)。 |
| D7 | (2026-07-22 三次追加)`usernameClaim` 升级为**空格分隔字段名列表**,取值按序空格拼接(缺字段跳过、全缺回落推导链、256 截断);并新增**呈现名跟随刷新**:`user_identities.preferred_snapshot` 记录 IdP 侧拼接值快照,登录时(以 subject 定位)快照有值且与新值不同 → 刷新 `users.displayName`;快照初见只落库不覆盖(保护存量/邀请/关联用户的既有呈现名);仅 usernameClaim 配置时启用,未配置行为与现状逐字节一致。 |
...
- **AC-10 呈现名刷新**:usernameClaim 配置的 Provider,登录时拼接值与身份快照不同
  → `users.displayName` 刷新且快照更新;快照相同或 IdP 值缺失 → 不动;快照初见
  (存量身份/邀请绑定/账号关联)只落快照不覆盖既有呈现名;未配置 usernameClaim 的
  Provider 永不刷新(回归锁)。
