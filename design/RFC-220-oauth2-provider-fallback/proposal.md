...
5. **用户名字段可配置**:新增 per-provider 可选 `usernameClaim`,指定从身份响应
   (id_token payload / userinfo JSON)中读取用户名的字段名(如 `login` /
   `username`);留空 = 标准 `preferred_username`(D5,用户 2026-07-22 中途追加)。

## 3. 非目标

- **不做通用 per-provider claim 映射配置**。userinfo 按标准 OIDC claims 提取,仅内置
  `sub ← id` 单点容错(D2)与用户名单字段选择器 `usernameClaim`(D5);更复杂的映射
  需求(如 `email ← mail`)将来另立 RFC。
...
- **US-1(纯 OAuth2 IdP)**:管理员在 Settings → Authentication 新建 Provider:填
  ...
  填 `login`(该 IdP 的 userinfo 用 `login` 承载用户名),provisioning 选 invite。
...
6. 其余路径(discovery 健康 + 完整 OIDC + 未配手动字段)**逐字节等价**。

## 7. 验收标准

- **AC-1 Schema/API**:4 个端点字段 + `trustEmailVerified` + `usernameClaim` 可创建
  / PATCH / 读取(redacted 输出含新字段);全部可选,不传等价于 null/false;旧客户端
  不带新字段的 create/patch 请求继续可用;端点字段拒绝非 http(s) 协议 URL,
  `usernameClaim` 拒绝非法字段名(含 `__proto__` 等原型链毒键)。
- **AC-2 逐字段合并**:discovery 成功 → 其字段优先、缺字段用手动补;discovery 失败
  → 全用手动;有效集仍缺所需端点时报确定性错误码(见 design §10 失败模式表)。
- **AC-3 无 id_token 全链**:token 响应仅有 access_token 时,以 Bearer 调
  userinfoEndpoint,按 D2 提取 claims;login / 自动建号 / 绑定邀请 / 身份关联全链
  可用。
...
- **AC-5 D4 矩阵**:id_token 存在 + JWKS 配置不可得 + userinfo 可用 → userinfo 路径
  成功;+ userinfo 不可用 → 400 `jwks-unavailable`。
- **AC-6 trustEmailVerified**:开启后 email 视为已验证(两路径),invite/allowlist
  可放行且 identity.emailVerified 落库为 true;默认关,行为与现状一致。
- **AC-7 诊断与前端**:/test 报告 discovery 结果、每字段有效值与来源、loginReady
  结论;表单新增「手动端点」组(4 URL)与 trustEmailVerified 开关,复用公共表单原语;
  登录失败页对新错误码有友好文案;zh/en i18n 齐全。
- **AC-8 迁移与兼容**:migration 向后兼容(6 个可空/带默认新列);journal `when`
  单调递增;upgrade-rolling 计数锁同步 bump;全量后端测试绿。
- **AC-9 usernameClaim**:配置后 id_token 与 userinfo 两路径都按该字段取用户名并
  驱动自动建号的用户名推导;字段缺失时回落既有推导链;留空行为与现状逐字节一致;
  非法字段名在 create/patch 即被拒。
