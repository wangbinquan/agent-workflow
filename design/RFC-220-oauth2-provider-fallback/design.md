...
- `patch`:7 个 `if (body.x !== undefined)` 分支(nullable 字段 null 直写=清空)。
  ...
  **写入时重验**(设计门四轮 P1,补 PATCH 锁的 TOCTOU 缺口):在途 callback 以旧
  subjectClaim 取得 claims、PATCH 在其 identity 落库前通过(表内尚无行、锁放行),
  callback 随后按旧命名空间写入——PATCH 侧锁挡不住。因此 identity **写入点**
  (createIdentity 的三个调用路径)在 dbTxSync 事务内重读 provider 行,
  `subjectClaim` 与 callback 快照不一致 → 中止并 `friendly('provider-config-changed')`
  400(用户重登即走新配置);两道闸共同保证任一时刻落库的 subject 都属于当前
  命名空间。
...
interface ProbeResult {
   ...
   * - subjectClaim 配置(D6 模式)⇒ 必须 userinfo(该模式 jwks 不是身份通道,
   *   报 ready 而回调必抛 userinfo-unavailable 是假诊断——三轮 P2);
