...
- 失败结果:`Map<issuerUrl, {error, fetchedAt}>`,TTL 5min(`NEG_TTL_MS`)。
  **负缓存仅对「手动端点足以继续登录」的 Provider 生效**(设计门二轮 P1 勘正):
  读取时若当前 provider 的 manual authorization+token 不齐,负缓存按 miss 处理、
  照常重新探测——否则一次瞬时 discovery 故障会让**无手动端点的标准 OIDC IdP**
  在 IdP 已恢复后仍被压制 5 分钟(现状是每请求重试、自然恢复;负缓存不得把降级
  机制变成宕机放大器)。判定放读取侧而非写入侧:provider 的手动字段可能在窗口内
  被 PATCH,读取时以当下配置为准。
...
- **三方比对,不与 displayName 直比**:`user_identities.preferred_snapshot` 记录
  ...
  - `snapshot !== cur && cur === ''`(IdP 值消失)→ 快照 = `''`,displayName
    **不动**(值缺失不清名)。
- **原子性**(设计门二轮 P2):displayName、快照、`users.updatedAt` 三写在**同一
  db.transaction** 内——快照先落而 displayName 未落的崩溃窗口会让下次登录
  `snapshot === cur` 永久跳过刷新。
...
- **S3 resolveEndpoints**:D1 矩阵(全 discovery / 部分文档补位 / 整体失败全 manual /
  ...
  过期后重探)+ **条件化两 case**——manual authz+token 齐:窗口内直走 manual 不
  探测;manual 不齐:负缓存视为 miss、每次照常重探(P1 宕机放大锁);超时路径
...
- **S14 拼接与呈现名刷新(D7)**:`ClaimNameListSchema` 拒双空格/首尾空格/毒键
  ...
  不同且 cur 非空刷 displayName+快照(**同事务**,断言两写一致)、**快照 null 只落
  快照不刷**(存量保护锁)、**建号时字段缺失(快照 `''`)→ IdP 次登出值 → 正常
