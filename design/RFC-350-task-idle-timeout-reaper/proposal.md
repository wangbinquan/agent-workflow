...
- US-1 平台管理员：我把「不活跃超时」打开设成 24 小时。某个任务的 agent 在凌晨哑掉、进程还在但不再产出，
  第二天同一时刻它被自动取消，详情页写着「因超过 24 小时无任何活动被平台自动终结（静默 25.3 小时）」，
  而不是留一行永远转圈的 `running`。
- US-2 平台管理员：一个评审停在 `awaiting_review` 没人管。24 小时后它被收走；我在任务详情页的「恢复」区
  看到一条收割记录，知道是平台干的、什么时候、因为静默多久。
- US-3 平台管理员：我同时开了「不活跃超时」和「终态任务自动归档（90 天）」。僵尸先在 24 小时后变
...
- AC-11：一棵全 `interrupted` 的树，在 `taskArchive.enabled=true` 且过了 `retentionDays` 后会被正常归档出库
  （今天不会）。
- AC-12：`idleHours` 的可配范围为 1–8760 小时；设置页与后端保存门同源校验，越界拒绝。
