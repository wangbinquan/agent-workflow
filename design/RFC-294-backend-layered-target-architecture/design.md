...
箭头统一表示“左侧源码只可依赖右侧受控 `public/*` entrypoint”。`platform` 不在业务 DAG 中：各模块只依赖自己拥有的
platform-facing port，bootstrap 再注入 platform 实现。
...
- 跨异步边界只从 `ctx.authority.subjectRef` 持久化 `actorRef`、operation/correlation/causation 与授权所需 resource refs，不把宽泛权限快照当
  新授权。不同异步面必须保持当前能力语义，不得泛化成“全部重建原用户”或“全部 SystemActor”：
...
| backend 值级 SCC                                                                                                 |                                  5 |                          0 |
| 全仓值级 SCC                                                                                                     |                                  7 |                          0 |
| `KNOWN_VIOLATIONS`                                                                                               |                                 36 |                          0 |
| route→DB 值级文件                                                                                                |                                 15 |                          0 |
| route/MCP 反向 import `server.AppDeps`                                                                           |                                 45 |                          0 |
| production ambient wiring seam                                                                                   |                                  5 |                          0 |
...
| 跨 context internal import                                                                                       |                             未设门 |                          0 |
