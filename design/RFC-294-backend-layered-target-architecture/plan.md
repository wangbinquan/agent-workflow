...
| W9   | partial vertical slice / no exit            | RFC-322 收编 14 个 hourly phase；2026-09-20 background=333/ambient=494（见 `status.md`）；RFC-359 已统一 boot/shutdown；DA/DE/Event Center 等 worker 仍散落，ManagedBackgroundRegistry/readiness/stop receipt 未落                                                                                                                                                                                                                                                                                                  |
...
| N22 | [RFC-363 Task/SC 生产切换](../RFC-363-task-source-control-launch-cutover/proposal.md)（Draft） | durable source/group/revision、两 launch lane、reader 与该纵切 B/D | 尚未实施；不关闭完整 E1/W5 |
| N23 | [RFC-364 MCP diagnostics](../RFC-364-mcp-runtime-diagnostics-cutover/proposal.md)（Draft） | E6 编排/effects/public/roots，复用现持久化与 lease | C + E4b 已满足；未实施，不给 E6/W9 credit |
| N24 | [RFC-365 Event target providers](../RFC-365-event-automation-target-providers/proposal.md)（Draft） | 两 target SPI、origin/claim/receipt 与旧链去重 | 先裁决输入预算兼容；未实施，不关闭 E9 |
...
目前仍非 production seam，durable source/group/revision 和真实 provider 接线由 RFC-363 Draft 承接，不得谎称 W1 已完成迁位。
Task-owned `TaskWorkspaceReadPort` + SC offered `WorkspaceContentParticipant` 也已由 RFC-362 声明，生产 reader/分页/字节
与 W4-B task worktree route 切换留 RFC-363；
...
- [ ] **E1 task launch admission**（RFC-362 Done 仅合同准备完成；RFC-363 Draft 承接启动/工作区生产纵切，完整 E1 仍须逐项退出）：消费 W2-B 已获批并稳定的 task admission 合同，不在 W4 偷改 RFC-287 行为。
...
- [ ] **E6 resource-specific tools**（RFC-364 Draft）：MCP runtime test 归 `resource-catalog/mcp/application/diagnostics`；落
