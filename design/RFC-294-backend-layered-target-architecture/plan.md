...
> **当前入口（2026-09-20）**：RFC-359 已完成；以新增 §1.2 的 current-source 对账与 §3.2 后继队列为准。
> §1 的旧 measurement seed / 2026-08-30 数字保留作历史证据，不是当前债务额度。当前机器数据见 `status.md`。
...
| W4   | A/C/E0/E2/E3/E4a/E4b/E6/E7 Done；其余未关闭        | RFC-344 关 W4-A、RFC-345 关 W4-C、RFC-346 关 W4-E7、RFC-347 关 W4-E0（W4-C 于 2026-09-02 完成 T9/T10，取证见其 `plan.md §7.6`）。**route→DB 与 AppDeps consumer 在该快照均为 0**（2026-09-02 `current-report.json`；由 RFC-349 的 application-owned DB ports 顺带打掉，不等于 W4-B/W4-D 的正式退出，其 facade 22/exception 280 仍在）；**RFC-352 关 W4-E2（2026-09-03）、RFC-353 关 W4-E3（2026-09-04，见 N17）、RFC-355 关 W4-E4a（2026-09-04，见 N18）**；RFC-360 已关 E4b 与本域 B/D；RFC-361 仅关 EC provider，RFC-362 仅关 Task/SC 合同准备。RFC-364 关闭 E6 与 MCP diagnostics 的 B/D；RFC-363 只关闭启动/工作区纵切。RFC-365 已关闭 Event target 子切片（`4796407fa`，CI run `35610192882` 46/46）；W4-B、W4-D 余项与 E1/E5/E8、Reaction/完整 E9、E10 仍开放 |
...
| W6   | scenario 双库合一 / lifecycle 未统一        | RFC-310 development effect journal 属领域恢复，不是共享 AtomicApply；RFC-359 已统一各场景 provider 实现，但 Bundle/Intent lifecycle 仍为 2                                                                                                                                                                                                                                                                                                                                                                          |
| W7   | frame 与 single mint 已落 / identity 未完成 | RFC-306 consumed/skipped、RFC-354 frame/scope、RFC-359 唯一 nodeRun mint 与既有 provenance 已落；NodeRun v2 seq/identity/backfill/readers 尚未落                                                                                                                                                                                                                                                                                                                                                                    |
| W8   | deferred / optional                         | RFC-289 旧设计已关闭；只有 W7 后新号 fanout RFC 获独立批准才进入，不阻塞核心 W9                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| W9   | partial vertical slice / no exit            | RFC-322 收编 14 个 hourly phase；2026-09-20 background=332/ambient=494（见 `status.md`）；RFC-359 已统一 boot/shutdown；DA/DE/Event Center 等 worker 仍散落，ManagedBackgroundRegistry/readiness/stop receipt 未落                                                                                                                                                                                                                                                                                                  |
...
### 1.2 RFC-359 完成后的对账（2026-09-20）

核对基线 `9ba159a7f3b1688806e54f374ab30e2aca1a4bff` 已包含 RFC-359 收口
`55676871ccb3afc4bfe54e759652ce325d401a91`。Main CI `35419594466` 终态 success、46/46；同 SHA 的 visual、
evidence-soak、git-protocols、integration-opencode、webkit、windows-platform、full-e2e、maintenance-soak 八条 schedule 均 success。
RFC-359 的 full PostgreSQL evidence `35389688674` 在祖先 `e7831c498ffc550e78f5093c8b03b81d4ee21410` 上成功，
属于该 RFC 的既有证据，不冒充本次 SHA 的重跑。后续文档提交与此功能基线分开报告。

| 已落事实 / 源码                                                                                                        | 对 RFC-294 的抵扣                                                 | 仍需完成                                                                                     |
| ---------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `platform/persistence/databaseTransaction.ts` / `transactionProgram.ts`                                                | 新合同可用同一 async transaction；不再等待 SQLite/PG 事务分叉修复 | capability-scoped participant 与跨域 owner 仍按各 wave 切换；旧同步包装器仍禁 async callback |
| `modules/task-execution/infrastructure/taskRouteLaunchOperations.ts` / `taskRouteOperations.ts`                        | launch 与 route operations 已共用算法；E1 的 RFC-359 协调等待解除 | SC source/snapshot/preparation 的生产接入、legacy helpers、adapters 与 root 收缩                   |
| `modules/task-execution/infrastructure/nodeRunMintParticipant.ts`                                                      | node_runs INSERT 已为 1；W7 single-mint 前置已有实现              | seq/generation、identityVersion/role、backfill 与 freshness readers 尚未完成                 |
| `modules/intent/infrastructure/intentApplyEngine.ts` 与 `platform/persistence/postgresqlResourcePackageAtomicApply.ts` | 每个场景各自已经双库合一，后者名称保留历史 provider 字样          | Intent / ResourcePackage 两套 scenario lifecycle 仍须 W6 合成唯一 engine                     |
| `cli/daemonProviderBootstrap.ts` / `cli/start.ts`                                                                      | daemon boot/listener/shutdown 已共用流程                          | W9 DaemonContainer、managed background registry、ambient/global owner 未退出                 |
| RFC-354 frame/scope 与 RFC-359 nodeRun program                                                                         | W7 不再从空白设计 frame 或第二套 mint                             | freshness 仍有 ULID 比较；不得把 wrapper frame 记为 v2 identity Done                         |

源码路径均相对 `packages/backend/src/`。`IntentApplyResourceParticipantInTx` 已退役（RC public participant 注释有记录），
后继 W6 从当前 Intent resource bindings/scenario transaction 抽合同，不恢复旧接口或旧 SQLite engine。

当前 generated source digest 为 `sha256:de52245214be0dbf3fe6d0de6892a954c1828a8ae9ff14e16355f271e48d4812`：
backend/repo SCC=1/3、KNOWN=8、route→DB=0、transport→DB=0、AppDeps consumer=0；ambient=494、background=333；
facade/service entries=295（246 legacy implementation、49 thin facade），architecture exceptions=4630。
相对本轮实施前 4562 净增 68（精确新增/退役见共同验收记录及 canonical）；模块归位后根/机制边继续显式计数，不能把 owned ID 退役写成全局债务归零。
22 个 RFC362 public 合同与 1 个 required port 为待接入声明；这些分母不等于运行时 liveness，0 条 route→DB 也不等于 W4-B/D 关闭。

**owner 对账待办**：W4-E1 桶当前仍含 23 个按 fallback 归 TE 的 route 文件。例：
`owner:packages/backend/src/routes/config.ts#$file`、`owner:packages/backend/src/routes/health.ts#$file`、
`owner:packages/backend/src/routes/docs.ts#$file`、`owner:packages/backend/src/routes/databaseMigrations.ts#$file`。
...
E8 等 task/SC/integration exact seams。W5/6/7/9 的正式退出条件保持，W8 仍可选。
...
- 真实 SQLite/PostgreSQL fresh/upgrade 与 schema admission 验证，并覆盖适用的 Linux/macOS 平台。
...
RFC-345 已 Done（2026-09-02，见 N14）；W4-B、W4-E 其余 slices 与完整 W4-D 仍须另立 RFC 并单独获批。
...
| N16  | RFC-352 / W4-E2（Done）                 | 注入 / 蒸馏 / 调度 / 源上下文四块实现迁进 `modules/memory`，`canView`/`canManage` 收成 `domain/scopeAuthorization.ts` 单一判据，SC offered `RepositoryScopeAuthorizationInTx` 薄 participant 落地，列表 keyset 分页下推；facade 8 → 2（余 2 条按 owner 转交），W4-E2 exact 边 `67 → 43`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 链 `49b714d89` → `9954be7bd` → `e48508ee4` → `0dc1662ec` → `2bb6dfcc0` → `e211f1499` → `eb8b331db` → `1ab271af2`；T9 把 R4 判据由 EXACT 入口改为 `isPublishedSurface`（W4-E2 `54→35`、W4-E3 `2→13`）；T10 又把 `routes/memories.ts` 从兜底的 W4-E1 纠回 W4-E2（`35→43`、W4-E1 `846→838`，见 §14）；最终验收 `b3883154e` 的 Main CI `33722386454` 35/35 attempt-1 success                                                                                                                                                                                                       |
| N17  | RFC-353 / W4-E3（Done）                 | `modules/knowledge-evolution` 从零建成 domain/application/infrastructure/inbound/public/composition 六层：fusion 编排与双 provider 持久化整体归位；融合适配器对 `memories` / `skills` / `skill_versions` 的**写入归零**（各经 owner 的 tx-bound participant，判据收成一份）；技能回滚的成员关系判据自 RC 迁入 KE；`routes/fusions.ts` 收成 decode-call-map 并迁入 KE inbound；两个 legacy facade（`services/fusion.ts` / `services/skillVersion.ts`）删除。新增只读面 `GET /api/skills/:id/provenance` 与前端版本行展开。W4-E3 exact 边 `30 → 27`，facade `346 → 345`，public-surface pilot 债 `10 → 6`                                                                                                                                                                                                                                                                                                                                                      | 链 `9911b3a05` → `dd3d51345` → `652f4d31c` → `624a14647` → `7fdada126` → `51c900140` → `02958a8aa` → `082a88b1c` → `94767e66a`；**AC-13 取证 `fb608f89c` / run `33800356235` / 35-35 success**（并发 push 取消时按含本提交的后继 sha 判）。**代价如实记账**：跨 context 的 provider 装配收到 bootstrap 之后，exception 账本净增 17 条（11 条 bootstrap→module composition/public 的 `legacy-inbound`、6 条新 provider adapter→`db/*` 的 `legacy-outbound`，全部自动分桶入账），详见 RFC-353 `proposal.md §9`                                                                   |
| N18  | RFC-355 / W4-E4a（Done）                | `services/intent/` **5136 行整目录删除**，18 个文件归位到 `modules/intent/{domain,application}`（1 个迁进 resource-catalog 的 composition，2 个兼容门面删除）；`routes/intentSessions.ts` 1088 行迁进 `modules/intent/inbound/` 并收成 decode-call-map（836 行），详情 handler 的 ~180 行编排收进 application、草稿生命周期与挂载建议两条判据进 domain；技能工件补偿改经 intent 声明的窄端口 + RC 提供的两个 provider 适配（深取 30 → 2 条纯类型边）；apply 大事务内的**全部计算**收成一份（基线重验 / bundle 内创建名 / plan↔op 同序 / receipt 行 / 谱系与新 manifest / handle 水位 / 重放三档），两个 provider 的 apply 文件 842/684 → 653/520 行——**事务机制按设计裁决留在 provider**（同步 `dbTxSync` 与 async `db.transaction` 形状不同）；public 面按「无 consumer 不公开」26 → 5 个符号；会话事件收成 `ports/intentSessionEvents.ts`，intent 的 application / inbound 不再 import `@/ws/broadcaster`。W4-E4a exact 边 **176 → 41**，facade **18 → 0** | 链 `68ea535e1` → `a6bf2193a` → `2f54a8be8` → `9ef861059` → `7caf1e91f` → `c31844a30` → `db8dbf424` → `b21d102c2` → `83be39dc0`；**AC-9 取证 `bec6c29f0` / run `33834461744` / run 级 success**（T4b 的后继取证见 RFC-355 `proposal.md §9.5`）。**代价如实记账**：剩余 41 条里 34 条是 bootstrap→module 的装配边（那正是目标形态，仍记成 `legacy-inbound` 只因 bootstrap 文件还住在 `src/cli` / `src/server.ts`）、5 条 intent↔RC、2 条专职 WS 投影文件；**全局 exception 净减 116**（5313 → 5197），与 RFC-353 的净增 17 方向相反，差别在于本刀同时消灭了整目录与 18 个 facade |
| N19 | RFC-360 / W4-E4b（Done） | RM 管理/注册表/首次选择冻结归位；旧入口退役；本域 B/D 完成；60 条原桶债逐条交后继 | `123ce2dbc` / Main `35492271521` 46/46 success |
| N20 | RFC-361 / W4-E9 EC provider（Done） | RC resource / TE Script fixture provider 强制注入，EC 保留 validation/receipt | 同上；Reaction/Event/完整 E9 仍开放 |
| N21 | RFC-362 / W4-E1 合同前置（Done） | SC offered / Task required 声明、真实行为验证、逐入口后继清单 | 同上；仅合同准备，不切生产，不关闭 E1/W5 |
| N22 | [RFC-363 Task/SC 生产切换](../RFC-363-task-source-control-launch-cutover/proposal.md)（Done） | durable source/group/revision、两 launch lane、reader 与该纵切 B/D | 最终证据及 E1/W5 转交见 RFC363 acceptance.md；不关闭完整 E1/W5 |
| N23 | [RFC-364 MCP diagnostics](../RFC-364-mcp-runtime-diagnostics-cutover/proposal.md)（Done） | E6 编排/effects/public/roots，复用现持久化与 lease | 七操作及三个根已切 RC；E6 和本域 B/D 关闭，W9 timer 单独转交 |
| N24 | [RFC-365 Event target providers](../RFC-365-event-automation-target-providers/proposal.md)（Done） | 两 target SPI、origin/claim/receipt 与旧链去重 | T1 合同对拍 + T2～T6 实现完成并置 Done（hosted 取证 `4796407fa` / CI run `35610192882` 46/46）；保留现能力；只关闭 Event target slice，不关闭 Reaction/完整 E9 |
...
W4-C 立下的转交口径处理剩余 facade 与不属于 memory 的 exact id，逐条见其 `plan.md §4.1`）。RFC-360/361/362 已按 N19～N21 范围完成；W6 可消费 P0-B，但仍须通过本 wave 的功能、transaction、compatibility 与 exact-SHA hosted gate。
...
`file://` 拒绝。`PublicRepositorySourceSealPort + RepositoryLaunchSnapshotInTx` 已由 RFC-362 批准并完成 additive
contract 与行为 oracle（one-shot raw URL seal / current authority + sealed-or-versioned source → frozen ref）；
RFC-363 已完成 durable source/group/revision、真实 provider 与启动纵切验收；只领取本纵切，不代表 W1 完整迁位。
Task-owned `TaskWorkspaceReadPort` + SC offered `WorkspaceContentParticipant` 已在 RFC-363 接入生产 reader/分页/字节与 task worktree route，最终证据见其 acceptance.md；
...
- [x] 六类 selector summary 收为 `ResourceCatalogQuery`，仅返回 `ResourceSummary`；每类完整 List/Get/Filter 保留
      typed QueryService，filter/pagination 下推数据库；
- [x] `ACL_TABLES` 限 infrastructure；跨模块用 resource public authorization port；
- [x] 按 named consumer 落 `TaskExecutionResourceSnapshotInTx`、当时的 `IntentApplyResourceParticipantInTx`（RFC-359 后已退役）、
...
- [ ] **E1 task launch admission**（RFC-362 Done 仅合同准备完成；RFC-363 Done 关闭启动/工作区生产纵切，完整 E1 仍须逐项退出）：消费 W2-B 已获批并稳定的 task admission 合同，不在 W4 偷改 RFC-287 行为。
...
- [x] **E3 knowledge-evolution**（RFC-353 Done，见 N17；本域退役/跨域转交按其 plan 记账）：fusion aggregate、decision、task launch intent 与 skill/memory exact-set tx participants
      收口；落 RC `SkillProvenanceVisibilityQuery` + Memory `MemoryProvenanceVisibilityQuery`，provenance query 逐 skill/memory
      批量读取并保留既有 scope 过滤与返回合同；
- [x] **E4a intent**（RFC-355 Done，见 N18；W6 仍未关闭）：session/draft/working-set commands/queries 与 legacy apply adapter 收口；复用 RFC-302 已落的 pure domain
...
- [x] **E4b runtime-management**（RFC-360 Done，exact handoff 见其 implementation-residual-debt.json）：profile/admin/probe 与 runtime-selection participant 独立切换/回滚，保持 per-NodeRun
...
- [x] **E6 resource-specific tools**（RFC-364 Done；最终证据与 W9 timer 转交见其 acceptance.md）：MCP runtime test 归 `resource-catalog/application/mcps/runtimeDiagnostics`；落
...
- [ ] **E9 digital-employee / event-center / execution-contract**（RFC-361 Done 关闭 Execution Contract provider slice；RFC-365 已关闭 Event target 实现切片（`4796407fa`）；Reaction 与完整 E9 仍开放）：以 RFC-310 OS 已落的 Case/Context/Attention/Reaction/Channel、
      ...
      RFC-365 已关闭该 Event target provider adapter cutover（`4796407fa`）；这仍不等于 Reaction 或完整 E9 已完成。
...
- [ ] util/git 改纯参数/port 注入，销当前 git circular 4 + util→upper 2（以 scripts/depcheck.ts 的 8 条总清单为准）；
- [x] 历史 agent SCC 已从当前 KNOWN 清单消失；不再重复申领 W5 改造，剩余 RC consumer 债按实际 owner 处理；
- [x] 历史 workflow SCC 已从当前 KNOWN 清单消失；不代表全部 workflow adapter 已归位；
...
- [ ] Intent provider 从当前 `intentApplyEngine` 的 resource bindings 提炼 typed scenario tx，复用同 engine（不恢复已退役的 `IntentApplyResourceParticipantInTx`），platform 不做资源 writer；
...
- [ ] knowledge-evolution 的 fusion approve/skill-restore 做故障点 characterization；符合 lifecycle 后接 provider，
      skill version 与 memory membership 同一 typed async tx。Restore-forward re-fuse 能力/历史 provenance backfill 另 RFC 呈批；
...
- [ ] W7.0 补齐 row semantic/synthetic rows/repo physical shape inventory；当前 `node_runs INSERT=1` 已由 RFC-359 single mint program 提供，继续以负扫描锁住，不能单凭此关闭 W7.0；
...
RFC-359 已统一 boot/shutdown；以下 restore generation 仍未落。checkpoint/stage/replace/verify 按 provider capability
明确实现：WAL 与文件 generation 描述 SQLite 机制，不能照搬到 PostgreSQL。W9-E 的独立 RFC 必须补全两种 provider 的
恢复与失败矩阵，保留现有备份/恢复功能；不能因统一启动已完成就关闭本段。
...
- [ ] admission stop、task/worker/outbox/apply drain/fence、provider-specific checkpoint、stage/verify/safety backup；
...
7. 所有持久化变动用真实 SQLite/PostgreSQL 同一行为套件；涉及 migration 时两库 fresh/upgrade，并按平台能力覆盖 Linux/macOS/Windows；
8. 涉及用户流程时真实 E2E；
9. 独立实现门（合同核实 + 对抗破坏）；
...
- [ ] proposal AC-1～AC-12 全部兑现；
- [x] 所有 P0 独立 RFC Done；
- [x] RFC-287 Done；RFC-288/289 保持 CLOSED；新号 W2 implementation RFC Done；
- [ ] W8 仅在新能力 RFC 获批时要求其 Done；未获批则保持原能力并跳过，不阻断核心 W9；
