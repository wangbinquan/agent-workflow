...
| N24 | [RFC-365 Event target providers](../RFC-365-event-automation-target-providers/proposal.md)（In Progress） | 两 target SPI、origin/claim/receipt 与旧链去重 | T1 合同对拍 + T2～T6 实现候选完成；保留现能力，待 hosted exact-SHA 取证；只关闭 Event target slice，不关闭 Reaction/完整 E9 |
...
- [ ] **E9 digital-employee / event-center / execution-contract**（RFC-361 Done 关闭 Execution Contract provider slice；RFC-365 已完成 Event target 实现候选并等待 hosted 取证；Reaction 与完整 E9 仍开放）：以 RFC-310 OS 已落的 Case/Context/Attention/Reaction/Channel、
      ...
      RFC-365 已把该 Event target provider adapter cutover 落为候选；这仍不等于 Reaction 或完整 E9 已完成。
