...
| W9   | partial vertical slice / no exit            | RFC-322 收编 14 个 hourly phase；2026-09-20 background=331/ambient=494（见 `status.md`）；RFC-359 已统一 boot/shutdown；DA/DE/Event Center 等 worker 仍散落，ManagedBackgroundRegistry/readiness/stop receipt 未落                                                                                                                                                                                                                                                                                                  |
...
| `modules/task-execution/infrastructure/taskRouteLaunchOperations.ts` / `taskRouteOperations.ts`                        | launch 与 route operations 已共用算法；E1 的 RFC-359 协调等待解除 | SC source/snapshot/preparation 合同、legacy helpers、adapters 与 root 收缩                   |
...
当前 generated source digest 为 `sha256:57a28fe8441b0a45f3012f2ff8145260dba1381b8dc5a3634c9115ca251696ce`：
backend/repo SCC=1/3、KNOWN=8、route→DB=0、transport→DB=0、AppDeps consumer=0；ambient=494、background=331；
facade/service entries=296（247 legacy implementation、49 thin facade），architecture exceptions=4562。
这些是不同分母；本次只对账，不编辑 `status.md` 或 canonical，也不把 0 条 route→DB 等同于 W4-B/D 关闭。

**owner 对账待办**：W4-E1 桶仍含 24 个按 fallback 归 TE 的 route 文件。例：
...
后继顺序：RFC-360（E4b 全纵切）优先；RFC-361（E9 的 EC provider 第一刀）可独立；RFC-362（E1/SC 合同准备）
先产出合同与行为 oracle，生产 cutover 另号获批。三者已于 2026-09-20 获批并进入 In Progress。E6 等 E4b；E10 等 E9 与 task query；
...
| N19  | RFC-360 / W4-E4b（In Progress）         | RM profile/admin/probe/model/selection 纵切，同步本域 B/D                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 用户批准后优先；与 TaskExecution root 接线串行                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| N20  | RFC-361 / W4-E9 第一刀（In Progress）   | EC resource/fixture providers 归 RC/TE，保留 receipt/wire                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 可独立实施；不关闭 Reaction/Event/完整 E9                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| N21  | RFC-362 / W4-E1 合同前置（In Progress） | SC offered / Task required 合同与当前 launch 行为 oracle                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 本 RFC 不切生产；后继 cutover 另号获批                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
W4-C 立下的转交口径处理剩余 facade 与不属于 memory 的 exact id，逐条见其 `plan.md §4.1`）。当前 RFC-360/361/362 已获批；RFC-360 正在推进管理用例与 HTTP adapter migration；W6 可消费 P0-B，但仍须通过本 wave 的功能、transaction、compatibility 与 exact-SHA hosted gate。
...
- [ ] **E1 task launch admission**（RFC-362 In Progress 仅准备合同，生产 cutover 另号）：消费 W2-B 已获批并稳定的 task admission 合同，不在 W4 偷改 RFC-287 行为。
...
- [ ] **E4b runtime-management**（RFC-360 In Progress）：profile/admin/probe 与 runtime-selection participant 独立切换/回滚，保持 per-NodeRun
...
- [ ] **E9 digital-employee / event-center / execution-contract**（RFC-361 In Progress 先做 EC provider slice）：以 RFC-310 OS 已落的 Case/Context/Attention/Reaction/Channel、
