...
| N24 | [RFC-365 Event target providers](../RFC-365-event-automation-target-providers/proposal.md)（Draft） | 两 target SPI、origin/claim/receipt 与旧链去重 | T1 兼容报告与特征测试已交付；T2+ 等待预算兼容定稿，不关闭 E9 |
...
- [ ] **E9 digital-employee / event-center / execution-contract**（RFC-361 Done 仅关闭 EC provider slice；RFC-365 Draft 承接 Event target，兼容定稿先于生产切换）：以 RFC-310 OS 已落的 Case/Context/Attention/Reaction/Channel、
