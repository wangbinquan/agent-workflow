...
pairTables(leftTables, rightTables):
  ...
  阶段 2(同键剩余序数):**两侧都**"内容不在对侧集合"的剩余表,按
    ...
    → 两侧替换为同一占位符 ph;ph 按
      'tablepair\0'+左内容+'\0'+右内容 **内容寻址**(设计门三轮 P2:
      两对同键修改表内容必不同 → ph 必不同,pairLookup 无覆盖;完全
      相同的一对出现两次共享 ph 且还原目标一致,正确)
...
5. **配对行的 cell 级 diff**:
   ...
   - **原子化在 §2b 已完成**(code → link → math 顺序;link 的 label /
     ...
     `)`、math 内容无 `$`),**超出保护集的奇异行内语法**(嵌套括号
     URL、autolink、行内 HTML 等)显式继承现状:marker 落入对应节点
     value、由 `remarkDiffMarkers` 单侧化渲染,不构成新回归(三轮 P1
     按边界文档化)。循环后仍有已发放码点残留(真 bug)→ 整对放弃细化
