...
2. **GFM 等价与列数守卫**:(a) 表头 cell 数 ≠ 分隔符 cell 数时整对放弃
   ——remark-gfm 根本不会把它解析成表,细化无意义(设计门三轮 P1);
...
4. **变更 run 内相似度配对**:
   ...
   - 候选序:全部 (i=removed 序, j=added 序) 按
     ...
     **规模上限**(设计门八轮 P1):单 run 候选数 removed×added >
     2,500(50×50)时,该 run 跳过相似度配对、直接整行 DEL(旧序)+
     整行 INS(新序)呈现——candidate 枚举与排序是同步主线程路径,
     无界矩阵(如 5,000 行全改写 = 2,500 万候选)会冻结评审页;上限内
     复杂度 O(候选 × cell token) 与 jsdiff 主调用同阶。
...
5. **配对行的 cell 级 diff**:
   ...
   - 每对 cell:逐字节相等 → 原样;不等 → lead/tail 空白外置,inner
     ...
     **整 cell 降级的两条结构性规则**(四轮/五轮 P1,七轮 P1 收束为
     ...
     白名单覆盖不了 `.`/`@`,而 marker 落进字面 autolink 会让
     remark-gfm 无法建 link 节点,mailto/href 直接消失,设计门八轮
     P1)→ 整 cell 降级。这把「保护集外
...
4. 相似度配对:编辑行与原行配对;无关行不硬配——含纯标点包装
   ...
   **单 cell 行仅 URL 变化**(原子化后 Dice=0)→ 整行红/绿呈现锁定
   ...
   DEL+INS、渲染合法且耗时有界(八轮 P1,用行数构造锁定不做真 5000 行
   压测);**字面 autolink cell**(`foo@old.com`→`foo@new.com`)→ 整
   cell 红绿、两侧 mailto link 结构完整(八轮 P1);**pair 键单射**:
   构造含 NUL 的同键表组不串表(八轮 P2)。
5. 超列行 → 整对回退现行为(两张表)。
