...
> **后续修订（RFC-224 / RFC-226 / RFC-227，2026-07-24）**：OpenCode 不再是 daemon
> 启动硬门，且 RFC-227 已 supersede RFC-224 的 official v1.18.3 exact-hash/OS 准入：
> 自定义 binary/fork 按 byte snapshot、direct API 行为 codec 与 containment capability
> 判定，版本字符串只作 telemetry。注册表抽象与显式 runtime probe 继续有效。
