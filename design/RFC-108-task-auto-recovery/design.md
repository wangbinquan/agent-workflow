...
- spawn 时持久化 `node_runs.spawn_binary_path = cmd[0]`（migration 0051）。`killStaleRunProcessTree` 改按该**精确二进制路径**匹配活 pid 的 `ps command`（`pidCommandContainsBinary`，substring、跨 macOS/Linux 可移植），取代 `/opencode|bun/` 模糊正则。匹配 ⟹ 确是我们的子进程 → 杀（**身份匹配时不再受 48h 窗口保护**，长跑 agent 仍是我们的、且 T4 30min floor 使 >48h 近乎不可能）；不匹配 ⟹ 确信 pid 被回收 → `command-mismatch`（安全、不杀、照常翻行）。**legacy 行（spawn_binary_path NULL）保留旧 window+regex 兜底**。
