...
**T6（AR-15）GC 不反噬可恢复任务 + resume worktree 前检 + 多仓 GC**

- `gc.ts` `TERMINAL_STATUSES`（`:23-27` 含 failed+interrupted）→ GC 候选集**排除「failed/interrupted 且无后继 done 行」的可恢复任务**（或给独立更长 retention）。翻转 `scheduler-audit-gap3-gc-terminal-statuses.test.ts` 既有锁定断言（在测试头注明 RFC-108 改写意图）。
- `resumeTask` 顶部加 `isGitWorkTree` 前置检查（镜像 `getTaskDiff:1870`），在 pending-CAS **之前**抛干净 `410`（worktree 已不在）——而非现在的 CAS→warn-and-continue→对不存在 worktree kick 出 500。
- 多仓 GC：按 `task_repos` 逐子仓 `removeWorktree`+`deleteSnapshotRefs`+容器目录 rmdir（修 `07-GIT-05` 恒泄漏）。
- GC 永不 prune 被「非终态任务的最新 node_run」引用的 snapshot。

**T7（AR-17）resume 跨 node_run 原子回滚**

- `resumeTask` 的 `toRollback` 循环前加**跨行 pre-flight**：对每个待回滚行的 `pre_snapshot`/`pre_snapshot_repos_json` 先 `gitCommitExists`（无副作用）；任一缺失则**在 reset 任何 worktree 之前** `escalateSnapshotLost`。把 `nodeRollback.ts` 的「node 内 two-phase」抬到「跨 node_run」高度。
- 顺带：非 snapshot 的回滚失败（stash 冲突 / git lock）在写节点行上改为「有界重试后升级」而非 warn-and-continue。
- 红测：两 failed 顶层行、row2 被 gc-prune，断言 row1 worktree 未被 reset。
...
**T9（AR-14）fail-safe survivor-kill + 持久化 spawn 身份**

- spawn 时持久化「binary 绝对路径 + start-time token（用既有 `opencodeSessionId`）」到 node_run 行；`killStaleRunProcessTree` 改为按持久化身份匹配，取代 `/opencode|bun/` 启发式。
- **fail SAFE**：当 `isProcessAlive(pid)===true` 但某门拒绝杀（窗口过期 / 命令不匹配）→ **不静默 flip**，emit 高危 alert，并对该任务**拒绝 auto-resume** 直到 operator 确认。要求 alive + path + token **3-of-3** 才杀，歧义则告警不强杀。
- 48h 窗口可配；live pid 的「因窗口跳过」记日志。
- 注入 `isProcessAlive`/`pidCommandLooksLikeAgentChild` seam，测三场景：窗口过期但活 / 命令不匹配但活 / resume double-write（stub alive=true）。
