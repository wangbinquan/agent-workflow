...
| 配置分类工具箱     | 分类的“工具箱”页      | 固定职责小卡片；所选工作项的输入、输出、完成标准和已有工具                 | 为当前工作项增加/验证工具                        | 岗位模板所需工作项均有兼容工具             |
...
- **AC-51** 分类工具箱与岗位模板编辑器复用同一 manifest 小卡片图及 workItem identity；员工列表和普通页签不重复展示全景。
  工具箱执行“注册工具”，岗位模板详情执行“选择默认工具”；深链和刷新保持同一选中工作项。任务运行只展示当前 Case 所需的状态与
  下一步，不把配置全景当作每页固定页头。
...
- **AC-77** 工具箱的小卡片职责图桌面和窄屏都全量展示 20 个工作项，不出现线、端口圆圈或画布拖拽。平台、可定制工具、员工协同
  三类标签和可选泳道必须可辨；选择卡片只高亮当前职责，并在同页下方打开输入输出与工具配置，由功能 E2E 与目标视觉基线共同防护。
...
- **AC-86** 岗位模板创建/修改不使用弹窗，进入分类页内详情编辑状态并复用工具箱小卡片图。点击卡片只配置该职责；已配置为绿色，未配置
  或未启用为黄色。可选泳道不配置也可发布；提交时仅对真正缺失的必选职责闪烁、高亮并定位到第一项。
...
- **D42**：确定性职责全景的 authoring 表达采用固定区域中的小卡片，而不是 SVG 连线画布。工具箱常驻显示；岗位模板只在详情编辑态复用；
  员工列表和其他页签不重复占据页面。卡片内写明类型、配置状态和下一步。
- **D43**：岗位模板是页面内可编辑详情，不是创建弹窗。黄色只是“未配置/未启用”的事实，不等于阻断；只有分类 manifest 认定为必选且当前
  启用的职责缺失时，发布动作才触发高亮校验。
