...
T168 本地部分于 2026-08-21 完成：最终完整 `gate:local` 8m31s 全绿（backend 11,622 pass / 36 skip / 0 fail，
...
输入焦点和 `trigger.issue.issue_id` 实时路径。保持 🚧 只表示尚未取得推送后 exact-SHA hosted CI/visual 终态。
