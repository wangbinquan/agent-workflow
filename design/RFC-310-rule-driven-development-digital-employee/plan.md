...
> 其后按是否启用审核直接旁路，或显示“方案分析 → 人工审核方案”；两条路径汇入唯一的“分析与实现”后，“校验并冻结代码修改”和“提交 MR”
...
| PR-27   | 双来源入口与人工审核分支     | 界面输入/ISSUE 都归一到材料准备；关闭审核旁路，开启审核走方案分析→人工审核方案；两者汇入唯一分析与实现；真实 ISSUE→MR→修绿链可复跑            | PR-25,PR-26 |
...
- 1280×900 桌面视口仍需纵向滚动才能看全职责图，或测试继续靠 2400px 高视口掩盖布局；
- 卡片仍长期展开下一步、合同或参数，导致全景退化成详情列表；
- 业务名称仍被省略、裁切或靠继续缩小字号来满足一屏几何；
- 全景仍表现为泳道名称包裹若干卡片的分组矩阵，出现贯穿多泳道的竖线，或卡片为填满一行而宽度不一致；
- 分类产出 N 类但全景仍只显示一个笼统“修复流水线”节点，或派生节点能绑定不接受该 route 的工具；
- 运行详情另造不同拓扑、只展示当前节点，或历史阶段无法定位 exact session 和冻结输入输出；
- 工具箱或岗位模板继续在全景下方常驻所选职责详情；
- 岗位模板职责编辑页仍常驻名称/说明两个输入框，或新建时直接铺开全部职责参数；
- 名称确认后岗位仍只存在浏览器内，取消、刷新或缺少必选工具会让草稿消失；
- 集合输入职责仍画成普通单卡，或视觉层按开发类型/节点 ID 硬编码叠卡；
...
堆到来源列下方；
未选择“先评审方案”时直接旁路，选择后走“方案分析 → 人工审核方案”。两条路径必须汇入同一个“分析与实现”节点，再进入“校验并冻结代码修改”，
职责图不得复制“分析与实现”或额外渲染一个“实现”伪节点。方案分析工具必须由岗位模板配置并冻结 exact revision；内置工具只是默认选项，
其执行合同只要求 `analysis-plan` 文档路径端口，不要求 `agent-result` envelope。
