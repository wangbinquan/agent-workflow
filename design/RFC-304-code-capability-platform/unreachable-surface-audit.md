...
- §11.1 噪音上限 + §11.2 T59 人工指令回执 + readiness 陈旧——见 `plan.md` §2ter.8，含
  两处刻意收窄（回执入口只开给 issue 打标签；readiness 选读时推导而非写时失效，
  `readinessInvalidation.ts` 保留未删以便回退）；
- **AC-35 是本文件的一处误判，已勘误**：`updateSummary` 确实零调用方，但总览这件事
  mr-review 早就在做（`<!-- aw-review-overview -->` + `publishReview.renderOverview`，
  建一次之后编辑）。照本文件去「接线」会在 MR 上加出第二条竞争总览，e2e 当场抓到。
  残留缺口只是「另外四条能力不参与这条总览」。**教训**：本文件第 1 步给出的是线索，
  不是结论——见 `plan.md` §2ter.8 勘误段与 `docs/dev-gotchas.md` 同名条目；
...
上表中 **AC-38 灰度回退 / T63 批量启用 / T64 模板上游** 三条的共同
形状是：领域层写完了，但**没有路由、也没有界面入口**——接线不是加一行调用，而是要
先定产品面。其中「部门层模板的详细配置入口」用户在 2026-08-17 明确说过**先不做**，
所以接手者**不要**把这三条当成可以顺手补的接线，先与用户确认范围。

**T66 曾被我错误地归在这一类**：`/code` 的活动面本来就有，缺的只是「说出自己截断了多少」
与「单项放宽到 window」——纯接线，已在本轮做完（见 `plan.md` 同名段）。教训是：判断「缺
产品面」之前先去看那个面**是不是已经存在**，别从领域模块零导入直接推断到界面不存在。
