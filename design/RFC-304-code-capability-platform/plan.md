...
## §0 关闭记录（Done，2026-08-17）

RFC-304 关闭。五条能力（mr-review / ci-fix / mr-comment-fix / requirement /
mr-monitor）在 system-mock 上真事件跑通，E2E 全量 328 passed，CI 绿。

### 关闭前更正的两处账目错误

收尾核对时按「除定义文件外零生产引用」重扫了 `modules/code-capability/domain/**`，
发现**下表里标着 ✅ 的两项，代码在生产里根本到不了**：

| 任务                                                 | 表里原状态    | 实况                                                                                                                                                                                                                 | 归宿           |
| ---------------------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| T63 框架发布生命周期（revision / canary / 一键回退） | ✅ 2026-08-16 | `domain/frameworkRelease.ts` **零生产导入**——判定函数与状态机写完了，没有路由、没有界面，也**没有持久化**：canary 需要「每个仓钉在哪个 revision」与 last-known-good，而 `capabilityFrameworks` 只有一个 `version` 列 | **转出新 RFC** |
| T64 模板上游关系（四态 + 三方差异预览）              | ✅ 2026-08-16 | `domain/templateUpstream.ts` **零生产导入**，无路由无界面                                                                                                                                                            | **已由 RFC-309 T16 结清**（2026-08-17）：`application/templateUpstreamStatus.ts` 接线、`GET/POST /api/capability-templates/:id/upstream[/merge]`、模板详情页四态徽标 + 逐字段差异 + 「只合并未被覆盖的字段」；迁移 `0175` 补 `base_snapshot_json`（只有摘要答不了「是谁改的」） |

两项的共同前置都是**部门层模板/框架的配置界面**，而用户 2026-08-17 明确回复
「先不做」。T63 还额外需要 schema 变更。故不在本 RFC 内补，**转出**：需要时以新 RFC
立项，先定产品面与 schema，再接线。

> 教训（已进 `docs/dev-gotchas.md`）：**「域层写完 + 单测绿」不等于「这一项做完了」**。
> 这两行 ✅ 是按「模块和用例都在」记的，而它们从未被任何生产代码调用过。本 RFC 全程
> 修掉 20+ 处同款「两半都对、没接上」，账目自己也栽在同一个判据上。

### 本轮（2026-08-17）补完并 CI 绿的项

| 面                        | 此前状态                                                               | 提交       |
| ------------------------- | ---------------------------------------------------------------------- | ---------- |
| §2.3 lease 三行           | 心跳续租 / daemon 代际 / 启动回收全部零调用方                          | `19d430f9` |
| §2.2 发布临界区启动清理   | 崩在临界区 ⇒ 该 MR 永久静默停摆，清理函数零调用方                      | `3d0051a3` |
| §11.1 噪音上限            | `notificationsSpent` 无人赋值，上限从未生效过一次                      | `eb2b938d` |
| readiness 陈旧            | 存下来就不再更新，依赖删了仍显示 ready ⇒ 改**读时推导**                | `eb2b938d` |
| §11.2 T59 人工指令回执    | 零调用方；且发不出去——目录里没有 issue 评论端点                        | `96cc392b` |
| T65 配置规模化批量        | `configScale.ts` 零导入 ⇒ 批量 preview / apply / 一键回滚              | `6dd28b3c` |
| T66 状态图规模化          | 列表每项 3 轮且**不说自己截断了** ⇒ `roundsHidden` + 可放宽            | `c40644d4` |
| §7.2 发布意图**重启**恢复 | 只有「下一轮 publish 前按 anchor 对账」，够不着「此后再没有轮次的 MR」 | `1938ab16` |

> `6dd28b3c` 的 commit message 把这项写成了「T63」，实为 **T65**（T63 是框架发布
> 生命周期，见上表已转出）。此处更正，便于日后按号追溯。

### 关闭时的其余遗留，逐项归宿

1. **T4a3 真机验收（真实 GitLab 上出现行级评论）** —— 需用户的 GitLab / 仓库 / MR，
   代码面已由 system-mock E2E 全链覆盖（含 GitHub 双家参数化）。**归宿：用户环境待办**，
   不阻塞关闭。
2. **`domain/readinessInvalidation.ts`** —— readiness 改读时推导后结构上不可能陈旧，
   这套写时失效机制随之冗余。**归宿：可删**；本轮保留未删，以便用户若倾向写时失效可
   直接回退（理由见 §2ter.8）。
3. **`stateViewScale.describeHidden` / `stageMayHaveAttempts`** —— 前者返回英文散文而界面
   是双语的（数字过网线、措辞留 i18n），后者的规则页面已内联实现。**归宿：重复意图，
   标注保留**，不为接而接。
4. **不可达面全量盘点** —— 方法与逐条对账见 `unreachable-surface-audit.md`（可复跑的
   三步扫描）。**归宿：随本 RFC 归档**，供后续 RFC 复用判据。
...
| T46b | 三入口：issue 标签 webhook / `/code` 界面 / 平台 API                                                                                                                                                                                                                                                    | T16,T46a | **已由 RFC-309 PR-3 结清**（2026-08-17）：`POST /api/code/rounds` + 模板详情页发起面板；四条能力按判别联合给输入，`requirement` 用新增的 `anchorKind: 'platform'` |
...
| T63 | **框架发布生命周期**：不可变 revision + `draft→validated→canary→published→retired`；binding 声明 `pinnedRevision`/`followChannel`；发布前回放固定样本并显示受影响仓数；保留 last-known-good 可一键回退                            | T13,T14 | ⛔ 转出（见 §0） |
| T64 | **模板上游关系**：`upstreamRef`/`upstreamVersion`/`baseDigest`/`localOverrides` + `current\|update-available\|conflicted\|orphaned` 四态 + 三方差异预览与"只合并未覆盖字段"；配置包携带来源与基线摘要，连不上标 `detached`        | T17b    | ⛔ 转出 → **RFC-309 T16 已结清** |
...
**T66 一并接完，它不是「缺产品面」——是我先前归错了类**：`/code` 的活动面早就有了
（工作项列表、轮次切换器、阶段、按 AI 阶段惰性加载的 attempt）。缺的正是
`stateViewScale.ts` 文件头自己写的那句——「a view that renders 20 while the query
returns 3 shows a truncated list with no indication it was truncated」——而那**就是**当时
的实况：查询给 3 轮、页面渲染 3 轮，80 轮的 MR 上读者无从知道还有 77 轮。

- 后端：投影统计总轮数并返回 `roundsHidden`，切片走 `roundWindow`（页面与查询不可能对边界
  产生分歧——这正是常量与切片同住一个 domain 模块的理由）；新增 `rounds` 参数把**单个**
  工作项放宽到 window，且无论调用方要多少都在 window 处封顶（`?rounds=5000` 撑不开响应）。
- 前端：把数字念出来——不是「显示最近若干轮」，那种措辞正好让人以为自己看全了——并给一个
  只放宽**这一项**的按钮。按工作项而非全局：20 项 × 20 轮正是这条边界要拦的响应体积，一个
  人看一个 MR 不该让所有人的列表变大。
- `roundsHidden` **恒在、含 0**：调用方无条件渲染它，就不可能因为「没这个键」把截断提示吞掉。

**两处刻意不接**（避免为接而接）：`describeHidden` 返回英文散文而界面是双语的——数字过网
线、措辞留 i18n；`stageMayHaveAttempts` 的规则页面已内联实现（`stage.kind === 'ai'`）。
两者现在属重复意图，标注出来比硬接上更诚实。

**判定教训**：判断「缺产品面」之前先去看那个面**是不是已经存在**。我从「领域模块零导入」
直接推到「界面不存在」，因而把一件纯接线的活儿挂在了用户身上——与本轮另外两次误判
（总览「早就有人做了」、回执「下一层做不到」）同源：**符号级证据推不出产品级结论**。
