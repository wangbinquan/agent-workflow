...
- **新增 `POST /api/repo-groups/preview`（干跑展平）**。设计说编辑器预览调
  `/layout`，但那条只服务**已存在**的组，而编辑器要预览的正是还没保存的成员表。
  两个选择：前端另写一套近似展平（然后与服务端慢慢长歪、保存时才发现不一致），
  或让服务端用**同一份实现**干跑。选后者。它是**纯读**的：`repoUrl` 形态的成员
  会触发镜像 clone（写），预览一律不导入、只回报 `pendingImports` 计数——否则
  用户每敲错一个字符就在 `cached_repos` 里留一行垃圾。
- **AC-19 落成 `task_repos.readonly_dirty_count`（迁移 0133）**，而不是复用
  `lifecycle_alerts`。后者绑 `LifecycleAlertRule`，RFC-108 的自动修复循环会全局
  扫描并尝试**修复**每一条；「只读仓被改动了」不是待修复的不变量违反，是给人看
  的事实通报，塞进去会被误修。三态：NULL=从未检查 / 0=检查过且干净 / N>0=丢弃
  了 N 处（干净时也写 0，否则 UI 永远分不清「干净」与「没查过」）。
- **`RepoLayoutTree` 不自造空态**（RFC-214 Lock B）。它是纯投影组件，空与不空
  由持有 query 的调用方经 `QueryState.emptyText` 表达。这条是本仓设计系统棘轮
  在 review 之前就把问题拦下来的一个实例。
- **树按挂载路径重建，父取「最长祖先」且按段边界比较**。`vendor/sdk` 是
  `vendor/sdk/ext` 的父但**不是** `vendor/sdkx` 的父；只看第一段会把三层布局压
  成两层。两条都有专门的红绿测试。
- **T40 修掉一处真正的 join 断裂**：文本 diff 的挂根标记写作 `.`（空串在
  `# === Repo:  ===` 里既不可读也不可解析），而结构化侧的规范 key 是 `''`。
  不过 `parseRepoKeyWire` 就会拿 `.` 去拼 `./src/a.ts`，与结构化的 `src/a.ts`
