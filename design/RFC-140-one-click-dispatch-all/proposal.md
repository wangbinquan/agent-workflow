...
2. **塌缩边界**：改派到第三节点 → 常规 override（golden-lock 逐字不变）；questioner 行已
   ...
   建（seal 戳归一化）；**幸存 designer 行三分支**（Codex 设计门 P2 两轮）——未下发带旧
   override → 归一化清空（effective 回落设计节点）；已下发且 effective==设计节点 → 塌缩成功
   零改动（D6 对称）；已下发且 effective==第三节点 → 409 拒塌缩指引 reopen（post-dispatch
   不可改目标守卫不破）；scope 表双写 lockstep（clarify_rounds + cross_clarify_sessions）。
...
4. **越权防护**：仅 staged 未点发（`auto_dispatch_deferred_at` 为空）的条目在续跑 done 后
   **不被**自动下发；**deferred 条目被 unstage（撤回待下发）后登记随之清除、不再自动发，
   re-stage 也不复活登记**（stage 与 unstage 双向清列 + 盖登记观测值 CAS 关闭 dispatch
   pre-lock 窗口——Codex 设计门 P1 + 二/三轮竞态收紧）。
5. **重启韧性**：deferred 登记落库；daemon 重启 + 任务 revival 后 tick 照常自动补发。
6. **补发失败自愈**：补发**一次全量调用**（frontier 全局计算——逐 home 拆分会让 DAG 下游
   home 拿旧输入直接 mint，Codex 三轮 P1 否决）；可恢复 Conflict（in-flight /
   target-changed / designer-not-ready，复用既有 `DESIGNER_DEFERRABLE_CONFLICTS` 白名单）→
   静默留待下一 tick，不失败任务、不丢登记；**白名单外的不可恢复 Conflict → 清登记 + WARN
   回手动轨道**（「宁可回手动、不可错 frontier」裁决——清列不丢数据，看板批量下发照常可用并
   显示真实报错），绝不静默空转（Codex 设计门 P2）。
