...
> **实现门修订（Codex adversarial-review 2026-07-08，1 high + 1 medium）**：预置行变可删后，
> 删除完整性有两个新暴露的缺口——删被引用的 runtime 会静默降级、all-delete 会让 seed 复活。

8. **删被 config per-feature 引用**〔impl-F1〕：`findRuntimeReferences` 原只扫 `agents.runtime`
   - `defaultRuntime`，漏了 `memoryDistillRuntime` / `commitPushRuntime` / `mergeAgentRuntime`
     （经 `resolveInternalAgentRuntime` 消费）。删被它们引用的 runtime → 内部 job 静默落协议名
     fallback（NULL profile）。修：签名重构为 `RuntimeRefConfig`（4 字段），删除检查全覆盖并报出
     具体字段；`deleteRuntime` + route DELETE 传 config 四字段。
9. **all-delete 复活**〔impl-F2〕：seed「空表才建」判据不 durable——config.defaultRuntime 指向
   不存在名（PUT config 只挡 disabled 不挡不存在）时 F1 保护落空，可删空表 → 下次启动 seed
   复活预置行，违背「删了不补种」。修：`deleteRuntime` 加 last-row backstop（`length <= 1` → 409
   `runtime-last`），保证表永不空（除真首启），比改 config route 更轻且直接。
...
- **实现门 2 回归**：
  - **impl-F1**：`memoryDistillRuntime` / `commitPushRuntime` / `mergeAgentRuntime` 任一引用某
    runtime 时删它 → 409 `runtime-in-use`。
