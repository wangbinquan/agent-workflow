...
#### 5.2.14 mixed-path 写流程重构（Codex impl-gate 第七轮，RFC-076 否决区；用户拍板）

P5-BC 落地后跑 impl-gate Codex 对抗审，命中 **mixed path**（同一轮**控制通道**部分 seal/dispatch ⨯ **快通道**整轮 finalize 交织）三个真实正确性缺陷（逐条回源确认）。早稿的 concern (a) park-starve un-park + concern (b) submit-side in-flight guard **不足以**根治，且根治需改 `submitClarifyAnswers` 的 RFC-076 定序临界区（否决区）。**用户拍板：支持 mixed path、授权 RFC-076 临界区改写。** 三步连贯设计：

- **缺陷 1（数据丢失）**：读侧 `selectAnsweredRoundsForConsumer`（`clarifyRounds.ts:328-331` self / `:389-395` questioner，经 `roundsWithDispatchedEntries` keyed on `dispatched_at`、**永不清除**）把**任何有 dispatched self/q 条目的轮永久排除**出整轮渲染路径。故一轮一旦进入控制通道 dispatch 模式，对它 quick whole-round finalize 续跑出来的 continuation 整轮渲染**取不到该轮**、而未 dispatch 的题又无 per-question 队列条目 → **未下发的答案丢失**。早稿 (b) 只挡 in-flight（`!isDispatchedEntryConsumed`），放行了 consumed-dispatch → 仍丢失。
- **缺陷 2（双 mint race）**：submit 的 guard 是 mint 前的 async 预检，与 `mintNodeRun`（旧 `clarify.ts:505`，独立 await、不在 tx、不在锁）**非原子**；并发的 `dispatchTaskQuestions`（自带同步 dbTxSync 但**不持任务写锁**）可在「guard 通过 → submit mint」窗口里 commit 它的 stamp+mint → 同 home 双 rerun。dispatch 侧 in-tx recheck 只挡 submit-first，挡不住 dispatch-first。
- **缺陷 3（park 饿死 / 重复执行）**：concern (a) un-park 让 continuation 能跑，但 sealed-undispatched q1 仍在 → continuation consume 整轮后**又被 park 源重新 park** + 可被再次 dispatch（重复执行）。

**三步连贯修（self 路径 `submitClarifyAnswers` 本轮全落；questioner 路径 `submitCrossClarifyAnswers` 见「scope」）**：

1. **(b) guard 改 reject any-dispatched**：轮上有**任何** dispatched self/q 条目（in-flight **或** consumed）即 reject quick-finalize（错误码 `clarify-quick-finalize-round-dispatched`）。关缺陷 1——dispatch 模式的轮只能走控制通道（seal + dispatch 余题）。落点：`clarifyRerunLedger.ts:roundHasDispatchedSelfQuestioner`（async 预检，question er 路径用）+ `clarify.ts` 内 dbTxSync 里的同步 recheck。
2. **quick-finalize consume 整轮 open-undispatched self/q**：quick-finalize 成功时把该轮**所有** open + undispatched 的 self/q 条目标 `confirmation='confirmed'`（整轮已答、答案在 `answers_json` 由续跑渲染）。park 源（排除 `confirmation='confirmed'`）不再 park 该 home（**不饿死、不重 park**），且 dispatch 选条目加 `confirmation='open'` 过滤 → **不可再下发**（关缺陷 3，取代 concern (a) un-park——已回退）。`confirmed_by` 审计-only（RFC-099，不进 prompt）。无 self/q 条目的轮命中 0 行（黄金锁）。
   - **不只 sealed 条目（Codex impl-gate 第八轮 finding A）**：`reconcileDesiredEntries` 对**每个**问题建 self/q 条目；partial seal 后 q1（sealed）+ q2（unsealed，sealed_at NULL）都有行。早稿 consume 带 `sealed_at IS NOT NULL` 只确认 q1，q2 在 answered 轮仍 `open` → readiness gate（轮 answered 即放行未 seal 条目）下仍可被 dispatch → 为已渲染答案重复 mint。**修：consume 去掉 `sealed_at` 条件**，确认整轮所有 open-undispatched self/q（dispatched 的不可能到这——step 1 已拒整轮）。
   - **dispatch CAS 同步复核 confirmation（finding B）**：`dispatchTaskQuestions` 的 `confirmation='open'` 过滤原只在异步 `requested` 读上；若 quick-finalize 在「读后、tx commit 前」确认了某条选中 sealed-undispatched 条目、且其续跑已 done+output（open-ledger recheck 不再挡），则 in-tx CAS 只查 `dispatched_at IS NULL` → 仍 stamp/mint 重复 rerun。**修：in-tx CAS（stillNull 守卫）+ stamp UPDATE 都加 `confirmation='open'`**——被并发确认的条目使 stillNull 计数缩水 → `ConcurrentClaim` → 整 tx 回滚（不 stamp/mint）。
3. **submit 写临界区收进同步 dbTxSync（RFC-076 临界区改写）**：`submitClarifyAnswers` 把 \{dispatch-mode recheck → mint → consume → flip session → flip round\} 收进**一个同步 `dbTxSync`**。bun:sqlite 单线程 + dbTxSync 同步体不 yield → 与 dispatch 的 dbTxSync **天然互斥**：dispatch 要么在本 tx **前** commit（本 recheck 看见 → reject）、要么在**后** commit（dispatch 自己的 in-tx immediate-ledger recheck 看见本续跑 → reject）→ 关缺陷 2，**无需新锁**。
   - **mintNodeRun 可同步化**：`mintNodeRun` = `buildMintNodeRunValues`（纯同步）+ `db.insert`；故 tx 内 `tx.insert(nodeRuns).values(buildMintNodeRunValues(...))`（`rfc098-allow-direct-node-run-insert` 豁免，同 dispatch:679）。`buildFrozenAttributionSet`（async 读）在 tx **前**算好、tx 内套用。
   - **RFC-076 定序不变**：`mint → write(session/round) → close` 次序**保持**；阶段 (1)+(2)〔mint + flip〕收进 tx（**原子**，torn 窗口彻底消失、且比旧 async-ordered 更强）；阶段 (3) close-clarify 仍走 tx **后**的 async `transitionNodeRunStatus`——node_run 状态转移必须经 lifecycle CAS（s14 禁直写状态），且续跑已在 tx 内 commit，故 close-after 仍满足「never (clarify done ∧ rerun absent)」。rollback（git，async、持任务写锁）仍在 tx 前（与旧序一致；early guard 在 rollback 前 fail-fast，省去被拒时的 rollback 浪费）。
     **最终 adversarial impl gate（独立深审）3 findings（用户拍板 questioner 不停 step 1，全落）**：

- **finding 1（self/questioner 双提交双 mint）**：dbTxSync 原只 recheck dispatched、**没 claim session**。两并发 submit 同过 tx 前 awaiting_human 早读 → 串行进 tx → 第二个仍 mint + 重写 answered（tx 后 close 失败为时已晚，第二 rerun 已落）。**修：tx 内先原子 claim**——reselect `clarify_sessions`/`cross_clarify_sessions.status`，非 `awaiting_human` 即抛（`clarify-already-answered` / `cross-clarify-already-answered`）回滚。bun:sqlite 单线程 + dbTxSync 同步体不交错 → 输家必见赢家已 commit 的 answered。落点：`clarify.ts` tx 首检 + `crossClarify.ts` tx 首检。
- **finding 2（questioner 完整 step 2+3）**：questioner 不停 step 1。**关键可行性**：cross 的续跑 mint（questioner cascade / designer）**不必进 tx**——session CAS（finding 1）+ consume-in-tx（step 2）+ confirmation-gated dispatch（finding B）三者已封住「双提交」与「dispatch 竞态」两条双 mint 路；故 cross 把 `{session CAS → dispatched-recheck →（cascade 时）reconcile+consume questioner → flip}` 收进**一个同步 dbTxSync**，**异步的 questioner cascade / 多源 designer readiness / deferred 分支留在 tx 后**（peer-aggregation 需异步多行读，本就无法、也无须进 sync tx——races 已被 CAS+consume+confirmation-gate 封）。**consume 仅在本 submit 真 cascade questioner 时跑**（`directive==='stop'` 或 RFC-059 全 questioner-scope 快路径）；designer 路径不 consume（其 questioner 处理是 RFC-059 既有逻辑，不动）。RFC-076 对 cross 是「flip 必须在 mint 前（peer-aggregation）」——保持：flip 在 tx、cascade 在 tx 后、close 仍在 flip 后 branch 前。
- **finding 3（懒 reconcile 复活）**：consume 原只作用现有行；virgin quick-finalize（没开问题列表/没 seal）tx 中 0 行 → 之后 `listTaskQuestions` 懒 reconcile 按 `confirmation='open'` 新建 self/q 行 → answered 轮视 sealed → 可 dispatch → 重 mint。**修（option a，无新列、不破 §5.2.8）**：tx 内调 `reconcileRoundEntriesTx(tx, {...round, status:'answered', answersJson})` **物化**该轮 self/q 行、再统一 confirmed。`reconcileRoundEntriesTx` 同步 + 幂等、其 `onConflictDoUpdate` **从不动 `confirmation`**（`taskQuestions.ts:219/271`），故后续懒 reconcile 保留 confirmed。self 轮无 designer 行（cleanup no-op）；cross 仅在 cascade 分支物化 + 只 confirm questioner（designer 留给 deferred/designer 路径）。

- **scope**：self（`submitClarifyAnswers`）+ questioner（`submitCrossClarifyAnswers`）三步**全落**（含 session CAS + consume 整轮 + 同步 tx flip）。

**第二轮最终 adversarial impl gate（更深并发审）3 findings**：

- **finding 1（self rollback 在 CAS 之前、destructive）**：self submit 的 worktree `rollbackNodeRunWorktrees` 在 in-tx session claim **之前**。两 submit 同过 tx 前 awaiting_human 早读 → loser 仍在 task write sem 下 rollback、再 hit tx CAS 抛；若 winner 已 return+resume、其续跑已 start/write，loser 的迟到 rollback reset worktree → CAS 防了双 mint 没防 worktree 破坏。**修：把 task write sem 扩到 `{durable claim → rollback → mint/flip tx}` 全程；claim（锁内第一步 reselect session.status，非 awaiting_human 即抛 `clarify-already-answered`）在任何 rollback side effect 之前**——loser 在碰 worktree 前就 reject。rollback 仍在 mint 前（rerun 不得在 worktree reset 时已存在，否则并发 scheduler tick 会 pre-rollback 跑它）。落点：`clarify.ts` `getTaskWriteSem(taskRow.id).run(async()=>{ claim → rollback → dbTxSync })`。（cross 无 pre-claim rollback，不涉此 finding。）
- **finding 2（cross questioner cascade post-tx mint window）**：上一轮「cross mint 不必进 tx、留 tx 后」**被推翻**——answered-but-not-yet-minted window 里 `confirmation='confirmed'` 只挡 dispatch 同 entries，immediate-ledger gate 看不到还不存在的 pending `cross-clarify-questioner-rerun` → 并发 dispatch 另一 staged entry 到同 questioner home 可先 mint、cascade 再 mint → 同 home 双 pending rerun。**修：questioner cascade rerun 的 mint 进 tx**——`mintQuestionerRerun` 的异步读（lastRun）hoist 到 tx 外、同步 `buildMintNodeRunValues` + `tx.insert(nodeRuns)` 进 tx（同行同 cause），与 flip 原子 commit、对并发 dispatch 的 gate 立即可见；stop / continue-fast 分支复用 tx-minted id（不再 `triggerQuestioner*Rerun`）。**只 sync insert 进 tx；多源 designer readiness / designer rerun 仍异步留 tx 后**（peer-aggregation 不破）。落点：`crossClarify.ts:537` tx 内 `tx.insert(nodeRuns).values(questionerRerunValues)`。
- **finding 3（peer-aggregation designer double-mint）——判定：既有 RFC-059，非 P5-BC，单独跟进**：两 sibling cross session（同 designer）的 session CAS 各管各（per-session、不冲突），both flip answered 后各跑 `evaluateDesignerRerunReadiness` 于 stamp `designerRunTriggeredAt` 之前 → both ready+untriggered → both `triggerDesignerRerun` → 双 designer mint。**归属判定**：`designerRunTriggeredAt` stamp + readiness 由 RFC-059 期 commit（`7b566e85`/`70bcafd7`，2026-05-22/23）落定；P5-BC 提交（`46c8ca4`/`780cd92`）与本轮未追踪 diff **均未触** `evaluateDesignerRerunReadiness`/`triggerDesignerRerun`/`designerRunTriggeredAt`/`countDesignerScopedAcrossSources`（`git diff` 空）→ **pre-existing RFC-059 multi-source designer readiness 并发，非 P5-BC 引入/加重**。按 gate 指示**不塞 P5-BC scope、单独跟进**：建议修法 = sync tx 内原子 claim ready source set（`UPDATE 候选 sessions/rounds SET designerRunTriggeredAt WHERE IS NULL` + 校验 expected affected count，再 insert designer rerun；losers 见 claim 不 mint）——留独立 issue/RFC。

- **第二轮 scope**：finding 1（self rollback-before-claim sem 扩域）+ finding 2（cross questioner mint 进 tx）**已落**；finding 3（RFC-059 designer peer-aggregation）**判定为既有、单独跟进**（design 此处注明）。

#### 5.2.15 question-write 锁（第三轮 final gate，用户经决策界面授权）

第三轮独立 gate 命中根因：reciprocal 的 pre-rollback precheck 仍可 stale——`submitClarifyAnswers` 持**长任务写锁** A（`getTaskWriteSem`，rollback 的 writer 保护）跨 {precheck→rollback→tx}，但 `dispatchTaskQuestions` **不取任何共享锁**，故一个 deferred dispatch 可在 submit 的 precheck 之后、destructive rollback 之前 commit → precheck 失效 → rollback reset worktree clobber dispatched rerun（+ 残留双 mint 窗口）。封死需序列化 submit 与 dispatch。

- **新原语**：`taskWriteLocks.ts` 新增 **per-task short-lived `getTaskQuestionWriteSem`（锁 B）**，**独立 registry**，区别于横跨 agent run 的长锁 A（`getTaskWriteSem`，scheduler `1962→2872` 持）。所有持有者只做快 DB（+ self rollback 的有界 git reset）、**不跑 agent** → B 短时、绝不挂在 agent run 后。
- **谁持 B**：`submitClarifyAnswers` 的 {claim→precheck→rollback→tx} 全程（A 外 B 内）；`submitCrossClarifyAnswers` 的 tx（B only，cross 无 rollback → 无 A）；`dispatchTaskQuestions` 的 stamp+mint tx（B only）。
- **锁顺序 + 无死锁论证**：唯一嵌套是 self submit 的 **A ≻ B**（先取 A〔rollback writer 保护，RFC-098 B1〕，再取 B）。dispatch + cross-submit 只取 B；scheduler writer 只取 A。**无任何路径持 B 取 A** → 无 A→B/B→A 环 → 无死锁。且 self 先取 A 再取 B → **B 绝不在等 A 时被持有** → 等 B 的 dispatch 绝不被 A（agent run）的等待阻塞（B 保持短时、dispatch HTTP 不挂）。为何不用长锁 A 给 dispatch：会让「下发」HTTP 挂在运行中的 writer 后。
- **粒度 per-task**：不同 taskId → 不同 B 实例 → 不同 task 并发不互斥。
- **reciprocal 保留**：短锁封根因后，dispatch-scoped reciprocal（pre-rollback precheck + in-tx recheck）**保留为 defense-in-depth**（双层）。
- **第三轮 gate 三细化**：(P1) self 的 pre-rollback 段在 reciprocal 之外**再加 `roundHasDispatchedSelfQuestioner`（本轮 ANY-dispatched，含 consumed）**——home-only OPEN 检查漏 consumed，而 finding-3 数据丢失对 consumed 同样成立，须在 rollback 前拒；(P2) reciprocal 改 **ALL-ROLE**（`hasOpenDispatchedEntryOnHome`：home 上**任一** deferred role〔self/q/designer〕的 OPEN dispatched 条目都拒，一节点至多一个 open ledger，镜像 `assertNoInFlightDispatch` 的任一角色跨域）；(P3) `gcTaskWriteSem` **同时回收 `questionWriteLocks`**（同 idle 守卫 + 自愈），免长驻 daemon 无界增长。
- **不破 RFC-076**：A/B 只加序列化，mint→write→close 次序与 finding 1 的临界区不变；self A⊃B 只是把 finding 1 的 A 段体再包一层 B。
- **测**（`§5.2.14` + `cross-clarify-service`）：① 并发 dispatch+quick-finalize 同 home → 恰 1 rerun（无双 mint / 无 stale rollback）；② `getTaskQuestionWriteSem` per-task + 区别于 `getTaskWriteSem`；③ 锁短时（持有者不跑 agent，源码 + 注释锁定）。
- **不破 RFC-076 的逐行核对**：mint 仍最先（tx 内首个写）；session flip 在 mint 后（tx 内）；round flip 在 session flip 后（tx 内）；close 最后（tx 后）；rollback 数据安全（续跑字段全来自 `sourceRunRow`，独立于 rollback）。`clarify-rerun-write-ordering.test.ts` 源序锁 anchor 从 `mintNodeRun(` 改 `tx.insert(nodeRuns).values(rerunValues)`，三条 `<` 不变量保持。
- **测**（先红后绿，`rfc128-p5-bc-self-questioner-rerun.test.ts §5.2.14`）：step 1 — 控制下发 q1（in-flight）后 quick-finalize → reject、无双 mint；step 1 — **consumed** dispatched q1（done+output）→ **仍** reject（缺陷 1，翻早稿「consumed 放行」错误测试）；step 2 — seal q1 后 quick-finalize → q1 **与未 seal 兄弟 q2 都** `confirmation='confirmed'`（finding A）、home 不 park、q1+q2 都不可再 dispatch、续跑已 mint；step 2 golden-lock — 无前置 seal 的 quick-finalize 零 confirmed；step 3 — quick-finalize 后 rerun(pending)+session(answered)+clarify(done) 终态（RFC-076 定序）；step 3 源锁 — `submitClarifyAnswers` 在 dbTxSync 内 mint（race-close 源码层文本断言）。+ 回归：`clarify-rerun-write-ordering` 源序锁、RFC-076/127/128 全套、全量 clarify/cross-clarify。

**第二轮 final gate（临界区边界三 high，承上 question-write 锁 B）**：B 加上后仍有三处「临界区边界没盖严」的数据丢失/原子性洞——

- **①（cross deferred designer 行建在 B 外）**：`submitCrossClarifyAnswers` 的 deferred-designer 续答路径原先在 B-tx **之后**才 `reconcileTaskQuestionsForRound`（独立 tx）建 designer `task_questions` → answered flip 与 designer 行**不原子**，并发 dispatch / scheduler park 可观测到「round 已 answered 但 designer 行还没建」的中间态而误判。**修**：`taskRow` 提到锁前加载；在 B-tx 内（flip 前）按 `isDeferredDesignerPath`（continue + designer-scope 子集 + 有 designer target + task 开 deferred）调 `reconcileRoundEntriesTx(tx, {...round, status:'answered', answersJson, directive, questionScopesJson})` materialize designer 行（与 questioner cascade 的 in-tx reconcile 互斥、幂等、`onConflictDoUpdate` 不碰 `confirmation`）；删掉锁后的 `reconcileTaskQuestionsForRound`（连带删除其 import）。
- **②(a)（quick submit 在取锁前算 lockedIds → 覆盖 sealed answer）**：self/cross quick-finalize 原先在取 B **之前**读 `loadSealedQuestionIds` + `mergeSealedAnswers`（基于 pre-lock 快照）；一个并发 seal（`sealRoundQuestions`）在该 pre-lock 读之后、quick 的 tx 之前 commit 一个 locked answer → quick 用 stale 整轮 `answersJson` 覆盖它（数据丢失，破 P2-2 locked-answer 保证）。**修**：把 lockedIds 读 + answer/scope merge（cross 连带 attribution/designerSplit/cascade 判定/questioner cascade mint 值）全部移进 B 临界区，**在锁内重读当前 `answersJson`/scopes 再 merge**；hoist `sealedAnswers`/`effectiveScopes`/`designerSplit` 供锁后 `sessionAfter`/分支用。B 持有跨整个 run 回调（含其中 awaits）→ 锁内读与 tx 间无其他 writer 插入。
- **②(b)（seal 自身不持 B）**：`sealRoundQuestions`（控制通道逐题 seal）写 round `answersJson` + `task_questions`，原先是**自己的无锁 tx** → 与 quick submit 互不序列化。**修**：锁前先读 `round.taskId`，把其 `dbTxSync` 包进 `getTaskQuestionWriteSem(taskId).run(runSealTx)`（round 不存在则不加锁直跑、由 tx 抛 NotFound）；锁后 `setNodeClarifyDirective` 留在 B 外（异写他表）。
- **锁顺序复核（②(b) 新增持 B 者）**：`sealRoundQuestions` 仅被 `routes/clarify.ts` 调（HTTP-route-only，scheduler 不引用）→ 与其它 B 持有者同样**绝不在 scheduler 长锁 A 下**取 B → 不引入 A→B/B→A 环，死锁自由不变。
- **测**（`rfc128-p5-bc §5.2.14 final-gate (2nd round)`）：②(b) 源锁 `sealRoundQuestions` 的 tx 在 `getTaskQuestionWriteSem(...).run(runSealTx)` 内；②(a) self+cross 源锁 `loadSealedQuestionIds`/`mergeSealedAnswers` 在 B 闭包内（index > `.run(` index）；① 源锁 deferred reconcile 在 flip-tx 内且在 flip 前、`reconcileTaskQuestionsForRound(` 已绝迹；②(a) 行为锁 — 控制 seal q1=A 后 quick-finalize 投不同 q1=B → 终态 q1 仍 A（locked-answer 不被覆盖）。① 行为由既有 `cross-clarify-service` deferred 用例兜底（designer 行 submit 后即在）。
