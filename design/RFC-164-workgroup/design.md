...
> **RFC-215 修订（2026-07-21）**：free_collab 分支已被
> [RFC-215](../RFC-215-fc-dual-track-batch-claim/design.md) §2 取代——任务轨/消息轨
> 双轨拆分互不占用（成员在跑消息回合时仍可被配任务批，反之亦然）；fc 成员认领从
> 「一人一张」改为**批量均分**（每批上限 5，`fc_claim.assignmentIds[]`）；派发顺序改为
> 恢复批→新配批→消息回合（预算末端保任务）。lw 行保持原语义。
...
> **RFC-215 修订（2026-07-21）**：fc 的失败重开预算从「重派次数上限 = 全局
> defaultNodeRetries（按 shardKey 数 node_runs 行）」改为
> `workgroup_assignments.attempt_count` 列（认领即计数，批量 shardKey 下行计数失效）；
> 崩溃恢复矩阵见 RFC-215 design §3.4（reconcile 改按卡上 nodeRunId 直查）。
...
> **RFC-215 修订（2026-07-21）**：free_collab 下「同成员有 in-flight run 则消息并入
> 该 run 结束后的下一次注入」不再成立——消息轨与任务轨并行，成员在跑任务批时其消息
> 回合照常被唤起；fc 任务 run 不再注入/消费 @ 消息（游标单一归属消息轨，RFC-215 §4）。
> lw 保持本节原语义。
