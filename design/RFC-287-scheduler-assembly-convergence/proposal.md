...
| C7  | **收缩**：启动接口不再同步保证「返回成功 = 工作树就绪」（**限定无上传的 JSON-body 分支**；有上传的 multipart/agentLaunch 分支保持同步物化）                                                                                            | 调用方拿到的是「任务已受理」，工作树就绪与否看任务状态。**仓库准备成为任务的第一个执行步骤**（合成运行记录 `__repo_prep__`，时间线可见、可单独重试），启动失败**从此有任务记录与 git 报错原文可查**（今天什么都不留）                                                                                                                                                                                                                                                                                                                               |
