...
- **G4 配额面可配（2026-08-13 用户拍板纳入本 RFC）**：并发/配额共 6 项，设置页
  ...
  修成真即时生效**（见 §4 C9）。
- **G5 `file://` 不对用户开放（2026-08-13 拍板并入；口径以 design §10.7「第二轮门
  P1-1 定音」为准）**：拒绝点是服务层的来源汇流点 `resolveRepoSourceSingle` +
  `refreshCachedRepo`，一处覆盖 URL 直填 / `cachedRepoId` 反查 / 仓库组成员 /
  多仓循环 / 重放 / webhook 命中存量缓存；**注册面（批量导入 / 仓库组保存）刻意
  不拒**（存量可见不可运行）。
  ⚠️ 本行原写「平台内部启动面与测试通道保留（118 个测试/e2e 文件依赖它造仓）」
  ——**已作废**：`internalSource` 是 local-path 面、在汇流点之前就早返回，根本不
  承载 `file://` URL，所以没有任何「内部通道」还能跑它；测试侧已整体改用本地
  smart-HTTP 夹具（`tests/helpers/gitHttpRemote.ts` / e2e 同源那份）。留着这句会
  让人以为服务层还有豁免，照它写代码就会重新开洞。目的：消灭「用户以为基于远端、
  实际基于本机仓」的
...
- AC-8（G5）**运行**两面拒绝 `file://`（启动的来源汇流点 + 镜像刷新），错误可读且
  带稳定错误码；注册面不动（存量可见不可运行）。以 `cachedRepoId` / 仓库组启动
  存量 `file://` 镜像时，**同步段**即拒（proposal §G7 把「地址格式」列为同步校验），
  不得延后成「201 之后后台失败」。
  ⚠️ 原文「内部与测试通道不受影响（118 文件全绿）」已作废，理由同 §2 G5 那条。
