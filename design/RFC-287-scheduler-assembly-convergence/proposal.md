...
| C5  | **收缩**：用户不能再注册/使用 `file://` 仓库源                                                                                                                                                                                         | 「让 agent 在本机某个 checkout 上直接干活」这一入口消失；替代=把仓推到真实远端后注册，或用临时空间。**存量已注册 `file://` 仓与其定时任务的处置见 §7 待定项**。**离线/内网隔离部署将完全无法启动任何任务**（`file://` 是唯一 fetch 恒可达的源；临时空间没有用户代码，不能替代）——用户 2026-08-13 明确拍板「彻底不支持」，不留管理员开关 |
...
**决策：从此再也不支持运行——任何以 `file://` 为仓库源的启动一律以「非法参数」
拒绝**，不区分新建与存量。含手动启动、定时任务触发、webhook 触发三条路径；
