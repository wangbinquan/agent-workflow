...
## T13 / T12 实施记录（2026-08-14，已完成）

### T13（G7 仓库准备异步化）

测绘先推翻了自己的估计：三个准备动作早就住在独立顶层函数里，`materializeSpace`
是单一入口、产物经 8 个字段扇出。所谓「1591 行准备段」实为 `startTaskImpl` 里
两处调用到 INSERT 之间约 150 行。**唯一的死结**是 id 在物化过程中才铸，于是
落行必须等物化。第一刀（`e921c102`）把 id 提成可选入参解开它。

分刀落地：延后准备（`391b49e7`）→ 合成 `__repo_prep__` 行（`15438053`）→
打开开关（`56aca71e`）→ 孤儿回收豁免锁（`7f9fb748`）→ 重试语义（`0919667b`）。

**实测逼出的四处修正**（都不是设计时能想到的）：

1. 准备失败有**两种形态**：`earlyError` 与**抛出**。`resolveCachedRepo` 的超时
   抛 `repo-cache-locked` 504，压根不经 `earlyError`——只接前者会让最常见的一类
   失败（拉不动远端）重新变回「什么都不留」。
2. `task_repos` / `task_space_nodes` 回填缺口。与任务行路径回写放进同一事务：
   多仓的 repos 与 `worktreePath` 必须同时可见，否则中间态是「有工作树但没有
   成员仓」。映射抽 `taskRepoRowsFor` 供两个写入时机共用。
3. 所有权租约要从空租约换成真的，否则清理协议管不到新建的工作树。
4. 响应体拿到空 `worktreePath`（它在准备前用占位值构造）——改为准备后重读。

**撞到并遵从的两条既有护栏**：RFC-098 修订 #10 禁止顶层行 born-running（改为
先 pending 再转 running）；孤儿回收器「无驱动即判死」（靠 `activeTasks` 豁免，
而注册时机已在 G7 核心那刀挪到准备之前，两件事共用同一前提，已加锁）。

### T12（G6 基线同步窗口化重试）

新增 `shared/gitFailureClass.ts`：网络类可重试，鉴权/不存在/无权限**不占窗口**，
认不出来的保守按不可重试。**顺序不可换**——先判 permanent，因为真实 stderr 常
同时含两类词（认证失败时 git 先报连接诊断再报 Authentication failed），先判网络
会让这类失败白等一个窗口。窗口按**总容忍时长**（默认 60s，可配，0=关闭）而非
固定次数：用户关心「最多等多久」。

**设计门点名的前提实测确认缺失并补上**：git 输出此前没有强制 locale。分类器读
stderr 原文，中文环境下 git 会把 `Repository not found` 整句翻译掉，特征词全灭
——网络抖动被误判成不可重试、退化回硬失败，G6 直接失效。已强制 `LC_ALL=C`。

漏斗接线用 `!== undefined` 而非 `> 0`：0 是合法值（关闭重试），用 `> 0` 会让
「显式关掉」被静默忽略、退回默认 60s。
