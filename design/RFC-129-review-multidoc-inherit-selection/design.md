...
selectionStale: integer('selection_stale'), // 0 | 1 | null
```
...
```ts
// —— 新增：加载「上一轮」多文档成员（本 review 节点、同 workflow iteration、item_index 非空、
//        且不属于正在 mint 的当前 round）。跨 node_run（覆盖 US-2 新 run），按匹配键取最新 versionIndex。
const priorMembers = await loadPriorRoundMembers(db, appHome, {
  taskId, reviewNodeId: node.id, iteration, currentReviewNodeRunId: reviewNodeRunId,
})
...
`loadPriorRoundMembers`（新私有 helper，`review.ts`）：

```ts
// 取本 review 节点、同 workflow iteration 内、所有多文档成员行（跨 node_run），
// 排除 currentReviewNodeRunId（正在 mint 的当前 round——其行此刻尚不存在，双保险再排一次），
// 按匹配键（item_path 优先、否则 item_index）取 versionIndex 最大者 = 最近一轮该篇。读其 bodyPath 正文。
...
- **跨 node_run**：iterate / reject / refresh 复用同一 review `node_run`（上一轮成员同 `reviewNodeRunId`）；
  US-2 重开 mint 新 run（上一轮成员在旧 run）。用 `docVersions.reviewNodeId`（表已有该列）+ iteration 过滤即可
  同时覆盖两种（不局限单一 reviewNodeRunId）。
- **取最近一轮**：每个匹配键（path / index）可能有 v1..vN 多轮历史 → 取 `versionIndex` 最大的那行做继承源。
- **`selectionStale` 读取**：`row.selection_stale === 1`。
...
`rowToDocVersion` 映射 `selectionStale: row.selectionStale === 1 ? true : row.selectionStale === 0 ? false : null`
（供 `DocVersionSchema`）。
...
```ts
stale: m.selectionStale === 1,
```

（单文档分支 / `selectionStale` NULL → `false`。）
...
- iterate 重开：新一轮各篇 `selection` == 上一轮匹配篇（含 unselected）；改过正文的篇 `selection_stale=1`。
- reject 重开：同上（覆盖回退重生路径）。
- US-2 新 run：跨 node_run 继承（by reviewNodeId + iteration）。
- 人工重标清 stale：`setDocumentSelection` 后该行 `selection_stale=0`。
- **单文档 golden**：单文档 iterate/approve 后 `selection_stale` 恒 NULL、行为逐字不变。
- loop 隔离：不同 `node_runs.iteration` 不互相继承。
- detail：`getReviewDetail().documents[i].stale` 正确反映列。
...
| **D8** | 单列 `selection_stale`（nullable int），不加索引、不加 CHECK | 与 RFC-079 三列同惯例；数据量小 |
