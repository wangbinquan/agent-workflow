...
- **D6 — 人工环节不冒泡**：子任务进入 `awaiting_review` / `awaiting_human` / 反问时，
  ...
  自身页面。不做跨任务状态机联动。
- **D7 — 允许 fan-out 内子执行，受限额约束**：`wrapper-fanout` / `wrapper-loop` 内允许
  调用节点（每分片/每轮一个独立子任务）；新增全局活跃子任务并发限额与引用嵌套深度上限，
  父任务启动时对冻结闭包做环检测，运行时深度守卫兜底，超限 fail-fast。
...
### 4.2 fan-out 每分片一个子流程

作为工作流作者，我把 `call-workflow` 节点放进 `wrapper-fanout`，以文件分组分片：
每个分片各起一个独立子任务并行执行子工作流，全局活跃子任务限额约束并发；任一子任务
失败按既有 fan-out fail-all-after-join 语义收场。
