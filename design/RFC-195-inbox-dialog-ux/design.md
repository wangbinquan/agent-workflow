...
### 11.4 实施收口证据（2026-07-15）

- clean detached worktree：workspace typecheck/lint/format、backend+shared `5632 pass / 23 skip / 0 fail`、
  frontend `3855 pass`、binary smoke 与定向 e2e/visual 全绿；Codex 实现门最终 `APPROVE`；
- Option B 首轮 Linux run `29407137852` 只缺 dark baseline；artifact 人工读图确认真实暗色后回填，
  dark SHA-256 `10cb3eba…` 与 light `75bbead3…` 不同；
- 首轮同时暴露测试直接改 `data-theme` 会被 `useApplyTheme` 的异步配置读取覆写；改为经配置 API 切换并等待
  `html[data-theme]` 后，主题竞态消失；完整 visual spec 暴露的既有 agents 基线漂移经双平台读图后同步刷新；
- 最终 Linux visual run `29407415776` 为 `12 passed`；实现 SHA `6225130d` 的主 CI run
  `29407388716` 全绿（双平台全量测试、binary smoke 与 8 个 Playwright shard 均通过）。
