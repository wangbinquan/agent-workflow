...
## 0b. 验收记分板（as of `b5347f30b`，2026-09-08）
...
| AC    | 判据                                                                         | 实测                                                                                                                                                                                         | 状态   |
| ----- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| AC-1  | 成对文件数 → 0（**已按实测修订**，见 proposal §7 的修订段）                  | 153 → **10**，`UNVERIFIED_PAIR_COUNT = 0`——十对全部登记在册、各点名双引擎对拍文件、各带逐条裁决。**未登记的成对实现为 0**（88 条 provider 命名文件里 70 条同目录无孪生，详见 §5e 及其口径盲区）  | ✅     |
| AC-2  | `cli/start.ts` 无 `provider === 'sqlite'` 分支；`servePostgresqlDaemon` 删除 | 分支 **0**；该函数已删，仅注释里留历史引用                                                                                                                                                   | ✅     |
| AC-3  | 统一事务原语的双引擎原子性对拍；裸 `db.transaction(` 收敛                    | **裸驱动事务 17 → 0**。账本剩的 27 条不是债，是守卫正则超出自己声明的判据（`this.runner.transaction()` 内部本就走中立原语），收紧判据的方案见 §5f                                              | ✅     |
| AC-4  | 方言表 exact 清单，每条两个 provider 各真实执行一次                          | **`RAW_DIALECT_DEBT` 6 → 0**；`UNSHIMMED_FUNCTION_DEBT` **0**。同批发现 `greatest()` 两个引擎都没有真实执行断言、且**跨引擎不是 NULL 等价**，已钉成两侧显式前提                              | ✅     |
...
| AC-8  | 用户可见行为逐字不变                                                         | 由各波双引擎对拍逐条验证；本 RFC 明确修复的缺陷逐条记在各波小节（W7 的 14 处 PG-only 分叉、W11 的 PG 侧缺 `inClaimTxAfterJournal` 等）                                                        | ✅     |
...
| AC-12 | 组合根全量，`*-not-bound` 为零（第三款已按 AC-1 对齐修订，见 proposal §7）   | 组合根构造 70 → 20；占位账本 49 → **32**（W11 在三个文件上 26 → 9，正解是词法作用域而非改必填，见 §5g）。provider 命名文件 176 → **88**，其中 18 条是登记在册的机制分叉（本款豁免）、70 条命名债交由收尾一刀 | 进行中 |
...
**仍在路上的只剩两件**：①**命名债收尾**（70 条同目录无孪生的 provider 命名文件，分类为
死代码 / 改名两格，是 AC-12 第三款的最后一段）；②**AC-3 的守卫判据收紧**（见 §5f，
把「任意接收者的 `.transaction(`」收窄到「接收者是数据库句柄」，收紧后账本降到 0）。
