...
对「读次数主导、单次查询本身几乎不花时间」的微端点，两侧发同样多次读时
**gap = N × (X_pg − Y_sqlite)**，而 `X_pg > Y_sqlite` 恒成立——进程外一次往返
不可能比进程内一次函数调用更便宜。所以只要 N > 0，差就恒为正；
**任何查询优化都只能按比例缩小它，不能让它变号。**
...
**这不是我可以自行决定的**——本仓硬规则写着访问控制只按显式功能需求与用户可见行为验收，
不由跑测试的人评价或调整其语义。所以 AC-11 的收口是一个**产品裁决**：
是接受「微端点上 PG 慢 0.05ms、重端点快一倍」这一事实（改判据口径），
还是接受「权限变更延迟一个缓存窗口才生效」（改行为）。
两者都不是工程缺口，工程面已经走到头了。
