...
前端「重试准备仓库」按的就是它）：远端仍不可达时，

| | 响应 |
| --- | --- |
| SQLite | **409** `task-execution-outcome-unknown`（`unresolvedDecisionRefs` 指向那次失败的效应尝试） |
| PostgreSQL | **400** `repo-clone-failed`（同步重跑了一次克隆，把 git 原文回给调用方） |

也就是说同一个按钮在两个引擎上给出**两种完全不同的失败**：一个说「这个继续里有结果未知的
操作，请用手动 resume/retry/sync」，另一个直接把克隆错误回给你。

**没有当场定性**，两个原因：①这不在本刀（G7 落地）的范围里，硬塞进去会把一条已经验证过的
判据面搅浑；②我没有证据说明哪一侧是**对的**——`rfc287-t13-deferred-prep` 的 AC-11 用例
（58/58 绿）说明 SQLite 那条路在**它的**夹具里是通的，所以 409 很可能是我这个夹具特有的状态
（效应失败回执被 fence 掉之后留下的 unresolved ref），而不是 AC-11 本身坏了。
...
### 第 8 刀第 2 步落地　preflight 对拍——照出一处真缺陷

**先解锁**：那份「SQLite 的」修复实现（`taskLifecycleRepair.ts` + 各 `options-*.ts`）
**没有任何 SQLite 专有原语**——零同步终结符、无 `dbTxSync`、不 import bun:sqlite。
把它绑死的只有几行 `db: DbClient` 类型标注。放宽到中立句柄之后，两份实现能在**同一个库**上
并排跑，于是这条对拍在**两个引擎的 lane 上都成立**，比的是「两份实现」而不是「两种数据库」。

**第一跑就红 6/8**，分叉稳定且两个库上一致——证实是实现差异。`available` /
`unavailableReasonKey` / 选项集合全部一致，分叉只在 **`previewSteps`**，而且分两类：

1. **信息丢失（真缺陷，已销账）**：`CR-1/S5/S6.acknowledge` 三条在 PG 那份里共用一句
   `Resolve alert <id>.`，而 classic 那份对每条规则各给**两步**——第二步是
   「**这一步不改任何数据**，你接下来该做什么」（S5 去看 pid 然后 cancel/resume、
   S6 去恢复一个成员、CR-1 说明会话已被扫描器改成 abandoned）。
   丢掉它的后果是**用户点完「确认告警」以为问题解决了，而 acknowledge 本身什么都没修**。
   取 classic 那份逐字并按规则分开（共用一个分支正是那句话丢失的原因）。
2. **文案风格（登记在案，不销）**：`S4.kick-task` 的预览，classic 那份把**字面 SQL** 与内部
   函数名露给运维界面（`UPDATE tasks SET status='interrupted'…` / `resumeTask('…')`），
   shared 那份给一句人话摘要。**这里没有信息丢失**——摘要说的正是 SQL 做的事。
   改哪一边都会动某个部署已经在显示的文案，属于产品判断，**留给合并那一步连同它一起定**；
   在那之前先钉住现状，免得它在无人注意时漂出第三种写法。

### 顺带修掉一处「刚变成活风险」的违规

放宽句柄之后 `helpers.ts` 进入了 `rfc359-w5-t19f`（PG 执行面禁止模块顶层捕获 schema 列）的
射程，它那 7 处顶层捕获当场被报。**这一条选择修而不是记账**：顶层常量会把 SQLite 的具体列
对象带进 PG 查询，`bigint({mode:'number'})` 的 mapper 整个丢失、数值列以字符串回到调用方
——**不报错，只是结果错**（W4-B4a / W4-D10 各撞过一次）。把投影挪进函数体即可。
这也说明「中立化一份实现」不是改类型就完了：它同时把这份代码放进了一批**它此前不在射程内**
的守卫里，那些守卫报的是真风险。

## 第 8 刀第 2 步推的红：一条**墙钟判据**的守卫自己假红（与本刀无关，就地修掉）

`b4b1b4cfc` 的 CI 只红一格：macOS 分片 1/6 的
`rfc349-target-coverage-linear-grouping > ten thousand chunks verify without a quadratic
regrouping pass`，`花了 182ms`（预算 150ms）。本刀改的是修复选项的预览文案与
`taskLifecycleRepair` 的句柄类型，**一行都不在 `finalizeSchema` 的回执分组上**。

**先取证再判**（本仓禁止「重跑就过了」）：

- 本机连跑三轮全绿；
- 写了一份微基准直接量 `finalizeSchema` 的规模曲线：`chunks=0 → 0.06ms`、`1k → 0.25ms`、
  `10k → 1.48ms`、`40k → 5.72ms`、`80k → 10.80ms`——**完美线性**，固定开销 0.06ms 可忽略。
  也就是说 CI 那 182ms 不是「分组变慢了」，是在一个 **1.5ms 宽的窗口**上挨了一次约 180ms 的停顿；
- `docs/audit-backlog.md` 里**已经挂着这一条**：2026-09-06 `0e2343757` 的 macOS 分片 1/4 上
  量到的同样是 **182ms**。同一个数字出现两次，说明它是「一次同等堆规模的 full GC」这种**可重复**
  的停顿形态，不是随机噪声——再调大预算只会把偶发变成更慢的偶发。

**判据本身有病，就换判据**。用例的文件头原本就写着「measures the shape, not the wall clock」，
而断言写的是裸墙钟——按 backlog 早就登记的正解改成**比倍率**：同一形状测 k 与 8k
（10 000 / 80 000），线性实现耗时比约 **8**、二次方约 **64**，判据取几何中点 **21**。
机器快慢对两次测量同等作用，被比值约掉;两个规模**交替**测，让某段时间的整体变慢两边同吃;
每个规模取 **5 轮最小值**——停顿只会抬高某一轮，压不低任何一轮，最小值对它免疫。
另加一道 4s 的「失控闸」，它离线性实现两个数量级、**不参与判定快慢**，只为在复杂度真回潮时
当场给出可读报错，而不是让五轮各跑十几秒最后撞用例超时。

**变异实证（两道判据各验一次）**：把分组改回 `receiptsByTable.set(id, [...current, receipt])`——

| 判据 | 结果 |
| --- | --- |
| 失控闸 | 第一轮 8 万分片 **4888ms** 咬住，整条用例 5.1s 收红 |
| 倍率（把闸抬到 600s 让它单独作用） | **52.3 倍**（`93.21ms → 4879.67ms`），阈值 21 |
| 线性实现（还原后）同机对照 | **7.3 倍**（`1.48ms → 10.80ms`） |
...
### 再更正一次：上一节那个「位置差异」也**还没有被行为证实**，而且探针照出了第三处盲区

按上一节的定式（构造状态、驱动两侧、比错误码与落库结果）写了探针：
任务 `canceled`、一条 `failed` 的行带一个**不存在的** `pre_snapshot` 哈希，然后驱动两侧 `retry`。

实测结果**没有够到那道门**：

- SQLite：`code=no-throw`、事后任务 `status=pending`——重试**直接成功了**，
  回滚判据压根没触发；
- PG：走到 `children.resume` 才炸（w7 里那是替身 `{} as never`）。

原因是 `rfc359-w7` 的 `seedTask` 默认 `worktreePath: ''`——**没有工作树，git 一步都跑不到**，
于是「快照丢了」这个条件在这套种子下构造不出来。

**这本身就是第三处盲区，比原来那条勘察更值钱**：`rfc359-w7` 是这一对适配器唯一的双引擎对拍，
而它的种子让**两侧的回滚路径都不可达**——不是一侧弱，是这一段在对拍里整体零覆盖。
第 9 刀的基线因此必须带**真 git 工作树夹具**（`git init` + 真 stash），不能沿用 w7 的合成种子。

**当前如实的结论**（三次收敛后）：
1. `assertChildTaskDrivable` / `assertNotSourceTerminated` 两格**没有缺口**（A18 已锁，另一格由 CAS 抛同码）；
2. 回滚基线判据两侧都有，**位置不同**（PG 在准入 CAS 前、SQLite 在后）——这一条**源码上可见、行为上未证**；
3. w7 的种子够不到回滚路径，所以 ①② 之外还欠一份带真工作树的基线。
...
### 第 9 刀第 1 步再收口　门序也进对拍——共 **7 格**，仍然逐格相同

再加两格，取自 `retry-node-guard-order.test.ts`（RFC-099 审计 2026-07-15 锁的那条门序：
**校验 nodeRunId 归属必须早于准入 CAS**；旧顺序先 CAS 再查归属，于是一个伪造 / 跨任务的
nodeRunId 会把已完成的任务打成「没有调度器的 pending 僵尸」并抹掉完成元数据，然后才报 404）。
那条锁只在 SQLite 侧；门序在两份实现里**各写了一遍**，此前没有任何东西比较过它们。

| # | 格 | 实测 |
| --- | --- | --- |
| 6 | 伪造的 nodeRunId | `404 node-run-not-found`，任务的 status / finishedAt / errorSummary 一个字段都没动，两侧相同 |
| 7 | 跨任务的 nodeRunId | 同上，且**两个任务**都没动，两侧相同 |

变异实证（两半各验一次、分别落在两侧）：SQLite 的归属校验放宽成「只查存在」⇒「跨任务」那格红
（证明锁的是**归属**不只是存在）；PG 的 `node-run-not-found` 换成别的码 ⇒ 两格都红
（证明错误码那一半也在被断言）。
...
## AC-1 第 13 刀（上）　`assertManualExecutionAllowed` 合一——`TaskRouteOperations` 的第一处真分叉

`TaskRouteOperations` 这一对今天是「一份实现（`taskRouteOperations.ts` 2273 行）+ 两个绑定
（215 / 274 行）」。逐个动词量过之后，**两个绑定里只剩三处是真分叉**，其余全是命名或
「谁来构造」：`get` 两侧分别叫 `loadTask` / `loadTaskProjection`（后者是前者的纯别名）、
`listItems` 同理、`nodeRuns` / `diff` / `stdout` / `events` 只差传 `dependencies` 还是 `{db}`。

三处真分叉是：①`assertManualExecutionAllowed`；②`syncWorkflow`；③启动参与者的取用位置。
本刀收 ①。

**分叉长什么样**：SQLite 那半是本地 27 行——`getTask` + bun:sqlite 的**同步读** `.all()[0]`
取 `workgroup_config_json` + `getWorkflow` 判内置；PostgreSQL 那半内联在绑定里，除了同一条内置
判据，还多一道**「这个工作流现在还在不在、我还看不看得见」**。`rfc359-w7` 的 B3 当年就是为这条
差异开的账：RFC-285 起 `tasks.workflow_id` 是软链，工作流被删之后同一个任务在 SQLite 上**能**
手动执行、在 PostgreSQL 上 404。

**处方是各取更强的一半**：抽成 `assertManualExecutionAllowedProjection`，依赖面刻意只收两样
（库句柄 + 资源权威工厂），好让两条绑定都交得起——PG 在 `dependencies.launch.resourceAuthorityFor`，
SQLite 在 `dependencies.resourceAuthorityFor`，同一个东西、两个位置。

**对拍里的一处「假合一」也一并修掉**：W7 的 SQLite 泳道此前把 `resourceAuthorityFor` 写成
`() => ({}) as never`，因为那一侧的实现根本不碰资源权威。合一之后两条泳道必须用**同一个**替身
（读同一张 `workflows` 表），否则「两个引擎跑同一份代码」这句话在对拍里是假的——本刀实撞：
先换实现、不换替身，SQLite 泳道当场 `undefined is not an object (evaluating
'authority.resources.loadAuthorized')`。

**顺带的里程碑**：B 段（「实测分叉」）**已经没有一条按引擎分叉的断言了**——
`const isPostgresql = …` 随 B3 销账而删除。这一段现在整段是「不许再分家」的锁。

### 账

- `rfc345-resource-acl-facade-compatibility` 36 → 35：`services/workflow.ts →
  sqliteTaskRouteOperations.ts` 那条兼容边随本地实现删除而出账（**债是还掉的，不是挪走的**）。
- T19d 的 `TaskRouteOperations` 由 `sqlite 10/2` 降到 `sqlite 9/2`。
- 第 12 刀的一次性 `allowGrowth` 在本笔退役（本笔不涨）。

## AC-1 第 13 刀（下）　`syncWorkflow` 合一——`TaskRouteOperations` 的第二处真分叉，顺带清掉路由层最后一处 `StartTaskDeps`

**分叉是两台机器，不是两层壳。** SQLite 那一侧转给 `services/task.ts` 的 `syncTaskWorkflow`
（160 行），终端是 `resumeKick` 的**一段式**准入（直接落 `pending`）；PostgreSQL 那一侧走共用的
`syncWorkflow`（288 行），**两段式交棒**——准入 CAS 落 `interrupted` 并打 `continuationHandoff`
标记，随后才交给复活端口，由它 CAS 回 `pending`。两份都真在生产上跑，谁也没跟谁比过。

**共用那份严格更全**，多两样 SQLite 一直没有的东西：
- **回滚基线的跨行零副作用预检**（`assertRollbackBaselinesPresent`）：任一行的 `pre_snapshot`
  已被 GC 回收时**干净失败**，而不是把任务推进 `interrupted` 之后在半截回滚里才发现；
- canceled 写节点的回滚交给 `selectSyncRollbackTargets(['canceled'])`，failed / interrupted
  两档由紧随其后的复活连同 native-session 租约围栏一并处理，**不重复动工作树**
  （退役那份一次性传 `['failed','interrupted','canceled']`，在租约围栏之前就动手）。

### 收窄依赖面才是合并的前提

共用那份原本收整个 `TaskRouteOperationsDependencies`，而 SQLite 侧（尤其 `server.ts` 那条
**不装配完整 runtime** 的路）交不起。于是先把它收成 `TaskWorkflowSyncDependencies`，三格收窄：
...
| `children` + `topology` + `resumeRuntimeFor` | `resumeTaskAs` | `retryNodeProjection` 早就是这个形状 |

**收窄的副产品比合并本身更值钱**：`syncWorkflow` 是路由层最后一个持有 legacy `StartTaskDeps`
的动词。它一合，`SqliteTaskRouteOperationsDependencies` 的 `startDepsFor` / `recovery` 两格
**整格消失**，`cli/start.ts` / `server.ts` / 测试 harness 三处的 `startDepsFor` 绑定一并删除；
RFC-103 的那条计数判据从 `toBe(1)` 变成 `toBe(0)`，并补一条「路由层不得再长回 `StartTaskDeps`」。

### 判据：整套迁走，不是删掉

`tests/rfc109-sync-task-workflow.test.ts`（RFC-109 的 AC 锁，11 个用例）此前钉的是退役那份、
**只在 SQLite 上跑过**。本刀把它整套迁到 `describeEachProvider` + **生产装配**
（`createEachProviderTaskExecution` 交出的 `provider.routes.tasks.syncWorkflow`），
两条泳道各自跑自己部署里真正会执行的那条路；原判据一条不少，30 pass / 0 fail。

迁移照出两处**测试侧的「假合一」**，都已修：
1. `eachProviderTaskExecution` 的 `routeLaunch.agent.resources` 两条泳道都是「一碰就炸」的桩，
   于是 sync 一调用就撞桩——换成生产那份 `composeAgentLaunchResourceOperations({ db })`。
2. `rfc359-w8-task-route-capability-parity` 的 SQLite 泳道此前把 `resourceAuthorityFor` 写成
   `() => ({}) as never`、`resumeTaskAs` 写成空桩，还为「SQLite 会真起调度」专门留了一个
   `driver: 'real' | 'noop'` 旋钮。两侧收成同一个替身 + 同一个交棒记录点之后，**那个旋钮没有了**。

AC-2（「done 任务上新增的节点在同步后被派发」）改成**有界等待**：复活在生产上是 fire-and-forget，
派发是它之后调度器读新图的结果。等不到就红，不接受「重跑就过了」。

### 账

- `services/task.ts` 少 160 行（`syncTaskWorkflow` 整条退役）；`resumeKick` 仍有两个消费者
  （gate continuation / dynamic-workflow confirm），不在本刀范围。
- `rfc359-w5-test-engine-hardcoding` 317 → 316（`rfc109-sync-task-workflow` 那格销账）。
- `rfc104` 的内置判据锚点从 SQLite 专属文件改到共用实现（`assertNotBuiltin('workflow', candidate)`）。
- `TaskRouteOperations` 这一对剩下的最后一处差异是**启动参与者的取用位置**
  （PG 在绑定里现造，SQLite 由装配方交）——那是「谁来构造」，不是引擎差异，随下一刀收。

## AC-1 第 13 刀（收尾）　`/api/tasks` 只剩一个工厂——`TaskRouteOperations` 这一对销账

上下两半把两处真分叉合掉之后，把两个绑定逐行摆在一起看，**第三处**也现形了，而且它同样不是
引擎差异：PostgreSQL 那个绑定除了收 `launches`（批次二 ⑥ 交进去的启动参与者），还自己用
`dependencies.launch` 再 `createTaskExecutionLaunchParticipant(...)` 造**第二个实例**给
`launchWorkflow` 用——同一份依赖造两台，两个启动动词各用各的；SQLite 那侧一直是一台。

收掉它之后两个绑定**逐行同义**，于是合成一个中立工厂 `createTaskRouteOperations`，
两份 provider 前缀文件退役。依赖面同时做了三处整理：

- `launch: Omit<TaskRouteLaunchDependencies,'db'>` 整格删掉，换成它真正用到的两样
  （`resourceAuthorityFor` / `validateHostWorkflow`）——**路由不需要整束启动依赖，它只需要两个函数**；
- `resumeTaskAs` / `cancelChildTaskForCascade` 两个 lambda 折回 `children` + `topology` +
  `resumeRuntimeFor`（PostgreSQL 原本的形状，一个端口比两个 lambda 少一层可漂移面）；
- `users: TaskRouteUserDirectory` 是**零消费者**的摆设，一并删除。

`server.ts` 那条**不装配完整 runtime** 的回退路因此要自己装一份子任务生命周期参与者——
第 12 刀之后那就是一句 `createChildTaskLifecycleParticipant({…})`，端口按这条路的部署形态绑。
它的取消 / 复活 / 级联取消三处从此走**同一个参与者**，不再各拼各的
（此前 `cancel` 走 `composeTaskCancellation`、`resume` 走 `resumeTaskProjection`、
级联取消又单独写一份 `composeTaskCancellation(...).cancel(id, {kind:'parent-cascade'})`）。

### 对拍里的第三处「假合一」

W7 / W8 两份对拍此前**各有两个构造函数**，喂的替身还不一样（SQLite 泳道的资源权威是空对象、
`children` 根本不存在、W8 还为「SQLite 会真起调度」留了个 `driver: 'real' | 'noop'` 旋钮）。
那种形状下两条泳道驱动的根本不是同一份装配。合并后两份对拍各只剩**一个** `operations(harness)`，
唯一按引擎变的是库句柄——A 段 64 格 + W8 能力 22 格全绿，一格没改判据。

### 账

- `PROVIDER_PAIR_COUNT` **5 → 4**；`PROVIDER_NAMED_FILE_DEBT` 29 → 27；
  T19d 的两条配对条目销账（`rfc359-w5-coverage-parity` 5 → 4、`rfc359-w5-inverted-pairs` 3 → 2）。
- provider 适配器语料 68 → 66、provider 命名函数语料 71 → 69（**是合一不是删覆盖**）。
- `rfc294-review-off-dag-offered-edges` 41 → 39（两份文件的 collaboration offered 边一起出账）；
  `server.ts → sqliteTaskRouteOperations` 那条 legacy inbound 边改指共用实现（**债没增**）。
- RFC-305 的已审消费者账本少一条（SQLite 那半的 `OwnerIdentityQueries` type-only 边）。

### AC-1 现状

账本剩 **4 对**，逐条指得出 §5fq 的哪一条：
`ResourcePackageMaintenance`（②两套落盘工件格式）、`LogicalSource`（②文件代号 vs 活跃生成代）、
`Migrator`（②编号 SQL 文件 vs advisory lock 下按声明准备 schema）、
`SourceTerminationParticipant`（待复核——它的理由写的是「提交后事件 / 停止位置 + SQLite 无 driver
收尾机制」，那更像部署形态，**下一刀先量它**）。

## AC-1 第 14 刀　源终止参与者合一——W12 那三条「不合」的理由逐条量过，一条也不是引擎差异

`SourceTerminationParticipant` 是账本上最后一对「理由看起来像机制、其实是装配」的孪生。
W12 写的理由是「两侧既有的提交后事件 / 停止位置与 SQLite 无 driver 收尾机制」。逐条量：

| 差异 | 是什么 | 处置 |
| --- | --- | --- |
| 发布 / `requestStop` 相对**评审锁**的位置（SQLite 在锁内，PG 三样全在锁外） | **时序选择**，不是引擎强加 | 取强的一半：`requestStop` 只是取一张停机票据（非阻塞），放进锁内让「状态写入」与「停机请求」相对任务评审 FIFO **原子**——中间插不进一次评审写入；真正会久等的 `awaitStopped` 两侧本来就在锁外 |
| 运行时注册表（PG 收参数 / SQLite 读模块全局） | **部署形态**（同第 12 刀） | 做成参数，两个组合根各绑各的 |
| 无 driver 时的工作区收尾（SQLite 有、PG 没有） | 两侧收尾器**确实不是同一个函数**（PG 是 source-control 的 `finalizeClaimedWorkspace`，SQLite 是 `finishClaimedWebhookWorkspacePrune`） | 正因如此它该是**注入端口**——但「要不要收尾」两侧必须一致 |

第三条值得单记：查下来**两条 boot 都装了同一条 durable 消费者**（`nudgeWorkspacePrune`，
`cli/start.ts` 的两个 provider session 各一处）作为兜底，所以 PG 并没有「收不掉」；
但同步收尾让回执（`no-active-owner`）在**返回时**就是真的，而不是「稍后由事件泵补」。
本刀把它抬成两侧一致：`composeTaskSourceTermination({ db, finalizeWithoutDriver })`，
两条 boot 各交自己那一份。

### 账

- `PROVIDER_PAIR_COUNT` **4 → 3**；`PROVIDER_NAMED_FILE_DEBT` 27 → 25；
  同文件孪生账本 14 → 13（`sourceTermination.ts` 那条的原话就是「先合下层，这一层自然塌成一份」，
  下层一合它真的塌了）；T19d 的配对条目销账（`rfc359-w5-coverage-parity` 4 → 3）。
- provider 命名函数语料 69 → 67、适配器语料 66 → 64、组合根语料 15 → 14（**都是合一不是删覆盖**）。
- W12 对拍里那条 `const publishInsideLock = harness.capabilities.isolation === 'exclusive'`
  翻成 `expect(heldAt).toEqual([true, true, false])`——**两侧同一档**。

### AC-1 现状：账本剩 3 对，全部命中 §5fq 的第②条
...
## AC-12 裁决与收口　占位计数不是完工度量，**逐条取证**才是；已知的那条逃逸早已不存在

§5fi 把 AC-12 停在一次方向裁决上，两个选项：①按形状豁免 token 注册表，**并同时**补上
`server.ts:2525` 那类文案逃逸（两件事要一起做，否则账本更不准）；②做
`CollaborationCommandContext<C>` 的能力入类型（API 形状变更，影响全部调用方）。本轮裁决如下。

### 一、那条已知逃逸**已经不存在了**——§5fi 的记载过期

账本与 §5fi 都记着「`server.ts:2525` 有一处与 PG 根逐字同构的兜底，文案写成
`'task catalog requires the digital employee runtime'`，marker 与 prose 两条判据都咬不到」。
实测：**`packages/backend/src/` 全树零出现**，`git log -S` 定位到它在
`41c7316a6`（「完整装配 daemon 依赖并补双引擎执行验证」）那一刀里随装配补齐而删除。
所以选项①的后半句已经**不需要做**——它要补的那个洞自己被堵上了。

### 二、选项①的前半句**单做会让账本更不准**，而后半句已无对象 ⇒ 不做

§5fi 自己写明两件事要一起做。后半句没有对象之后，只做前半句（豁免 token 注册表、把计数
从 9 降到 6）就是纯粹地放宽守卫——账本总则原话「加一条豁免比修一处问题便宜，这正是账本会
失控的方向」。**不做。**

### 三、试过选项①后半句的**一般化**版本，实测过不了精度关（这是本轮新增的证据）

不靠文案、只靠结构能不能咬住那类逃逸？本轮真写了一条判据并实测：
**嵌套闭包（depth ≥ 1）里对属性访问链判空并当场抛**即算（`testsNullishPath` + `accessPath`）。

- 第一版：**13 个文件 / 44 处**。
- 第二版加上「排掉闭包自己的形参」（装配占位的根是从外层捕获的装配产物，业务校验的根是入参）：
  **仍有 38 处**。

抽查下来绝大多数是**业务参数校验**（`employeeTypePackage.ts` 的
`'collect-pipeline requires the current MR and a pipeline patch'` 一类），而且它连**守卫头注里
明确写着「不是债」的那两处**都收了进来（`webhookAdmission.ts` 的 `resourceSnapshot`、
`taskDagScope.ts` 的 `f`）。**结论：在这个粒度上，结构式判据分不开「装配占位」与「运行期收窄
兜底」。** 把它接进账本会让 9 变成 47，而多出来的 38 条里没有一条是晚绑定装配槽——
那不是让数字变准，是让它变得更没意义。判据已回滚，只把这次测量留在案。

### 四、裁决：AC-12 按**逐条取证**收口，不按数字收口

AC-12 的判据原文是「全量装配，无晚绑定占位，退役未豁免 provider 文件」——三件事，只有第二件
被人当成了一个数字。逐条核完当下源码（不是读注释，是重新打开文件看）：

| 条目 | 实测形态 | 是不是晚绑定装配槽 |
| --- | --- | --- |
| `cli/start.ts:3315` / `server.ts:3064` / `cli/postgresqlDaemonApplication.ts:1719` `marker=1`×3 | `intentCatalogActors.get(context)` 取不到就抛；`contextFor` 在**同一个对象字面量**里 `set` | **否**。不透明 token 的运行期不变量：来了一个不是本目录铸出来的 context |
| `commandContext.ts` `prose=5` | 五个**可选能力**（`artifacts` / `reviewDecisions` / `questionDispatches` / `clarifyDecisions` / `taskExecutionReadModels`） | **否**。`infrastructure/review.ts:1078` 与 `:1598` 实测只传 `{ db, appHome }`，造出来的 context 本来就没有那几个命令口 |
| `taskEngineApplication.ts:572` `marker=1` | `dynamic-workflow-operations-not-composed`，在 `dynamicWorkflowArgs()` 里 | **否**。**条件依赖**的 fail-closed：只有选了动态工作流生成引擎的任务才走到它 |

**另外两件事本轮都在推进且可量**：`未构造组合根` 保持 **0**；provider 命名文件本轮从
27 一路降到 **25**（第 13 / 14 刀各带走两个），全程 88 → 25。

所以 AC-12 的收口姿势是：**占位账本保持现状（不豁免、不改名、不改必填），并在此记下每一条的
取证**；把「计数降到 0」从完工线上拿掉——它度量的不是 AC 要的东西，而这一点现在有测量支撑
