...
| AC    | 判据                                              | 实测                                                                                                                                                                                                                            | 状态   |
| ----- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| AC-1  | 已登记的机制差异保留对拍，其余重复实现合一        | 同目录153→9对均已登记；W49再将两处字符串数组decoder复用既有parser，五个原调用与完整调用者保持。跨目录/内联真实重复仍待清理                                                                                                      | 进行中 |
...
| AC-6  | 全量 backend 行为套件在真 PostgreSQL 上进 push CI | 1962测试文件、605构库文件/1195调用，无harness556→545、有harness474→485。W49在11旧文件中迁移75原功能声明（展开77例），55原例保持native；真实双库待新SHA。W48主1339/1339全部通过，不替代全量覆盖                                  | 进行中 |
...
| AC-9  | 含全部 RFC 改动的 exact-SHA CI 全绿               | W48 exact509e0f35d Main34411434156终态38 success/2 failure，13后端全过；主1339/1339、新增196/196、独立2/2与原hook18/18全部通过，两OS各1962文件恰一次。完整Main仍未绿。W49新SHA仍待验证                                          | 待办   |
...
| AC-12 | 全量装配，无晚绑定占位，退役未豁免 provider 文件  | 原始占位文本32→9、未构造根账本0；provider命名88→59保持。W49仅收敛两处重复decoder，不能据此关闭其余装配/命名与真实重复                                                                                                           | 进行中 |
