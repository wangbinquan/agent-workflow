...
| AC    | 判据                                              | 实测                                                                                                                                                                                                                                           | 状态   |
| ----- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| AC-1  | 已登记的机制差异保留对拍，其余重复实现合一        | 同目录153→9对已登记；W50再将catalog私有字符串数组decoder复用既有parser，原完整caller保持。跨目录/内联真实重复仍待清理                                                                                                                          | 进行中 |
...
| AC-6  | 全量 backend 行为套件在真 PostgreSQL 上进 push CI | 1962测试文件、602构库文件/1182调用，无harness545→532、有harness485→498；T19f对应三文件归零退役，账本688→685。W50迁移68原声明/72展开，未选30例保持；W49主1580/1580、新增241/241实际通过，不替代全量覆盖，W50新SHA待验                           | 进行中 |
...
| AC-9  | 含全部 RFC 改动的 exact-SHA CI 全绿               | W49 exactbb2127ad Main34417055489终态38 success/2 failure，13个后端与普通lint全部通过；主1580/1580、新增241/241、独立2/2及原hook18/18全部实过，两个OS各1962原生文件无遗漏/重复。完整Main仍未绿，两个非后端失败只记录任务元数据。W50新SHA待验证 | 待办   |
...
| AC-12 | 全量装配，无晚绑定占位，退役未豁免 provider 文件  | 原占位文本32→9、未构造根账本0，provider命名88→59保持；W50仅减少一项真实decoder重复，装配/命名与其余重复继续开放                                                                                                                                | 进行中 |
