...
- W12 第八批：插件写入与工作流投影合一、多库对拍夹具已落候选；逐批 CI 与未达 AC 继续以记分板为准
...
- AC-6 仍未达成：CI 的真 PG 服务已经到位，但仍有大量行为用例直接创建 SQLite 内存库。
  W12 已迁 58 个技能/MCP/memory/开发自动化等行为套件，真实任务经过完整 provider 工厂执行到 done；
...
- AC-12 继续按实际装配能力收口；完整构造参数、精确返回类型与后声明闭包已经替代多处可漏绑空槽。
  未构造根已归零，数字员工两种执行类型、协作路由与实时回放都有真实行为用例；原始占位仍有 11 处。
