...
- [x] **RFC-238-T3 Shared + migration**：strict request/DTO/status/receipt schemas；新增 session/
      ...
      migration fresh/upgrade/rollback fixture 与约束测试均已落地。

- [x] **RFC-238-T4 Service 与 HTTP/WS**：owner ACL、stable-id + operation hash、runtime
      fingerprint/capability、create/message idempotency、single-flight/idle CAS、cancel/end、
      SessionView endpoint、owner-scoped private WS locator 与 1.5 秒 polling fallback 均已完成。

- [x] **RFC-238-T5 进程生命周期抽取**：从 `runSystemAgent` 抽 runtime-neutral captured attempt，
      ...
      daemon shutdown hook、周期 reconciliation、cleanup retry 与 receipt GC 已接入并覆盖恢复测试。

- [x] **RFC-238-T6 OpenCode capability**：独立 `mcp-test-v1` verified plan、closed controlled
      ...
      capability-driven plan 不调用 legacy task spawn，也不以 protocol 白名单放宽准入。
...
- [x] **RFC-238-T7 Claude Code capability**：专用 one-MCP spawn；built-in tools disabled、
      ...
      qualification；已复用 RFC-237 的公共 binary snapshot 原语并保持能力声明独立；真实进程
      两轮 stateful fixture MCP E2E 已通过。

- [x] **RFC-238-T8 Capture 与 Session view**：抽 ordered sink core；session-wide event cursor、
      canonical external-key dedupe、root-id CAS、cross-turn `extraUserPrompts`、capture limit/
      incomplete/truncated/WS final cursor；durable ordered sink、跨轮 dedupe、limits、Session view
      与终态 cursor refetch 均已完成。
...
- [x] **RFC-238-T10 自动化与真实验证**：shared/backend/frontend 定向；mock runtime + fixture
      MCP 两轮 E2E；cancel/end/expiry/config drift/ACL/response-loss/recovery；desktop light/dark、
      390px、keyboard、axe；全仓门禁与双 binary smoke。最终本地结果：Backend
      `7696 pass / 26 skip / 0 fail`、Shared `1531/0`、Frontend `5375/0`；真实进程 Claude
      两轮 MCP fixture、浏览器双 runtime picker/失败态/responsive/a11y、production + E2E binary
      build/version smoke 均通过。OpenCode verified plan 与 RFC-224 source reachability 已闭合。

- [x] **RFC-238-T11 实现门与收口**：从精确实现 commit 的分离 worktree 跑 Codex 实现门，
      `P0=0 / P1=0 / P2=0`；RFC/STATE/index 已更新。按用户授权精确提交、push 与 exact-SHA CI
      由本次发布流程完成并以 Git/CI 记录为准。
...
- [x] strict schemas 与 unknown-field/size/enum 负例。
- [x] create receipt + session + first turn 原子；active partial unique。
- [x] create clientCreateId exact replay/mismatch 在 active-conflict 判断之前；旧 transcript
      清理后重放仍不产生 side effect。
- [x] message single-flight、clientMessageId replay/mismatch。
- [x] idle/message/end CAS race。
- [x] queued cancel/end/hard deadline 与 semaphore 释放竞态不 spawn。
- [x] cleanup-complete ended replacement；pending/quarantined row 不被误删。
- [x] cleanup pending 可新建、quarantined 阻止新建且不丢 recovery row。
- [x] secret absence audit。

### ACL / mutation

- [x] viewer 可创建；另一 viewer 不可读；owner 与普通 MCP manager 不可跨用户读；system admin
      exact-id 可审计。
- [x] revoke/visibility/disable 与 session-ending 同事务；delete restrict + 显式 teardown；
      runtime mutation 触发正确 end/block。
- [x] user deletion 先 teardown test sessions，owner FK 不允许级联丢 recovery identity。
- [x] delete 在 live child 未证明回收时失败关闭。

### Runtime / lifecycle

- [x] driver capability source/completeness lock。
- [x] OpenCode exact-one-MCP + effective MCP-only permission。
- [x] OpenCode new/resume owner/lease/ACK pre-prompt。
- [x] OpenCode persistent identity 不含 per-turn path，但锁 exact execution digests。
- [x] Claude exact argv/env + 首轮预持久化 `--session-id` + MCP-only behavior + resume。
- [x] systemAgentRun 既有路径行为不变。
- [x] timeout/cancel/TERM→KILL/group reap/pipe flush。
- [x] raw frame/单 event/stderr tail 有界，超限后固定 buffer drain。
- [x] PID reuse guard、graceful shutdown、boot recovery、cleanup quarantine。
- [x] plan identity receipt 与 PID/raw+wrapped command receipt 均在 prompt/stdin 前持久化。
- [x] idle single timer + periodic reconciliation。

### Capture / frontend

- [x] session-wide event sequence + resume replay dedupe。
- [x] root session mismatch fail closed。
- [x] 多轮 `extraUserPrompts` 与完整 tool events。
- [x] capture terminal precedence、limits、final cursor refetch。
- [x] Dialog public primitives、dirty paths、runtime lock、close/reopen。
- [x] Cancel 与 End 独立；idle deadline 只展示、不客户端续期。
- [x] `SessionConversationPanel` source guard。
- [x] zh/en、desktop light/dark、390px、keyboard、axe。

### E2E / gates

- [x] mock provider + fixture MCP 至少两轮 native resume。
- [x] close/reopen while running。
- [x] cancel then continue；running end；idle expiry；new session。
- [x] builtin/other MCP negative access。
- [x] config/runtime drift、ACL revoke、create/message response-loss。
- [x] `bun run typecheck`
- [x] `bun run lint`
- [x] `bun run test`（Backend `7696 pass / 26 skip / 0 fail`；Shared `1531/0`；
      Frontend `5375/0`）
- [x] `bun run format:check`
- [x] `bun run depcheck`
- [x] production + E2E binary build/version smoke。
- [x] Codex 实现门 0 open P0/P1/P2。
...
设计门关闭后，用户已进一步明确回复“批准”，构成本 RFC 的实施批准。实现、强化验证与
实现门均已完成；用户随后要求“完整实现并提交上库”，本次发布流程负责精确提交、push 与
exact-SHA CI 核验。
