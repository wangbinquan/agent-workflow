...
- 状态：Implemented / release pending（2026-07-24；独立复审与本地全门已完成，
  累计 23 组 P1 / 14 组 P2 全部 resolved，实现门为 `APPROVED / 0 open`；首个
  implementation SHA `b4b3e082c0bf010f123c3e93c7b9abbd1f4f877e` 已推送但
  远端门禁失败，仅作诊断历史。T32 仍待 follow-up 修复 SHA 的 exact-SHA
  CI / Linux `integration-opencode`（以及实际触发的 visual）terminal green）
...
- AC10 CLAUDE.md 的 inline-恒胜断言与运维文档已勘误；follow-up 完整 gates、
  compiled binary smoke、实现门与修复 SHA 的 exact-SHA CI 全绿后才可完成。
