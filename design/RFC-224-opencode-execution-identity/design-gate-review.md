...
| 同步 POST 会把长 tool-loop 绑定到 HTTP request 生命周期；仅凭 idle 也可能误报成功     | 使用 `prompt_async` 只接受 204；成功必须同时观察 idle，并在其后 GET final WithParts 与 SSE 终态逐字段对账；error/permission/question 一旦发生不可逆失败                                                                                             |
...
| null model、plugin、dependsOn 与普通 retry/followup 行为在各入口不一致                | shared effective-runtime policy + stable failure union；OpenCode model 必选，unsupported 状态覆盖 save/launch/schedule/probe/UI；identity failure 不进 followup，stream-failed 是唯一有界 fresh-process retry 例外，normal/wrapper/workgroup 同分类 |
