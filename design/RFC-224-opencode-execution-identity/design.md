...
> v5，2026-07-23。上游基线固定为官方 `anomalyco/opencode` tag
...
验证 namespace 消失。SIGTERM-resistant、double-fork/setsid fixture 必须无孤儿。
macOS 不运行模型，因此不声称仅靠 process group 回收 daemonized descendant。
...
- Linux bwrap 有 gated real escape + double-fork/setsid orphan matrix；macOS
  business/system/smoke 有 fail-closed matrix，darwin 仅跑 binary/managed-path
  diagnostic（含真实 `plutil` exec count 0）。
