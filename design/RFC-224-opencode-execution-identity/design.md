...
> v7，2026-07-24。上游基线固定为官方 `anomalyco/opencode` tag
...
4. depcheck、`bun run build:binary`、compiled hidden-command smoke；
...
两者互相独立。修复后的新 SHA 及其 CI/integration 终态完成前，T32 保持未完成。
...
**317 pass / 1382 assertions**；typecheck、lint、format、depcheck、
`git diff --check`、single-binary build 与上述 compiled smoke 均完成。本地实现门
终裁恢复为 **APPROVED / 0 open**：累计 **23 组 P1 / 14 组 P2** 全部 resolved，
最终未关闭 **0 P0 / 0 P1 / 0 P2**。这只关闭本地 T28/T30，不提前关闭发布：
proposal/release 仍为 pending，修复 SHA 的 exact-SHA CI、Linux
`integration-opencode`（以及实际触发的 visual）terminal green 前，T32 继续未完成。
