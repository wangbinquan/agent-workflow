...
七轮复审（对六轮补丁）折出 **1 P1，已采纳**：unchanged 跳过判在 tree 记账上——
`submodule update` 是 fail-soft 的，部分失败时实际 checkout 可落后于记账（recorded ==
target 却不等于 actual），六轮的 tree-OID 快路径会跳过修复，让节点从 stale 子仓基线起跑。
修：快路径改判**实际 worktree HEAD**（每子仓一次 `rev-parse`，等于目标才跳过 checkout，且
仍递归校验嵌套实况——父等于目标时子仍可能落后）；O(子仓) 个轻量进程，无 checkout 风暴。
附带闭环两笔 CI 账：`rfc187-wg-merge-conflict-abandon` 的 T8 源码锁按契约演进重写（冲突路
径仍 abandon+discard 不变；merge THROW 保留 iso + pending-merge 可重放 + 五轮 replay-discard
收口生命周期——原"无条件 discard"锁文本自身要求变更时重审，本次即重审）；再扫掉两个真 git
测试文件的 5s 默认 timeout（dirty-submodule-snapshot 实测 5001ms 超时 + nonroot 同险）。

八轮复审（对七轮补丁）折出 **1 P1，已采纳**：七轮的实况 HEAD 快路径会**递归进未声明的
initialized stray 嵌套 repo**（`git add .` 误提交、无 `.gitmodules` 条目、用户已在其中初始化并
推进）——`actual !== sha` 落到 `checkout --detach sha`，而记录的 c0 在其 ODB 可达时 checkout
**成功并静默倒回**用户工作（round 6 的 `currentSha === sha` 恰好整条跳过、未暴露）。修：
initialized 分支在快路径**之前**先查 `submoduleNameForPath`，未声明即 stray、整条 `continue`
（既不 checkout 也不递归——与 uninitialized-stray 分支同策略，平台对未声明嵌套 repo 零接触）。
红→绿实证需真初始化嵌套 repo（`git worktree add` 只留空目录、走已有的 uninitialized 守卫；
须 clone 共享 c0 对象方能让 checkout 成功从而复现倒回），钉 7cde9324 实测红。

九轮复审（对八轮补丁、pin 到本方父提交 73978f78 隔离并发 RFC-222 diff）折出 **1 P1 + 1 P2，
全采纳**：①P1：`submoduleNameForPath` 对「条目缺失」与「`.gitmodules` 存在但 git 无法解析」都
返回 null，八轮的 stray-skip 遂把「配置损坏的**受管**子仓」误判为 stray 静默跳过其更新
（merge-back 报 clean 而子仓停在错误 base）。修：新增 `readDeclaredSubmodulePaths` 区分
`ok:false`（`.gitmodules` 存在但 `git config --get-regexp` exit>1 解析失败）与「干净无声明」；
解析失败对受管子仓**抛 materialize-failed**，不静默跳过。②P2：八轮每个 gitlink 各跑一次
`git config --get-regexp`（每次又扫全部 N 条）=O(N²)+N 进程/materialize 趟；改为**每层
`checkoutMergedGitlinks` 只加载一次**声明集、initialized/uninitialized 两分支复用。红→绿：
committed 损坏 `.gitmodules`（materialize 第③步会把它从 tree 还原，故必须落进 tree）令受管子仓
materialize 抛错（钉 ee797977 实测红——旧路径静默 resolve）。十轮复审再折出 **1 P1，已采纳**：`readDeclaredSubmodulePaths` 只取路径值、丢了名字校验——
`[submodule ""]` + `path = nestedrepo` 可解析（git config exit 0）但 git 自身忽略空名条目；旧
`submoduleNameForPath` 一贯要求非空名，新解析器漏了，遂让 crafted `.gitmodules` 把 stray 仓
重标为受管、被 `checkout --detach` 倒回（绕过八轮守卫）。修：加回名字非空校验（键按
`^submodule\.(.*)\.path$` 提取、空名 continue，对齐 git 行为）。红→绿：committed
`[submodule ""]` + 已推进的初始化 stray 仓，materialize 后仓仍在用户 HEAD（钉 2fdb3a0d 实测
红）。十一轮复审再折出 **1 P2，已采纳**：名字校验用的 JS 正则 `.` 不匹配 U+2028/U+2029，含此类
字符的**合法**子仓名会让正则 null、路径被丢，受管子仓静默漏更新。改 `[\s\S]*`（dot-all，
只拒零长名）。新增 `readDeclaredSubmodulePaths` 直测五例（无 .gitmodules/合法/空名忽略/U+2029
保留/解析失败）钉三轮边界。十二轮复审再折出 **1 P2（本方）+ 1 P1（并发 RFC-221 视觉基线，非本方）**：本方 P2——
`submoduleNameForPath`（attach 查名用）仍用 `.` 正则，与十一轮改过的分类器 name-set 不一致，
U+2028/2029 名的子仓分类器认可但 attach 抛 `no .gitmodules entry`；改同款 `[\s\S]*`，加两
helper 一致性单测。P1 是并发 `e4814bf3`（RFC-221 只刷了 mobile-home-nav 整页基线、漏刷
mobile-nav-open 组件裁剪）遗留，与 RFC-210 无涉、未触碰。**十二轮后本方 diff 仅剩并发噪声，
RFC-210 实现门 4 critical + 十二轮复审全部闭环、helper 家族 Unicode 名一致。**
