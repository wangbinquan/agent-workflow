...
>
> **v3 处置**：`--remote` 的语义改为「刷新时把子仓上游最新**提交进 cache repo 的分支**」——
> 即在读侧跑完 `submodule update --remote` 后，若 gitlink 确有推进，则在 cache repo 上
> 用 `AW_INTERNAL_GIT_IDENTITY` 造一条 `chore: bump submodules` 提交（**仅当** cache repo 处于
> 平台自建镜像、非 path 模式）。这样任务从该分支派生 worktree 时才真正看得到新版本。
> 新增 **AC-23**：`gitSubmoduleRemote=true` 时，上游子仓推进后新起的任务 worktree 里子仓内容为新版本。
> 若评审认为"平台不该在镜像上造提交"，则本目标应**降级为非目标**并从 proposal 删除——
> 保留一个无效开关比没有开关更糟。
...
submoduleAutoRefresh: z.object({
  enabled: z.boolean(), intervalMs: z.number().int().min(60_000).max(7*24*3600_000),
  onlyRecentDays: z.number().int().min(1).max(3650),
}).optional(),
```

> **v1 自相矛盾**（design 写 `.optional()`、plan 写"必填含默认"）：**统一为 `.optional()`**，
> 真实默认在 `resolveSubmoduleParams` 里兜（与既有 `gitRecurseSubmodules` 同构）。
> 但**必须**同步 `config/index.ts` 的 `mergeDefaults`(:104-122) 与 `mergePatch`(:125-152)——两者
> **硬编码**只认 `worktreeAutoGc` / `eventsArchiveThresholds`。不改的后果（审计 P1-6）：
> ① 将来往该对象加第四个字段时，老 `config.json` 原样透传 ⟹ `safeParse` 缺字段失败 ⟹ **daemon 起不来**；
> ② `PATCH {submoduleAutoRefresh:{enabled:false}}` 因 `.partial()` 是浅 partial ⟹ 400，与同形状的
> `worktreeAutoGc` 行为不一致。

### 10.2 管道（v1 只列了叶子）

`resolveSubmoduleParams` 改签名收 config 后，还需打通 **4 层**：

1. `createIsoUnderLock`（`isolatedAgentRun.ts:49-74`）**根本不传** `submoduleMode/Jobs` → 补。
2. `mergeBackNodeIso`（`nodeIsolation.ts:336-340`）签名无配置位，其 5 处 `materializeTree` 全裸调 → 补。
3. `materializeTree` 的 6 个调用点全部补传。
4. `util/git.ts` 目前对 `gitSubmodule`/`gitRepoCache` 用**动态 import 避环**（`:745-748` 注释）；
   让它静态 import `@/config` 有 binary-build 模块初始化环前科 ⟹ **配置一律由调用方传入，`util/git.ts` 不自取**。
