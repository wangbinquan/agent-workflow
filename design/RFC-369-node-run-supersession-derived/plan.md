...
**状态**：**Done（2026-09-23）**——T1–T8 全部落地，实现门已处置，CI 绿（`6a76a82fd`）
...
- [x] AC-2 冲突为 0：以确定性回归锁落地（`rfc369-node-run-supersession` AC-2：两笔同任务铸造都铸完再提交，PG 上事务体各只执行一次；换回改前铸造即红）
...
- [x] AC-6 两条间歇红用例在 `6a76a82fd` 的 CI 全绿、backlog 条目改已修（间歇性问题，后续 CI 再撞即重开该条目）
