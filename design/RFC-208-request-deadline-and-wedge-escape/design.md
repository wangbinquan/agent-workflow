...
```
deadlineMs = max(CLIENT_HARD_DEADLINE_MS, BASE_MS + bytes / MIN_THROUGHPUT_BYTES_PER_MS)
```

取 `BASE_MS = 60_000`、`MIN_THROUGHPUT ≈ 64 KiB/s`（即容忍 512 Kbit/s 的劣质链路）。
200 MiB 上限对应约 55 分钟，足够宽松；而**任何**大小的载荷都有界。

> **`max(档A, …)` 这个下限是必需的（设计门二轮，见 §6-8）。** 初稿公式在小载荷时会低于档 A：
> `tasks.new.tsx:846` 的注释明确写着「**任何 upload-kind 输入都走 multipart 提交，即使
> 零个文件**，以便后端的 min/max 门禁运行」——于是一个 0 B 的 `POST /api/tasks` 按初稿公式
> 只有 60s，而这个端点在服务端还要做仓库解析、worktree 创建，daemon 允许它跑到 255s。
> 那会掐断本可成功的启动，并留下一个结果未知的任务。**传输预算只能在档 A 之上叠加，
> 不能替代它。**
...
> **取消的语义必须按幂等性分裂（设计门二轮，§6-11）。** abort 只终止**浏览器的等待**，
> 服务端可能已经提交。因此：
> - **幂等调用**（GET、显式声明幂等的写）：取消 = 解冻 + 可直接重试。
> - **非幂等写**（`POST /api/tasks`、创建 / 删除 / 升级 …）：取消后**必须进入 outcome-unknown**
>   （走对账或幂等键），**不得**呈现为"已取消，随便重试"——否则用户一按重试就可能重复
>   启动任务，或在已变更的状态上二次写入。
>
> 落地要求：实现前先**逐个登记可取消的 mutation 及其幂等性**（作为 T12 的前置产物）。
> 任何非幂等面若没有对账或幂等键，**就不提供取消按钮**——宁可只有硬截止兜底，也不给
> 一个会造成重复提交的"解冻"。
...
/** 档 B 的纯函数——独立可测，新增上传接口自动继承。档 A 是下限（§6-8）。 */
export function payloadDeadlineMs(bytes: number): number {
  return Math.max(
    CLIENT_HARD_DEADLINE_MS,
    PAYLOAD_DEADLINE_BASE_MS + Math.ceil(bytes / PAYLOAD_MIN_BYTES_PER_MS),
  )
}
...
`apiPostMultipart` **必须**同样接受 `deadlineMs`，默认走 `payloadDeadlineMs(上传字节数)`
（初稿只给 `RequestOptions` 加了字段，multipart 入口没有覆盖手段——设计门 §6-1）。

**`apiGetBlob` 归入档 A，不走公式**（设计门二轮，§6-9）：它的签名只有 path/query/signal，
调用方在拿到响应前**根本不知道字节数**，服务端契约也不保证 workflow export 带
`Content-Length`。硬套档 B 只会退化成"要么 300s、要么继续无界"。若将来出现确实大体积的
下载，再由调用方显式传 `deadlineMs`（或 `expectedBytes`）—— 契约里写明这一点，不留
"默认自动推导"的假象。
...
| B3 | `cli/start.ts:85`(+`:144`) 探针传 `timeoutMs`；**超时仍按探测失败 fail-closed**（释放锁并退出），不得改成继续监听。**同时覆盖 git 启动门禁**：`services/gitVersion.ts:92` 的 `runGit(['--version'])` 同样无超时且在持锁期间执行（设计门二轮 §6-12） | `util/opencode.ts:93-117` 已支持 `timeoutMs` |
...
- **RFC-203 错误呈现层**：新增的 `request-timeout` 码走既有 `resolveApiError` 通道，
  不新建呈现路径。
- **`ResourceSplitPage` busy 令牌 —— 需要的是操作句柄，不是时间戳**（设计门二轮，§6-10）。
  初稿设想"在 `beginBusy` 返回值里带个起始时间戳，守卫自行判断"，**不可实现**：
  - `beginBusy` 目前只返回一个 `release`；`ResourceSplitPage` 只存 `Set<symbol>`；
    `UnsavedChangesGuard` 只拿到一个 `RefObject<boolean>`。时间戳既**不能 abort 请求**，
    也**不能让迟到的 `onSuccess` 失效**——而 D3 两者都需要。
  - 更关键：`SettingsDraftProvider` 与 `workgroups.detail` 的守卫**根本不走 `beginBusy`**，
    只加时间戳无法覆盖它们。

  故改为定义一个共享的**操作句柄**：`{ startedAt, abort(), generation }` ——
  `generation` 用于作废迟到回调（回调回来时比对当前代号，不匹配则丢弃副作用，尤其是导航）。
  该句柄必须**贯通所有守卫调用方**，而不只是 split 页。这是 T9 的真实工作量，
  也是 T10/T15 能否落地的前提。
...
| T-9 | 分类函数**显式接收幂等性**；分别断言幂等 status-0（retriable）与**非幂等 status-0（unknown）** | 单元（纯函数） |
| T-10 | `useActor` 的 queryFn 消费了 `signal` | 单元 |
| T-11 | 档 B 纯函数逐点：`payloadDeadlineMs(200 MiB)` 远大于 300s，**且 `payloadDeadlineMs(0)` 不小于档 A** | 单元（防 §6-1 / §6-8 复发） |
| T-11b | 零文件的 upload-kind 启动（`tasks.new.tsx:846` 那条路径）不被掐断 | 集成（防 §6-8） |
...
| T-B4 | git 缓存 **refresh / delete / resolve 的 warm fetch / cold clone 四条路径**超时 → 底层 git 子进程**确实被 kill**、串行队列被释放 | 后端单元（防 §6-5 / §6-13） |
| T-B5 | git 启动探针挂起 → daemon 释放 PID 锁并退出，不持锁空转 | 后端集成（防 §6-12） |
...
### 第二轮（同日，RFC 提交 `0a67cdc5`）

8. **档 B 公式在小载荷时低于档 A** —— `tasks.new.tsx:846` 注释明确：任何 upload-kind 输入
   **即使零文件**也走 multipart，而该端点服务端还要解析仓库、建 worktree。60s 会掐断合法启动。
   → 公式加 `max(档A, …)` 下限 + 补 0 B 用例。
9. **`apiGetBlob` 无法"默认走公式"** —— 签名里拿不到字节数，服务端不保证 `Content-Length`。
   → 明确归入档 A，需要时由调用方显式传 `deadlineMs`，不留自动推导的假象。
10. **时间戳不足以实现 D3** —— `beginBusy` 只返回 release、守卫只拿到 `RefObject<boolean>`，
    时间戳既不能 abort 也不能作废迟到回调；且 `SettingsDraftProvider` / `workgroups.detail`
    的守卫根本不走 `beginBusy`。→ 改为 `{startedAt, abort, generation}` 操作句柄并贯通所有守卫。
11. **取消必须按幂等性分裂** —— abort 只停浏览器等待，服务端可能已提交；"取消后可重试"
    会造成重复启动任务。→ 非幂等面必须进 outcome-unknown（对账 / 幂等键），否则**不给取消按钮**。
12. **git 启动门禁同样无界** —— `services/gitVersion.ts:92` 的 `runGit(['--version'])` 无超时，
    且在持 PID 锁期间执行，与 opencode 探针是同一个"持锁空转、重启也救不回"的形状。
13. **`withTimeout` 的真实用武之地是 `resolveCachedRepo`** —— warm fetch 与 cold clone 都在
    同一 URL 队列内；只改 refresh/delete 会让冷克隆挂起时子进程与队列继续永久占用。
14. **落档即登记** —— 初稿为规避并发 session 的索引冲突而推迟登记 `design/plan.md` / `STATE.md`，
    但这让其他 session 无法从规定入口发现本 RFC。→ 按 CLAUDE.md 立即登记；混入的他人行按
    多人协作规则保留、commit message 只描述自己的改动。
