...
### D1 —— 硬截止锚定在 daemon 自己的天花板之上

后端 `cli/start.ts:394` 已把 Bun `idleTimeout` 设为 **255s（Bun 硬上限）**，注释写明理由是
插件 `npm install` 最长 60s。这意味着**服务端本来就存在 255s 的天花板**：任何请求若空闲超过
255s，Bun 自己就会关闭 socket，客户端的 `fetch` 随即 reject。

因此前端硬截止取 **`CLIENT_HARD_DEADLINE_MS = 300_000`（5 分钟）**：

- **构造上不可能误杀**：任何今天能正常返回的请求，要么在 255s 内返回，要么被 daemon 自己掐断
  （前端拿到的是 reject 而非超时）。5 分钟这条线只会在"daemon 的关闭信号根本传不回来"
  ——即半开 TCP / 黑洞代理——时触发，而那正是我们要兜的唯一场景。
- 因此**不需要长接口白名单**。这是选 300s 而非"30s + 白名单"的关键理由：白名单是会漂移的
  维护负担，且漏一个就是一次误杀事故。
...
`isDefinitiveSkillWriteError` 的二分（4xx = 确定 / 其余 = 未知）过粗。改为三分：

| 分类 | 判据 | 语义 | 是否持有导航令牌 |
| --- | --- | --- | --- |
| `definitive` | 4xx | 服务端明确拒绝，客户端状态可信 | 否 |
| `retriable` | `status === 0`（传输失败：连接被拒 / 超时 / abort） | **请求未被服务端接受**，安全重试 | 否 |
| `unknown` | 5xx | 服务端可能已提交 | 是，但**可解除**（见 D3 逃生口 + 现有「重新检查」） |

关键修正是把 `status 0` 从 `unknown` 移出：连接被拒绝/超时意味着请求根本没落地，把它当作
"结果未知"从而没收导航权是错误且高频的（daemon 重启即触发）。
...
export interface RequestOptions {
  ...
  signal?: AbortSignal
  /** 覆盖硬截止。仅供确有需要的调用方；省略即 CLIENT_HARD_DEADLINE_MS。 */
  deadlineMs?: number
}
```
...
| 编号 | 修法 | 依据 |
| --- | --- | --- |
| B1 | `scheduler.ts:1083` 改为**先** `releaseGlobal()` **再** `await discardNodeIso` | 与 `:3645` / `:5098` / `:5464` 三处对齐 |
| B2 | 把 `:2875` 的 `await persistIsoBase(...)` 移入带 `finally { releaseGlobal() }` 的保护区 | 与 `:743` / `:4943` / `:5336` 三处对齐 |
| B3 | `cli/start.ts:85`(+`:144`) 探针传 `timeoutMs`；超时按"探测失败"处理并**继续启动到监听**，绝不在拿着 PID 锁的状态下无限等待 | `util/opencode.ts:93-117` 已支持 `timeoutMs` |
...
| B6 | `gitRepoCache.ts:716` / `:812` 套 `withTimeout`；顺带接上死配置 `gitCloneTimeoutMs` | 与 `:637` 已有的 30 分钟兜底对齐 |
...
| T-B1 | 注入一个永不 settle 的 `discardNodeIso` → permit 仍归还（先红后绿） | 后端单元 |
| T-B2 | 注入抛异常的 `persistIsoBase` → permit 仍归还（先红后绿） | 后端单元 |
| T-B3 | 探针挂起 → daemon 仍进入监听状态 | 后端集成 |
