...
| 风险 | 缓解 |
| --- | --- |
| 超时值选小 → 误杀合法长请求 | 两档预算：档 A（体积有界接口）固定值且大于 daemon `idleTimeout`；档 B（上传）= `max(档A, 基线 + 传输预算)`，由字节数推导。**残留风险仍在**：档 B 是总时长而非真正的"无进展"检测（浏览器 fetch 上传侧进度支持不完整），只是把窗口放到足够宽松。**不得**再声称"构造上不可能误杀"——该论证已被设计门推翻（design §6-1 / §6-8） |
