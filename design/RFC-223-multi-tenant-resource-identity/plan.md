# RFC-223 任务分解与 PR 拆分（v5，实现期依赖校正）
...
>
> **实现期依赖校正（2026-07-23）**：live code 证明 skill API 若先切 id、内部仍回落 name service/FS，会保留 rename/delete→同名重建 ABA；fusion 同名全生命周期又依赖 id 磁盘。因此剩余落库顺序固定为 **deferred 0115 → PR-5 → PR-4 → PR-7 → PR-8 → PR-9**。PR-7 可先在隔离副本实现，但不得早于 PR-5/4 集成。
...
- 当前迁移链已到 deferred `0115`（`caf52127`）；PR-5 从 `0116` 起。marker `--> statement-breakpoint`；`when` 严格递增；journal SQL/tag 1:1 + `breakpoints:true`。
