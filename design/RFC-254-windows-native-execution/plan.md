...
| 簇                                        | fail        | 诊断（根因类型）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| rfc253-script-execution                   | 13          | **D3/T22 生产解释器解析已实现**（`scriptRun.ts` WINDOWS_INTERPRETER_CANDIDATES python3→python→py / bash-from-git / node，非生产缺口）；13 fail 全**测试侧可移植性**：①POSIX 路径分隔符断言（`join` 出反斜杠）；②containment 渲染测（bwrap `--unshare-net`/macOS SBPL——Windows 无 provider，应注入 platform 或 skipIf）；③`runContainedProcess` 测硬写 `/bin/sh`+`PATH=/usr/bin:/bin`（Windows 无 `/bin/sh` ⇒ spawn-failed，需 Windows 命令或按 containedSpawn 的 win32 降级姿势改夹具）。同 T31 共性修法。 |
