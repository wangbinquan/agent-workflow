...
- **T40b · win32 file PRIVACY 原语（DACL）· 已完成：ARM64 验收机 + GitHub x64 runner 双双端到端通过（全簇 0 fail）** ——
  `2d01bde7`（核心）+`5609ef0e`（封根推广）+`e01c4464`（末 2 诊断收口）+`ccde2603`（source-guard 回归修复）+`2081a8ed`（x64 别名修复）。**x64 分叉已解**：GitHub `windows-latest` 以 RID-500 内建 Administrator 跑，
  owner ACE 序列化为 SDDL 别名 `LA`，`verifyDaclPrivate` 现认 `LA`/`DA` 为 TCB（RID-500 时 `LA` 满足
  userGranted）；`windows-platform.yml` 已跑真 icacls 隐私簇、x64 转绿。`mode` 在 Windows 合成（可写文件恒 0o666 与 ACL
...
| 簇                                        | fail         | 诊断（根因类型）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| rfc253-script-execution                   | ~~13~~ **0** | ✅ **已修（真机 22 pass / 7 skip / 0 fail）**。D3/T22 生产解释器解析本就实现（`scriptRun.ts`）。13 fail 全测试侧：①HOME/traversal 断言改平台感知（`join`/`+sep`）；②network-fence + readonly 两个 describe（bwrap/SBPL POSIX 渲染，Windows 无 provider）`describe.skipIf(win32)`（登记 policy=2）；③4 个 contained-spawn 硬写 `/bin/sh` 改可移植 `bun -e`（runContainedProcess 本就平台无关，只是夹具 POSIX）。                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| rfc248-materialize-group                  | ~~10~~ **0** | ✅ 已修（真机 13/0）。全测试侧、无生产改动：①afterEach EBUSY（clone 开 bun:sqlite 缓存库句柄，Windows GC 才释放）⇒ `removeTempDirSync`；②`url: file://${dir}` 在 Windows 成 `file://C:\…` 畸形，git clone 拒 ⇒ `pathToFileURL(dir).href`（合法 `file:///C:/…`）；③真 git 物化（file:// clone + 多 worktree）在 Windows 慢，默认 5s 超时 ⇒ 13 条 `test(...)` 统一 `}, 60_000`；④`worktreePath` 形状断言 `/worktrees\/…/` 只认 `/` ⇒ 分隔符无关 `/worktrees[\\/]…/`（生产路径本就对，Windows 反斜杠）。                                                                                                                                                                                                                                                                                                                                                            |
| sandbox-allowback-audit-2026-08-04        | ~~7~~ **0**  | ✅ 已修：三个 `computeSandboxPolicy`→`renderBwrapArgs`/`renderSeatbeltProfile` describe 是 POSIX 沙箱规格（Windows 无 provider、`validatePolicyPath` 拒 POSIX 夹具路径）⇒ `describe.skipIf(win32)`（policy=3）；第四个 runner 源码文本锁平台无关、win32 仍跑。真机 3 pass / 7 skip / 0 fail。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| rfc252-git-hardening                      | ~~6~~ **0**  | ✅ 已修（真机 9/0），纯测试侧。**2 纯函数**：断言用 `toEqual` 固定 argv，未注入 platform ⇒ win32 host 上 D18 的 `core.longpaths`/`autocrlf=false`/`eol=lf` 随行使数组变长/错位 ⇒ 改注入 `platform` 参（`hardenedGitLeadingArgs`/`hardenGitArgs` 本就带该参默认 `process.platform`），linux+win32 两平台各显式断言（win32 序为 hooksPath→D18→fsmonitor）。**4 集成**：陷阱 `.sh`（hook/fsmonitor/diff.external）的 `>> hitsFile` 与 config 值都是**反斜杠 Windows 路径**，git-for-Windows 经 MSYS `sh`（shebang 仿真）执行时 `\` 被当转义 ⇒ 静默写空 ⇒ 连未硬化对照组 `hits()` 都是 `[]`。修法：陷阱内 hitsFile + `core.hooksPath`/`core.fsmonitor`/`diff.external` config 值一律 `replace(/\\/g,'/')`（git 与 sh 三平台都吃正斜杠）。证实 git-for-Windows **确实**跑 `.sh` 钩子/fsmonitor/diff.external（生产 `-c` 硬化本就平台无关，只是夹具路径 POSIX 偏置）。 |
| migration-0102-rfc210-submodule-isolation | ~~4~~ **0**  | ✅ 已修：`withMigratedDb` 泄漏 drizzle 句柄未关（+ bun:sqlite 在 Windows 于 GC 才释放句柄）⇒ 关句柄 + 复用增强后的 `removeTempDirSync`（win32 先 `Bun.gc(true)` 逼终结器释放，再删）。真机 4/0。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| task-start-git-identity                   | ~~3~~ **0**  | ✅ 已修（真机 8/0）。纯测试侧：`makeEnvCapturingStub` 的假 opencode 是 `#!/usr/bin/env bash`+`jq` 脚本（Windows 无 shebang launcher、无 jq）⇒ 端口成 `.ts`、经 `opencodeCmd:[process.execPath,'run',stub.ts]` 换缝（e2e-stub-argv-contract 既有先例），输出字节与 bash 版一致（四变量 capture JSON + `{"type":"text",...}` 行载 envelope，靠 runner raw-text fallback + envelope 正则抽取，wrapper 行故意非严格 JSON）。捕获键用 `basename(OPENCODE_CONFIG_DIR)` 剥 `.opencode` 后缀、PID 兜底，忠实复刻 bash。                                                                                                                                                                                                                                                                                                                                                  |
| rfc213-pre-migration-backup               | ~~3~~ **0**  | ✅ 已修：①afterEach EBUSY 同 migration-0102（`removeTempDirSync`）；②`maybePreMigrationBackup` 的 tarGz 撞 **GNU tar 把 `C:\` 当 rsh host**（git-bash tar 抢占 PATH）⇒ archive.ts 改走绝对 `System32\tar.exe`（bsdtar，与 macOS CI 同 dialect）。真机 4/0。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| migration 红簇（8 文件）    | ~~21~~ **0** | 0002/0021/0041/0042/0057/0058/0104/0138：统一 bun:sqlite GC-gated 句柄致清理 EBUSY，按形态修（beforeEach 重建前 `if(win32)Bun.gc(true)`；目录 rm→`removeTempDirSync`；0104 `withMigratedDb` 捕获 close 泄漏的 openDb 句柄）。0117/0118（`:memory:`）实为绿——scan 的 toBe/toContain hint 是命中断言源码文本的误判、真错全 EBUSY。真机全绿。 |
| rfc099-migration-0045       | ~~4~~ **0**  | 同 0002：beforeEach `Bun.gc` + afterAll `removeTempDirSync`。真机 4/0。                                                                                                                                                                                                                                                                    |
| workflow-schema-migrate     | ~~4~~ **0**  | afterEach 只 `rmSync(tmp)` 不关 describe 级 openDb `db`（被引用 gc 收不掉）⇒ 关 `$client` + 置空解引用 + `removeTempDirSync`。真机 7/0。                                                                                                                                                                                                   |
| rfc223-pr5-boot-restore     | ~~6~~ **0**  | swapInDbFile 修好后暴露**真生产缺陷③**（见下）+ afterEach tmps `rmSync` EBUSY→`removeTempDirSync`。真机 8/0。                                                                                                                                                                                                                              |
| subagent-live-capture       | **0**        | scan 报 41 fail 系假阳性（fail 正则命中被捕获的子代理输出文本），真机 13/0 双跑稳定绿。                                                                                                                                                                                                                                                    |

**真生产缺陷③（RFC-223 技能身份迁移在 Windows 整条不可用，已修 `80285b95`）**：`skillMigrateOp.ts
renameAndSyncParent` 对父**目录** fd 无保护 `fsyncSync`——Windows 拒绝目录 fsync（EPERM）、openSync 目录本身
也可能抛 ⇒ 每次 boot + restore post-swap 链跑的技能身份 barrier 一开始就死、任何技能迁不动。修法：目录 sync 改
best-effort try/catch（rename 本原子，与 `restore.ts fsyncDir` 同款）。回归覆盖=rfc223-pr5。
