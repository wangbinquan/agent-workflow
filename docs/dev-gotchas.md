...
- **共享索引文件要按「错了会怎样」分档，不能一律套「混改一起提」**（2026-08-19，由并发 session 提出、双方各自实测确认）。同一个机制（`git add` 整份共享文件必然裹进他人在制内容）在不同文件上后果差一个数量级：
  ...
  - **最阴的一条：改了已应用的迁移，drizzle 不会报错，而是永远不再执行它**。`sqlite-core/dialect.cjs:672-687` 的判据是 `Number(lastDbMigration.created_at) < migration.folderMillis`——**`hash` 只写不比**。所以「哈希不一致会报错」这个直觉是错的：真实后果是那条迁移被静默跳过，库与代码永久分叉，症状要到几周后某个查询报 `no such column` 才浮出来。本机实测：186 条已应用里有 **8 条**（0052/0069/0084/0085/0095/0107/0125/0139）哈希与仓库文件不符且一切照常运行。**判据**：怀疑分叉时不要看 daemon 起不起得来，直接对每条 `<tag>.sql` 算 `sha256` 与 `__drizzle_migrations.hash` 逐条对拍。
- **别对 `STATE.md` / `docs/**` / `design/**` 跑 prettier——它们不在 format 门里，"顺手格式化"等于替别人改内容**（2026-08-19 实撞，并发 session 独立复核确认；与上一条同族）。我习惯性 `prettier --write STATE.md`，它把 RFC-310 段落里一行以 `+` 开头的正文当成列表项归一成 `-` 并缩进后续行——**一次重排 22 行我根本没打算碰的第三方段落**，混在我的 diff 里提上去就是「谁也没要求、谁也看不懂」的噪音。
  - **判据**：动这三处之前先问「它在 format 门里吗」。答案在 `package.json`：`format` 只覆盖 `packages/**/*.{ts,tsx,json,md}`，`format:check` 另加的 repo-ui 串只有 `package.json` / `playwright.config.ts` / `e2e/**` / `.github/workflows/*` / 两个 scripts。**这三处一个都不在**，跑了既无 CI 收益、又有真实代价。
