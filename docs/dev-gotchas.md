...
- **win32 上「刚拷贝出来的大 `.exe` 立即执行」会与 Defender 实时扫描竞争、约 ⅓ 概率瞬退（exit code 5=ACCESS_DENIED / 段错误、零输出）**：RFC-224/227 每次运行把 175MB 的 opencode 二进制新拷进私有密封目录再 exec；Defender（真机默认 ON）在拷贝落盘瞬间开始扫描并短暂持锁，exec 撞进扫描窗口就失败——`Bun.spawn` 本身成功（有 pid），是**进程瞬退**，所以症状是 launcher 见服务端 stdout 流关闭且无 listen 行 → `bootstrap-failed`（易被误判成慢启动或超时）。真机实证：re-hash 那次整文件读**不**足以喂饱扫描（exec 是另一种访问类型、Defender 会再扫），有界重试才收敛（fresh-copy exec 重试至多 8×@300ms → 12/12 成功，Defender 首次扫净后缓存 clean）。**判据**：Windows 上「写完一个可执行文件立刻 spawn 它」若间歇 exit 5/段错误，先想 AV 扫描窗口；缓解＝对密封根加 Defender 排除目录，或「未就绪即瞬退」时有界重生 spawn。（RFC-254 续三十五记为唯一已知 win32 可靠性缺陷，见 `audit-backlog.md`。）
