...
- **复杂度守卫别拿墙钟毫秒当判据，比「倍率」**（2026-09-17 收，同一条用例假红两次）：上一条讲的是「扫描型守卫要有预算」；这条讲的是**另一类**——判据本身就是时间的那种。`rfc349-target-coverage-linear-grouping` 原断言「1 万个分片 < 150ms」，防 `finalizeSchema` 的收据分组回到 O(k²)。它在 `0e2343757`（2026-09-06，macOS 分片 1/4）与 `b4b1b4cfc`（2026-09-17，macOS 分片 1/6）各红一次，**两次量到的都是 182ms**，而本机连跑五轮是 **1.5ms**。150ms 名义上有 100 倍余量，实际却架在一个 **1.5ms 宽的窗口**上——一次 GC 停顿就够越线（两次落在同一量级，正合「同等堆规模的一次 full GC」的形状）。**墙钟阈值把「复杂度」和「这台机器此刻有多忙」混成一个数**，注定间歇假红；而假红会把人训练成「重跑就好」，真的复杂度回潮反而被淹掉。**正形态是量同一形状的两个规模、断言耗时倍率**：k 与 8k，线性约 8 倍、二次方约 64 倍，判据取几何中点 **21**；两个规模**交替**测（某段时间的整体变慢两边同吃，被比值约掉），每个规模取**多轮最小值**（停顿只抬高某一轮、压不低任何一轮，最小值对它免疫）。另配一道离线性实现两个数量级的「失控闸」，它不参与判定快慢，只为在真回潮时当场给出可读报错，不让五轮各跑十几秒最后撞用例超时。**变异实证**：把分组改回 `set(id, [...current, receipt])` 后，闸在第一轮 4888ms 咬住；再把闸抬高、让倍率判据单独作用，量到 **52.3 倍**（同机线性侧 7.3 倍），两侧各留 2.5 倍以上。
...
- **`architecture/guard-manifest.json` 里有一批「手工维护格」，重跑 census 不会带上它们**（2026-09-18 实撞，推红一次）：`corpusScanner` / `minCorpusFiles` / `assertsAbsence` 这几格是**种子**，`scripts/architecture-census.ts` 读它、原样写回，**不重算**。于是「改完代码跑一次 census」这条定式在它们上面**不成立**：本地跑的那几个账本守卫全绿（它们读的是别的账本），推上去 CI 才在`rfc317-guard-corpus-floor` / `rfc317-guard-negative-fixture` 上翻红。触发条件很平常——守卫的扫描形状变了（本次：从「扫一个目录」变成「读一个文件」⇒ `corpusScanner` 该翻 false），或者给守卫补了一条空集断言（⇒ `assertsAbsence` 该翻 true）。**定式**：动了任何 `GUARD_FILE_NAME_PATTERN` 命中的测试文件（名字里带 architecture / boundary / ratchet / lock / guard / invariants / preflight / callsite / extinction / interlock）之后，**把 `tests/architecture/` 整目录跑一遍**（69 个文件、分片 8 个一批约两分钟），别只跑你以为相关的那几个；真值可以用 census 导出的 `isCorpusScanner` / `assertsAbsence` / `corpusFloor` 三个纯函数直接算出来对账。
- **退役一份实现时，守卫的「语料」会静默缩小——抽取器只认一种写法的那些尤其危险**（2026-09-18 RFC-359 第 8 刀实撞）：`rfc317-transition-table-oracle` 靠 AST 抽 `setTaskStatus({ to, allowedFrom })` 这种**调用参数对象**来数「CAS 站点」。合并两份修复实现、退役其中一份之后，语料从 **35 掉到 15**——而掉走的那 20 个正是这条预言头注释里逐条点名的**反例来源**。留下的那份把同样的 `(from, to)` 放在 `kind: 'task-transition' 的动作描述符里，一样静态可知，抽取器却看不见。**症状是「守卫还绿着，射程却少了一半」**，比红更难发现：只有「语料非空」那条下限断言偶然挡了一下（`>= 35` 变成量到 15）。**定式**：①这类守卫必须有「语料下限」断言，且下限要够高、能在射程塌一半时报出来（只写 `> 0` 等于没有）；②被测实现换写法（哪怕只是把参数对象挪进一个描述符）时，要回头问一句「原来那个抽取器还认得它吗」；③给抽取器补第二种形态时，把两种形态各写一条自证 fixture，别让「只认一种」再次静默发生。本次补上第二种形态后语料回到 28，三条越界边按新锚点重新入账。
- **源码锁的锚点文件被删掉时，要跟着实现走、不是删条目**（同一刀，8 处）：`lifecycle-repair-grep-guard`（不许裸写 node_run 状态 / 不许 delete）、`rfc165 A7c`（会复活执行的修复选项必须盖章）、`rfc193 case 8d`（S1 的 scopeRoot 要按 wrapper 血缘推导）、`s13 G5`（fork 排序不得回来）这些锁**锁的是行为约束，不是那个文件**。实现搬家 / 合并时把锚点换到留下的那一份，约束就继续生效；顺手删掉条目则等于把约束一起退役——而这次重挂锚点**当场照出一处真缺陷**：留下的那份 S1 直接传 `task.worktreePath`，正是 case 8d 明令禁止的写法（wrapper 内的评审修复会落在任务根而不是 iso 工作树上）。
...
- **`bun test` 全绿 ≠ 类型对**（2026-08-16 实撞）。给共享测试 harness 的构造函数加了个字段却忘了加进它的 `interface`，33 条用例全绿、`tsc` 红。bun 跑测试时不做类型检查，所以**测试绿只证明运行时行为，证明不了类型面**——这正是门禁把 typecheck 单列一条车道而不是让它跟着测试走的原因。改完 harness / fixture 的**形状**（不只是值）时，跑一次 `bunx tsc --noEmit` 再说。
  **2026-09-13 又撞一次，代价是把 main 推红**（RFC-359）：一批 27 个测试从单引擎迁到双引擎 harness，本地「改过的文件跑 eslint + prettier + 那几个测试」全绿、两个引擎都过，CI 红在 `Lint + Typecheck + Format`——11 个文件把更宽的 db 类型传给了形参写死窄类型的函数，**运行时正好都只用到两个类型共有的 API**，所以只有编译期看得见。⇒ **凡是动到类型面的改动**（换参数类型、换 import、放宽/收窄签名、加泛型），提交前必跑 `bun run typecheck`；eslint 不做类型推断，替代不了它。
  **反过来也要跑**（2026-09-13 同日第三次推红）：放宽一个形参会让原来的窄类型 import 变成死的，`tsc` 不管这个、`eslint --max-warnings 0` 会当场红。所以「改完类型跑 typecheck」和「改完任何文件跑 eslint」是**两件事**，缺哪件都能把 main 推红——**提交前对着 `git status` 的****完整改动列表**各跑一遍，别只跑「我觉得动过的那几个」。
...
## 单引擎测试迁双引擎：先搜 `.run()`，它是唯一编译器帮不上忙的那一种（2026-09-14 连撞三批）

把一份钉死在 bun:sqlite 上的测试改成 `describeEachProvider` 时，真正的工作量不是换 harness，
是把 bun:sqlite 的**同步执行面**全部换成 `await`。三个 API 的危险程度差得很远：

| API | 中立客户端上的行为 | `tsc` 拦得住吗 |
| --- | --- | --- |
| `.all()` | 返回 `Result<…>` 而不是数组 | **拦得住**，`.length` 一取就编译错 |
| `.get()` | 返回 `Promise` | **多半拦得住**，下游取字段会报 possibly undefined |
| `.run()` | 返回**没人 await 的 Promise** | **完全拦不住** |

`.run()` 是唯一一种「类型完全合法、`tsc` 一声不吭、运行时那句写根本没发生」的写法。
它在 SQLite 上是同步执行所以一切正常，换到 PostgreSQL 上立刻变成「写了个寂寞」——
症状通常离现场很远（本仓实例：草稿行没落库，apply 读到的是上一版，整批用例报
`intent-draft-superseded`）。

**定式**：迁移第一步是 `grep -n '\.run()' <file>` 并把它们逐个改成 `await`；改完再让 `tsc`
去扫 `.all()` / `.get()`。反过来做的话，`tsc` 绿了你会以为迁完了。

同一条也适用于**生产代码**：drizzle 的查询构建器是惰性 `QueryPromise`，两个引擎上都只有
`.run()` 或 `await` 才真的执行；**既不 `.run()` 也不 `await`** 的写在两个引擎上都不会发生。

### 还有第三种假绿：套在 `db` 上的故障注入 `Proxy`，在 PG 上进不了事务（2026-09-14 实撞）

比 `.run()` 更阴的一种。测试为了"在导出中途插一笔写"这类判据，会用 `Proxy` 包 `db` 并拦
`select` / `from`。**在 SQLite 上它白拿了一份便宜**：`createSqliteDatabaseSession` 的事务实现是
`const tx = db`，句柄就是原对象，Proxy 顺理成章地跟进了事务。PostgreSQL 侧
`db.transaction(async (tx) => …)` 交回的是**另一个对象**，钩子被整个绕开——

而生产读写口几乎都跑在 `databaseSessionFor(db).serializable(...)` 里面，**于是钩子一次都不触发**。

后果不是报错，是**静静变绿**：那些"注入一笔冲突写、断言操作仍应成功"的用例，在 PG 上因为
根本没注入而必然成功。`rfc271-export-closure-authz` 的六处注入就是这样，四条"必须成功"
的判据在 PG 上零预言力（同文件头注释记着它此前已经以别的形态发生过一次）。

**定式两条**：
1. 包 `db` 的 Proxy **必须同时拦 `transaction`** 并把交回的 tx 句柄重新包一层。
2. 更重要的是给 seam 加一个 `fired()` 计数器，**在每条用它的用例里断言它真的触发过**。
   没有这个计数器，"seam 坏了"和"功能正常"在绿色里长得一模一样——而 seam 坏掉的方向
   恰好是让判据变绿的方向。
...
### 变异验证要挑**读点**，别挑写点（2026-09-14 实测，否则会得到假阴性）

修完上面任何一种假绿，标准做法是**变异验证**：把修的那一处改回错的写法，确认判据真的变红。
但**变异点选写入侧会得到假阴性**——

去掉某个 `INSERT` 的 `await`，PG 道**照样绿**：那个没人等的 promise 在后续若干个
`await` 掉的 git / 文件系统操作期间自己 settle 完了，等读到它时数据已经在库里。
这不代表「不 await 也没事」——换一段没有中间 await 的代码路径它就会漏，
只是这一条判据碰巧遮不住它。
...
每条验收组挂一个 `{ testFile, testName }`，守卫拿 `testName` 去目标文件里
`toContain("test('<名字>'")`。于是**只改标题、一行断言都没动**也会红，
而且红的是**另一个文件**（`rfc349-functional-evidence.test.ts`），
报错里看不出「是你刚才那次重命名干的」。

这跟「按文件名钉死的 digest 守卫」是同一个家族：**跨文件、按字符串做外键、编译器管不着**。
`tsc` 不知道测试名，`eslint` 不知道，改名时 IDE 也不会跟着改。

**定式**：改了 `describe` / `test` 的标题（或挪动、合并了某个用例），
除了跑被改的文件，还要跑一遍**提到这个文件名的所有测试**：

```
grep -rln "<被改文件的 basename 去掉 .test.ts>" packages/backend/tests --include='*.ts'
```
...
的测试文件数」，判据跑在整份文件文本上——**注释里写了一次也算一次引用**。

实撞：一条用例的注释里写了「靠读时投影补（`postgresqlTaskRouteOperations.ts:413`）」，
账本因此 11 → 12；等那处差异修好、那句话删掉，又退回 11。**两次移动都与覆盖无关**。

**看到这条账本的数字变化时先分清是哪一种**：真的多/少了一个测试文件在驱动它，
还是只有某段散文改了。前者要解释覆盖，后者照实改数字并写明是注释所致就行。
...
**更狠的一条：声明了 `allowGrowth` 之后，下一笔提交必须把它删掉——哪怕那笔是纯文档。**
（2026-09-17 实撞，同一族第三次，这次是**一笔只改 `design/**/plan.md` 的提交把 main 推红**。）

判据是「**这个 commit 没涨就必须删掉它**」，而**没涨**包含「什么都没动」。
于是节奏是强制的两步：

```
提交 A：账本涨了 → 声明 allowGrowth（带 why）
提交 B（下一笔，任意内容，含纯文档）→ 必须先删掉 A 留下的 allowGrowth
```

**最反直觉的地方**：纯文档提交在本仓其余所有门禁下都是安全的（不跑测试、不碰源码），
唯独这一条会因为它「没让账本继续涨」而红。**推纯文档之前也要看一眼
`architecture/ledger-baselines.json` 里还有没有上一笔留下的 `allowGrowth`。**

**第四次（2026-09-18）撞在「紧急修红」上，值得单记一条**：上一笔声明了 allowGrowth 并推红了
`rfc331` 拓扑判据，于是赶着推了一笔**只加 4 行判据登记**的 hotfix——没重跑 census、没清上一笔的
allowGrowth，结果 hotfix 自己又推红两条（`T16` 的登记数 vs 基线、`T17` 的过期 allowGrowth），
红从一个 shard 挪到另一个 shard。**越是急着把 main 修绿，越要先走这两步**：
清掉上一笔的 allowGrowth + 重跑 census。它们加起来不到一分钟，比多转一轮 CI（当天排队 20 分钟起）便宜得多。
第五次（2026-09-23，RFC-369）同形：刀 2 声明了五条 allowGrowth，紧接着为修刀 1 推红的广播快照推了一笔
**只改一个测试基线**的 hotfix（`2b2a92835`）——那笔没涨，五条许可随之过期，hotfix 自己又红了 `T17`。

删文件 / 改文件名时同理（见本文件「删文件要连着账本一起删」那条）。
**注意方向**：这类账本红的时候往往是**好事**——「少一个未受审调用点」「少一条 deep import」
正是棘轮要的方向，处置是**删行销账**，不是放宽判据。

### `allowGrowth` 的退役时机是「下一笔**不涨**的提交」，不是「下一笔提交」（2026-09-19 一天撞两次）

上面那条说的是 `allowGrowth` 一次性、必须退役。**踩的点不在「要退役」，在「哪一笔退役」**：
守卫（`rfc317-ledger-highwater` 的「allowGrowth 无过期条目」）判的是**本次提交有没有涨**——
声明的那一笔涨了所以绿，之后任何一笔**没涨**的提交都会因为条目还挂着而红。

本轮两次都栽在同一个形状：**声明的那一笔和我打算退役的那一笔之间，插进了一笔不涨的提交。**

- 第一次：`6d7dbed53` 声明 → 打算在下一笔功能提交里退役 → 中间 `b4cf1f9e5` 不涨 → 红。
- 第二次：`9bef03c4b` 声明三条 → `a520adee6` 只改了一个 e2e spec、一格没涨 → 两个 shard 红。

**定式**：声明了 `allowGrowth` 之后，**下一笔提交（无论它是什么）先跑一次**
`bun test packages/backend/tests/architecture/rfc317-ledger-highwater.test.ts`——
它一秒出结果，且是唯一能告诉你「这一笔该不该带上退役」的东西。别按「这一笔是不是功能提交」推理：
一个只动 e2e spec / 只动文档的提交同样会让它过期。
...
而改动**在原地留下一段解释「原来这里是 X，为什么换掉」的注释**——注释里当然带着 X 这几个字。
于是判据继续绿，绿在注释上，而它要证的事实早已反转。

实撞的那条：某守卫用「准入预检是 bun:sqlite 的同步读（`.all()[0]`）」当作「两个引擎的实现
不能互换」的机械证据；那句同步读早被改成 `await db.select(...)`，原地留下的注释里写着
`` `.all()[0]` ``。加一层去注释之后判据当场红——也就是说**在那之前它一直是假绿**。

这是「零与合规同形」的近亲，而且更隐蔽：那一类至少还能靠语料下限兜底（扫空即红），
这一类连语料都是满的，只是内容变成了历史说明。

**定式**：任何对**源码文本**做 `toContain` / `not.toContain` / 正则的判据，
断言之前先过一层去注释：
...
（`[^:]` 是为了别把 `https://` 当行注释切掉。）**负向断言尤其危险**：
`not.toContain(X)` 在注释里出现 X 时会**误红**，正向断言则会**假绿**——两个方向都要过这一层。
...
### `bun run architecture:write` **不重钉 provenance**——要传 `--snapshot-sha`（2026-09-22 实撞两次）

上一条的定式（手改在 census 之前）照做了还是会红 `RFC-294 N1a`，因为漏了一件事：
`scripts/architecture-census.ts` 里**重钉 provenance 的那整段挂在 `if (requestedSnapshot !== undefined)` 下**
（`:161`）。而 `package.json` 的 `architecture:write` 不带这个参数，于是：

- 产物照写（`sourceDigest` 等会随源码更新），
- `provenance.contentDigest` **原封不动**——它还是上一次带 `--snapshot-sha` 跑时那份 payload 的摘要。

只要这一轮改动动了任一 `PROVENANCE_ARTIFACTS`（`guard-manifest.json` / `ledger-baselines.json` /
`commons-manifest.json` / `commons-debt.json` …）的内容，N1a 立刻红在 digest 不符上，而 `git diff`
看着完全正常、census 反复跑也不会自愈。

**定式**：所有手改（`allowGrowth` 增删、手工维护的 baseline 数字、`guard-manifest` 的种子格）做完之后，
最后一步跑**带 pin 的那条**：
...
`bun run architecture:write` 只适合「源码改了、但没动任何 provenance 产物内容」的场合；拿不准就一律用带
`--snapshot-sha` 的长写法，它是前者的超集。

> 2026-09-22 第一次撞的时候我把机制误判成「census 在算出来跟磁盘一样时不写文件」，并据此写过一条
> gotcha。那是错的：真实机制就是上面这条 `requestedSnapshot` 门控。误判的由来是当时做的验证实验
> 恰好把 payload 改回了已钉住的那一版，于是看起来「census 修好了」。留下这段是因为这个坑的**表象**
> （反复跑 census 不自愈）确实一样，别再顺着「是不是没写文件」去查。
...
- **「加心跳」这条缓解办法按直觉写是无效的**（同日实测，本条自我更正）：后台命令的 stdout
  不是 tty，stdio 走**块缓冲**，几行短心跳根本不会 flush——文件照样 0 字节，
  与「守候卡住」长得一模一样。真要心跳得逐行 flush（`stdbuf -oL` / `print -u1 -r --` 后
  显式 `exec 1>&1` 之类都不可靠），代价高于收益。
- **本仓的结论：这类「轮询到某状态再报」的后台守候，不值得写**。CI 状态本来就是一条
  `gh run list` 直查就有的东西；起一个守候换来的是「多一个会骗人的信号源」——
  它「还在跑」既可能是 CI 没完，也可能是脚本坏了，而两者外观相同。
  需要知道的时候直接查；真要等，用 `Monitor` 那种每行即事件的机制，别自己搓轮询循环。
...
**最隐蔽的一种：pathspec 是从 `git status` 生成的**（2026-09-03 实撞）。写法长这样——

```bash
# ⚠️ 这不是 pathspec，这是伪装成 pathspec 的 `git add -A`
git status --porcelain | awk '{print $NF}' | grep -v <一两个例外> > paths.txt
git commit -F msg.txt --pathspec-from-file=paths.txt
```

它**每一条规则都满足**：用了 `--pathspec-from-file`、没写 `git add .`、还额外排除了已知的
他人文件。但语料是「工作树里此刻的全部脏文件」，所以别人在你生成清单**之后**才动的文件、
或者你没意识到属于别人的文件，会原封不动进你的提交。我连着这样提了好几笔都没事，直到某笔
把并发 session 一个 489 行的 `git mv` 新文件一起发布了——而且只带走了新增、没带走对应的
删除，主干上于是同时存在新旧两份调度器（不是功能故障，但多了一份死代码，账本也跟着虚高）。

判据很简单：**pathspec 的来源必须是「我这次改了哪些文件」，不能是「工作树里现在有哪些脏
...
## 机械改名会把**判据里的正则字面量**一起改掉，判据当场反转（RFC-359 AC-6 实撞，2026-09-11）

AC-6 的迁移脚本做 `DbClient` → `ProviderNeutralDatabase` 的全文件替换。
`rfc349-intent-context-authorization-binding.test.ts` 里有一条源码锁：

```ts
expect(runnerCode).not.toMatch(/as unknown|DbClient|PostgresqlDatabaseClient|dbTxSync/)
```

替换把**正则里的** `DbClient` 也改成了 `ProviderNeutralDatabase`，判据于是从「运行器不许还留着
SQLite 专属类型」反转成「运行器不许出现中立类型」——而那个文件恰恰通篇是中立类型，两个引擎一起红。

**规律**：跨文件机械改名必须区分**类型位置**与**字符串 / 正则字面量**。纯正则替换做不到这件事，
所以：①改完先看 `git diff` 里有没有落在 `/…/` 或引号内的命中；②架构守卫、源码锁这类
「按名字扫源码」的文件要么排除在批量改名之外，要么逐条人工确认。命中点旁边补一行注释说明
「这里是判据里的字面量，不是标注」，下一个人就不会再踩。

## 同一文件里共享解析缓存的守卫，开销落在**恰好先跑**的那条上（RFC-305 实撞，2026-09-11）

`rfc305-architecture-lock.test.ts` 里十几条锁都要扫整棵 `src` 树，文件顶部因此按路径缓存
TypeScript 解析结果。缓存是**共享**的，于是全树解析那笔开销落在**第一条真正跑起来的锁**头上，
而 bun 的用例顺序在不同 run 之间并不稳定：

- 上一次绿的 run：先跑「roles 只是展示数据」（自带 `20_000` 超时），花 10.1s；
  「AST 字面量消费者」随后只花 **473ms**。
- 红的那次：顺序反过来，「AST 字面量消费者」先跑，**5.4s** 撞死在 bun 默认的 **5s** 超时上。

同一条锁的耗时差 10 倍，而代码一个字没改。**处置：凡是要扫整棵树的守卫都显式写超时**
...
它能证明适配器走了哪条事务 / fence 路径，**证明不了那条 SQL 在真 PostgreSQL 上跑不跑得动、
跑出来一不一样**。RFC-349 靠这一层过了全部门禁，接上真库之后连着暴出六个**只在 PostgreSQL 上
成立**的缺陷，每一个都是「SQL 合法、语义不同」：

1. **省略的 identity 列被渲染成字面量 `null`**。Drizzle 的 SQLite dialect 对「values 里没给、
   列也没有 default」的列一律填 `null`；SQLite 把 `INSERT INTO t(id) VALUES (NULL)` 里的
   `INTEGER PRIMARY KEY` 读成「分配下一个 rowid」，PostgreSQL 的
   `GENERATED BY DEFAULT AS IDENTITY NOT NULL` 直接拒绝。命中的是 `node_run_events`——
   **选 PostgreSQL 时一条 agent 输出都写不进去**。
2. **`LIKE` 的大小写语义相反**。SQLite 对 ASCII 大小写不敏感，PostgreSQL 敏感 ⇒ 用户搜索静默
   少召回。用 `ilike` 复刻；按存量 id / 常量前缀匹配的 `like` 要**保留**（那里敏感才对）。
3. **NULL 排序正好相反**。SQLite ASC 把 NULL 排最前、DESC 排最后；PostgreSQL 反过来。认领 /
   扫描类查询往往**故意**把 NULL 收进候选集（NULL = 还没被认领 / 还没扫过），切库后它们掉到
   队尾，存量填满 LIMIT 就**永远轮不到**——是饿死，不是排序不好看。
...
6. **SERIALIZABLE 的 predicate lock 是索引页粒度，不是行**。小表（每任务一行的那种）上两个改
   **不同**聚合的事务会互判读写依赖：8 并发满速冲突率 81.2%、重试预算耗尽逃逸 234 次。单聚合的
   读—改—写换成 `READ COMMITTED + 聚合根 FOR UPDATE` 后 0%。

**怎么防**：给真库加一条**写矩阵**——对 schema contract 里每一张表渲染并执行一次真 INSERT
（插完即回滚），判据按 **SQLSTATE** 分类而不是错误文本：`23503` 外键 / `23514` check /
`22003` 越界 / `22P02` 文本转换属于「合成值不满足业务约束」，其余（`23502` not-null、
`42703` 未知列、`42804` 类型不匹配、`42883` 未打 shim 的函数）一律算缺陷。再加一次读回，
按逻辑 codec 逐列比对 JS 运行时类型。写矩阵覆盖不到的读 / 表达式面（LIKE、ORDER BY、聚合、
布尔表达式）用静态守卫补，并且在 `bun:sqlite` 里**可执行地钉住前提**（"SQLite 的 LIKE 确实
不敏感"），前提哪天变了先红在前提上。本仓的实现见
...
## 「扫全源码树」的守卫用例要显式给超时，否则迟早在 macOS 上假红（2026-09-03 一天撞两次）

本仓有一类守卫:`readdirSync` / `walk` 递归整棵 `packages/*/src`(约 2700 个文件、17 MiB),
再逐文件做文本匹配。它们的**工作量确定且有界**,但成本随源码树增长,而 bun 的默认单测超时是
**5000ms**。本机跑整个文件通常 1 秒上下,hosted macOS runner 满载时同一条能到 5–7 秒——于是
以「超时」形态假红,而报错和你这次改了什么**看不出关系**。
...
**处置**(与 RFC-227 那条的既定做法一致):给这类用例一个**宽裕但有限**的显式上限
(`}, 30_000)`),并在原地写明「本机多少、runner 多少、为什么」。**不是**把上限抬到永不触发——
真挂住仍然要失败。

**别搞混的一条**:这跟本文件里「继续加超时是错解」那条(UI 竞态、轮询式等待跨 turn 的稳定态)
**不是一回事**。那类是**竞态**,调大只是把概率降低,正解是换事件驱动锚点;这类是**确定性计算**
在慢机器上越线,给足预算就是正解。判据:失败的是不是「同一份输入、同一段纯计算」——是,就给
预算;不是(涉及子进程、网络、UI 渲染、跨 turn 状态),就别加超时。

**写新守卫时顺手做的事**:如果它要读整棵源码树,就**按需读**而不是在模块作用域一次性 slurp。
2026-09-03 新写的 `rfc349-provider-completeness.test.ts` 初版就是模块作用域读进 1911 个文件 /
16.9 MiB,而六条用例里四条只需要两三个具名文件——改成惰性 + 具名直读之后成本回到与需求成比例。
...
**补一条更锋利的判据（W8 实测，20/20 反证）**：SERIALIZABLE **能不能**兜住「先查存在、再插唯一键」，
取决于**两笔事务有没有读过它们要插的那张表**——那次读留下的 **SIREAD 谓词锁**才让插入成为 SSI
可检测的读写冲突。把前置读换成读**另一张**表，同一段代码在 PG 上 20/20 全红。
...
所以这类守卫的判据里要带上 **opener 是什么**、**读之前有没有序列化**，否则预言力会落在错的一档。

**归纳判据时的一般教训**：从个例反推规则，要先问「这个例子属于哪一档」，再问「这一档的边界在哪」。
少了后一问，判据会稳稳地筛出安全的那一支——本条就是这么错了一次的。
...
## 扫语料的守卫里，局部 helper **不要叫 `scan`**——`census.ts` 解析被调用者时不分作用域（2026-09-07 实撞）

`tests/architecture/census.ts` 的 `negativeFixtureAssertions` **按名字**解析被调用者，且解析到
**文件顶层**的声明表——它**不区分作用域**。于是判据函数里一个局部的 `const scan = …`，
与文件顶层那个读磁盘的 `scan()` 撞名之后，那个**纯函数**被判定成「碰了语料」，
结果**整份文件的负 fixture 一条都认不出来**（`fixtures []`）。

坏在诊断上：T14 只会报「你没有负 fixture」，**不会告诉你为什么**——于是人会去补 fixture，
而不是去看名字撞了。实撞时把局部名从 `scan` 改成 `visitExtras`，12 条 fixture 全部被认出。

**规矩**：扫全源码树的守卫里，局部 helper 避开 `scan` / `walk` / `read` 这类与文件顶层
语料读取函数同名的名字。看到 T14 报「没有负 fixture」而你明明写了，先查同名遮蔽，别急着补 fixture。

## `bunx tsc --noEmit` 在**仓库根**什么都不检查——它**退出 1**，但输出里一条报错都没有（2026-09-07 实撞）

仓库根**没有 tsconfig**。在根目录跑 `bunx tsc --noEmit`，tsc 找不到工程、把它当成「没给输入文件」，
**打印版本号和用法**：

```
$ bunx tsc --noEmit > out.txt 2>&1; echo $?
1
$ head -2 out.txt
Version 5.9.3
...
**退出码是 1**，所以 `&&` 链、`set -e`、CI step 都会当场拦住它——这一半是安全的。
**危险的是输出**：里面**一条 `error TS…` 都没有**。于是所有「跑完看有没有报错行」的判断
（人眼扫一眼、`grep -c error`、agent 汇报「零错误」）**全部判成通过**。
实撞：一个 session 中途几次在根目录跑它、每次都当成绿，改到 `packages/backend` 下重跑，
当场照出 **7 个真错**。
...
要求跑 tsc 的流程（含派给 subagent 的指令）必须把**工作目录或 `-p`** 写进指令里，
不能只写命令名；收到「tsc 零错误」的汇报时先问一句**在哪跑的**。

### 附带一条更通用的：`cmd | head; echo $?` 报的是 `head` 的退出码

本条最初被我写成「退出 0、静默假绿」，**那是错的**，而错因值得单独记：我当时测的是

```
bunx tsc --noEmit 2>&1 | head -3; echo "exit=$?"      # ← 报的是 head 的退出码，恒 0
```

管道的退出状态取**最后一个命令**。凡是「跑个命令、管道给 head/grep/tail 截一下、再看 `$?`」
的写法，**测到的都不是那个命令的退出码**。要真值就别用管道：

```
cmd > out.txt 2>&1; echo $?          # 或
set -o pipefail                       # 让管道取第一个非零
```

**这条错还提醒了另一件事**：一条**写错了的**踩坑比没有更危险——它会被后来人当依据。
本文件 §「站点账本的『安全理由』只回答了一半危害时，它比没有更危险」讲的是同一个道理。
落这类条目时，判据本身也要实测一次，别凭一次顺手的观察就写死。

## 变异脚本用空串做锚点会把代码**贴到文件开头**，而且后续变异全部假绿（2026-09-07 实撞）

一个常见的变异 harness 写法：`s.replace(old, "")` 删掉一段、验证红、再 `s.replace("", old)` 还原。
**第二步是错的**——`String.replace("")` 匹配的是**位置 0** 的空串，于是那段代码被贴回**文件开头**。

后果有两层，第二层才是真正危险的：

1. 文件当场变成语法错误；
2. **此后的每一次变异都「通过」**——测试根本跑不起来，`fails=0`，而 harness 把 `fails=0`
   读成「变异被抓住了？没有 → 存活」或「全绿 → 还原成功」，取决于它怎么判。实撞：接下来
...
- 还原**用读到的原文整体写回**（先把原始内容存到变量/临时文件，还原时整体覆盖），
  不要靠「反向 replace」；
- 每次变异后**先确认测试真的跑起来了**（用例总数与基线一致），再看红绿。
  `fails=0` 与「测试没跑」在很多输出格式里长得一样——这正是本条的杀伤点。

顺带：共享工作树上还有一条相关纪律——变异**不要整文件备份-还原**，那个窗口会覆盖别人刚写入的内容；
用定点替换 + 原文整体写回。

## 落盘前只跑 `tests/architecture/` 不够：源码锁守卫**散在 `tests/` 根下**（2026-09-07 实撞）

落一个 178 文件的大波次时，我按「跑架构守卫」做了验证：`tests/architecture/` **610 pass / 0 fail**，
干净导出上验的，很干净。推上去 CI **6 个分片红**。
...
数量多、分布散、没有共同前缀。「跑架构守卫」这个说法会让人以为覆盖了全部守卫，其实只覆盖了一类。

**判据**：凡是改动**函数名 / 委托关系 / 事务边界 / provider 分派**的波次，落盘前要跑的不只是
`tests/architecture/`。可行的定位方式是找**读源码文本**的用例（`readFileSync` + `src/` 路径），
...
## 两条我传播过的粗判据，实测都要收窄（2026-09-07 两把刀分别纠正）

### 「漏 `await` 只有 PostgreSQL 才炸」——**有三档**，只有两档是 PG 独有

三把刀分别撞到不同的一档，合起来才是全貌。写变异 / 读汇报时要先问「这是哪一档」：

| 档 | 写法 | SQLite | PostgreSQL | 谁会红 |
| --- | --- | --- | --- | --- |
| ① 裸惰性 builder | 既不 `.run()` 也不 `await` | 语句**不发生** | 语句**不发生** | **两个引擎都红** |
| ② 读漏 await | `.all()` / `.get()` 不 await | 同步驱动当场执行并交回数据 | 拿到 Promise | **只有 PG 红** |
| ③ 写漏 await | `.run()` 不 await | `changes` 是真值 | `changes` 恒 `undefined` | **只有 PG 红，而且静默** |

- ① 之所以两边都红：drizzle 的查询构建器是**惰性 `QueryPromise`**——`.run()` 与 `await` 都会触发执行，
  两者都没有就一条语句都不发出去。把这一档记成「PG 独有」会让人以为双引擎对拍在这里没有增量，错。
- ③ 最阴险：语句**发出去了**、数据也写了，只是**结果没等到**。于是所有基于受影响行数的 CAS
  判据（`changes === 1` / `affectedRows`）在 PG 上静默失真，而 SQLite 侧全绿——
  实测就是这样把一处 CAS 变成了永假。

结论没变、但要说准：**双引擎对拍是抓 ②③ 的唯一手段**（`lint:promises` 与 SQLite 单跑都放行）；
①靠任一引擎都能抓，别把它算进「只有双引擎能抓」的功劳里。

**③ 还有一个前提**：它只在**下游真的有基于受影响行数的判据**时才可观测。实撞一次：某条 delete
之后没有任何 `changes` / `affectedRows` 判据，把 `await` 摘掉**两个引擎都零红**——语句照样按序
落在同一条保留连接上。所以变异 ③ 打不出红时，先问「这条路径有没有人看受影响行数」，
再下「双引擎抓不住」的结论。

## 「从 `sqlite-core` 取的东西都是 provider-盲」——太宽了，要看它读不读 SQLite 专属符号

本文件先前记过「凡是从 `drizzle-orm/sqlite-core` 里拿的东西，在 provider 中立代码里都是 provider-盲的」。
**那条说过头了。** PG 客户端就是 drizzle 的 **sqlite-proxy**（`platform/persistence/postgresqlDatabaseClient.ts`），
**表对象两侧是同一套**，所以 `alias` 这类只做包装的工具在 PG 上正常工作。

真正会炸的是**读 SQLite 专属符号**的那些——实撞的是 `getTableConfig`：它取
`SQLiteTable.Symbol.InlineForeignKeys`，而投影出来的 PgTable 上那个符号是 `undefined`，当场 `TypeError`。

**判据**：拿表对象做元编程（列清单、外键、索引）时用方言无关的 `getTableColumns`（从 `drizzle-orm` 取）；
只做包装 / 构造的（`alias` 等）照用不误。别按 import 路径一刀切。

### 「SQLite 上表达不出并发交错」——只对**事务内**成立

SQLite 会话是进程内单写者租约 + `BEGIN IMMEDIATE`，它串的是**事务**。
凡是「读在事务**外**、写在事务**内**」的形状（本仓 `setTaskStatus` 先读、再 `dbTxSync` 写就是），
**读—写窗口是真实存在的**，SQLite 上同样能被别人插进来——源终止那段 race-winner 复原分支
正是为这个窗口存在的。

推论有两条，方向相反、都要注意：
- 判定某处「SQLite 表达不出、只能 PG 测」之前，先看**读是不是在事务外**；是的话这个判断错了，
  两个引擎都要测。
- `holdAggregateRoot` 这类「占住聚合根撑开窗口」的手法在这条路径上**用不了**：SQLite 的
  `dbTxSync` 一旦检测到别人的显式事务开着就直接抛 `CrossContextTransactionError`
  （`src/db/txSync.ts:38`）——占住中立事务不是把参与者挡住排队，是**把它打死**。
  这类路径要找两个引擎都成立的确定性交错点（例如注册的策略回调，两侧都恰好 await 它一次）。

## 守卫的 `absent` 锚点只写强侧的字面量，会把「换个名字做同一件事」误判成「没做」（2026-09-07 实撞）

一条 dual-engine 缺口账本记着「SQLite 缺代际四分支**与路径 / 版本哈希校验**」。前半段属实，
**后半段两项都不成立**：

- 路径校验 SQLite **有**，字面量是 `resource-package-skill-version-artifact-path-mismatch`；
  账本的 `absent` 锚写的是 PG 命名的 `resource-package-skill-artifact-path-mismatch`，**扫不到**。
- 哈希校验 SQLite 也**有**，只是校验 live 树（`…-live-hash-mismatch`）而 PG 校验 version 目录——
  位置不同、覆盖等价。

**成因**：写缺口的人是从强侧读出实现、把强侧的错误码抄进 `absent` 锚，再去弱侧 grep。
弱侧只要用了自己的命名，grep 就落空，于是「做了但名字不同」被记成「没做」。

**规矩**：
- `absent` 锚点要按**行为**取，不要按**强侧的字面量**取——能落到「哪个函数在哪个位置做了这件事」
  最好；退而求其次，把两侧的候选字面量都列上。
- 读到「一侧缺 X」的账本条目，**动手前先在弱侧按行为搜一遍**（搜错误类型、搜调用点、搜测试名），
  别只搜那个字面量。
- 同一条教训的反面在本文件 §「账本的 `removeWhen` 是假设，不是事实」——**账本里所有由人写下的
  判断（`why` / `removeWhen` / `missingSide` / `absent` 锚）都是当时那个人的诊断，不是事实**。
  已经实撞过四种：removeWhen 指向不在环上的边、missingSide 记反、consequence 描述的行为不成立、
  absent 锚只认强侧命名。

## 抽公共 helper 会让「按字面量钉住某一侧」的守卫**静默失效**（同上）

把两侧各写一遍的判定抽成一份中立 helper，是本 RFC 要的方向；但**判定字面量随之离开了被点名的文件**，
于是「弱侧文件里不得出现 X」这类 `absent` 判据**恒真**、守卫绿着失效——它此后再也拦不住任何东西。

**处置（本仓已实践）**：抽完 helper **先跑一次守卫确认它红了**，再去销账。红出来的应当同时有两类：
「缺口已补」（弱侧文件现在含有那些字面量）与「有判据一侧锚点漂移」（强侧原函数名已不存在）。
**两类都没红 = 判据已经失效**，这时候删条目等于把守卫连同债一起丢掉。

## `allowGrowth` 是**一次性**的，而「落产物 → CI 报红 → 跟进修复」这个节奏**必然**让它过期（2026-09-07 连撞四次）

棘轮的设计是：许可授权**这一个 commit** 的上涨，**下一个 commit 没涨就必须删掉它**。设计是对的。
但它与一个很常见的工作节奏正面冲突：

> 落一笔带许可的账本产物 → 推 → CI 报出别的红 → 补一笔跟进修复 → **那笔跟进提交没涨，许可当场过期**。

只要还需要跟进修复（大波次几乎必然需要），这条就会红。一天之内我撞了**四次**，每次都要多花一个 CI 循环。

**2026-09-10 第五次**，形状略有不同、更容易漏：跟进提交是一笔**只改一个测试文件**的热修
（修一处守卫锚点），主观上跟账本毫无关系，于是根本没想到上一提留着许可。红出来的位置也毫无
提示性——`Backend tests (ubuntu-latest shard 6/8)`，与那个测试文件八竿子打不着。
**结论：判据要对每一次 push 都跑，不分「这提跟账本有没有关系」。**
...
**每一次** push 之前看一眼（不是「账本笔才看」，见上面第五次），非零就问自己「这一提涨了吗」。
...
**结构上是 `ProviderNeutralDatabase` 的子类型**。也就是说：

- 一个函数的签名写着 `db: PostgresqlDatabaseClient`，**只说明它被这么标注了**，不说明它做了任何
  PostgreSQL 专属的事；它的函数体很可能一行方言都没有，换成 `ProviderNeutralDatabase` 直接就能用。
- 反过来，`PostgresqlResourceCatalogTransaction` 这类由它派生的事务类型
  （`Parameters<Parameters<PostgresqlDatabaseClient['transaction']>[0]>[0]`）也一样——
  名字里有 `Postgresql` 不等于语义上绑定 PostgreSQL。

**实测影响**：一把刀按「吃 `PostgresqlDatabaseClient` = 真分叉」筛过一遍 13 个文件，
用这条事实复核后**有 4 个被重新归类成误名**（一份中立实现顶着 provider 的名字与类型标注）。

**判据**：判一处是不是真分叉，看**函数体里有没有方言 / 有没有第二份实现**，
不要看**类型名**、也不要看**文件名**。本文件另有两条同族教训——
§「`sqlite-core` 的 import 不是一律有毒」与 §「T17 的 provider 命名文件数不等于实现分叉数」。

## 文件头写着「这里是唯一的一份」**不是证据**（同上）

`resourceVisibility.ts` 的头注释自称「这里是唯一的一份」。实测：它的四个主题里**有三个**在
`sqliteResourceGrantRepository.ts` 里存在**逐字节的副本**，而且**两份在同一个进程里都活着**
（中立那份喂授权应用，副本被三个 legacy 调用点直接调用）。

两本账本都看不见它：T17 只看**文件名**；成对账本要求**同目录同词干**，而另一侧叫 `resourceVisibility`。

**规矩**：把「某某已经合一了 / 只有一份了」当结论之前，**自己 grep 一遍函数名**。
写下那句话的人当时可能是对的，也可能只合了一半——注释不会随代码演进自动作废。
发现不符时，**把原句留着、在后面追加更正**，别直接删——删掉会连同「这里曾经骗过人」这个信息一起丢掉。

## `dbTxSync` 在 PostgreSQL 上是**静默 no-op**：写既不落库也不报错（2026-09-08 实测）

`dbTxSync` 是 bun:sqlite 的同步事务面。把它跑在 PostgreSQL 客户端上时——PG 客户端是
drizzle 的 **sqlite-proxy**，它的 `transaction` 是**异步**的——`dbTxSync` 拿到一个
**未 await 的 Promise** 就返回了：**事务体里的写既不落库，也不抛错。**

**后果比「降级」严重**：账本里所有标着「钉死、暂时保留」的 `dbTxSync` 站点，在 PostgreSQL 上
**不是「能用但不理想」，而是整条路径死掉**。读到「这处钉死了，PG 上走另一条路」这种说法时，
要去确认**真的有另一条路**，而不是这一条在 PG 上静默失效。

**连带**：给这类路径写测试时，夹具**不能**用生产入口播种（`createAgent` / `createWorkflow` 之类，
它们内部就走 `dbTxSync`）——在 PG 上什么都不会写进去。要用直接 `insert` 播种。

## 账本的扫描正则漏掉泛型调用：`dbTxSync<T>(…)` 隐形（同上）

同步事务面账本用 `\bdbTxSync\(` 扫站点，**匹配不到带泛型参数的调用** `dbTxSync<T>(…)`。
实测全仓有 **5 处**这样的隐形站点，其中一个文件（`sqliteResourcePackageMaintenance.ts`）
**从来没在账本里出现过**。

> 谁想加一个 SQLite-only 事务而不惊动账本，只需要打一对尖括号。

已改成 `\bdbTxSync[<(]`。**一般规律**：用文本正则扫调用点的守卫，判据里必须把
**泛型实参**（`f<T>(`）、**可选链**（`a?.f(`）、**跨行调用**（`f(\n`）都想一遍——
少想一种就等于给出一个零成本的绕过方式，而绕过它的人**不需要有恶意**，写顺手了就绕过去了。

**顺带一提**：这条与本文件 §「守卫的 absent 锚点只写强侧的字面量」是同一族——
**按文本匹配的判据，漏的永远是「同一件事的另一种写法」**。

## 组合根的「装配未完成」占位：先分**可选参数**还是**模块局部槽**（2026-09-08 实测）

组合根里 `X is not composed` / `X-not-bound` 这类占位有**两种完全不同的形状**，处置手法不通用：

- **可选参数**（`foo?: T` + 进门 `if (!foo) throw`）：处置是**改必填**。
  验证方法是 grep 全仓有没有第二个传值点；类型层证据是 TS2345 / TS2739。
- **模块局部槽**（`let x: T | null = null` + 后面某处 `x = …` + 闭包里 `if (x === null) throw`）：
  **没有调用点可 grep**，上面那套方法完全不适用。这类的成因是**同一作用域内的循环依赖**
  （A 的构造要用 B，B 的构造要用 A），先用 `null` 占位、之后回填。

**模块局部槽的正解是词法作用域，不是「改必填」**：判别式是「**环的另一端在不在同一个函数作用域里**」。
在，就让闭包直接引用**后面那个 `const`**——JS 的闭包本来就允许引用声明在后面的绑定，
只要**执行时**已初始化即可。槽位、回填那一步、以及那句 throw **三样一起消失**。

**而且这个证明更强**：可空槽位你把回填那行删掉，代码**照样编译得过**（只是运行时炸）；
换成词法引用之后**根本没有"回填"可删**，删掉声明就是 **TS2304 `Cannot find name`**。
17 处占位里 16 处是这一类，实测证据就是 TS2304 而不是 TS2345。

**代价**：词法引用把「A 必须在 B 之后求值」变成了运行时约束。要用一条测试钉住求值顺序
（把引用改成**急切**求值，应当抛 `ReferenceError: Cannot access 'x' before initialization`）。

**探针写法的坑**：拿 `void x` 当 TDZ 探针**没用**——Bun 的 transpiler 会把它当死代码消掉，
TDZ 根本不触发。必须是一次**真实读取**（传参、取属性、放进对象字面量）。

## zsh **不对无引号参数展开分词**——`bun test $FILES` 会把整份清单当成一个参数，**静默只跑一部分**（2026-09-08 实撞，本仓累计第 6 次同族）

```zsh
FILES=$(grep -rl ... )                       # 444 个路径
bun test --isolate tests/architecture/ $FILES   # ← zsh 里 $FILES 是**一个**参数
```
bash 会分词，**zsh 默认不会**。于是那 444 个路径被当成一个不存在的过滤器，
bun **不报错**，只跑了 `tests/architecture/`——输出看起来完全正常（`Ran 623 tests across 62 files`），
而你以为自己跑了 500 多个文件。

**这次的代价**：我据此判定「全量验证通过」并推上主干，CI 随后在
`rfc345-resource-acl-facade-retirement`（一条只存在于 `tests/` 根、不在 `tests/architecture/` 下的
守卫）上转红。**验证的静默降级比红本身危险得多**。
...
**同族**（都是 zsh 与 bash 的差异，都会静默降级）：
- `grep --include=*.ts` 无引号 ⇒ zsh 当 glob 展开，`no matches found` 直接中止命令；
- `cmd | head -N` 之后拿 `$?` 或计数下全称结论（见本文件另一条）。

## 源码里混进裸 NUL 字节 ⇒ **grep 把整个文件当二进制，所有文本守卫对它集体失明**（2026-09-08，本日第二次）

写复合键分隔符时直接落了字面 `\0`（不是 `\u0000` 转义）。后果链条：
- `tsc` 通过、两个引擎都接受、**所有测试全绿**；
- 但 `grep` 判定该文件为 binary，**在里面什么都找不到**；
- 于是**每一条扫这个文件的源码文本守卫都变成零预言力**——不是红，是**静默失明**。

这是「改名会静默缩小守卫语料」那一族的**内容版**：不是有人改了名字，是**文件内容让它退出了语料**。
本仓今年已撞两次（另一次是测试文件里 3 个 NUL 让 git 把它当二进制）。

**规矩**：分隔符 / 控制字符一律写**转义**（`\u0000`），不要落字面字节。
批量改完后扫一遍：
```bash
grep -rlIP '\x00' packages/backend/src packages/backend/tests || echo "clean"
# 或：file packages/backend/src/**/*.ts | grep -v 'text' 
```

## 先红后绿的第二个用处：**发现自己对"现状"的假设是错的**（同上）

一把刀按「重复项应当落成 2 行」写了用例，**打在未修改的代码上就红了**——现状根本不是那样：
同一批里重复的 id 在**批量化之前**就已经会撞唯一约束抛错。

如果没有这一步红，它会照着错误的假设去「修」，把一个**显式抛错**悄悄改成
**「投递莫名少一条」**——从响亮的失败变成沉默的数据缺失，是更坏的结局。

**规矩**：先红后绿不只是「证明我的修改有效」，更是「**证明我对现状的理解正确**」。
用例第一次跑出**意料之外的颜色**时，先去查现状，别急着改代码让它变绿。

## 切并发刀的路径域之前，先查**消费者集合**（同上，我自己踩的）

我给一把刀列了 5 个目标文件，同时在**同一条消息里**禁掉了 `cli/start.ts`。结果其中 2 个文件
是**跨阶段的延迟绑定原语，各有一个消费者就在 `start.ts` 里**——结构上根本动不了，
而且只改一边会让两个 daemon 的装配形状不一致，比现状更糟。

**规矩**：切路径域时，对每个目标文件先 `grep -rn '<模块名>' packages/backend/src` 看消费者落在哪。
消费者跨进禁区的，要么把禁区一起划进来，要么这一刀就别列它——**列了等于让刀白跑一趟**。

## 守卫的正则按**错误消息措辞**认账，改个说法就逃逸（同上）

`server.ts:2525` 有一处 fail-closed 与被账本记着的那处**结构完全相同**，但账本不数它——
因为它的消息写的是 `'task catalog requires the digital employee runtime'`，
既不匹配 `marker` 正则（`not-bound|not-composed`）也不匹配 `prose` 正则（`is not composed` 等）。

守卫头注释里担心的「改个名就逃逸」**在树里已经是现实**了。而 `holder` 那条通道也盖不住它，
因为那个值是**属性**不是 `let`。

**一般规律**：凡是按**自然语言措辞**取语料的守卫，都有这个洞。设计时要么改成按**结构**认
（AST 上「一个 fail-closed 分支 + 抛出」的形状），要么接受漏检并把它写进头注释——
**别让读的人以为那个数字是全集**。

## 判据守卫的 `identifier` 锚点用**整文件作用域**，会把 `import` 也算成命中（2026-09-08 实撞）

「某处必须有 X 这道门」类的守卫，锚点若只写「文件里出现标识符 `X`」，那么**删掉那句门调用、
只留下一条没用的 `import { X }`，守卫照旧是绿的**——`import` specifier 在 AST 里也是一个
`ts.Identifier`。

实撞：给「SQLite 侧补上的门」写自证用例，第一版**删掉门调用后没红**，就是被残留的 import 骗了。
把锚点收窄到「`createSqliteResourcePackageApplyArtifactRecovery` 的 `rollForward` 函数体之内」
再跑，自证当场红。

这是本仓已知的「把逻辑抽成辅助函数让守卫看不见」那条陷阱的**反向变体**：
那条是**真有门但守卫找不到**（假红），这条是**门没了但守卫还找得到**（假绿）。假绿危险得多。

**规矩**：`identifier` / 字面量锚点必须落到**那道门该在的那个函数体**上，不能是整文件作用域。
写完用「删掉门调用」这一个变异自证一次——**这条自证是锚点作用域的唯一检验**。

## 又一例「测试在给死代码续命」：还带着一条不过新门的旧臂（同上）

`convergeResourceBundleApplies`（`platform/persistence/sqlite/legacyResourcePackageBundleApply.ts:630`）
**零生产调用方**，只有测试引用。危险点在于它**自带一条 committed roll-forward 臂，且不过新加的那道门**
——哪天有人「复活」它，仓里就凭空多出第三份实现，而且是没有门的那份。

**规矩**：清点死代码时，除了「有没有生产调用方」，还要看它**内部有没有绕过现有守卫的分支**。
零调用方 + 有绕过分支 = 优先删，不要留着。

## 变异不红，可能是**断言被它自己要变异的东西把门**（2026-09-08 实撞）

给一条「SQLite 侧要发 `INDEXED BY` 索引提示」的用例做变异：把 `indexHint()` 改成返回空串。
**第一次跑 0 fail。** 查下去是用例自己的两个缺陷：

1. **自我把门**：断言写成「**如果**渲染里有 `indexHint`，那就检查它对不对」——变异把
   `indexHint` 变没了，于是那条断言**自己把自己关掉了**。变异体越彻底，检查越不执行。
2. **预言错了对象**：原本用 `EXPLAIN QUERY PLAN` 判断有没有走索引。而在**空的内存库**上，
   planner 本来就会选那条覆盖索引——加不加提示计划都一样，**这个预言根本不区分**。

改法：判据换成「**实际执行的语句文本**里必须出现 `INDEXED BY`」，并按 `engine.isolation` 分支
（只在需要它的引擎上断言）。改完变异当场红。

**规矩**：
- **断言里不要出现被变异的那个符号做条件**。`if (用了X) expect(X 对)` 是空断言；
  要写成 `expect(渲染结果).toContain(X 的效果)`。
- **变异不红先怀疑用例，别急着下「此处不可达」的结论**——本仓另有先例是真不可达，
  但那要在排除「预言力为零」之后才成立。
- **`EXPLAIN` 类预言在空库 / 小库上常常不区分**：planner 在任何一种写法下都选同一个计划。
  要判「有没有发出某个提示」，判**语句文本**比判执行计划可靠。

## 能力矩阵的条目也可能**跨引擎不等价**（同上，`greatest` 实撞）

`EngineCapabilities` 的存在意义是「引擎差异只在这里表达一次」。但矩阵里的一项**本身**也可能
在两个引擎上语义不同：`greatest(a, b)` 渲染成 SQLite 的多参 `max` 与 PG 的 `GREATEST`，
而**两者对 NULL 的处理相反**——SQLite 传播 NULL，PG 忽略 NULL。矩阵不替调用方兜这件事
（该 `COALESCE` 还得自己 `COALESCE`）。

更糟的是它此前**在两个引擎上都没有一条真实执行断言**，而矩阵自己写明的纪律是
「每一项在两个引擎上各有一次执行断言」。**纪律写在注释里不等于被执行。**

**规矩**：往矩阵加算子时，除了「两边都能跑」，还要显式钉住**边界语义**——NULL、空集、
类型提升、排序中的 NULL 位置。这些正是两个引擎最常分家的地方，而且不写用例就永远不会有人发现。

## 双引擎对拍：**别预设哪个引擎会红**——两个方向都实证出现过（2026-09-08）

写双引擎对拍时很容易先入为主地假设「PostgreSQL 那半会红」（因为 PG 是后接的、隔离更弱）。
本仓两个方向都真实发生过，取决于缺陷属于哪一类：

- **PG 红 / SQLite 绿**：缺陷是**弱隔离**。SQLite 的中立会话是「进程内单写者租约 +
  `BEGIN IMMEDIATE`」，两笔写事务之间根本没有交错窗口，连**表达**这类时序都不行；
  同一段代码搬到 PG 的 READ COMMITTED 上就是丢更新。W8 的「读—改—写中间不锁」6/6 全是这个形状。
- **SQLite 红 / PG 绿**：缺陷是**同步包装器**。裸 `db.transaction(async …)` 在 bun:sqlite 上，
  async 回调在第一个 await 处被当作已返回并 COMMIT ⇒ 抛错也不回滚；而 PG 客户端本来就是 async，
  驱动自带事务**从一开始就是原子的**。W11 把裸事务改中立原语时，基线红的全是 SQLite 那半。
...
- **真缺陷**：`ORDER BY` 缺失。用例读回 `node_runs` 后直接按数组序断言 `[[0,'failed'],[1,'failed']]`，
  SQLite 上扫描序碰巧等于插入序，PG 上不是。补 `.orderBy(asc(nodeRuns.retryIndex))` 即绿。
  **这类是本 RFC 的正靶心**——同一段代码换引擎就出错结果。
- **不是缺陷，是余量没了**：另一条断言「两个写者的子进程生命期重叠」，靠的是
  「相邻节点派生间隔 < 写者时长」。实测同机同用例：SQLite 泳道派生间隔 **~170ms**、
  PostgreSQL 泳道 **~700ms–1s**（PG 测试拓扑走完整 HTTP 应用 + 真库往返）。写者时长 300ms
  于是在 PG 上恒不重叠。两侧隔离都生效、锁序未变——**变的只是一次派生要多久**。

**规矩**：把一条**依赖时序**的既有用例接上双引擎前，先量两件事：
1. 该断言的余量是什么（「A 必须在 B 结束前开始」⇒ 余量 = B 的时长 − A 的启动延迟）；
2. 两个泳道的**同一段开销**各是多少。PG 泳道往往贵一个量级——不是因为 PG 慢，
   是因为它的测试拓扑更重（完整应用装配 + 网络往返 vs 进程内直连）。

余量不够时**优先把余量做成结构性的**（拉大被测时长直到 ≥ 2× 最坏开销），并把**实测数字**
写进常量声明处。**不要**按引擎分叉常量——那等于承认两个引擎语义不同，而事实并非如此。
也不要一路往上抬数字：下次再红先查开销为什么涨（同 `rfc321-cached-repo-refresh-credential` 的教训）。

## 分辨「runner 慢了」与「这一条挂住了」：看**同分片邻居的耗时**（2026-09-08 实撞）

CI 上一条用例撞满超时时，最容易的解释是「CI 机器慢」，处置就是抬超时。本仓的
`rfc321-cached-repo-refresh-credential` 被这么抬过两次（5s → 60s → 120s），两次的理由都写着
「整台 runner 慢了 2–4×」。**第三次撞上时一查，那个诊断是错的。**

**决定性判据是同分片其它用例的耗时**：
- 真·runner 慢：**所有**用例按比例变慢，包括同文件的邻居。
- 单点挂起：邻居**正常速度**，只有它一条爆表。

实测数据长这样（后者）：同一文件第一条 4100.73ms 绿（基线 2.7–4.6s）、第二条 120089.25ms 挂死，
同分片其余用例除一条 11.6s 的架构守卫外**全部 ≤4.1s**。同机同文件相邻两条，一条基线速度一条挂死
——runner 没有慢。

**取证姿势**（日志已下载到本地时）：
```bash
# 同文件所有用例的结果与耗时
grep -aoE "\((pass|fail)\) <describe 名> > [^[]*\[[0-9.]+ms\]" job.log
# 分片里最慢的十几条，用来定「正常水位」
grep -aoE "\(pass\) .*\[[0-9]{4,}\.[0-9]+ms\]" job.log | sed -E 's/.*\[([0-9]+)\.[0-9]+ms\]/\1 &/' | sort -rn | head -12
```
再用**时间戳空窗**定位挂点：把日志按行首时间戳解析出来，找 ≥20s 的间隔，看空窗**前后各是哪一行**。
上面那次的空窗夹在「冷克隆成功」与 bun 的 `killed 1 dangling process` 之间，一句话锁死了挂在哪一步、
而且 `killed 1 dangling process` 说明**有个子进程还悬着**。

**一般规律**：**抬超时前先证明「邻居也慢」。** 证不出来就不是墙钟不够，是某处没有上界——
抬多少都会再撞。本仓这一例的根因是 **git 对 `credential.helper` 子进程没有任何超时**，
会无限期等它退出（`GIT_TERMINAL_PROMPT=0` 只挡 tty 提示，挡不住助手自己卡住）。

## `| head -N` 之后不能下**全称结论**（2026-09-08 又撞，本日第二次）

实撞两次，都是同一个形状：
- `cmd | head -3; echo $?` —— 报的是 **head 的**退出码，不是 cmd 的。据此说「`tsc` 退出 0」是错的，
  真实退出码是 1（输出里没有 `error TS` 行才是那次的真相）。
- `grep -rn "batchInsertMax" src tests | head -8` —— 只看到 `capabilities.ts` 自己，
  据此说「全仓零消费者、这个能力没人用」。**是 `head -8` 截断了**：设施
  `platform/persistence/batchInsert.ts` 与 5 个消费点、一整份双引擎测试都在后面。
  由此把一件**已完成**的任务判成「只落了一半」，还据此排了工。

**规矩**：`head` / `tail` 只用于**看一眼样子**。凡是要下「有几个 / 有没有 / 全都是」这类
**全称或计数**结论，命令里就不能有 `head`——用 `wc -l`、`sort -u`、或者干脆全量输出。
判「零消费者」尤其危险：那类结论会直接导向「删掉它」或「重做它」。

## 漏 `await` 的第四档：**行为测试在两个引擎上都抓不到，只有源码守卫能抓**（2026-09-08 实测，推翻此前记载）

此前本文件记「`.run()` 不 await ⇒ PG 上静默出错、SQLite 上没事」。**「PG 红」那一半不成立。**
实测两个变异（中立事务体内摘掉围栏写的 `await`、摘掉任务铸造事务里 `taskCollaborators` 插入的
`await`），**两个引擎的行为测试全绿**——drizzle 的 sqlite-proxy `.run()` 是**急切发射**的，
它的 microtask 会被事务体后面任何一个 `await` 冲掉，于是照样赶在 COMMIT 之前落库。

**真正的暴露面只有两条**：① **返回值**——`changes` / `rows` 变成 undefined，CAS 判定当场误读；
② **顺序**——同一事务内多条写之间的相对次序不再保证。

**推论（重要）**：这一类缺陷**写行为测试是抓不住的**，写了也只是给自己一个假的安全感。
唯一能抓的是**源码守卫**：AST 扫「中立事务体内每个 `.run()/.get()/.all()` 是否都在 `await` 下」。
本仓已有一条这样的守卫，新迁的事务体要记得纳入它的语料。
`bun run lint:promises`（`no-floating-promises`）抓的是**丢弃整个 Promise**，与这一档**不是**同一件事
——这里 Promise 没被丢，是**没被等**，值照样拿不到。

## 改名会**静默缩小**守卫的语料（同上）

W8 把 `setNodeRunStatusTx` 改名成 `setNodeRunStatusInTransaction`，而
`rfc317-transition-table-oracle` 的 `NODE_WRITERS` 里写的是旧名——**那批调用点从此不在语料里**，
守卫继续绿，预言力已经没了。

这是「零与合规同形」的一个变种，但触发方式更隐蔽：不是有人把语料扫空，是**有人改了个名字**。
本仓的守卫大量按**符号名 / 文件名**取语料，改名就是在动它们的分母。

**规矩**：改任何被守卫点名的符号名 / 文件名时，`grep -rn '<旧名>' packages/backend/tests/architecture/`
一遍，把命中的语料清单一起改。评审改名类 diff 时也照此查一遍——
**守卫没红不是证据**，它可能只是不再看那些代码了。

## 用 `db.transaction` 下毒的测试，会在生产路径迁到中立原语时**停止下毒**（同上）

`rfc165-scratch-space` S8 靠 hook `db.transaction` 强制回滚来验错误路径。中立 SQLite 会话
**根本不调 `db.transaction`**（它自己发 `BEGIN IMMEDIATE` / `COMMIT`，把 client 直接交给事务体），
于是这条毒一滴都没下到。

这次它**红了**（`Received promise that resolved`），所以被发现；但那是运气——毒失效之后测试
**照样可能是绿的**，只是不再验任何东西。改法是把毒改打在真正会被调用的面上（这里改成 `db.insert`）。

**规矩**：迁移事务面时，`grep -rn "db.transaction\|\.transaction =" packages/backend/tests/` 找出所有
拿 `db.transaction` 做注入点的测试，逐个确认它们的目标路径迁走之后毒还在不在。

## 普查账本会造成「滚动红」：每笔红都被下一笔顺手修掉，同时开出新的红（2026-09-08 实撞，main 连红四提交）

`6075eb93e` / `6d098349f` / `26af7203f` / `1b5e74339` 连续四笔 CI 红，**三个不同成因**：
T17 `allowGrowth` 过期 → N1b 投影不一致 → RFC-345 门面守卫 → T17 `allowGrowth` 过期。

危险之处在于**每一个单独看都像已经处理过了**：普查是「读旧产物 + 重算 + 写回」，
下一笔提交跑普查时会顺手把上一笔的红修掉，同时因为本笔的债务变化又开出一个新的
一次性 `allowGrowth`——它在**再下一笔**上过期转红。于是账面上每笔都有人动过，
实际主干**一次都没绿过**。

更隐蔽的一层：**分片号每次都不一样**（2/4 → 4/4 → 3/4 → 2/4）。因为每笔都新增了测试
文件，守卫文件在分片间漂移。看「又是 shard 2/4 红」会以为是同一个问题复发，看「这次
是 4/4」又会以为是新问题——**分片号不携带任何信息，别拿它做归因**。

**规矩**：
- **推完必须盯到绿再推下一笔**，尤其是带普查重采的提交。连着推等于把「上一笔红没红」
  这个信息永久丢掉——后面每一笔的 CI 结果都混着前面的账。
- 归因只看**失败用例名**：`gh run view --job "$JID" --log | grep -oE "\(fail\) .*" | sort -u`。
  取 job id 要用管道给 jq，`gh run view` 本身**不支持 `--jq --arg`**（会报
  `accepts at most 1 arg(s), received 4`）。
- 判「这一刀能不能补绿」不要只跑自己改的那个守卫：普查产物被多个守卫消费，
  要在 HEAD 只读导出上跑**全量 `tests/architecture/`**，看到 0 fail 才算数。

## `allowGrowth` 一次性授权：这是第五次踩（同上，累计）

`allowGrowth` 授权的那次上涨一落地，它**当场就该删**——下一个不涨的 commit 上即判过期转红。
本仓已因此红过五次。**开豁免和删豁免必须是同一次提交的两半**，不存在「先留着，回头清」。

删它**不能只改 JSON**：`ledger-baselines.json` 会被 N1b 投影进其他清单，必须在 HEAD 只读
导出上**重跑全量普查**（`bun run scripts/architecture-census.ts --write --snapshot-sha HEAD`），
让 contentDigest 与另外几份产物的 currentSnapshotSha 一起前移。普查会
`readSeedJson` 读回旧账本并**保留人写的 `allowGrowth`**（`rfc294Canonical.ts:4067`），
所以必须先手工删掉再跑，指望普查自己清是清不掉的。
...
## 覆盖度守卫按「提到模块名」计数：**在注释里写一句文件名就算一次覆盖**（2026-09-08 实撞）

`rfc359-w5-t19d-coverage-parity` 的 `ref` 口径是「测试文件里提到该侧的**模块名或其导出符号**」。
实撞：一把刀在新对拍的**头注释**里写了一句 `postgresqlTaskRouteOperations.ts` 作为背景说明，
当场把那一对顶成 `9/2 → 10/2`、守卫转红——**而它没有给那一对加任何判据**。

守卫头注释里本来就警告过「覆盖数会虚高」，但没想到**虚高会来自文档**。

**规矩**：
- 在测试文件里写背景注释时，**不要写另一侧的模块文件名 / 导出符号**——写「PostgreSQL 侧的任务路由适配器」
  这类描述性说法，或者干脆写清「本文件不覆盖那一对」。
- 反过来，**读覆盖数时要知道它可能虚高**：一个对子的 ref 数上去了，不代表真的多了判据，
  可能只是有人在注释里提了一句。

**一般规律**：凡是「按文本出现次数」计的守卫，**注释与字符串字面量都会计入**。设计这类守卫时
要么剥掉注释再扫（本仓有几个守卫这么做），要么把口径写清并接受虚高。

## 「测试在给死代码续命」：零生产调用方，但被一条守卫养着（同上）

`postgresqlTaskLifecycleTransaction.ts` 的两个导出**零生产调用方**，唯一引用来自
`rfc359-w5-t18-…test.ts`。于是「零生产消费者」类守卫扫不出它——**测试自己就是那个消费者**。

同族的更狠一例：效应观察者有两份实现，`application/*` 是生产路径、`infrastructure/sqlite*` 是死的
第二份；而 RFC-328 那条 1495 行的正确性矩阵里，**唯一锁住某条不变量的用例 import 的是死的那份**。
也就是「**被跑的那份没人用，在用的那份没人跑**」——两边都绿，两边都没意义。

**判据**：判「零生产消费者」时，**要把测试排除在消费者之外**（本仓 `rfc359-w5-adapter-production-consumer`
正是这么定义的，别退回去）。反过来，看到一份实现「有测试覆盖」时，**先确认那些测试驱动的是不是
生产路径上的那一份**——同名/同形的两份实现里，测试很容易钉错一份。
...
- **固定 `setTimeout` 等一个「事情会发生」= 墙钟赌博，负载高的 runner 必输**：`memory-distill-scheduler` 的重入用例先关闸、`release()` 后 `await sleep(80)` 再断言 `calls === 2`，对着一个 5ms 间隔的循环。本机连跑 5 次全绿、CI 的 macos shard 上 `Received: 1`（run 30886241395）。**判据是断言的方向**：证明「某事已发生」（正向）必须**轮询到条件成立 + 一个宽松 deadline**。写成固定 sleep 的正向断言迟早会在别人的 PR 上红，而且看起来像别人的锅。
  **勘误（2026-09-12，RFC-359 AC-20）**：本条原写「证明『没有多余发生』（负向）**只能**靠固定等待，无法轮询」——**那句是错的**，照它写就会留下一枚固定 sleep。负向断言的正解是**因果屏障**：让一件**必定会发生、且因果上排在被断言者之后**的事成为谓词，等它成立，再断言那件不该发生的事没发生。三种取材（都已在仓内落地，见 `design/RFC-359-database-provider-unification/plan.md` §5bf）：
  ...
  ② **把一趟不走那把锁的请求完整驱过同一个 app**——用于「证明另一个在飞的请求还卡着」（`tests/helpers/stillParked.ts`，`plugins-http` / `runtime-routes-registry` 三处同形）；
  ③ **应答本身就是屏障**——去重 / OCC 这类在应答**之前**就已判定的，拿到那个 `duplicate` / `409` 就等于拿到了「此后不会再有一次派发」（`rfc259-github-ingress:191`）。
  **屏障分两种强度，注释里必须写清是哪种**：投递路径若是**同步**的（如 repo-import 频道无 frameGate，`gatedSubscribe` 走同步 `sendJson`），「后播的到了 ⇒ 先播的已判完」是**严格蕴含**；若是 **fire-and-forget** 的（workflows 频道有 frameGate，`.then(...)` 起，跨帧无序），那只是**相对屏障**——仍远好于固定毫秒（机器越慢屏障自己越慢，窗口跟着放大，而固定 30ms 会被慢机器吃掉），但别让下一个人以为拿到了证明。
  **还有一类真的没有谓词可等**（刻意推进时钟让 `installedAt` 严格递增之类）：那就在**紧上一行**写 `// sleep-ok: <理由>` 显式豁免——守卫只读紧邻的那一行（`tests/test-suite-policy.test.ts` 与 `scripts/rfc359-ac6-preflight.sh` 同判据）。零容忍 + 带理由的豁免，比让判据去猜意图诚实。
  **改完要变异验证**：屏障换错了会让负向断言**永远绿**。逐条改源码制造那个回归（让 frameGate 漏帧、让 `channelKeyOf` 不分批次），确认转红，再把源码 revert 干净。
...
### sync → async 的转换里，「忘了 await」会**静默通过类型检查**

- **现象**（RFC-359 W4-D23b 实做，2026-09-06）：把一层同步机器改成异步（`dbTxSync` → 中立事务原语）时，
  端口签名通常要放宽成 `void | Promise<void>`（因为两侧实现一时长短不一）。这一放宽，**漏 await 的调用点
  tsc 就不再报错**——返回的 Promise 被当成合法的 `void` 丢掉。实撞：`deleteSkill` 漏了一个 await，
  `getSkill` 随后仍返回那一行，测试红在「删除没删掉」，而类型检查全绿。那一轮共找出 **13 处**同类调用点。
- **为什么 tsc 帮不上**：`no-floating-promises` 只对**明确返回 Promise** 的表达式生效；一旦联合类型里
  混进 `void`，规则就放行了。真正危险的是端口层（依赖注入的成员），因为实现和调用点不在同一个文件里。
- **做法**：转换完**不要只信 tsc**。按「这一轮改成异步的导出名」列一张清单，用
  `grep -rn "name(" src | grep -v "await \|export \|import "` 逐个过一遍调用点；
  跑测试时优先跑**写路径**的用例（删除 / 提交 / 发布），漏 await 的症状总是「写好像没生效」。
- **更阴的一种：漏 await 的 Promise 被当成布尔判据**（RFC-359 W4-D23c 实做，2026-09-07）。把一个
  返回 `boolean` 的读函数改成 `async` 之后，调用点写在合取里：`const ok = a === b && isCanonical(db, id)`。
  tsc **不报错**——`ok` 的类型成了合法的 `false | Promise<boolean>`；而 Promise **恒真**，于是这条判据
  从此永远成立，屏障 / 围栏形同虚设，并且**没有任何测试会红**（它只在"本该拒绝"的分支上失效）。
  同一轮还撞到「两条 authority 断言整个被丢掉」——`await` 缺失让断言函数根本没被等待，异常无人接。
  **凡是把 `boolean` / 断言型函数改成 async 的，调用点必须逐个看**，别指望类型或测试兜底。
- **摘掉 `.run()` 时必须同时补 `await`**（同轮自伤）：drizzle 的查询构造器是惰性的，
...
- **现象**（RFC-310，2026-09-07 定位一条查了一周的 CI 间歇红）：`getMission()` 读出 revision，
  紧接着 `occUpdate(id, revision, epoch, patch)`，**返回值不看**。两步之间只要有并发写手 bump 了
  revision（本仓里路由的 fire-and-forget reconcile、定时器、另一轮泵都会），补丁就静默丢失。
- **为什么后果远比「少写一行」严重**：丢的往往是**决策依据**（这里是 `repositoryFactsRef`）。
  丢了之后系统看起来仍处在「还没做那件事」的状态，而决策的去重键一个字节都没变 ⇒ 下一轮被
  去重 ⇒ handler 再也不跑 ⇒ **永久停顿**。表现是 status 一直停在中间态、没有错误、没有阻塞码，
  把任何预算耗光——极难归因，也不会有测试自然红。
- **做法**：这类「把刚产出的东西记回去」的 OCC 写必须**重读重试**（本仓的 `recordOnMission`）。
  重试只对**修订漂移**做；epoch 冲突不重试——epoch+1 是 cancel/handover 有意让在途工作过期，
  那时候把旧产物记回去才是错的。**同一文件里出现一处，通常就有一族**：这轮一口气找到 7 处。
- **顺带**：给长跑循环的「预算耗尽」错误加上**状态 dump**（未结算行的 kind:state、轮数、关键
  引用是否为 null）。这条红此前六次只留下 `status=working blockCode=null`，每个撞上的人都要
  从头查；加了 dump 之后第七次一红就当场否掉了错误假说，直接指向真因。

### 把测试命令管进 `tail` / `head`，你看到的退出码是**管道末端**的，不是测试的

- **现象**（RFC-359 W4-D23c，2026-09-07）：为了只看结尾摘要，把全量后端套件跑成
  `bun run test:backend 2>&1 | tail -40` 丢进后台。任务回报 **exit code 0**，我据此当作全绿推了 main
  ——CI 上 backend shard 2/4 立刻红（一条源码锁指着我刚删掉的函数）。`tail` 成功了，所以管道退出码是 0；
  测试的非零退出码被吞得干干净净。更糟的是 bun 的分片输出被缓冲，`tail` 那一头**一个字节都没落盘**，
  于是「0 字节输出 + exit 0」看起来像「跑完了、没事」。
- **做法**：跑门禁类命令**不要接管道**。要摘要就先落盘再读：
  `bun run test:backend > run.log 2>&1` （退出码就是测试的），然后 `tail -40 run.log`。
  非要管道时用 `set -o pipefail`，或显式检查 `${PIPESTATUS[0]}`。
- **顺带**：本仓「本地不跑门禁、以 CI 为准」的约定不受影响——但**自己主动跑了本地套件、又据此判断能不能推**的时候，
  这条决定了你判断的是不是真的。

### 合一之后，还锁着「已退役那一份」的测试是**沉默的**——重指它们才是找回丢失行为的手段

- **现象**（RFC-359 W4-D19c-tail，2026-09-06，一刀连出四处）：工作组回合引擎合一后，SQLite 侧十几个行为套件
  仍直接调已经没有生产消费者的 legacy engine。它们**全绿**——绿的是死代码。逐组把判据重指到两个 provider
  真正在跑的那份中立驱动，连续照出四处合一时丢掉的行为，每一处都是用户可见的：
  ① 系统署名房间消息的分类判据没带过来，平台手写英文兜底文案可以直接落进房间且无从本地化；
  ② 收尾轮被丢弃的派单在房间里没有任何说明（模板早在、没有调用方）；
  ③ 重启杀掉的「已回答澄清」续跑不再按原样血缘重铸，人回答过的 Q&A 再也注不回提示词（RFC-187 T13 原地复发）；
  ④ 流中断的重跑不推进 `retryIndex`、还挂着主 cause，撞号破坏 node_run 身份，且**白吃掉一整轮**轮次预算；
  ⑤ 房间消息 id 退回普通 `ulid()`（合一前是 `monotonicFactory()`）——游标按 `message.id > cursor` 的字典序
  判「我没看过的」，同毫秒两条消息没有稳定序，游标先落在字典序较大那条上时，另一条对该成员永远不再出现。
- **为什么合一评审看不见**：两份实现对着 diff 只能看出「两边都有的东西哪里不一样」。一侧**独有**的分支在
  diff 里长得像「另一侧的多余代码」，把弱的那份提为中立基线时它就静默消失了——而唯一会发现它的那条断言，
  还锁在被退役的实现上。
- **找全引用要用两种形态**：`import` 与**按路径段拼出来的 `readFileSync(resolve(dir, '..', 'src', …, 'engine.ts'))`**。
  `scripts/tests-referencing.sh` 只找前者；后者要按**被删文件的 basename 加引号**去 grep
  （`grep -rn "'engine\.ts'\|'turnExecution\.ts'" tests`）。D19c-tail 收尾时正是这一步又找出八个套件，
  其中一个就藏着上面第 ⑤ 处缺口——typecheck 与「删完跑一遍」都照不到它们，因为它们在**运行时**才 ENOENT。
- **规矩**：**合一不是删掉一侧就完事，删之前必须把所有还指向它的测试逐条重指到留下的那份**。
  重指时逐条问「这条断言在新实现上还成立吗」，红了先假设是**合一漏带**而不是断言过时；确认属实就先写一条
  能复现的双引擎用例（红），再补回行为（绿）。等到所有引用都指向生产那份，删岛才是安全的——
  在那之前，被退役实现上的绿灯不构成任何证据。
...
## 「减少往返」这个杠杆只对**串行**往返成立（2026-09-10 实测，推翻了我自己的判断）

RFC-359 AC-11 上折过三个端点，两成一败，差别只有一条：**那几次往返是串行的还是并发的**。

- **串行**（`await a; await b; await c;`）——N 段就是 N × RTT。评审徽标与反问徽标都是这一类：
  「先把候选行捞出来 → 再问一次可见性 → 在 JS 里数一遍」。折成一条 `count(*)` 之后，
  PG 侧省掉的是真实网络时间。
- **并发**（`Promise.all([...])` + 足够的连接池）——墙钟只有 ~1 个 RTT，**折叠省不下时间**。
  `/api/overview` 的六个 ACL 计数就是这一类：`poolMax` 缺省 16，六条查询各占一条连接同时发。
  我按同一个杠杆把它们折成一条（每维一个标量子查询），语句数 13 → 8，P95 **3.24ms → 3.17ms**
  ——在噪声里。整刀回退。

**规矩**：动手折之前先看调用点是 `await` 串起来的还是 `Promise.all` 并起来的；分不清就先量
...
**⚠️ 判据只能报「直线式睡眠」，而且报出来要逐条分类。** 这条判据改了四版才对
（报出的数字 20 → 11 → 12 → 7），每一版的错都只能靠**跑真实数据**发现：

| 版本 | 漏判的正当形态 |
| --- | --- |
| 只看「有没有 setTimeout」 | 有界轮询（`for(;;){ …break; if(Date.now()>deadline) break; await sleep }`） |
| 循环 + `Date.now() +` | **计数式**上界（`for (attempt < 500 && isActive(id))`） |
| 循环里有 `break` | 用 **`return` / `throw`** 退出的轮询（`waitFor` 辅助函数的标准写法） |

**真正的区别不在「有没有上界」，而在谁在做同步**：坏形态里睡眠时长**就是**同步手段；
好形态里 `loop { 查一下; 满足就退出; sleep(退避) }`——**那个检查**才是同步。

**而且判据推断不了意图**：`plugins-http` 有一处睡眠是刻意推进时钟（让 `installedAt` 严格
递增），改成谓词等待反而是错的。所以这条检查是顾问式的，报出来分三类：
① 等写入落库 ⇒ 改 `eventually`；② 断言某事没发生 ⇒ 补因果屏障；
③ 与写入无关 ⇒ 上一行写 `// sleep-ok: <理由>` 豁免（守卫与 pre-flight 都认这个注释）。

**假阳性在这里的代价不是噪声——是有人照着把一个正当的轮询循环、或一次刻意的时钟推进改坏。**
...
- **凡是「按文件内容分类」的守卫，判据都要跑在剥掉注释的 token 流上**（TS scanner + `skipTrivia`），
  不能裸文本扫。2026-09-14 实撞：RFC-359 的单引擎分类把 `rfc305-architecture-lock` 判成
  「用了裸驱动」，因为它的**注释里**写着 `db.$client`——而那段注释恰好就是在解释
  「裸文本扫描会撞上自己」。**分类判据的失效方向总是让数字变好看**（把真待办挪进「已裁决」），
  所以这类守卫宁可多花一次解析。token 用空格拼回之后，所有判据要写成容忍空白的形式
  （`new\s+Database\s*\(` 而不是 `new Database\(`）。
...
- **一个测试文件在单引擎账本里有多个条目时，先分清它们归属哪个块**。常见形状是同一个文件里
...
- **测试被 CI 掐红时，先问「这条断言真的需要那段慢准备吗」，再考虑加时间**。
  `rfc210` 那条「没有 `.gitmodules` 时短路」为了断言真去 `git init/add/commit` 造了个仓，于是在
  共享 runner 上被时间掐红两次（5s → 补到 30s → 又在 30s 超时）。**继续加时间是错的方向**：
...
## 合一两个 provider 实现前先**数清差异的条数与方向**——未必只有一条，也未必同向（RFC-359 AC-10 实测，2026-09-15）

账本 / 守卫记的通常是**分派点**（`provider === 'sqlite' ? A : B`），但分派存在的理由是
「A 和 B 不同」。动手前要把 A 与 B 逐方法对一遍，把差异列成表。RFC-359 实测那次：
账面是 1 个分派（2 处等值比较），逐方法对下来是 **4 条差异**，而且**方向相反**——
两条让 SQLite 更弱（写回行数不校验、漏传 `now` 导致同一行里两个时钟），
一条让 PostgreSQL 更弱（手抄的事务内联版漏了归属闸），还有一条无优劣但必须一致（写入次序）。

如果只按「哪个引擎是参考实现」一刀切，无论切哪边都会**把另一边弱的那条扶正、把强的那条弄弱**。
正确姿势是**逐条定方向、各自取强的一侧**，然后差异归零、分派自然消失。

两条推论：

- **「差异只有一条」是要验证的假设，不是可以省掉的步骤。** 源码注释里写的那条
  （「这条历来走宽判据」）往往只是**当时那个人注意到的**那一条。
- **手抄件会悄悄漂移。** 那份漏了归属闸的内联版，最初多半是照着共享原语抄的；
  抄完之后共享原语加了闸，手抄件没跟上。发现「一份共享实现 + 一份内联同义实现」时，
  默认假设是**内联那份已经漂了**，逐行对一遍再决定留谁——通常是删手抄件。

### 顺带：账面清零 ≠ 形状消失，守卫会替你咬住

上面那次收敛的第一版把两个入口留成了薄别名（函数体逐字相同、只有形参类型不同），
分派确实没了、账本确实清零了，但 `rfc359-w5-identical-provider-twins` 当场红，
...
## 验证跑的时候别改源码——改了那一跑的结果就作废了（2026-09-15 实撞）

后台起了一个 70+ 文件的双引擎验证跑（4 分钟），等它的时候顺手把下一处收敛也改了。
跑完红四格，其中三格是**我自己在跑到一半时改出来的**——bun 是边跑边读文件的，
那一跑看到的是一棵**跑到一半被改过的树**，红哪些、绿哪些都无法归因。

这和共享工作树上跑 `gate:local` 红了无法归因（本文件上面那条）是同一个病，只是这次
并发的另一方是我自己。规矩一样：

- **一次验证跑对应一个确定的树状态。** 起跑之后到出结果之间，源码和测试都不要动。
- 等待期间想干活，只能挑**不在测试语料里**的：写 `design/**` / `docs/**` / commit message、
  读代码、查 CI。`architecture/*.json` **不算**安全——有守卫会读它。
- 真的必须改，就先把跑停掉（`TaskStop`），改完重跑，别让两件事叠在一起。
...
**它不是稀有事件**：同一天连着两笔纯文档提交各中一次，先 macOS `shard 1/3`、
后 Windows `shard 1/4`，三信号完全一致。所以**把 steps 数直接做进你的 CI 盯绿脚本**，
别等出事再去查：
...
`[steps=0]` 一眼就能和真失败分开——第二次就是靠这个立刻认出来的，没再花时间翻日志。
顺带一个旁证：同 run 里正常执行的兄弟分片 `steps` 是 15，没跑起来那个是 0。


## 单引擎测试迁双引擎：先问「判据依赖哪个引擎的**执行模型**」，再谈换 harness（RFC-359 AC-6 实撞，2026-09-15）

把 `describe` 换成 `describeEachProvider`、库取 `harness.db`——大多数迁移到此为止。
但有一类测试换完会**静默变成假绿**：它的判据本身依赖某个引擎的执行模型。

实撞的是 `rfc097-task-status-cas.test.ts` 的 CAS 竞态用例。它靠一个 db 代理，在
`transaction` / `run` 被调用的瞬间**同步**插入竞争写者，好让 helper 的
`WHERE status = from` 必然 miss。文件头注释把前提写得很清楚：
「bun:sqlite 全同步（`.run()` 立即落库），时序 100% 确定」。

这个前提**只对 SQLite 成立**。PostgreSQL 驱动是异步的，同一个代理会在竞争写者**还在飞**时就
放行事务：CAS 照常命中、helper 照常成功、用例**照常变绿**——竞态根本没发生，判据退化成
「没有并发」，而且你从绿色里看不出来。

**判据：迁移前先问一句「这条用例的预言力，依赖的是被测代码，还是某个引擎的执行模型？」**
依赖执行模型的典型信号——同步落库、`.get()` 立即返回、代理拦同步调用、`sleep(10)` 之后断言。

**正解是找一个「两个引擎都被 await」的注入点**，而不是把判据降级。这次用的是 helper 自己就有的
`await args.beforeCas?.()`（在 SELECT + 双闸校验之后、CAS 事务之前），代理整个删掉。
生产侧没有这种点时，**去生产侧开一个**（本次顺带给 `trySetTaskStatus` 补了 `beforeCas` 的类型
声明——它本来就原样转交 `args`，运行时一直是通的），也别把断言改松。
...
## 按「测试文件**文本**」计数的棘轮，要在**所有编辑做完之后**再跑最后一遍（2026-09-17 推红一次）

`rfc359-w5-t19d-coverage-parity` 的 `ref` 列数的是「提到该侧模块名或任一导出符号的测试文件数」，
**按纯文本扫**——写在**注释**里的一个文件名也算一条引用。

实撞：一次合并里我先把账本改成 `sqlite 11/3`、跑全量守卫 781/781 绿，**之后**又在另一个测试
文件的注释里写了一句「（`sqliteTaskExecutionRuntimeParticipants.ts` 逐字同形）」——引用数变成
12，而账本还是 11。那一提就这么推上了 main。

**判据**：这类「按文本数引用」的棘轮（t19d 是一例，`rfc359-w5-identical-provider-twins` 的分母、
`rfc345` 的兼容边同族）对**注释**同样敏感。因此：

- 改完**最后一个字**再跑一次 `bun test tests/architecture/`，别把中途那次绿当成结论；
- 尤其是「写注释解释为什么合一」这种收尾动作——它**本身**就会改这些数；
- 提交前 `git diff --cached` 里如果还有任何 `tests/**` 的文本变动，上面那一跑就得重来。
...
### 检查次序类的判据：要让两道门**同时**失败才测得出来

（RFC-359 纯读三件合一时实撞。）两份实现都先查 A 再查 B，你想锁住「次序不能调」，于是写了
三格用例：A 坏 B 好、A 好 B 坏、两个都好。**这三格都测不出次序**——只有一道门失败时，
先查谁都得同一个答案。把两道门对调的变异会静静地绿过去（实测如此）。

补一格「A 和 B 同时失败」，断言回的是 A 的那个错，变异随即转红。

具体形态：`GET /api/tasks/:id/diff` 先查 `base_commit`（缺 → 409）再查工作树（坏 → 410）。
「没有 base commit **且** 工作树也没了」必须回 409——「任务在准备阶段就没成」是主因，
工作树在不在是次要信息。同类判据还有：先查存在性再查权限、先查版本再查状态、
先查配额再查参数合法性。**凡是「先 X 后 Y」的契约，用例矩阵里必须有 X∧Y 同时为假的那一格。**

### 源码锁的半径是**跨包**的：frontend 会读 backend 的源码

（RFC-359 纯读三件合一时实撞，`5d78a3eaf` 推红一格。）删函数 / 搬家时的半径要取
「谁提这个符号」∪「谁把这个文件当文本读」——这条已经写过，但那次的文本读者**在另一个包里**：
`packages/frontend/tests/diff.test.ts` 用 `readFileSync('../../backend/src/services/task.ts')`
grep 多仓 diff 的表头模板，确保前端解析器与后端发射点用同一个字面量。
搬走发射点之后它以「格式变了」的名义红，真实原因是「文件不在那儿了」。

**扫半径时 `grep` 的范围要覆盖整个 `packages/`（外加 `e2e/` 与 `scripts/`），不是只扫自己那个包。**
这类跨包锁本身是好东西（它挡的是「前后端对同一个字面量的理解悄悄分家」），代价就是搬家时要记得改锚。

**而且 grep 的模式要能匹配分段拼出来的路径**：同一次搬家还漏掉了 backend 自己的
`source-text-rfc066-pr-b-guards`（PB-G5），因为它的路径写成
...
### 「一侧有、另一侧没有」的判据，在双引擎对拍里两边都不进——合并时才第一次被要求覆盖

（RFC-359 第 6 刀实撞。）本仓双引擎对拍的惯用结构是 A/B 两段：**A 段收「两侧同义的公共子集」，
B 段收「架构不同、合并会抹掉的分叉」**。这个结构有一个盲区：**一侧有、另一侧根本没有的那种门**
两边都不属于——它不是公共子集，也不是「两种写法的架构差异」，它只是一侧更强。

于是这类门可以长期零覆盖。第 6 刀把 PG 独有的两道预览门（非工作流任务、`task-active`）
抬进共用路径时实测：各自变异成 `if (false)`，**一条用例都不红**。

**处置**：合并把「一侧更强」的判据落到共用路径上时，**它就变成了必须自带测试的新行为**，
按「改动自带测试」补覆盖，并用变异实证（改掉它必须在**两个引擎上**同时红）。
反过来，在合并之前也可以主动扫一遍：对着 B 段的清单问一句「有没有哪一侧的门根本没进这张表」。

### 按**文件名**配对 / 筛选的守卫，都会漏掉名字不规整的那一对

（RFC-359 第 8 刀勘察时发现，与前面「棘轮清单按名字筛连漏三次」是同一类脆弱性。）

`rfc359-w5-t19d` 这份「成对适配器覆盖对等」账本靠 `sqliteFoo.ts` / `postgresqlFoo.ts`
的自述式**文件名前缀**配对。于是它看不见这一对：

- `platform/persistence/sqlite/taskLifecycleRepair.ts`（靠**目录**表明引擎，文件名里没有前缀）
- `modules/task-execution/infrastructure/postgresqlTaskRouteRepairOperations.ts`（基名也不同）

后果不是「记账少一行」，而是**这处覆盖倒挂从来没被报过**：SQLite 侧 13 个套件天天跑，
PostgreSQL 侧的手动修复一条行为判据都没有。账本存在的全部意义就是报这个。

**教训**：任何以「名字长这样」为判据的守卫，都要配一个**显式的手工兜底表**，
收容名字不规整但确实成对/确实该扫的那些。名字是启发式，不是事实源——
`grep` 筛棘轮清单如此，`classify()` 配对适配器也如此。

**同一次勘察里我自己也栽在名字上**：数「PG 侧手动修复有没有覆盖」时只数了文件名带
`lifecycle-repair-` 的 13 个套件，得出「PG 零覆盖」的结论并写进了 plan / STATE / 提交说明。
