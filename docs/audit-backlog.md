...
## `createPostgresqlDatabaseClient` 把 schema provider 全局翻成 postgresql 且**丢掉 restore**（2026-09-15 发现，未修）

`packages/backend/src/platform/persistence/postgresqlDatabaseClient.ts:380`：

```ts
export function createPostgresqlDatabaseClient(runtime) {
  selectDatabaseSchemaProvider('postgresql')   // ← 返回的 restore 函数被丢弃
  …
}
```

`selectDatabaseSchemaProvider` 返回一个 `() => void` 的还原函数（`db/providerSchema.ts:150`），
这里没有接。于是**进程里只要建过一次 PG 客户端，全局 schema 前缀就永久停在 `agent_workflow.`**。

**实撞**：同一进程里先跑 `rfc359-t19h-postgresql-upgrade.integration.test.ts`（它内部
`createPostgresqlDatabaseClient`）、再跑 `rfc359-t19h-generation-upgrade.test.ts`，后者的
**SQLite** 查询会带上 PG 的 schema 前缀：
...
SQLiteError: no such table: agent_workflow.workflows
  ...
  at createPortableBackupArchive (services/portableBackupArchive.ts:123)
```

单独跑任一文件都是绿的（24/24、7/7）；换成
`rfc349-daemon-provider-bootstrap` + `generation-upgrade` 这对也绿。**只有这一对会红。**

注意那个集成测试**自己是守规矩的**：它在 `try/finally` 里 `selectDatabaseSchemaProvider(...)`
并 `restoreProvider()`。漏的是**客户端内部**那次——它发生在测试自己那次之后，还原函数被丢掉，
所以测试的 `restoreProvider()` 还原到的是它自己调用前的值，客户端那次的影响留了下来。

**为什么现在还没红过 CI**：backend 测试按路径确定性分片，这两个文件目前不在同一片；
`--randomize` 只在片内打乱。分片一变就可能红，而且红在一个与改动毫无关系的文件上。

**候选处置**（都没做，待定）：
1. 客户端不再动全局——由装配处（已经知道 provider 的那一层）选一次；
2. 客户端接住 restore 并在 `close()` 时还原；
3. 兜底：给那个集成测试加 `afterAll` 复位（治标，且下一个建 PG 客户端的测试还会再撞）。

第 1 条最合乎 RFC-359 的方向（品牌只在装配期出现一次），但它会改动一条热路径，
需要单独确认；本次发现时不在手头改动范围内，先记账。
