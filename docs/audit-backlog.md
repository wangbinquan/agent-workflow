...
**但它是间歇的，不是常驻的**：紧接着的下一个提交（`c2096da9`）同一条腿**全绿**。所以
目前的口径是「在 Windows 腿上间歇复现（已观察 2 红 1 绿），会不定期把 `main` 打红」，
**不要**写成「必红」——第一版记录我就写过头了，这里订正。

- ⏳ **(P1，会不定期拦门禁)** **第一步不是排除规则，是让它可诊断**。现在这条断言只
  打印 `v.id`，所以两次红都拿不到「哪个元素、前景/背景各是什么色、算出的对比度是
  多少」——没有这些就没法判断它到底是「Windows 渲染导致 axe 取样不同」还是「一个真
  实的暗色对比度缺陷只在这台机器的渲染下越线」。把 `nodes`/`any[].data` 一并打进失败
  消息（或存成 artifact），代价一行，下一次红就能定性。
- ⏳ **(P1)** 定性之后再选处置：若确属平台渲染差异，**按规则 + 平台**排除并写明理由
  （不是全平台排除，那会连 Linux/macOS 的真实覆盖一起丢）；若是真实对比度问题，改
  样式。**在拿到节点级证据之前，两种处置都不要做。**
- **间歇性本身是一条线索**：同一份构建产物在同一镜像上时红时绿，更像「渲染/取样时序」
  而不是「某个元素的配色越线」——但这仍然只是假设，节点级证据到手前不要据此结案。
- **顺带一条口径**：把 e2e 翻到 Windows 腿时按标题排除了「恰好两条」已知红，本条不在
  其中——即那次翻腿的证据面没有覆盖到这个 spec。再翻别的腿之前，先连着跑几轮看常驻红
  是不是只有那两条。
