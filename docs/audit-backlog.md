...
- ⏳ **RFC-254 容器簇尾部 2 文件（RFC-238 MCP-runtime-测试 feature，非核心 Code→Audit→Fix 路径，须 fresh-context 逐条真机核实非「快 skipIf」）**：bulk-sync 当前 src 后真机诊断：①`rfc238-mcp-test-execution-material`（1pass/2fail）——`binarySnapshot`/`frozenFileDigest` 冻结/封 MCP 测试可执行文件：`frozenFileDigest` 的 mode 断言**已 win32-gated**（非 bug#2 类），红在 `lstat`/`executableFile`（POSIX 可执行 fixture 在 win32 无 `.exe/.cmd` 形态、`Bun.which('sh')` 解析等）——疑 POSIX-fixture，但须确认非真 win32 冻结缺陷。②`rfc238-mcp-runtime-test-real-e2e`（0pass/1fail）——真进程多轮 fixture `ConflictError: the MCP test session cannot accept another message`（`mcpRuntimeTest.ts:993`）：真进程 + session 状态，疑 win32 时序/状态，**须辨真 bug vs 测试时序**（killGroup：real-e2e 的 ConflictError 不可仓促 skipIf）。**下一步**：逐条真机诊断，POSIX-fixture 类补 win32-executable fixture 或 skipIf+登记，real-e2e 类先确证时序 vs 真 bug。
- ⏳ **RFC-254（承上）**：以上全是 plan **构建**；真 opencode 子进程在 Windows 的实际**执行**（loopback/直 API/FFF）尚未端到端跑（需真 opencode 二进制 + 真模型，属更后 T）。
