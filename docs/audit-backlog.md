...
- 🔴 **RFC-271 批次 G 引入的前端 UI 回归（CI e2e 持续红，归属明确是我）**（2026-08-09 定位，`a3cc99ca` run 31297879784）。**此前一直被掩盖**：`focus-ring-clip.spec.ts` 在「找不到 `Import YAML` 按钮」上提前失败，于是那个对话框与相关页面**从未真正被扫到**；把按钮选择器修好之后，18 个裁剪一次性暴露。诊断已精确到元素与像素：
