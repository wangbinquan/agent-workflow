...
- ⏳ **改 agent 没有下游守卫，删 agent 有**（RFC-358 设计门对抗审查查实，2026-09-04，本 RFC 按用户裁决 D6 只做知情提示、不拦截）：
  `deleteAgent` 会全表扫 `workflows` 并以 `agent-in-use` 拒绝删除一个仍被引用的 agent（`legacy/agent.ts:766-816`），
  而 **update 路径两个 provider 都零下游检查**——`legacy/agent.ts` 的 prepare/commit 与
  `postgresqlIntentApplyResourcePorts.ts` 的 agent 分支，校验方向全部朝上游（「A 引用了谁」），没有一条问「谁引用了 A」。
  于是把 agent 的 `outputs` 删掉一个端口，可以让全库任意数量的既有工作流变得不可启动，全程零提示；最早在
  `taskLaunchGate` 抛 `workflow-invalid` 才暴露。RFC-358 只在意图确认页列出「引用该 agent 的既有工作流 N 个，本次未校验」
  （`validation.agentDownstreamWorkflows`，不进 INTENT.md），**没有**加拦截——理由是「先改 agent、再改工作流」是合法的
  迭代节奏，拦截会破坏它。真要守，需要产品先定失败语义（保存期拒绝 / 启动期拒绝 / 仅告警），扫表逻辑本身现成。
...
- ✅→⏳ **`/ws/repo-imports/:batchId` 无 gate 频道——WS 半边已修，REST 半边待拍板**（RFC-285 B6②，2026-08-13）：BatchRecord 自创建携 `ownerUserId`（发起路由 actor 注入，绝不来自请求体），WS 升级门=发起者 ∨ 资源管理员，缺行/非发起者**同形拒绝**（batch-not-found；矩阵锁 ws-repo-imports.test.ts）。**剩余待拍板**：同数据的 REST 面 `GET /api/cached-repos/imports/:batchId`（getBatchSnapshot）与 `POST .../rows/:rowId/retry` 仍是 token-only、任何持凭据者可读/可重试他人批次——收紧属能力收缩、不在 RFC-285 已批准的 E 清单内，按 CLAUDE.md 收缩门槛呈用户逐项确认后另行落地（判定读点 `batchOwnerUserId` 已就位，接线是小改）。 **2026-08-26 追记（RFC-329 设计门 P1-8）**：影响面比原记载更大——`mcp_only` 令牌今天被 purpose 门挡在 REST 之外（`routes/registry.ts:188-193`），但 MCP dispatch 按 RFC-247 D2 **清除 purpose**（`mcp/dispatch.ts:125-139`），所以一旦给这两条路由补 MCP 工具，就等于把这个已知无门的跨用户读取/重试**第一次开放给自动化通道**。RFC-329 因此明确**不补**这两个工具（用户 D12 拍板），并在 `rfc329McpSurfaceLedger.ts` 里以 `not-in-scope` 登记。收紧仍待拍板；收紧之前不应给它们做工具。
...
- ⏳ **MCP 收敛工具只覆盖 CRUD**（RFC-247 实现门 P2）：`resource_read`/`resource_write` 的 `method` 枚举只有 list/get/create/update/delete，因此 workflow copy·export、workgroup rename、repo refresh、memory archive·unarchive 这些**已对令牌开放**的路由，MCP-only 客户端够不着。要么扩 method 枚举，要么给它们具名工具——属 v2 的「MCP 面做多宽」范围，本轮未做。**2026-08-14 收口方向（对齐 RFC-294 pin `be31dd62`；按 `6e8c4f9f` 仓规只引小节号）：不要扩 `method` 枚举。** `McpBinding = {operationId, toolName}`（`design.md` §13.1）是 operation↔tool 一对一，`plan.md` **W4-A** 要求「HTTP RouteMeta 与 MCP tool 映射引用同一 operation id/handler」，§13.1 结尾又明禁 catalog 导出 generic invoker——扩枚举正是往 generic invoker 方向加固。等 W4-A 落地时这条自动消解。 **2026-08-26 现状更新（RFC-329）**：人工门那部分已由 RFC-329 以**具名工具**补齐（反问 / 工作组 / fusion 三个门共 21 个），本条剩余范围收窄为**三类资源域运维面**：skills 内容面 11 条、workflows validate、四类 rename、workflows·workgroups copy、`cached-repos refresh`、mcps probe + runtime-test 9 条、全部 `GET .../acl` 读面、`scheduled-tasks run-now`、memory archive/unarchive、`POST /api/fusions`（发起融合，是发起不是门）。这些今天在 `packages/backend/tests/architecture/rfc329McpSurfaceLedger.ts` 里以 `not-in-scope` 逐条精确登记、受高水位约束只许降——不再是「无人看管」的状态。
- ⏳ **`users:search` 是系统域点，于是 `launch_task` 的 `collaboratorUserIds` 在 MCP 上无解**（RFC-329 设计门 P1-1，2026-08-26）：`GET /api/users/search` 与 `POST /api/users/lookup` 都要 `users:search`，而它在 `SYSTEM_DOMAIN_POINTS`（`shared/schemas/permission.ts:808`）里被 `resolveTokenPermissions` 显式剔除（同文件 `:1293-1301`）——**任何 PAT 都不可能持有**。RFC-329 曾设计 `find_users` 工具，实测发现它必死：声明该权限则永不出现在 `tools/list`，声明空权限则每次调用恒 403，故从该 RFC 删除。**补工具解决不了，真因是权限目录**。修法方向：新增一个可授予 PAT 的窄点（只回公开字段，如 `users:lookup:minimal`）并进令牌矩阵——跨 RFC 的面（连带账号页矩阵 UI 与文档），需单独立 RFC。这两条路由今天在 `rfc329McpSurfaceLedger.ts` 里以 `system-point` 登记。
- ⏳ **memory candidate 的人审发布在 MCP 上做不了，且 promote 路由自身有 ACL TOCTOU**（RFC-329 设计门 P1-6 / P1-7，2026-08-26）：`POST /api/memories/:id/promote` 本身 PAT 可达（`memory:update` 是矩阵点），但**发现 candidate 需要 `resource-acl:bypass`**——`routes/memories.ts:139-143` 对非 bypass 者过滤掉全部 `status='candidate'` 行，而该点同样是系统域点（`permission.ts:846`）。只补 promote 工具会造出「看不到、却能对已知 id 下决定」的形状，与「可发现、可读、可处置」矛盾，故 RFC-329 未做。修法方向同上一条：一个可授予 PAT 的 `memory:candidate:decide` 类窄点 + 统一 list/get/promote 的判据。**附带一条既有缺陷**：promote 路由先在事务外 `loadManagedMemory`（`routes/memories.ts:55-71,313-332`），最终事务只重查 memory 状态、**不重查当前 scope owner/grant**（`services/memory.ts:330-350`）——撤权或 owner 转移可以夹在检查与提交之间。它今天就存在于网页端路径、不因 MCP 而恶化，但补 MCP 面前应一并处理。
- ✅ **MCP 缺 review 逐文档操作与 clarify 子集/延后**（原 RFC-247 实现门 P1/P2）——**两半都已收口**：review 那半由 RFC-326 补齐（7 个具名工具 + `submit_review` 的 `comments[]`/`selections[]`）；clarify 那半由 **RFC-329 PR-B** 补齐——`answer_clarify` 现在收 `defer` / `questionIds` / `resubmitQuestionIds`，逐题封存→待指派→批量下发整条控制通道在 MCP 上可达（另加看板六工具 + 节点开关两条 + 协作草稿）。**收口方式与原定方向不同且是有意的**：原文写「补法是各操作在 W4-A 的 operation catalog 里各拿一个 `McpBinding`」，实际两个 RFC 都走了具名工具 + 双向守卫、没等 W4-A（W4-A 排在 RFC-328 P0-D 之后，短期内 MCP 面不会变全）。原约束仍被遵守：**没有扩 `resource_read` / `resource_write` 的 `method` 枚举**，未往 generic invoker 方向加固；W4-A 落地时这批具名工具直接成为 `McpBinding` 的实现，RFC-329 全域守卫产出的「路由⟷工具」映射表即其输入清单。
...
## 归属人徽章在用户表加载完之前先把 ULID 显示出来（2026-09-04 记，RFC-099/231 ACL 展示面）

`e2e/rfc319-workflow-list-and-acl.spec.ts:578`（WF-03）在 `e2e-webkit-nightly` run `33818674946`
webkit shard 3/4 上首跑红、retry 绿。**不是用例竞态，是产品的加载态缺一档**：

```
- Expected  "rfc319w-u-wf03-1"        ← 归属人用户名
+ Received  "01M1MVFVMNFT9K119TVB33CQ5A"  ← 原始 user id
```

`components/ResourceBadges.tsx:25` 写的是 `owner?.displayName ?? props.ownerUserId`，这一个
`??` 同时承担了两件**语义不同**的事：①「这个用户确实查不到」（该回落到 id，合理）；
②「用户表还没加载完」（不该回落——此刻把 ULID 直接摆在用户眼前）。列表数据与 owners 查询各自
返回，于是加载窗口里卡片上会闪一下 ULID，然后变成用户名。

**影响面不止工作流列表**：`ResourceBadges` 是六类资源列表共用的公共组件（见
`routes/workflows.tsx:163` 等调用点），所以这是一处共享展示契约的问题，不是单个页面的。

**修法**（需先定方向，故未在本轮动手）：让 `useResourceList` 把 owners 查询的 loading 暴露出来，
`ResourceBadges` 在 loading 期间不渲染归属人（或给骨架），只有在**确实查不到**时才回落到 id。
纯改用例（poll 等它稳定）会把这个用户可见的瞬时错误盖掉，不采纳。

## webkit 引擎自身崩溃导致的 nightly flake（2026-09-04 记，e2e-webkit-nightly）

同一轮 run `33818674946` 里 `e2e/rfc319-users-and-account.spec.ts:1064`（IAM-44）首跑红、retry 绿，
错误是 `page.goto: WebKit encountered an internal error` —— **浏览器引擎级崩溃，不是产品缺陷，
也无法靠改用例稳定**（Playwright 的 retry 已经是现有兜底）。记在这里是为了让下一个人看到这条
...
## ✅ 已收（2026-09-17）O(k²) 守卫用**墙钟毫秒**当判据，在共享 runner 上会假红（2026-09-06 实撞）

`rfc349-target-coverage-linear-grouping.test.ts` 断言「1 万个分片的分组 < 150ms」，用来防
`finalizeSchema` 的分组回到 O(k²)。2026-09-06 在 `0e2343757` 的 macOS 分片 1/4 上量到 **182ms** 而红；
同一份代码本机连跑五次全绿，线性实现本身只要 **0.6ms**（用例注释自己写着：二次方版本 300ms、
线性版本 0.6ms，150ms 的预算里绝大部分是同一用例中 `finalizeSchema` 的其余部分）。也就是说这条红
量到的是**机器负载**，不是算法复杂度——同一次 CI 里 e2e 的 Windows 腿在刷
`event loop stalled gapMs=19534`。

**为什么值得改**：墙钟阈值把「复杂度」和「这台机器此刻有多忙」混成一个数，注定间歇假红；而假红
会训练所有人「重跑一下就好」，真的 O(k²) 回潮反而被淹掉。**更稳的形状是比 scaling 而不是比绝对值**：
同一形状跑 k 与 2k，线性实现耗时比应当接近 2（给足容差，例如 < 3），二次方会接近 4。这样判据只依赖
同一台机器上的**相对**耗时，负载被两次测量共同吸收。

**判据**：本条的签名是 `分组回到了 O(k²)：1 万个分片花了 <1xx>ms`，且同用例本机单跑亚毫秒级。

**已收（2026-09-17）**：本条在 `b4b1b4cfc` 的 macOS 分片 1/6 上**原样复发**，量到的还是 **182ms**——同一个数字出现两次，说明它不是随机噪声而是「一次同等堆规模的 full GC」这种可重复的停顿形态，再靠调大预算只会把偶发变成更慢的偶发。按本条给的正解改掉了判据：同一形状测 k 与 8k，断言**耗时倍率** < 21（线性约 8、二次方约 64 的几何中点），两个规模交替测、各取 5 轮最小值，另加一道离线性两个数量级的失控闸只为给出可读报错。变异实证：分组改回 `[...current, receipt]` 后，闸在第一轮 4888ms 咬住；抬高闸让倍率判据单独作用，量到 52.3 倍（同机线性侧 7.3 倍）。通用教训已落 `docs/dev-gotchas.md` §测试 / CI。
...
## `e2e-webkit-nightly` 已连红 8 天以上（2026-09-13 发现，**非本轮改动引入**）

**证据**：workflow `280919052` 最近 8 次 run 全红，最早一次是 `eb64bfe8`（2026-09-06），
一路到 `7c3781dc`（2026-09-12）、`b7a7fff9`（2026-09-13）——**全部早于或无关于 RFC-359 W5 这批
测试迁移**。失败集中在 `Playwright webkit (shard 2/4)` 与 `(shard 3/4)`，两类：

1. `e2e/rfc310-zero-config-onboarding.spec.ts:393` —
   `dragged Pipeline gates handle never reached exact slot zero`。看形状是 WebKit 上的拖拽
   交互问题（重试也没过）。
2. `e2e/rfc319-agent-delete-and-refs.spec.ts:828`（`@nightly`）—
   `body.details?.visible?.map(v => v.name)` 期望等于 `[workflow.name]`，实际不符；
   用例自带的失败说明是「拒绝信息不说是哪个工作流拦住的 ⇒ 用户知道『有引用』却不知道去哪里改，
   这道闸就成了死胡同」。**这是一条功能判据**——删除被引用的代理时，409 要点名是哪个工作流拦住的。
   122ms 就失败，说明挂在 API 响应体形状上，不是 UI 超时。

**为什么一直没人发现**：②带 `@nightly` 标签，**push 门禁不跑**；而整个 nightly 已经长期是红的，
于是一条真的功能回归混在里面没人看得见——「长期红的流水线 = 没有流水线」。
...
### ✅ ① 已收口（RFC-359 AC-1 第 9 刀，2026-09-18）

`retry` 的两份实现合一之后，这条分叉**从根上消失了**：留下的那份（`retryNodeProjection`）
把复活整段交给注入的 `resumeTaskAs`，两个引擎因此都走 resume 的准入，对同一个请求给同一个答案。

**统一到哪一侧**：410 那一侧。一个既没有工作树、也没有墓碑、也没有待跑 `__repo_prep__` 行的任务，
重试出来的那一步没有任何地方可跑；resume 早就对同一个任务这么答了，retry 没有理由更宽。
（上面提到的**错误归因**问题——空路径可能是「从未建出」而不是「被 GC 回收」——是
`worktreeResumePreflight.ts` 自己的文案问题，与本条分叉无关，仍然留着，见该文件头注。）

那 4 条判据（flips-to-pending / 保留 clarifyIteration / cascade 下游继承 / 历史行重试）
已**去掉单引擎早返回、改在两个引擎上各跑一遍**。它们的本意是铸行的形状，与工作树无关，
所以夹具改成给任务一棵**真的**工作树（顺带把编出来的 `preSnapshot: 'snap-abcdef'` 换成夹具仓
真实的 HEAD——那条 RFC-098 WP-9 的基线存在性门此前因为空工作树从没被这几条走到过）。

### ⏳ ② 仍未定（`POST /api/tasks` 的 201 vs 400）

第 ② 条判据仍按 `scope.harness.capabilities.provider !== 'sqlite'` 登记为单引擎，用例注释里写明了
两侧各自的返回与原因，**不掩盖**。要收口得先定语义（决定 G7 的「先接受、后台失败」形态是否在
PG 上也要成立），属于产品行为决策，不夹在测试迁移里做。
...
**对手方已查明：回合引擎的并发是它自己。** 不必再找外部写者——

· `workgroupTurnsDriver.ts` 同时驱动多个回合：`Promise.allSettled(inflight.values())`
  （`:2657` / `:2661` / `:2668`）与 `Promise.race(inflight.values())`（`:2776`）。
· 每个回合各自是一笔 SERIALIZABLE 事务（`runResourceCatalogTransaction` → `serializable`）。
· 它们读写的是**同一个 task** 的 `node_runs` / `tasks` / 工作组行——天然互为读写依赖。
· 失败那条用例把这件事**推到最大**：它脚本化地让成员连失败 `DEFAULT_PROTOCOL_RETRY_BUDGET + 1`
  次（`rfc359-w4-d19c-adapters.test.ts:387`），**每一次重试都要新铸一个 `node_run`**——
  正是日志里失败的那条 insert。于是这条用例是全文件铸 run 最密集的一条，也就最先输。

**这就是典型的「一个好一个不好」**：同一套并发设计在 SQLite 上没事（没有 SSI，写被单写者
租约串起来），在 PostgreSQL 上会被 SSI 判成依赖环并抛 40001。

**处置要的是一次设计裁决，不是一行改动**（呈用户）：

1. **让同一 task 的回合不并发**——`inflight` 已经按 assignment / member 去重，但同 task 的
   不同回合仍会重叠。串起来最彻底，代价是吞吐与回合交错行为可能变化（**用户可见**）。
2. **缩小 SERIALIZABLE 的覆盖面**——只有真正需要可串行化的那一小段用 `serializable`，
   其余用更弱的隔离。代价是要逐段论证不变量，工作量大但不改用户可见行为。
