...
- ⏳ **RFC-254（承上，剩余待查，别 skipIf 遮掉）：完整 `buildVerifiedOpencodeBusinessPlan` 在真 win32 是否还有 bug#3+？** 已证的是两个**子函数**在真 win32 OK；完整 plan build 还有许多步未在真 win32 端到端跑过。**卡点**：`rfc224-verified-plan` 的 `activateVerifiedMac/Linux` 强制 POSIX 平台 + seatbelt/bwrap containment admission；Windows v1 **无 containment provider**（RFC-254 D1，Job Object/AppContainer 属未来），故真 win32 plan build 需一个 **win32/无-containment admission**（mode `off`/`warn→none`）——这本身是 RFC-254 核心问题（「verified 执行在无 provider 的 Windows 上能否成立」，跨 T32+）。**下一步**：构造一个 win32 无-containment admission 的完整 plan-build probe（或最小集成测试）端到端跑真 win32，若现新失败即 bug#3+；**在此之前不要 skipIf `rfc224-verified-plan`**——它强制 darwin 故真 win32 必红，仓促 skipIf 会遮掉未发现的 bug#3+（killGroup 纪律：本 P0 正是逐条深挖才挖出，两次奏效）。
