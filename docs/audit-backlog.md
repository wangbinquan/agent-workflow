...
- **claude 业务节点世代差**（不封印、`bypassPermissions`、full env 继承）由 `design/RFC-242-claude-runtime-security-parity/` 承载，T1-T3 已上库（受控节点已封印 + 工具门 + controlled env；未声明权限的存量节点按用户决策保持全权 + 告警）。核心矛盾：`agent.permission` 是 opencode 词汇的 verbatim 透传（`shared/schemas/agent.ts:196`），claude 侧无等价词汇 → 已由 `claudeCode/permissionMap.ts` 冻结映射表回答。剩余 T6（ratchet 扩展）。
