...
- ⏳ **RFC-254（疑似真生产缺陷 P0，须 fresh-context 端到端核实后修）：`services/runtime/netlessProjection.ts:resolveNetlessGitCommonDirs` 在 Windows 上拒 git 的正斜杠 `--git-common-dir` 输出 ⇒ 疑似 verified 业务 plan 在 Windows 构建失败（Code→Audit→Fix 主线）**。链路：`verifiedPlan.ts:518` **无条件**调 `resolveGitCommonDirs`（注释「a verified OpenCode business plan always runs in a real repository」）→ `resolveNetlessGitCommonDirs` 跑 `git rev-parse --path-format=absolute --git-common-dir`。**真机 probe 实测**：git-for-Windows 返回 `C:/repo/.git`（正斜杠，即使 `--path-format=absolute`），而 `isLexicallyCanonical`（`resolve(p)===p`，line ~195）+ 其后 `realpath(p)!==p`（line ~207）都按 host 反斜杠判 ⇒ 判非 canonical ⇒ `execution-identity-source-changed`/`store-unsafe` ⇒ **plan 构建失败**。**候选修法**（probe 验 `canonical-after-normalize=true`）：win32 上把 git 输出 `replace(/\//g,'\\')` 归一到 host 分隔符（POSIX 不变）。**为何未提交（本 session 已 revert 候选修法）**：①**测试无法端到端验证**——`rfc224-verified-plan` 等**强制 `process.platform='darwin'`**（确定性 POSIX 模拟；真机 debug 实证 `plat=darwin`），故 win32 代码路径**无任何测试覆盖**；真机上强制-darwin + 真 Windows git（返回 `C:/...`）⇒ 测试以 fixture 不一致失败，与本 bug 表象混在一起；②敏感 RFC-224 verified 核心，CLAUDE.md 要求动前重跑 qualification；③候选修法用 `process.platform` 而测试强制平台，二者模型需先厘清（生产是否该用注入平台）。**为何验收漏掉**：VM 的 `C:\aw` 是**非 git overlay**，acceptance 从未跑过「带真 git repo 的 verified 业务 plan 构建」。**fresh-context 须**：(a) 先确证生产 win32（**非**强制-darwin）下确实 source-changed——写一个不强制平台的 win32 集成测试，或真机跑一次带真 git repo 的真 business 任务；(b) 确认后落 normalize 修法 + 重跑 rfc224/227 qualification；(c) 顺带处理「平台-强制测试在真 Windows 上做 POSIX 模拟」的 cluster-wide 模式（skipIf(win32) 或改注入平台 + mock git）。**这是本 session 挖出的最重要线索——可能整条 verified 业务执行在 Windows 上是坏的**（killGroup 级或更大），但须 fresh-context 端到端确证，不可仓促改 verified 核心。
