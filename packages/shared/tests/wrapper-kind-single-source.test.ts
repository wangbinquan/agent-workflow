...
describe('WRAPPER_NODE_KINDS 单源（flag-audit W0）', () => {
  ...
  test('重灾区文件不得再手抄三连并集（防 re-fork 源码锁）', () => {
    ...
    const files = [
      ...
      'packages/backend/src/modules/task-execution/engine/wrapper/loopStrategy.ts',
      'packages/backend/src/modules/task-execution/engine/wrapper/gitStrategy.ts',
      'packages/backend/src/modules/task-execution/engine/wrapper/fanoutStrategy.ts',
