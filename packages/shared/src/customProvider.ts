 ...
 * Hence this explicit snapshot, generated from the models.dev catalog OpenCode
 * embeds at build time (`opencode/packages/opencode/script/build.ts` bakes in
 * `OPENCODE_MODELS_DEV`), plus the canary probe the backend runs for ids that
 * are new to a config.
 *
 * A first cut listed only the ids the credential-env table names — 32 of the
 * catalog's 170. That left plausible private-relay names (`deepseek`,
 * `moonshotai`, `siliconflow`, `ollama-cloud`, `lmstudio`, …) accepted at save
 * time and then failing every run with an unactionable
 * `provider-untrusted`, because the catalog's models had been merged in behind
 * the operator's back. Refresh this list when OpenCode ships a newer catalog;
 * the canary probe is what covers the window in between.
 */
