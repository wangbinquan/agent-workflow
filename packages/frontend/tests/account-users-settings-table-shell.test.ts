...
describe('account and admin responsive shells', () => {
  ...
  test('/account is route-backed and keeps identity unlink removed', () => {
    ...
    const codePush = read('components/account/AccountCodePushCredentialsPanel.tsx')
    const gitIdentity = read('components/account/AccountGitIdentityCard.tsx')
    ...
    expect(overview).not.toContain('AccountGitIdentityCard')
    expect(codePush).toContain('<AccountGitIdentityCard me={me} />')
    expect(codePush).toContain('className="form-grid account-code-push-form"')
    expect(gitIdentity).toContain('<SettingsCard')
    expect(gitIdentity).toContain('className="form-grid account-code-push-form"')
    expect(styles).toMatch(/\.account-code-push-form\s*\{\s*width: 100%;\s*\}/)
