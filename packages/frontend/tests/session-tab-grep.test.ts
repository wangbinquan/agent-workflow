...
const SESSION_CONVERSATION_PANEL = readFileSync(
...
describe('RFC-027 source-code wiring', () => {
  ...
  test('SessionTab uses the shared SessionConversationPanel backed by ConversationFlow', () => {
    expect(SESSION_TAB).toContain('SessionConversationPanel')
    expect(SESSION_CONVERSATION_PANEL).toContain('ConversationFlow')
