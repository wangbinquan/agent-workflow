...
describe('RFC-310 Digital Employee OS information architecture', () => {
  ...
  test('tool configuration is anchored to a selected work item on the fixed graph', () => {
    ...
    expect(typePage.match(/<EmployeeCapabilityPanorama/g)).toHaveLength(3)
    expect(typePage).toContain('cardIdPrefix="job-duty"')
    expect(typePage).toContain('cardIdPrefix="employee-duty"')
    expect(typePage).toContain('onSelectAdapterSlot={setToolboxAdapterTarget}')
    ...
    expect(typePage).toContain("const business = props.item?.nodeKind === 'business-tool'")
    expect(typePage).toContain('enabled: business && contractKey !== null')
    expect(typePage).toContain('{business ? (')
  ...
  test('ingress and human-review cards are generic read-only projections', () => {
    ...
    expect(runtime).toContain('runtimeIngress={{')
    expect(runtime).toContain('onSelectIngress={selectRuntimeInput}')
    expect(runtime).toContain('data-testid="employee-case-input-inspector"')
    expect(runtime).not.toContain("ingress.configurationSurface === 'task-creation'")
  ...
  test('work intake and runtime are first-class routes in the unified task surface', () => {
    ...
    expect(create).toContain('<TaskCreationAdvancedSettings')
    expect(create).not.toContain('autoCommitPush')
