...
describe('RFC-198 visual infrastructure source gates', () => {
  ...
  test('visual spec declares exactly 51 counted scenes', () => {
    const source = repoFile('e2e/visual-regression.spec.ts')
    expect(source).toContain('const EXPECTED_VISUAL_SCENE_COUNT = 51')
    expect(source).toContain('const HOMEPAGE_VISUAL_TIME = new Date(2026, 6, 23, 14, 0, 0)')
    expect(source.match(/^\s{2}test\(/gm)).toHaveLength(51)
    ...
    // One declaration plus nine locator screenshot callsites.
    expect(source.match(/COMPONENT_SNAPSHOT_OPTS/g)).toHaveLength(10)
  ...
  test('Linux instructions match the locked Playwright and CI Noble environment', () => {
    ...
    expect(readme).toContain('66 pixel baselines')
