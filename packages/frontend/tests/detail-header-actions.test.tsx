...
//      its own form-actions__error span (multi-channel pages depend on it).
...
describe('DetailHeaderActions', () => {
  test('renders title children + actions cluster in the header, errors AFTER it', () => {
    ...
    expect(header!.querySelector('.form-actions')).toBeNull()
    const errorRow = document.querySelector('.form-actions')
    expect(errorRow).not.toBeNull()
    expect(errorRow!.previousElementSibling).toBe(header)
    expect(errorRow!.textContent).toContain('boom happened')
  })

  test('errors: nullish channels filtered, each present channel gets its own span', () => {
    wrap(
      <DetailHeaderActions
        {...BASE}
        title="t"
  ...
  test('no present errors → no form-actions row at all', () => {
    wrap(
      <DetailHeaderActions
        {...BASE}
        title="t"
