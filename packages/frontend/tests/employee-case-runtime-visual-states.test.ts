...
describe('digital employee task runtime visualization', () => {
  ...
  test('the page renders the active capability panorama and every round on a chronological timeline', () => {
    ...
    expect(detail).toContain('adapterSlotState={runtimeAdapterSlotState}')
    expect(detail).toContain('data.capabilityActivation.exactAdapterBindings.find(')
    expect(detail).toContain('Task-frozen revision:')
