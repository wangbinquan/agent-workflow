...
export const enUS: Resources = {
  ...
  settings: {
    webhookEndpoints: {
      ...
      noPublicBaseUrl:
        'publicBaseUrl is not configured — set it to get the full URL (path: {{path}})',
      ...
      protocolHint: 'Which clone URL to use when the event repo is not imported yet',
  ...
  webhookTriggers: {
    ...
    subtitle:
      'Route code-host events by rule: repo scope × event type × branch × command → launch a workflow / agent / workgroup.',
    new: 'New trigger',
    empty: 'No triggers yet.',
    emptyDescription:
      'Set up an ingress endpoint under Settings → Webhook Endpoints first, then bind rules and a target here.',
    ...
    fields: {
      ...
      endpointImmutable: 'The endpoint cannot change after creation (recreate the trigger instead)',
      scope: 'Repo scope',
      events: 'Event types',
      eventsHint:
        'Pipeline events are NOT filtered by the ignore list (fix-until-green depends on it)',
      branchFilter: 'Branch filter (glob)',
      branchFilterHint:
        'MR-type events match the target branch, others the event branch; empty = no filter',
      commandPrefix: 'Comment command prefix',
      commandPrefixHint: 'MR comment events only, e.g. /fix',
      ignoreUsernames: 'Ignored usernames',
      ignoreUsernamesHint:
        'Push / MR / comment events by these GitLab users never fire (add your bot account); pipeline events are unaffected',
      launchKind: 'Target kind',
      kindImmutable: 'The target kind cannot change after creation',
      target: 'Target',
      targetPlaceholder: 'Select a target',
      inputMappings: 'Input mappings',
      inputMappingsHint:
        'Map event fields onto workflow inputs; git-kind inputs always take the event branch',
      noInputs: 'This workflow declares no inputs.',
      eventBranch: 'Branch from event',
      templatePlaceholder:
        'Template vars (mr_iid, branch, … wrapped in double braces) are supported',
      unmappable: 'This input kind cannot be event-mapped (save will reject it)',
      ...
      maxFiresHint:
        'The circuit opens after this many consecutive fires on one MR/branch (manual reset or a human-authored event restores it)',
      autoRegister: 'Auto-register repos',
      autoRegisterLabel: 'Clone by URL when the event repo is not imported yet',
    },
  ...
  webhookDeliveries: {
    ...
    subtitle:
      "Endpoint-level audit: one row per HTTP delivery; per-trigger outcomes live in each trigger's fires.",
    filterAria: 'Filter by status',
    empty: 'No deliveries yet.',
    emptyDescription: 'Once the GitLab webhook is configured, events will appear here.',
    filterAll: 'All',
    replay: 'Replay',
    replayBadge: 'Replay',
    rejectedNotReplayable:
      'Signature-rejected deliveries cannot be replayed (fix the secret first)',
