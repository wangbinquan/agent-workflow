...
function ScheduledDetailPage() {
  ...
  const runNowEligibility = scheduleRunNowEligibility(s)
  ...
  return (
    <ScheduledRunNowAction
      ...
      variant="primary"
      testid="scheduled-run-now"
      errorTestid="scheduled-run-now-error"
      onSuccess={(taskId) => {
        invalidate()
        void navigate({ to: '/tasks/$id', params: { id: taskId } })
      }}
    >
      {({ action: runNowAction, feedback: runNowFeedback }) => (
        <div className="page" data-testid="scheduled-detail">
          ...
          <section className="page__section">
            <dl className="detail-grid">
              ...
              <dd>
                {s.enabled && s.nextRunAt != null ? new Date(s.nextRunAt).toLocaleString() : '—'}
              </dd>
