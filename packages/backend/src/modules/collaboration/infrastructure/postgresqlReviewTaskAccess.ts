// RFC-349 — PostgreSQL review/task relationship projection.
...
async function hasActingMembership(
  db: PostgresqlDatabaseClient,
  taskId: string,
  userId: string,
): Promise<boolean> {
...
export function createPostgresqlReviewTaskAccessPort(
  db: PostgresqlDatabaseClient,
  ...
  return {
    ...
    async resolveRelationship(actor, taskId, taskOwnerUserId) {
      const taskVisible =
        ...
        (await hasMembership(db, taskId, actor.user.id))
      const actingMember = await hasActingMembership(db, taskId, actor.user.id)
      return {
        taskVisible,
