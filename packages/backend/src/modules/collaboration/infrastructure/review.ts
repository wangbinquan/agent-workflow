...
async function dispatchReviewNodeUnlocked(args: DispatchReviewArgs): Promise<DispatchReviewResult> {
  ...
  if (
    ...
    try {
      await finalizeCommittedHumanGate(collaboration, {
...
async function loadPriorRound(
  ...
  for (const r of byIndex.values()) {
    let body = ''
    try {
      body = await readCommittedReviewArtifactBody(collaboration, r.bodyPath)
    } catch {
...
export async function countPendingReviews(
  ...
  if (actor !== undefined && !actor.permissions.has('tasks:read:all')) {
    ...
    conditions.push(
      or(
        // 可见性判据取自共享持久化词汇 `db/taskVisibility`——与 `taskAuthorization.visibleIds`
        // 同一份代码，这里只是把它下推进本条语句（AC-11：省掉「先捞列表再问一次」的那次往返）。
...
export async function getReviewDetail(
  ...
  let body: string
  try {
    body = await readDocVersionBody(db, appHome, dv)
  } catch {
    body = ''
  }
...
export async function getDocVersionDetail(
  ...
  const body = await readDocVersionBody(db, appHome, dv)
  const comments = await commentsForDocVersion(db, dv)
...
async function buildRoundMember(
  ...
  let mbody = ''
  try {
    mbody = await readDocVersionBody(db, appHome, mdv)
  } catch {
    mbody = ''
  }
...
async function addReviewCommentUnlocked(
  ...
  const body = await readDocVersionBody(args.db, args.appHome, dv)
...
async function submitReviewDecisionUnlocked(
  ...
  const bodyCache = new Map<string, string>(
    await Promise.all(
      effectiveDvs.map(
        async (document) =>
          [document.id, await readDocVersionBody(args.db, args.appHome, document)] as const,
      ),
    ),
  )
  const readBody = (d: DocVersion): string => {
    const body = bodyCache.get(d.id)
    if (body === undefined) throw new Error(`review body was not prefetched: ${d.id}`)
    return body
  }
