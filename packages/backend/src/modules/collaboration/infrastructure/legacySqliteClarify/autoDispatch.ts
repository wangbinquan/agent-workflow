// RFC-128 / RFC-349 — SQLite quick-channel seal + AUTODISPATCH.
//
...
async function sealRoundAsWholeFinalize(
  ...
  if (round.kind === 'self') {
    await args.memoryDistillEnqueuer
      .enqueue({
        sourceKind: 'clarify',
...
export async function autoDispatchClarifyRoundWithDecision(
  ...
  if (replay !== null) {
    const converged = await finishCommittedClarifyAutoDispatch({
      db: args.db,
      memoryDistillEnqueuer: args.memoryDistillEnqueuer,
  ...
  try {
    ...
    if (prepared.capture.eventRefs !== undefined) {
      await publishCommittedEventsAfterCommit(prepared.capture.eventRefs)
    }
  }
