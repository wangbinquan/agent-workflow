...
import { canViewResourceInTx, requireResourceGovern } from './resourceAuthorization'
...
export async function getResourceAcl(
  ...
  const [aclRevision, grantRows] = await Promise.all([
    getSqliteResourceAclRevision(db, type, row.id),
    listResourceGrants(db, type, row.id),
  ])
  const wantedIds = [
    ...new Set([...(row.ownerUserId ? [row.ownerUserId] : []), ...grantRows.map((g) => g.userId)]),
  ]
  const byId = await loadAclUsers(db, wantedIds)
  const owner =
...
export async function updateResourceAcl(
  ...
  const updatedRow = withSqliteResourceAclMutation(db, type, row.id, (context) => {
    const current = context.current
    if (!canViewResourceInTx(context.tx, actor, type, current)) {
    ...
    if (body.grants !== undefined) {
      ...
      nextGrants = listResourceGrantsInTx(context.tx, type, row.id)
    }
    if (
      nextOwner !== previousOwner &&
      previousOwner !== null &&
    ...
    try {
      ...
      if (
        ...
        isSqliteOwnerNameConstraintError(error)
      ) {
        throw new ConflictError(
          'resource-name-conflict',
          `${type} '${current.name}' already exists for the target owner`,
