// RFC-223 (PR-1) — Resource Catalog application owner for create/import-time
// cross-resource references resolved to
...
import type { AclResourceType, AgentSkillRef } from '@agent-workflow/shared'
import { decodeAgentSkillRef, resourceRefKey } from '@agent-workflow/shared'
import type { Actor } from '@/auth/actor'
import type { RefCheckGroup, ResolvedRefsById } from './referenceTypes'
...
export interface AgentReferenceResolver {
  ...
  assertNoMissing(
    missing: readonly { readonly type: AclResourceType; readonly name: string }[],
  ): void
}
...
export async function resolveAgentRefsUsable(
  ...
  const mcp = await resolver.resolveUsableById(actor, 'mcp', input.mcp, {
    grandfatheredIds: existing ? new Set(existing.mcp) : undefined,
  })
  const plugins = await resolver.resolveUsableById(actor, 'plugin', input.plugins, {
    grandfatheredIds: existing ? new Set(existing.plugins) : undefined,
  })
  const dependsOn = await resolver.resolveUsableById(actor, 'agent', input.dependsOn, {
  ...
  const skillRes = await resolver.resolveUsableById(actor, 'skill', managedTokens, {
