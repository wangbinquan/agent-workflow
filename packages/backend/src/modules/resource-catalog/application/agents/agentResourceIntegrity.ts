...
import type {
  ...
  AgentResourceInventoryMcp as InventoryMcp,
  AgentResourceInventoryPlugin as InventoryPlugin,
  AgentResourceInventorySkill as InventorySkill,
...
import type {
  AgentLaunchResourceIntegrityParticipant,
  AgentOperationContext,
} from '../../public/participants'
import { agentLaunchResourceIntegrityParticipantBrand } from '../../domain/participantBrands'
import { ValidationError } from '@/util/errors'

export type { AgentResourceStatus } from '../../public/types'

type AgentResourceDisplayRef = AgentResourceStatus['references'][number]
type AgentResourceDisplayIssue = AgentResourceStatus['issues'][number]
type AgentResourceDisplayState = AgentResourceDisplayRef['state']
type AgentResourceIssueCode = AgentResourceDisplayIssue['code']
type AgentResourceRefKind = AgentResourceDisplayRef['kind']
...
export type {
  ...
  AgentResourceInventorySkill as InventorySkill,
  AgentResourceInventorySource,
} from '@/modules/resource-catalog/application/agents/ports'
 ...
 */
export async function loadAgentResourceInventory(
...
export async function assertAgentResourceIntegrity(
  ...
  let inventory = opts.inventory ?? (await loadAgentResourceInventory(source))
...
export async function getAgentResourceStatus(
  source: AgentResourceInventorySource,
  actor: AgentOperationContext,
  root: Agent,
): Promise<AgentResourceStatus> {
  const inventory = await loadAgentResourceInventory(source)
  ...
  const visibleAgents = await source.filterVisible(actor, 'agent', closureAgents)
  ...
  const [visibleSkills, visibleMcps, visiblePlugins] = await Promise.all([
    source.filterVisible(actor, 'skill', closureSkills),
    source.filterVisible(actor, 'mcp', closureMcps),
    source.filterVisible(actor, 'plugin', closurePlugins),
  ])
...
/**
 * Compatibility-free application handle consumed by the Agent transport.
 * Bootstrap selects the provider-owned inventory source; neither the public
 * query nor the route receives a database client or persistence row.
 */
export function createAgentResourceIntegrityQueries(
  source: AgentResourceInventorySource,
): AgentResourceIntegrityQueries {
  return Object.freeze({
    status: (authority: AgentOperationContext, input: GetAgentResourceStatusInput) =>
      getAgentResourceStatus(source, authority, input.root),
    async closureStatus(
      ...
      const result = evaluateAgentResourceIntegrity(
        await loadAgentResourceInventory(source),
        input.rootAgentIds,
      )
      return {
        ok: result.ok,
...
/** Exact TaskExecution-facing capability; inventories and providers remain internal. */
export function createAgentLaunchResourceIntegrityParticipant(
  source: AgentResourceInventorySource,
): AgentLaunchResourceIntegrityParticipant {
  return Object.freeze({
    [agentLaunchResourceIntegrityParticipantBrand]:
      'agent-launch-resource-integrity-participant' as const,
    assertUsable: (input: GetAgentResourceClosureStatusInput) =>
      assertAgentResourceIntegrity(source, input.rootAgentIds),
  })
}
