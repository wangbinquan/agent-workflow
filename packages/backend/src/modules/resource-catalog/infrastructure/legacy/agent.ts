// Resource Catalog SQLite compatibility implementation — CRUD on the agents table.
...
} from '../../application/agents/agentReferences'
import { filterVisibleRows, resolveResourceAccessForInTx } from '../../composition/resourceAcl'
import {
...
import { listGrantedResourceIds } from '../sqliteResourceGrantRepository'
...
import { isSkillAvailableThisBoot } from './skillBootVerify'
...
const agentReferenceResolver = (db: DbClient) => ({
  ...
  ) => resolveRefsUsableById(db, actor, type, tokens, options),
  assertNoMissing: (missing: Parameters<typeof assertNoMissingRefs>[0]) =>
...
const agentResourceInventorySource = (db: DbClient): AgentResourceInventorySource => ({
  async load() {
    ...
    const availableSkills = skillRows.map((row) => ({
      ...row,
      available:
...
export async function prepareAgentCreate(
  ...
  const resolved = await resolveAgentRefsUsable(agentReferenceResolver(db), opts?.actor ?? null, {
    mcp: input.mcp,
  ...
  await validateAgentDependencies(
    { get: (agentId) => getAgentById(db, agentId) },
    id,
    persistedOnly(dependsOnIds),
  )
  ...
  if (pending.size === 0) {
    await assertAgentResourceIntegrity(agentResourceInventorySource(db), [id], {
      overrides: [candidate],
    })
  }
...
export async function prepareAgentUpdate(
  ...
  if (pending.size === 0) {
    await assertAgentResourceIntegrity(agentResourceInventorySource(db), [candidate.id], {
      overrides: [candidate],
    })
  }
