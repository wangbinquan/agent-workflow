...
import type {
  ...
  McpRuntimeTestCancelPersistenceResult,
  McpRuntimeTestCleanupCandidate,
  McpRuntimeTestCreatePersistenceInput,
  McpRuntimeTestCreatePersistenceResult,
  McpRuntimeTestEndPersistenceResult,
...
export function createSqliteMcpRuntimeTestPersistence(db: DbClient): McpRuntimeTestPersistence {
  const persistence: McpRuntimeTestPersistence = {
    ...
    async create(input: McpRuntimeTestCreatePersistenceInput) {
      return dbTxSync(db, (tx): McpRuntimeTestCreatePersistenceResult => {
        const replay = tx
          .select()
          .from(mcpRuntimeTestCreateReceipts)
          .where(
    ...
    async cancel(input) {
      return dbTxSync(db, (tx): McpRuntimeTestCancelPersistenceResult => {
        const session = tx
          .select()
          .from(mcpRuntimeTestSessions)
          .where(eq(mcpRuntimeTestSessions.id, input.sessionId))
    ...
    async end(input) {
      return dbTxSync(db, (tx): McpRuntimeTestEndPersistenceResult => {
        const session = tx
          .select()
          .from(mcpRuntimeTestSessions)
          .where(eq(mcpRuntimeTestSessions.id, input.sessionId))
    ...
    async loadTurn(turnId) {
      return (
        db.select().from(mcpRuntimeTestTurns).where(eq(mcpRuntimeTestTurns.id, turnId)).get() ??
        null
      )
    },
    ...
    async invalidateMcp(input) {
      return dbTxSync(db, (tx): readonly McpRuntimeTestRunningRef[] => {
        const rows = tx
          .select()
          .from(mcpRuntimeTestSessions)
          .where(
    ...
    async markMcpConfigChanged(input) {
      return dbTxSync(db, (tx) => {
        ...
        const changedSessionIds = tx
          ...
          .all()
          .map((row) => row.id)
        return { idleSessionIds, changedSessionIds }
      })
    },
    ...
    async assertMcpDeleteReady(mcpId) {
      const unsafe = db
        ...
        .all()
        .find((row) => row.status !== 'ended' || row.cleanupState !== 'complete')
      if (unsafe !== undefined) {
        throw new ConflictError(
    ...
    async deleteExpiredReceipt(receipt, now) {
      db.delete(mcpRuntimeTestCreateReceipts)
        .where(
          and(
    ...
    async listQuarantinedCandidates() {
      const sessions = db
        ...
        .all()
      return sessions.map(
        (session): McpRuntimeTestQuarantinedCandidate => ({
          session,
          turn:
            db
              .select()
    ...
    async listDurableIntentCandidates() {
      return db
        ...
        .where(inArray(mcpRuntimeTestSessions.status, ['active', 'ending']))
        .all()
        .filter((row) => row.status === 'ending' || row.continuationBlockedReason !== null)
    },
    ...
    async prepareCleanup(sessionId, now) {
      return dbTxSync(db, (tx): McpRuntimeTestCleanupCandidate | null => {
        const session = tx
          .select()
          .from(mcpRuntimeTestSessions)
          .where(eq(mcpRuntimeTestSessions.id, sessionId))
