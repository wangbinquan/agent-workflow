...
import type {
  McpRuntimeProtocol,
  McpRuntimeTestLeaseInput as LeaseInput,
...
export function createSqliteMcpRuntimeTestLeaseOperations(
  db: DbClient,
): McpRuntimeTestLeaseOperations {
  return Object.freeze({
    claimNew: async (input: LeaseInput) => claimNewMcpRuntimeTestSessionLease(db, input),
    preclaim: async (input: LeaseInput) => preclaimMcpRuntimeTestSessionLease(db, input),
    rotate: async (token: McpRuntimeTestLeaseToken, nextRuntimeSessionId: string) =>
      rotateMcpRuntimeTestSessionLease(db, token, nextRuntimeSessionId),
    release: async (token: McpRuntimeTestLeaseToken) =>
      releaseMcpRuntimeTestSessionLease(db, token),
    repairAfterReap: async (testSessionId: string, turnId: string, childReaped: true) =>
      repairMcpRuntimeTestSessionLeaseAfterReap(db, testSessionId, turnId, childReaped),
  })
}
