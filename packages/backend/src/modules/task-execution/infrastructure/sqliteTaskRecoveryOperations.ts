...
async function hasUndispatchedDesignerQuestions(db: DbClient, taskId: string): Promise<boolean> {
  const projection = {
...
export type SqliteTaskRecoveryMutationOperations = Pick<
  TaskRecoveryOperations,
...
export function createSqliteTaskRecoveryOperations(
  db: DbClient,
  mutations: SqliteTaskRecoveryMutationOperations,
): TaskRecoveryOperations {
  const operations: TaskRecoveryOperations = {
    async recordEvent(input: RecordTaskRecoveryEventInput): Promise<void> {
      await db.insert(recoveryEvents).values(input)
    },
    ...
    async recordAutoRecoveryAttempt(input) {
      ...
      const reset = row.windowStart === null || input.now - row.windowStart >= input.config.windowMs
