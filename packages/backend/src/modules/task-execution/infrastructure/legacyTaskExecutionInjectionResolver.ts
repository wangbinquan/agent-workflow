// RFC-349 compatibility adapter for the retired SQLite injection resolver.
// Production task execution consumes TaskExecutionResourceBinding; this file
// remains only as the behavior oracle for the pre-cutover focused tests.
//
...
export interface ResolveInjectionOpts {
  ...
  readonly log: Logger
  /** Selected-provider Agent lookup. Dependency traversal must not infer a
   * SQLite client from the legacy resource hydration adapter. */
  readonly agentDependencies: AgentDependencyLookup
  /**
...
export async function resolveInjection(
  ...
  const closure = await resolveDependsClosure(opts.agentDependencies, agent, {
    call: DISPATCH_CALL_POLICY,
  }).catch((err: Error & { code?: string; details?: unknown }) => {
  ...
  const plugins = await loadPluginsByIds(
    {
      async loadByIds(ids) {
        if (ids.length === 0) return []
        return db
