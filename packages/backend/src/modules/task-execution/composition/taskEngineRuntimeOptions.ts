...
export interface RunTaskOptions {
  ...
  /** RFC-349 bootstrap-selected task-execution persistence bundle. */
  ...
  collaborationRuntime?: CollaborationRuntimeMechanics
  /** Provider-selected Resource Catalog turn ledger. */
  workgroupTurns?: WorkgroupTurnsOperations
  /** Provider-selected durable child creation used by call nodes. */
  childLaunch?: ChildExecutionLaunchOperations
  /** RFC-349 provider-selected dynamic-workflow state and Resource Catalog
   * validation projection. Required only when the durable task selects the
   * dynamic-workflow generation engine; that engine fails closed if absent. */
  dynamicWorkflow?: Readonly<{
    persistence: DynamicWorkflowPersistence
    validationContext: DynamicWorkflowValidationContextSource
  }>
  /** Provider runtime identity used only to share daemon-wide process pools. */
  processConcurrencyScope?: object
...
export type BoundRunTaskOptions = RunTaskOptions & {
  ...
  readonly dynamicWorkflow: NonNullable<RunTaskOptions['dynamicWorkflow']>
}
