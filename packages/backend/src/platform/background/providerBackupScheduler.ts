// RFC-213/RFC-349 — provider-aware scheduled backup + retention + safety net.
//
...
} from '@/platform/persistence/sqlite/systemBackupManifest'
...
export type BackupSchedulerOptions = BackupSchedulerCommonOptions &
  (
    | Readonly<{ db: SqliteMaintenanceDatabase; createScheduledBackup?: never }>
...
export const checkpointWal = checkpointSqliteWal
...
export const runWalCheckpointTick = runSqliteWalCheckpointTick

interface WalCheckpointCommonOptions {
  /**
...
export type WalCheckpointOptions = WalCheckpointCommonOptions &
  (
    | Readonly<{
        db: SqliteMaintenanceDatabase
        runStorageMaintenance?: never
      }>
    | Readonly<{
        db?: never
        /** Active-provider storage maintenance. PostgreSQL observes
         * autovacuum/catalog health and never receives a SQLite PRAGMA. */
        runStorageMaintenance: () => Promise<unknown>
      }>
  )
...
export function startWalCheckpointLoop(opts: WalCheckpointOptions): BackupSchedulerHandle {
  ...
  const handle = setInterval(() => {
    ...
    try {
      ...
      if (running) return
      if (opts.runStorageMaintenance === undefined) {
      ...
      void opts
        ...
        .catch((err: unknown) => {
          log.warn('storage maintenance failed', {
            error: err instanceof Error ? err.message : String(err),
          })
          lastCheckpointAt = now()
        })
