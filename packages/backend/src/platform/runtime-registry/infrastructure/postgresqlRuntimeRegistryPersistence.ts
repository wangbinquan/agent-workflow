...
import { agents, mcpRuntimeTestSessions, runtimes } from '@/db/schema'
...
async function transitionRuntimeTests(
  ...
  for (const session of sessions) {
    await transitionRuntimeTest(transaction, session, input.reason, input.now)
  }
}

async function transitionRuntimeTest(
  transaction: PostgresqlTransaction,
  session: RuntimeTestSession,
  reason: 'runtime-profile-changed' | 'runtime-disabled' | 'runtime-deleted',
  now: number,
): Promise<void> {
  if (reason === 'runtime-profile-changed') {
    await transaction
...
export class PostgresqlRuntimeRegistryPersistence implements RuntimeRegistryPersistence {
  ...
  async invalidateInheritedRuntimeProbeReceipts(
    ...
    return await this.db.transaction(async (transaction) => {
      ...
      const updated = await transaction
        ...
        .returning({ id: runtimes.id })
        .all()
      for (const runtime of inherited) {
        await transitionRuntimeTests(transaction, {
