...
// System Operations SQLite generation-safe restore adapter. Restore orchestration
// selects this mechanism only for a verified SQLite target.
...
/** The restore path owns the newly opened SQLite generation itself, so its
 * recovery audit lands through the same provider transaction mechanism before
 * that handle closes. Business services never receive this DbClient. */
async function recordSqliteRestoreRecoveryEvent(
  db: DbClient,
  input: {
  ...
  try {
    ...
    log.warn('restore recovery audit dropped', {
...
export interface RestoreOptions {
  ...
  /**
   * Bootstrap-composed cross-context recovery that must run against the
   * restored generation before its database handle closes. SQLite restore owns
   * the handle lifecycle, while Memory owns fusion provenance semantics.
   */
...
export interface SqlitePostRestoreRecovery {
  recover(input: { readonly db: DbClient; readonly appHome: string }): Promise<void>
}
    ...
    try {
      ...
      const migrated = willMigrate
      {
        ...
        if (hasFusionProvenanceSchema(db)) {
          await opts.postOpenRecovery.recover({ db, appHome })
        }
        ...
        await recordSqliteRestoreRecoveryEvent(db, {
