...
async function finalizeRound(
  ...
  // §11.2 T59/AC-34 — close the receipt, if a person opened one.
  //
  // AFTER the work-item transition, never before: this talks to the code host,
  // and the item's state is what a person and the `/code` page watch. A network
  // round-trip in front of it publishes "the round is over" while the item
  // still reads `running`, which is how this turned three CI shards red once.
  //
  // `createIfMissing: false` keeps the automatic paths silent without needing
  // to know which they were: only the ingress opens a receipt, and only for an
  // instruction somebody typed, so "is there a receipt for this round's
  // delivery" IS "was anybody waiting".
  //
