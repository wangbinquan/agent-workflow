...
//
// RFC-359 AC-6：`getTaskFileContent` 那一组跑双引擎（服务层收的是中立的
// `RepositoryWorkspaceStore`）。`openContainedFile` 那一组**一条数据库语句都没有**
// ——纯文件系统判据，双跑只是白花墙钟，留单份。
...
describeEachProvider('getTaskFileContent（双引擎）', (harness) => {
