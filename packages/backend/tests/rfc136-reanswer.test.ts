...
async function roundRow(db: ProviderNeutralDatabase, taskId: string) {
  const rows = await db.select().from(clarifyRounds).where(eq(clarifyRounds.taskId, taskId))
  return rows[0]
}

async function entriesOf(db: ProviderNeutralDatabase, originNodeRunId: string) {
...
describeEachProvider('RFC-136 reseal providers', (harness) => {
  describe('RFC-136 — 重答（reseal）正向路径', () => {
    test('待指派已答题重 seal：answers_json 覆盖 + sealed 戳前移 + autoStage 回待下发 + resealedQuestionIds', async () => {
      ...
      await sealThenUnstage(
        db,
        taskId,
        originNodeRunId,
        'q1',
        makeAns('q1', 0, 'OLD-MARKER'),
        1_000,
      )
      ...
      await db
        ...
        .where(
          and(
            ...
            eq(taskQuestions.questionId, 'q1'),
          ),
        )
