...
describeEachProvider(
  'RFC-053 PR-D — R1 (approved doc_version ⟹ done review node_run)',
  (harness) => {
    let h: Harness
    afterEach(() => h?.cleanup())
...
describeEachProvider(
  'RFC-053 PR-D — R2 (done review node_run ⟹ ∃ approved doc_version)',
  (harness) => {
    let h: Harness
    afterEach(() => h?.cleanup())
    ...
    test('violated: done review run with only rejected doc_version → R2 alert', async () => {
      h = await buildHarness(harness.db, 'failed')
      const run = await insertReviewRun(h.db, h.taskId, { status: 'done', finishedAt: Date.now() })
      await insertDoc(h.db, h.taskId, {
        reviewNodeRunId: run,
        decision: 'rejected',
