...
async function seedSchedule(db: ProviderNeutralDatabase, ownerUserId: string): Promise<string> {
  const id = ulid()
...
describeEachProviderHttpApplication(
  'RFC-324 §7 —— 定时任务两档授权',
  {
    ...
    dbVersion: 1,
    tempPrefix: 'aw-rfc324-scheduled-task-acl-',
  },
  (scope) => {
    ...
    test('owner 照常：改节奏、改绑目标、删除都放行', async () => {
      ...
      const del = await req(h.app, h.owner.token, `/api/scheduled-tasks/${id}`, {
        method: 'DELETE',
      })
