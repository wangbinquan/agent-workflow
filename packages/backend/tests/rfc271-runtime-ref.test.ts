...
/** 只回答 getById 的 provider-neutral 桩——resolver 不该碰别的。 */
const stubAgents = (rows: Record<string, unknown>) => ({
  async getById(agentId: string) {
    return (rows[agentId] ?? null) as never
  },
})
...
describe('resolveNodeAgentRef —— typed Result，绝不 throw', () => {
  ...
  test('两种失败是**不同的 reason** —— 合并它们会让两个错误码塌成一个', async () => {
    const missing = await resolveNodeAgentRef(stubAgents({}), {}, DISPATCH_CALL_POLICY)
  ...
  test('两种 call policy 走同一条解析、**返回形状一致**——分支在调用点不在这里', async () => {
    const dispatch = await resolveNodeAgentRef(stubAgents({}), {}, DISPATCH_CALL_POLICY)
    const hydrate = await resolveNodeAgentRef(stubAgents({}), {}, FANOUT_HYDRATE_CALL_POLICY)
  ...
  test('任何输入都不 throw（runScope 会把异常冒泡成任务级 scheduler error）', async () => {
    for (const node of [{}, { agentId: '' }, { agentId: 123 }, { agentId: '01J' }]) {
      await expect(
        resolveNodeAgentRef(stubAgents({}), node as never, DISPATCH_CALL_POLICY),
