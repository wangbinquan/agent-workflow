...
describeEachProvider(
  'RFC-074 — review awaiting-refresh: supersede + recomment-drop + v(n+1) (B14-B15)',
  (harness) => {
    ...
    test('awaiting review + fresher upstream → v1 superseded, comments dropped, v2 minted, consumed restamped', async () => {
      ...
      const allVersions = await db
        ...
        .from(docVersions)
        .where(
          and(eq(docVersions.reviewNodeRunId, reviewRunId), eq(docVersions.decision, 'pending')),
        )
    ...
    test('a T6 review refresh reuses one stable gate and advances its committed revision without overwriting v1', async () => {
      ...
      const versions = await db
        ...
        .orderBy(docVersions.versionIndex)
      ...
      const operations = await db
        ...
        .orderBy(collaborationGateOperations.resultGateRevision)
    ...
    test('refresh projection failure preserves the old pending round and retry commits the prepared operation', async () => {
      ...
      const clearProjectionFailure = async () => {
        await harness.executeFixtureDdl(
          exclusive
            ? 'DROP TRIGGER IF EXISTS rfc333_refresh_projection_down'
            : 'DROP TRIGGER IF EXISTS rfc333_refresh_projection_down ON doc_versions',
        )
        if (!exclusive) {
          await harness.executeFixtureDdl(
            'DROP FUNCTION IF EXISTS rfc333_refresh_projection_down()',
          )
        }
      }
      try {
        ...
        `)
          ...
          await harness.executeFixtureDdl(`
          CREATE FUNCTION rfc333_refresh_projection_down() RETURNS trigger AS $$
          BEGIN RAISE EXCEPTION 'rfc333-refresh-projection'; END;
          $$ LANGUAGE plpgsql;
        `)
          await harness.executeFixtureDdl(`
          CREATE TRIGGER rfc333_refresh_projection_down
          BEFORE INSERT ON doc_versions FOR EACH ROW
          EXECUTE FUNCTION rfc333_refresh_projection_down();
        `)
        }
        ...
        await expect(
          dispatchReviewNode({
            ...
            let failure = error
            while (failure instanceof Error && failure.cause instanceof Error) {
        ...
        expect(
          (await db.select().from(docVersions).where(eq(docVersions.id, v1Id)))[0],
        ).toMatchObject({
          decision: 'pending',
          decisionReason: null,
        })
        ...
        expect(
          (await db.select().from(nodeRuns).where(eq(nodeRuns.id, reviewRunId)))[0],
        ).toMatchObject({
          status: 'awaiting_review',
          consumedUpstreamRunsJson: JSON.stringify({ src: '01A_OLD' }),
        })
        ...
        expect(
          (
            await db
              ...
              .orderBy(docVersions.versionIndex)
          ).map((version) => [version.versionIndex, version.decision]),
        ).toEqual([
          [1, 'superseded'],
          [2, 'pending'],
        ])
