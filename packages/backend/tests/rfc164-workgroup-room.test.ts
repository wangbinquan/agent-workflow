...
// 纯函数，不碰库——不给它套双引擎 harness，否则白开一个 PostgreSQL 库跑字符串断言。
...
describeEachProviderHttpApplication(
  'RFC-164 room — endpoints',
  {
    token: 'a'.repeat(64),
    opencodeVersion: '1.14.25',
    dbVersion: 1,
    tempPrefix: 'aw-rfc164-workgroup-room-',
  },
  (scope) => {
    ...
    test('RFC-229 room aggregate exposes a message trigger reference', async () => {
      ...
      expect(body.messages.find((message) => message.id === childId)?.triggerMessageId).toBe(
        parentId,
      )
    })
    ...
    test('cancel: dispatched card cancels + system message; running card 409; stranger 404', async () => {
      ...
      expect(
        frames.some((f) => f.type === 'wg.assignment.updated' && f.status === 'canceled'),
      ).toBe(true)
...
describeEachProviderHttpApplication(
  ...
  (scope) => {
    ...
    test('deliver 双形态：body 直交付 / summary+detail 归一；卡 dispatched→delivered + delivery 消息', async () => {
      ...
      const r1 = await req(
        owner.token,
        `/api/workgroup-tasks/${taskId}/assignments/${a1}/deliver`,
        {
          method: 'POST',
      ...
      const r2 = await req(
        owner.token,
        `/api/workgroup-tasks/${taskId}/assignments/${a2}/deliver`,
        {
          method: 'POST',
    ...
    test('上线前加固：confirm 恢复失败时 gate、holder 与消息全部保持可重试', async () => {
      ...
      //
      // RFC-359 AC-6 —— 这条用例改成双引擎跑之后，PostgreSQL 那半**当场是红的**：它的
      // `continuation.assertResumable` 注入的是空操作，于是这里拿到 200、闸门关上、holder
      // 释放，随后 daemon worker 驱动失败只打一行 warn，任务永久搁浅且没有第二次 confirm 的
      // 入口。判据已收进 `modules/task-execution/application/worktreeResumePreflight`，两个
      // provider 共用一份（两个依赖都是 neutral）。别把任一边再退回空操作。
      const res = await req(owner.token, `/api/workgroup-tasks/${taskId}/confirm`, {
        method: 'POST',
        body: JSON.stringify({ decision: 'approve' }),
      })
    ...
    test('RFC-099 audit: adding a human member writes task_collaborators so they can enter the room', async () => {
      ...
      const add = await req(owner.token, `/api/workgroup-tasks/${taskId}/config`, {
        ...
        body: JSON.stringify({
          addMembers: [
            { memberType: 'human', userId: bob.id, displayName: 'bob', roleDesc: '把关' },
          ],
        }),
      })
      expect(add.status).toBe(200)
