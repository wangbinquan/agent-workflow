...
describe('buildClarifyPromptContext', () => {
  ...
  // RFC-070: `historyCutoffClarifyIteration` is gone — aging is per-row
  // `consumed_by_consumer_run_id IS NULL`. The semantic ("drop sessions
  // already baked into a prior done-with-outputs run") is covered end-to-end
  // by `rfc070-aging-stamp-behavior.test.ts` B-group.
