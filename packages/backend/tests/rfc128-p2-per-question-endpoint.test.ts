...
async function clarifyAnswerReruns(db: ProviderNeutralDatabase, taskId: string): Promise<number> {
...
async function nodeRunStatus(db: ProviderNeutralDatabase, id: string): Promise<string | undefined> {
...
describeEachProviderHttpApplication(
  'RFC-128 P2 — T6 defer=true 控制通道 (seal 进待下发/staged, 不续跑)',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
    tempPrefix: 'aw-rfc128-p2-',
  },
  (scope) => {
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-128 P5-BC (route) — deferred cross full-seal of self/questioner → 200 + park (no strand)',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
  ...
  (scope) => {
    ...
    test('cross full seal + directive=stop (deferred) → 200 + questioner park（无 designer 条目）', async () => {
      ...
      expect((await loadUndispatchedSelfQuestionerTargets(h.db, taskId)).has('questioner')).toBe(
        true,
      )
    })
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-128 P2 — defer=false/缺省 快通道逐字不变 (黄金锁)',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-128 P2 — T7 待下发 gate (stage 须先 seal)',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-128 P2 — 端点鉴权 403 (ensureClarifyMember)',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-128 P2 — Codex P1 / P5-0: full defer seal 关闭中介 node_run (designer) vs guard (self/q)',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-128 P2 — Codex P2-1: questionIds 须配 defer',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-128 P2/P5-BC — defer 透传 directive (stop): full deferred→200+park, partial→不落 directive',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
