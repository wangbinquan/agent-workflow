...
async function seedUsers(db: ProviderNeutralDatabase): Promise<Users> {
...
describeEachProviderHttpApplication(
  'RFC-099 — review membership + attribution',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
    tempPrefix: 'aw-membership-attr-',
  },
  (scope) => {
    ...
    beforeEach(async () => {
      db = scope.harness.db
      // 文档版本落在 app home 下，而应用用的是**作用域现建的那个**——用例自建一个再设
      // `AGENT_WORKFLOW_HOME` 会被整份绕开，路由一路 404 而夹具自己看起来一切正常。
    ...
    test('stranger cannot read or decide; collaborator comment + decision record role snapshots', async () => {
      ...
      const daveDecision = await req(
        app,
        users.dave.token,
        `/api/reviews/${reviewRunId}/decision`,
        {
          method: 'POST',
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-099 — clarify membership, drafts, attribution freeze',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
  ...
  (scope) => {
    ...
    test('stranger cannot submit answers or read detail; list/pending-count filtered', async () => {
      ...
      const daveList = (await (
        await req(app, users.dave.token, '/api/clarify')
      ).json()) as unknown[]
      expect(daveList.length).toBe(0)
    })
  },
