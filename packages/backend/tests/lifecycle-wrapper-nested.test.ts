...
async function buildFixture(harness: ProviderHarness): Promise<Fixture> {
  ...
  await db.insert(tasks).values({
    ...
    inputs: '{}',
    startedAt: Date.now(),
    // RFC-359: the SQLite-only `rfc328_tasks_lineage_after_insert` trigger has no
    // PostgreSQL counterpart by design, so a direct fixture INSERT must write both
    // causal columns itself (root task ⇒ lineage id = own id, single `task-root` frame).
...
describeEachProvider('循环 / 扇出 / 包装器嵌套（双引擎）', (harness) => {
  describe('RFC-053 PR-A T1f — loop / fan-out / wrapper nesting', () => {
    let h: Fixture

    beforeEach(async () => {
      h = await buildFixture(harness)
    })
    afterEach(() => h.cleanup())
    ...
    test('W1 iter scope — dispatch at iter=1 ignores iter=0 done rows', async () => {
      ...
      const iter0After = (
        await h.db.select().from(nodeRuns).where(eq(nodeRuns.id, iter0Review))
      )[0]!
