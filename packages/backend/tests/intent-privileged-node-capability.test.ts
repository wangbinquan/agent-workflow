...
async function applyAs(
  ...
  const draft = await installDraft(session.id, bundle, manifest)
...
// 这一份尤其值得喂两个引擎：§5ea 照出的「特权节点回填」缺陷正落在它锁的那条路上。
describeEachProvider('特权节点能力门（双引擎）', (harness) => {
  beforeEach(async () => {
        ...
        [
          {
            ...
            slots: [
              {
                slotId: 'secret:op-1:/definition/nodes/1/env/TOKEN',
                value: 'real-value-in-storage',
              },
            ],
          },
        ],
