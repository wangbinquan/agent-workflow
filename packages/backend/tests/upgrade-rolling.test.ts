...
// idempotently, (b) end up with the HEAD schema (all 226 entries in
...
describe('RFC-054 W1-6 — rolling upgrade from old home reaches HEAD + runs toy task', () => {
  ...
  test('HEAD journal has 226 entries (sanity — records the reviewed migration head)', () => {
    ...
    // TaskEngine catalog visibility bump 到 203 with 0203_task_catalog_visibility：
    // 内部执行保留为可审计任务，但不进入公共任务目录；调用子树继承同一可见性。
    ...
    // 任何人都够不到的行。存量行不回填。
    // RFC-310 task backlink bump 到 205 with 0205_rfc310_task_employee_case_link：
    // TaskEngine 行冻结所属 Case，历史 OS round 由 employee_reaction_rounds 回填；
    ...
    // RFC-330 bump 到 211 with 0211_rfc330_employee_authoring_acl：工具 / 岗位
    // 模版 owner+visibility+acl_revision、模版 owner 内同名唯一、案例成员表。
    // RFC-333 T3 bump 到 212 with 0212_rfc333_human_gate_operations：
    // collaboration gate operation / artifact journal（inactive additive slice）。
    // RFC-333 T12 bump 到 213 with 0213_rfc333_gate_intent_handoff：claimed
    // owner 与唯一 pending gate successor 分开约束，支持同 coordinator 的 durable handoff。
    // RFC-335 bump 到 214 with 0214_rfc335_oidc_git_name：独立保存展示名和
    // Git user.name，并给 Provider 增加独立映射字段。
    // RFC-336 bump 到 215 with 0215_rfc336_employee_case_advanced_options：Case
    // 时长/Token 上限、累计量与 exact-once 计量 receipt。
    // RFC-338 bump 到 216 with 0216_rfc338_maintenance_runs：维护日程、lease、
    ...
    // RFC-359 T19h bump 到 225 with 0225_rfc359_task_count_indexes：为任务计数和
    // 缓存仓引用查询追加两条覆盖索引，与 PostgreSQL 0001 增量对应。
    // RFC-359 W28 bump 到 226 with 0226_rfc359_task_list_facets_index：追加任务
    // 列表 facet 覆盖索引，与 PostgreSQL 0002 增量对应。
    //
    // 注意：本条断言与上面的 test 标题在 0223 那一轮曾经脱节（标题停在 222、断言已是 223）。
    // 两处都要改——标题是 CI 日志里唯一能看见的那一行。
    expect(HEAD_TOTAL_MIGRATIONS).toBe(226)
  })
