...
describe('RFC-054 W1-6 — rolling upgrade from old home reaches HEAD + runs toy task', () => {
  ...
  test('HEAD journal has 100 entries (sanity — locks the freeze target indices)', () => {
    ...
    // RFC-204 T2 bumped to 98 with 0100_rfc207_task_running_time.
    expect(HEAD_TOTAL_MIGRATIONS).toBe(100)
  })
