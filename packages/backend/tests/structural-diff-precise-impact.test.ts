...
describe('computePreciseImpact', () => {
  ...
  test('callers = all non-def occurrences, def excluded, extracted confidence', () => {
    const out = computePreciseImpact(graph, [
      { changedSymbolId: 'A.foo', scipSymbol: SYM_A, ownerFile: 'src/a.ts' },
    ])
  ...
  test('symbol with no references → no ImpactItem', () => {
    ...
    expect(
      computePreciseImpact(lonely, [{ changedSymbolId: 'X', scipSymbol: 'X', ownerFile: 'a.ts' }]),
    ).toEqual([])
  })

  test('symbol absent from graph → omitted, no throw', () => {
    expect(
      computePreciseImpact(graph, [
        { changedSymbolId: 'Z', scipSymbol: 'no-such', ownerFile: 'a.ts' },
      ]),
    ).toEqual([])
  })
  ...
  test('PRECISION WIN: two same-named methods disambiguated by resolved symbol', () => {
    ...
    const out = computePreciseImpact(g, [
      { changedSymbolId: 'A.run', scipSymbol: SYM_ARUN, ownerFile: 'a.ts' },
    ])
    expect(out).toHaveLength(1)
