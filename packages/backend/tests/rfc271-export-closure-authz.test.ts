...
interface MidExportWrite {
  ...
  /**
   * 注入点实际触发的次数。**每条用例都要断言它**——注入一旦失效，「必须成功」那几条
   * （基线 / private→public / 插件重装 / 混类型闭包）会安静地变绿，正是本文件开头记的
   * 那种假绿。触发计数把「注入没跑」变成一条明确的红。
   */
  fired(): number
}
...
function dbWithMidExportWrite(
  ...
  /**
   * 把这次查询的**执行**推迟到注入写完成之后。
   *
   * RFC-359 双引擎：SQLite 的写是同步的，注入过去在 `from()` 里当场跑完即可；PostgreSQL 上
   * 写是 async，必须在这条查询真正执行**之前**被 await 掉，否则读写乱序、注入形同虚设。
   * drizzle 的查询构造器是 thenable，接管 `then` 就能在执行前插进一次 await；链式方法
   * （`where` / `orderBy`）返回 `this`，要把代理身份带下去，否则一调用就脱代理。
   */
  ...
  const wrapHandle = (handle: object): object =>
    new Proxy(handle, {
      get(target, property, receiver) {
        ...
        // PostgreSQL 上事务句柄与客户端**不是同一个对象**（SQLite 上是同一个），而读端口的
        // 每次 `listByIds` 都在事务里发 select。不把事务句柄一并代理，注入点在 PG 上一次
        // 都不会触发。
          ...
          return new Proxy(builder, {
            get(queryTarget, queryProperty, queryReceiver) {
              ...
              return (t: unknown) => {
                ...
                if (!(nth < 0 ? reads >= -nth : reads === nth)) return next
                fired += 1
                let started: Promise<void> | undefined
                // 注入写走**这次读所在的那个句柄**：PG 上就是那笔事务的连接，写与读因此
                // 天然有序，也不会与外层 SERIALIZABLE 事务互相冲突。
...
// RFC-359 AC-6 —— 导出闭包的读端口、授权复核与产物复核在 RFC-359 之后是「一份实现两个
// provider 共用」，所以这一整组判据在两个引擎上各跑一遍。中途注入的那道 seam 也随之双引擎
// 化：PostgreSQL 的事务句柄和客户端不是同一个对象、写又是 async，两处都在
// `dbWithMidExportWrite` 里按引擎无关的方式接住了（见该函数注释）。
describeEachProvider('RFC-271 导出末端双复核（双引擎）', (harness) => {
  beforeEach(() => {
    db = harness.db
  })
  ...
  describe('AC-7 · 授权复核：闭包成员在导出中途失去可见性 ⇒ 拒绝', () => {
    test('基线：grant 在，导出成功且包含该依赖', async () => {
      const { appHome, rootId } = await seedGrantedClosure()
      try {
    ...
    test('**撤权后**导出必须拒绝 —— 而不是带走一个已经不可见的资源', async () => {
      const { appHome, rootId, childId } = await seedGrantedClosure()
      try {
        ...
        const raced = dbWithMidExportWrite(db, agents, 2, async (handle) => {
          await handle.delete(resourceGrants).where(eq(resourceGrants.resourceId, childId))
          await handle
            .update(agents)
            .set({ aclRevision: 1, updatedAt: 2 })
  ...
  describe('AC-7 · 产物复核拿**序列化器的产出**比，不是拿「行」近似「包」', () => {
    ...
    test('闭包里的插件在导出中途被重装（只动本机安装态）⇒ 不得误拒', async () => {
      ...
      try {
        ...
        const raced = dbWithMidExportWrite(db, plugins, 2, async (handle) => {
          await handle
            .update(plugins)
  ...
  describe('AC-7 · 第六轮：产物复核自己不能制造误拒 / 盲区', () => {
    ...
    test('② 导出中途**改名** ⇒ 必须发现（序列化器读的是顶层 name，不是 row）', async () => {
      ...
      try {
        const raced = dbWithMidExportWrite(db, agents, 2, async (handle) => {
          await handle.update(agents).set({ name: 'renamed' }).where(eq(agents.id, childId))
        })
