...
// 这四条会 spawn 一个真的 daemon 并等它打出 ready，等待预算 10~15s；bun 的**每条用例默认
// 超时是 5s**，于是它们在慢机器上必然先被 5s 掐掉——不是偶发，是等待预算天生大于用例预算。
// 2026-09-02 macOS shard 1/4 就以 `[5025.77ms]` 红在 RFC-300 那条上。给它们一个大于自身
// 等待预算的显式超时，让超时真的由 `waitForReady` 的诊断信息来报。
...
describe('CLI subcommands (P-1-05)', () => {
  ...
  test(
    'start --port overrides config.json bindPort (RFC-319 T36)',
    async () => {
  ...
  test(
    'status + stop: end-to-end against a real daemon',
    async () => {
      const child = Bun.spawn({
        cmd: ['bun', 'run', mainPath, 'start', '--port', '0'],
        env: { ...(process.env as Record<string, string>), AGENT_WORKFLOW_HOME: tmp },
  ...
  test(
    'RFC-300 boot resumes an already-authorized scratch prune before serving',
    async () => {
  ...
  test(
    'daemon writes .daemon.info on start, removes it on SIGTERM',
    async () => {
      const child = Bun.spawn({
        cmd: ['bun', 'run', mainPath, 'start', '--port', '0'],
        env: { ...(process.env as Record<string, string>), AGENT_WORKFLOW_HOME: tmp },
