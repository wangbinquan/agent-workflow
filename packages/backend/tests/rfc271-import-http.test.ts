...
describeEachProvider('资源包 multipart 提交的路由接缝（双引擎）', (harness) => {
  beforeEach(async () => {
  ...
  describe('POST /api/resource-packages/commit multipart seam', () => {
    ...
    test('PAT preview 猜中 human username 也不泄漏用户 UUID', async () => {
      await seedUser(db, 'route-user', 'route-user', 'admin')
      await seedUser(db, 'private-user-id', 'source-alice', 'user')
      ...
      expect(
        preview.humanMembers.find((member) => member.username === 'source-alice'),
      ).toMatchObject({
        username: 'source-alice',
    ...
    test('forwards human mappings and per-field secret inputs into one durable commit', async () => {
      await seedUser(db, 'route-user', 'route-user', 'admin')
      await seedUser(db, 'local-alice', 'alice', 'user')
      ...
      const landedMcp = await db.select().from(mcps).where(eq(mcps.name, 'tools')).get()
