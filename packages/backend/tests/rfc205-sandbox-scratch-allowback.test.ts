...
// 工作树的 .git 指针指向 scratch/{taskId}/.git/worktrees/{runId}，基仓不进
// allow 集时 agent cwd 里所有 git 命令 EPERM（文件写反而成功——iso/{taskId}
// 本身是 allow 的）。实测后果：成员宣布工作区不可用（"git 无法运行 / 仅 .git
// 指针"），转而去改 appHome 边界外不设防的用户真仓。修法：buildRunSandboxCtx
// 在 scratch/{taskId} 存在时把它并进 taskWorktrees。
...
describe('buildRunSandboxCtx — scratch 基仓 allow-back', () => {
  test('scratch/{taskId} 存在 ⇒ 并入 taskWorktrees（iso run 的 gitdir 可达）', () => {
    const taskId = 'T-SCRATCH-1'
    mkdirSync(join(appHome, 'scratch', taskId), { recursive: true })
  ...
  test('worktreePath 就是 scratch 目录（canonical run）⇒ 不重复并入', () => {
