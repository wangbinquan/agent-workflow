...
/**
 * RFC-359 AC-6 —— 权限门的负集合在**两个引擎**上各钉一遍。
 *
 * 合一前这份用例只跑 SQLite 内存库：它锁的是「哪些端点对普通用户返回 403」，而路由门本身
 * 与引擎无关——但**门后面的东西不是**（`/api/config` 的读、资源路由的 404 与 422 都要真库）。
 * 只跑一个引擎意味着 PostgreSQL 部署上这张负集合从来没有被验证过。
 */
async function buildHarness(
  provider: ProviderDatabaseHarness,
  appHome: string,
  configPath: string,
): Promise<{ harness: Harness; application: ProviderHttpApplication }> {
  ...
  return { harness: { db, app, userToken: token }, application }
}
...
describeEachProvider(
  'regular-user session token — admin-only endpoints all return 403',
  (provider) => {
    let h: Harness
    ...
    beforeEach(async () => {
      const tmp = mkdtempSync(join(tmpdir(), 'aw-admin-gate-'))
      ...
      const built = await buildHarness(provider, appHome, join(tmp, 'config.json'))
      h = built.harness
...
describeEachProvider(
  'regular-user session token — endpoints that are intentionally open',
  (provider) => {
    let h: Harness
    let application: ProviderHttpApplication | undefined
    let cleanup: (() => void) | undefined
...
describeEachProvider('PAT-bearing actor cannot escape role limits', (provider) => {
  let h: Harness
  let application: ProviderHttpApplication | undefined
  let cleanup: (() => void) | undefined

  beforeEach(async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'aw-admin-gate-pat-'))
    const previousAppHome = process.env.AGENT_WORKFLOW_HOME
    cleanup = () => {
      rmSync(tmp, { recursive: true, force: true })
      if (previousAppHome === undefined) delete process.env.AGENT_WORKFLOW_HOME
