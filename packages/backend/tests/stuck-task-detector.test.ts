...
describeEachProvider('RFC-053 PR-E — S4 (pending too long)', (harness) => {
  let h: Harness
  afterEach(() => h?.cleanup())

  test('stuck: pending > 5 min → S4 alert', async () => {
    h = await buildHarness(harness.db, 'pending', T0 - 10 * MIN_MS)
  ...
  test('not stuck: pending < 5 min → no S4 alert', async () => {
    h = await buildHarness(harness.db, 'pending', T0 - 2 * MIN_MS)
    const r = await runStuckTaskDetector({
      operations: createTaskExecutionPersistence(h.db).recoveryAdministration,
      now: () => T0,
    })
  ...
  test('D11 child: pending 40 min → S4 alert with 30min threshold + childBudgetWaitHint', async () => {
    h = await buildHarness(harness.db, 'pending', T0 - 40 * MIN_MS)
    await markAsChild(h)
    const r = await runStuckTaskDetector({
      operations: createTaskExecutionPersistence(h.db).recoveryAdministration,
      now: () => T0,
...
describeEachProvider('RFC-053 PR-E — S1 (awaiting_review without pending dv)', (harness) => {
  let h: Harness
  afterEach(() => h?.cleanup())

  test('stuck: awaiting_review > 30 min + no pending dv → S1 alert', async () => {
    h = await buildHarness(harness.db, 'awaiting_review', T0 - 60 * MIN_MS)
...
describeEachProvider(
  'RFC-053 PR-E — S2 (awaiting_human without open clarify_session)',
  (harness) => {
    let h: Harness
    afterEach(() => h?.cleanup())

    test('stuck: awaiting_human > 30 min + no open session → S2 alert', async () => {
      h = await buildHarness(harness.db, 'awaiting_human', T0 - 45 * MIN_MS)
      const r = await runStuckTaskDetector({
        operations: createTaskExecutionPersistence(h.db).recoveryAdministration,
        now: () => T0,
      })
...
describeEachProvider('RFC-053 PR-E — S3 (running but all node_runs terminal)', (harness) => {
  let h: Harness
  afterEach(() => h?.cleanup())

  test('stuck: running > 30 min + all runs done → S3 alert', async () => {
    h = await buildHarness(harness.db, 'running', T0 - 60 * MIN_MS)
...
describeEachProvider('RFC-098 WP-8 — S5 (running, active runs, events stalled)', (harness) => {
  let h: Harness
  afterEach(() => h?.cleanup())
  ...
  test('stuck: events exist but stalled 40 min ago → S5; lastEventTs reports the run own ts', async () => {
    h = await buildHarness(harness.db, 'running', T0 - 120 * MIN_MS)
    const runId = await insertRun(h.db, h.taskId, { nodeId: 'b', status: 'running' })
...
describeEachProvider('RFC-053 PR-E — reconcile + WS onAlert', (harness) => {
  let h: Harness
  afterEach(() => h?.cleanup())
