...
async function harness(
  ...
  // 开关是**守护进程配置**，而应用读的是作用域现建的那份 config——自建一份写 `mcpSurfaceEnabled`
  // 会被整份绕开（迁移当天实测：关掉开关的用例照样拿到 200）。
  const app = (await scope.open({ config: { mcpSurfaceEnabled: surfaceEnabled } })).app
...
describeEachProviderHttpApplication(
  'RFC-247 §4.1 — only personal access tokens open the pipe',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: null,
    dbVersion: 1,
    tempPrefix: 'aw-mcp-transport-',
  },
  (scope) => {
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-247 D10 / F6 — the surface switch closes the endpoint',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: null,
    dbVersion: 1,
  ...
  (scope) => {
    test('a valid PAT is refused while the switch is off', async () => {
      const h = await harness(scope, false)
      const res = await mcpRequest(h.app, h.patToken)
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-247 — tools/list reaches the model',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: null,
    dbVersion: 1,
