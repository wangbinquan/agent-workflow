...
interface Harness {
  ...
  app: Hono
  /**
   * 应用**真正在用**的 app home（`scope.open()` 现建并写进 `AGENT_WORKFLOW_HOME`）。
   * 别再自建一个：合一前那份模块级 `beforeEach` 建的 home 会被 `open()` 覆盖掉，
   * 于是 `join(appHome, 'evidence', …)` 断言的是一个应用从没写过的目录。
   */
  appHome: string
  tokenA: string
  tokenB: string
...
describeEachProviderHttpApplication(
  'rfc310 pr3 upload security — HTTP surface',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
    tempPrefix: 'aw-rfc310-pr3-',
  },
  (scope) => {
    test('upload routes create the selected app-home evidence store on the first request', async () => {
      ...
      const evidenceRoot = join(h.appHome, 'evidence')
    ...
    test('binary bytes land byte-identical in the blob pool — no transcoding, no newline normalization', async () => {
      ...
      const blobPath = join(h.appHome, 'evidence', 'blobs', expectedSha.slice(0, 2), expectedSha)
    ...
    test('upload idempotency key replays to the same ref (no unrecognizable duplicate rows)', async () => {
      const h = await buildHarness(scope)
      const first = (await (
        await uploadBytes(h, h.tokenA, 'same content', {
          'x-upload-idempotency-key': 'retry-key-1',
        })
...
function previewDeps(db: ProviderNeutralDatabase): {
  deps: LaunchDeps
  sessions: UploadSessionPersistence
} {
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'rfc310 pr3 upload security — real git baseline chain',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
  ...
  (scope) => {
    test('dispositions come from the frozen HEAD: binary already-present, symlink/dir/mode-drift blocked; preview never claims', async () => {
      const db = scope.harness.db
      // 这一块不打 HTTP，但仍要 `open()`：被测的上传落盘走 `Paths.root`，也就是
      // `AGENT_WORKFLOW_HOME`——不 open 的话那个变量指向的是**真实用户目录**。
      const appHome = (await scope.open()).appHome
      ...
      expect(await db.select().from(developmentRepositoryUploadPlans)).toHaveLength(0)
