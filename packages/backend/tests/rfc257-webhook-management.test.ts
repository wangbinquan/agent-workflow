...
async function harness(scope: ProviderHttpApplicationScope) {
  ...
  // RFC-359 AC-6：派发器是**按用例**的（每次 harness 各自一个 `dispatched` 数组），所以走
  // `open()` 的逐例覆盖；secretBox 不再自建——作用域交出的就是**应用自己装配的那一份**，
  // 本文件也从不用它自己加解密。
  const app = (await scope.open({ webhookDispatcher: dispatcher })).app
  ...
  (scope) => {
    ...
    test('轮换 secret：新明文一次性返回且 hint 更新；轮换 url token 改变入站地址', async () => {
      ...
      const rot = await call(
        ...
        h.admin,
        'POST',
        `/api/webhook-endpoints/${ep.id}/rotate-secret`,
      )
...
describeEachProviderHttpApplication(
  ...
  (scope) => {
    ...
    test('PUT：kind/endpoint 不可变 422；合法 patch 生效；DELETE 级联 fires/streams', async () => {
      ...
      expect((await call(h.app, h.alice, 'DELETE', `/api/webhook-triggers/${tid}`)).status).toBe(
        200,
      )
