...
// RFC-359 AC-6（收尾）：本套件迁到双引擎。需求提供方 mock 是**进程侧**的可变状态
//（用例会改 `revision` 再还原），所以每条泳道起一台自己的——共用一台会让一条泳道的
// 「刷新到 r2」漏给另一条泳道先跑的用例。库侧夹具本来就是每个用例现建，直接接 harness 的库。
describeEachProvider(
  'rfc310 pr3 — external requirement acquisition via real adapter subprocess',
  (harness) => {
    beforeAll(async () => {
