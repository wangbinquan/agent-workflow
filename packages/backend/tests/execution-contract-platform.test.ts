...
/**
 * 直插 `tasks` 的夹具必须自带 lineage：SQLite 的 `rfc328_tasks_lineage_after_insert`
 * 触发器按设计没有 PostgreSQL 对应物，两个引擎上都显式写同一份谱系，读点才拿到同样的行。
 */
function taskLineage(taskId: string): { executionLineageId: string; lineageSlotPathJson: string } {
  return {
...
describeEachProvider('平台执行合同（双引擎）', (harness) => {
  describe('platform execution contracts', () => {
    ...
    test('direct Agent prompts contain only the action, business result, and business input', () => {
      ...
      expect(prompt).toContain(
        'Action: Read the requirement materials and complete the code change',
      )
    ...
    test('pipeline tool guides expose the classified problem-set handoff instead of treating tool configuration as task input', () => {
      ...
      expect(
        classifier.output.fields.find((field) => field.path === 'contextPatches'),
      ).toMatchObject({
        label: {
          'zh-CN': '问题种类与问题记录',
    ...
    test('human-review projection reads the frozen host input and durable review receipt', async () => {
      ...
      await db.insert(tasks).values({
        ...
        ...taskLineage('review-projection-task'),
