...
describe('RFC-359 W5 —— provider 组合根必须被测试真正构造过', () => {
  test(
    ...
    () => {
      ...
      // RFC-359（2026-09-13，第四档）：58 → 57。资源包 apply 两台引擎合一——SQLite 专属的
      ...
      // `composeIntentApplyConvergence`、`composeSqliteSkillArtifactCompensation`、
      ...
      //
      // RFC-359（2026-09-13，第二档）：76 → 70。同一批继续还掉四对
      // （`CollaborationRouteOperations` / `WorkspaceMaintenanceCommand` / `ResourceCatalog` /
      // `EventsArchiveStore`），plan §5ds 的名单又短了四行。
      //
      ...
      // RFC-359 AC-1（2026-09-15，plan §5fu）：45 → 31。一次退役 14 对纯装配别名——
      // 这条判据按**名字**派生「provider 组合根」，别名退役后分母自然一次掉 14 组。
      // 掉得多不代表覆盖变差：那些名字背后本来就是同一个函数，判据此前是在
      // 「同一个组合根」上数了两遍。
      // RFC-359 AC-1（2026-09-16，plan §5fw–§5gf）：31 → 19。本轮又退役一批 provider 命名的
      // 装配入口（别名 / 转交层 / 体逐字节相同的中立+品牌 / 装配责任上提），
      // 这条判据按**名字**派生组合根，分母跟着掉。掉的是重复计数与已消失的品牌名，不是覆盖。
      ...
      // RFC-359 AC-1（第 14 刀）：15 → 14。源终止的装配两个引擎合成一份，
      // 退役 `composePostgresqlTaskSourceTermination`。**是合一不是删覆盖**，分母少一。
      expect(ROOTS.length, '命名式一条都派生不出来 ⇒ 清单本身失效').toBeGreaterThanOrEqual(14)
