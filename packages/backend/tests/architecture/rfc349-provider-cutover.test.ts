...
const PROVIDER_SPECIFIC_BUSINESS_DEPENDENCY_DEBT = [
  ...
  'packages/backend/src/services/task.ts -> @/modules/task-execution/infrastructure/legacySqliteTransportMechanisms :: LegacyProviderNeutralDatabase,LegacySqliteTaskDatabase,LegacySqliteTaskTransaction,SQL,agents,and,asc,cachedRepos,clarifyRounds,count,desc,docVersions,eq,gt,inArray,isNotNull,isNull,lifecycleAlerts,nodeRunEvents,nodeRunOutputs,nodeRuns,runtimeSessionLeases,sql,taskCollaborators,taskExecutionIntents,taskExecutionOwners,taskRepos,taskSpaceNodes,tasks,users,workflows,workgroupTaskState',
