...
async function harness(scope: ProviderHttpApplicationScope) {
  ...
  // 这份 config 必须并进**作用域现建的那一份**——应用读的是它，用例自建一份会被整份绕开。
  ...
  await db.insert(webhookEndpoints).values({
    ...
    provider: 'gitlab',
    urlToken: URL_TOKEN,
    // 加密列必须用**应用实际装进去的那个盒子**：另起一个新钥匙的盒子写进去，
    // 读面解出来是乱码（两个引擎会一起红）。
    secretEnc: secretBox.seal('s3cret-value'),
    enabled: true,
  })
...
describeEachProviderHttpApplication(
  'RFC-260 · AC-1 端点读面与 URL 响应分层',
  {
    token: 'a'.repeat(64),
    opencodeVersion: '1.14.25',
    dbVersion: 1,
    tempPrefix: 'aw-wh-read-vis-',
  },
  (scope) => {
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-260/RFC-283 · 端点写面仍 admin 独占',
  {
    token: 'a'.repeat(64),
    opencodeVersion: '1.14.25',
    dbVersion: 1,
  ...
  (scope) => {
    test('user/manager 对端点写与 replay 全 403；admin 全通', async () => {
      ...
      for (const token of [h.userSession, h.managerSession]) {
        ...
        expect(
          (await send(h.app, 'PUT', '/api/webhook-endpoints/ep-1', token, { enabled: false }))
            .status,
        ).toBe(403)
        ...
        expect(
          (await send(h.app, 'POST', '/api/webhook-deliveries/dl-1/replay', token)).status,
        ).toBe(403)
      }
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-283 · manager 触发规则 owner 写边界',
  {
    token: 'a'.repeat(64),
    opencodeVersion: '1.14.25',
    dbVersion: 1,
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-260 · AC-3 触发器全量只读（D1 推翻 owner-404 读语义）',
  {
    token: 'a'.repeat(64),
    opencodeVersion: '1.14.25',
    dbVersion: 1,
