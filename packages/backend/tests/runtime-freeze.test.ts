...
): Promise<{ db: ProviderNeutralDatabase; id: string }> {
...
describeEachProvider(
  'resolveFrozenRuntime — built-in protocols (RFC-111 D15)',
  (harness) => {
    ...
    test('first dispatch falls back to config.defaultRuntime, then opencode', async () => {
      const a = await seedRun(harness.db)
      expect((await resolveFrozenRuntime(a.db, a.id, null, 'claude-code')).protocol).toBe(
        'claude-code',
      )
      const b = await seedRun(harness.database(1).db)
...
describeEachProvider(
  'resolveFrozenRuntime — custom runtimes freeze a (protocol, binary) snapshot (RFC-112 P1)',
  (harness) => {
...
describeEachProvider(
  'resume inherits the session owner’s frozen runtime (RFC-112 Codex impl-gate P1)',
  (harness) => {
...
describeEachProvider(
  'resolveFrozenRuntime — freezes the runtime PARAMS too (RFC-113 Codex P1-2)',
  (harness) => {
