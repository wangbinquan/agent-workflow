...
/**
 * RFC-359 AC-6 —— 本文件的库由 `describeEachProviderHttpApplication` 的 harness 提供，
 * 每个用例前重置到「刚迁移完」的状态。`fixtureModule()` 与各用例都从这里取句柄，
 * 因此不再有任何一行自己建 SQLite 内存库。
 */
...
describeEachProviderHttpApplication(
  'RFC-310 数字员工编排层级（双引擎）',
  {
    tempPrefix: 'rfc310-digital-employee-authoring-',
    token: 'a'.repeat(64),
  ...
  (scope) => {
    ...
    describe('RFC-310 Digital Employee OS authoring hierarchy', () => {
      test('type packages declare complete responsibility lanes for deterministic graph layout', () => {
        ...
        expect(
          delivery.responsibilityLanes.find((lane) => lane.laneId === 'delivery-main')
            ?.adapterSlots,
        ).toEqual([])
        expect(
        ...
        expect(
          descriptor.reactionRules.find((rule) => rule.ruleId === 'handle-review'),
        ).toMatchObject({
        ...
        const pipelineRule = descriptor.reactionRules.find(
          (rule) => rule.ruleId === 'handle-pipeline',
        )!
        ...
        expect(employeeTypePackageDescriptorSchema.safeParse(invalidAdapterPurpose).success).toBe(
          false,
        )
      })
      ...
      test('pipeline attention is active only while the current head gate is pending', () => {
        ...
        expect(
          attentionEventIds('development.pipeline', { ...pipeline, status: 'passed' }),
        ).toEqual([])
        expect(
      ...
      test('HTTP names malformed type refs and empty or declared-oversized uploads', async () => {
        ...
        expect(await malformedContract.json()).toMatchObject({
          code: 'execution-contract-ref-invalid',
        })
      ...
      test('pure migrations stay resource-empty and daemon seeding installs Agent templates once', async () => {
        ...
        expect(
          templates.map((template) => template.frontmatterExtra.digitalEmployeeTemplate),
        ).toEqual([
          'code-writing',
          'problem-diagnosis',
          'pipeline-repair',
        ...
        const historicalReviewTools = JSON.parse(
          catalog.listJson(
            JSON.stringify({ typeId: 'development', revision: 8 }),
            'repair-feedback',
          ),
      ...
      test('repository admission is inherited from a fixed employee scope or required at task launch', () => {
        ...
        expect(() =>
          repositoryRef(
            request(
              { kind: 'repository', repositoryId: 'repo-fixed' },
      ...
      test('unconfigured optional lanes stay disabled without blocking employee save', async () => {
        ...
        const coreItems = typePackage.authoringManifest.workItems.filter(
          (item) =>
            item.responsibilityLaneId === null || !optionalLanes.has(item.responsibilityLaneId),
        )
        ...
        const enabled = (await module.queries.getEmployee(employee.id)).definition
          .enabledWorkItemRefs
      ...
      test('type -> work item -> tool registration closes an exact employee definition', async () => {
        ...
        const typePackage = await module.queries.getType(typeRef)
        expect(manifest.lifecycleRegions.map((region) => region.regionId)).toEqual([
          'delivery',
          'care',
        ])
        expect(manifest.workItems).toHaveLength(20)
        expect(
          manifest.workItems.find((item) => item.workItemRef === 'repair-conflict')
            ?.nextWorkItemRefs,
        ...
        await expect(
          invalidAdapterJob({ id: 'wrong-purpose-adapter', revision: 1 }),
        ).rejects.toThrow('requires pipeline-gate')
        ...
        await expect(
          invalidAdapterJob({ id: 'pipeline-adapter-hidden', revision: 1 }),
        ...
        expect(overridden.definition.exactToolBindings).toEqual(
          current.definition.exactToolBindings,
        )
        ...
        expect(
          (await module.queries.getEmployee(employee.id)).definition.exactAdapterBindings,
        ).toEqual(exactDefaultAdapterBindings)
        ...
        expect(
          (await module.queries.getEmployee(cppEmployee.id)).definition.jobTemplateRef,
        ).toEqual(cppTemplateRef)
      })
      ...
      test('pipeline problem types follow the employee-configured priority and handler list', () => {
        ...
        const failureTypeDefinitions = orderedDispatchConfigurations[0]!.routes.map(
          (route, index) => ({
        ...
        expect(firstRepair).toMatchObject({
          caseState: 'active',
          nextWorkItemRef: 'repair-pipeline',
        })
        expect(
          JSON.parse(
            firstRepair.contextPatches.find(
              (patch: { contextTypeId: string }) =>
                patch.contextTypeId === 'development.problem-set',
            ).stateJson,
          ),
                  ...
                  },
                  {
                    ...
                    stateJson: JSON.stringify({
                      ...
                      threads: problemIds.map((threadRef) => ({
                        threadRef,
                        revision:
                          reviewThreads.find((thread) => thread.threadRef === threadRef)
                            ?.revision ?? 'unexpected',
                        acknowledgement: null,
                        disposition: null,
                        replyBody: null,
