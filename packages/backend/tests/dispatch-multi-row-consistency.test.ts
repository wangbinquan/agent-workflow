...
describeEachProvider('RFC-359 W25 multi-row dispatch providers', (harness) => {
  interface Harness {
  ...
  describe('RFC-053 PR-A T1b — multi-row dispatch consistency', () => {
    ...
    test('C4 retry=0 canceled (superseded) + retry=1 pending → scheduler picks retry=1 + dispatch parks it', async () => {
      ...
      const pendingAfter = (
        await h.db.select().from(nodeRuns).where(eq(nodeRuns.id, pendingId))
      )[0]!
    ...
    test('C10 RFC-052 ANY-done short-circuit ignores reviewIteration disagreement', async () => {
      ...
      expect(
        (await h.db.select().from(nodeRuns).where(eq(nodeRuns.id, pendingId)))[0]!.status,
      ).toBe('pending')
    })
  })
})
