...
// RFC-359 AC-6（收尾）：**每条泳道一个自己的 appHome**。共用一个会让第二条泳道的
// `git init` 落在第一条已经建好的仓上，`commit -m base` 当场 "nothing to commit" 失败。
let HOME = ''
...
// RFC-359 AC-6（收尾）：本套件迁到双引擎。真 git 仓是进程侧的一次性世界；库侧夹具
// **每个用例重建**（PG 泳道每个用例前会整库快照回滚）。
function startLaneWorld(): string {
  HOME = mkdtempSync(resolve(tmpdir(), 'rfc310-pr4-journey-'))
...
async function seedLaneDatabase(db: ProviderNeutralDatabase, repoPath: string): Promise<void> {
  launches.length = 0
  outcomes.clear()
...
describeEachProvider(
  'rfc310 pr4 journey — real workspace / validator / candidate chain',
  (harness) => {
    let repoPath = ''
    beforeAll(() => {
      repoPath = startLaneWorld()
    })
    beforeEach(async () => {
    ...
    test('positive: workspace materialized with evidence mount; changed outcome derives a real git candidate', async () => {
      ...
      expect(readFileSync(join(mountDir, manifest.bundleId, 'body.md'), 'utf8')).toBe(
        'do the thing',
      )
