...
//
// RFC-359 AC-6 —— 发布凭据的持久化（`DrizzleRepositoryTransportCredentialRepository`）在
// RFC-359 之后是「一份实现两个 provider 共用」，所以这条 E2E 在两个引擎上各跑一遍，
// 库由 `describeEachProvider` 给。
//
// 双跑要过的坎不在库上，在**库以外**：harness 只 TRUNCATE 数据库，出进程的 system mock
// 套件与磁盘夹具都不复位。第二遍再 `seedCodeHost('rfc321/managed-publication')` 会直接撞
// `already seeded`，而上一遍推上去的 `refs/heads/rfc321-*` 还留在 mock 的裸库里。既有的
// 顶层 `beforeEach` 已经在每条用例开局做控制面整套复位（code-hosts / 请求流水），
// 临时根目录也每条用例现建现删，所以两遍看到的外部世界与第一遍完全同形——判据一条都不放宽。
...
describeEachProvider('RFC-321 托管发布的真 smart-HTTP 证据（双引擎）', (harness) => {
  test('SSH metadata resolves to real smart HTTP; personal wins, absence uses global, and auth failure never retries global', async () => {
    ...
    const invalidGlobalRequests = gitRequestsFor(
      allRequests.slice(beforeInvalidGlobal),
      projectPath,
    )
