...
async function fixture(scope: ProviderHttpApplicationScope) {
  ...
  // RFC-359 AC-6 —— 读模型与协作上下文都取**应用自己装配好的那一份**。合一前这里在外面
  // 用 `createTaskExecutionReadModels(db)` / `createCollaborationCommandContext({...})` 再建
  // 一份一模一样的，再从 `AppDeps` 塞回 `createApp`——那个形状把整个文件钉死在 SQLite 上
  // （`PostgresqlApplicationInput` 没有、也不该有这些只服务测试的字段）。
  const opened = await scope.open()
  return {
    db,
    app: opened.app,
    appHome: opened.appHome,
    context: opened.collaborationContext,
    taskExecutionReadModels: opened.taskExecutionReadModels,
  }
}

// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-340 review access matrix',
  {
    token: 'daemon-token',
    opencodeVersion: 'test',
    dbVersion: 217,
    tempPrefix: 'aw-rfc340-review-access-',
  },
  (scope) => {
    ...
    test('owner full-replaces assignments; removal revokes and re-add restores node history access', async () => {
      const { db, context } = await fixture(scope)
      ...
      expect(
        await resolveReviewAccess(context, { actor: reviewer, nodeRunId: 'run' }),
      ).toMatchObject({
        commentAuthorRole: 'reviewer',
    ...
    test('HTTP reviewer surface lists assigned gates, permits own edits, and refuses delete/decision/selection', async () => {
      const { db, app, appHome, context } = await fixture(scope)
      // 文档正文经 `Paths.root`（= 作用域现建的 app home）读，写进那里而不是另建一个目录。
    ...
    test('HTTP configuration is owner-only, full-replace, and never creates task membership', async () => {
      const { db, app } = await fixture(scope)
