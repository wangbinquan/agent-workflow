...
async function seed(
  ...
  const id = ulid()
  await db.insert(subject.table).values({
    id,
...
async function readRow(
  ...
  const rows = await db.select().from(subject.table).where(eq(subject.table.id, id))
  const row = rows[0]
...
describeEachProviderHttpApplication(
  'RFC-317 C1 —— 配置资源写门只认 owner',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
    tempPrefix: 'aw-rfc317-config-resource-write-gate-',
  },
  (scope) => {
    ...
    for (const subject of CASES) {
      test(`${subject.type}：public 行——非 owner 的三个写入口全部 403 且零写入`, async () => {
        const harness = await buildHarness(scope)
        ...
        for (const attempt of writeAttempts(subject.base, id)) {
          ...
          expect(after, `${subject.type} ${attempt.label} 被拒后不得留下任何持久写入`).toEqual(
            before,
          )
        }
      })
      ...
      test(`${subject.type}：write 档 grant 能 revise/publish，archive 仍 403（治理面不放行）`, async () => {
        ...
        expect(
          await readRow(harness.db, subject, id),
          'archive 被拒后不得留下任何持久写入',
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-330 D-② —— digital_employee playbook 保存是内容写',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
