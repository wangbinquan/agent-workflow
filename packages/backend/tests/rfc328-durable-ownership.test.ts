// RFC-328 core correctness matrix. These fixtures use the real migration and the
// provider-neutral persistence adapters; no clock sleeps or process-global task
// map is involved.
//
// RFC-359 AC-6 —— 双引擎。整份用例此前钉死在 `createInMemoryDb(MIGRATIONS)` 上，锁的
// 因此只是 bun:sqlite 那一半；RFC-359 把持久化统一到中立实现之后，同一段断言两个引擎
// 都该成立，于是改走 `describeEachProvider`。
//
// **注意 `seedTask` 为什么显式写 `executionLineageId` / `lineageSlotPathJson`**：
// SQLite 上这两列由触发器 `rfc328_tasks_lineage_after_insert` 回填，PostgreSQL **按设计
// 没有对应触发器**——派生搬进了应用层（`application/buildNodeRunMintRecord.ts`，那里的
// 注释记着裁决：触发器只属于一种方言，用 plpgsql 再写一份等于把「一条规则写两遍」从一个
// 引擎变成两个）。所以直接插 `tasks` 的夹具必须自己把两列写全，两个引擎才同一个起点。
...
describeEachProvider('RFC-328 归属与持久执行（双引擎）', (harness) => {
  describe('RFC-328 ownership domain and durable owner adapter', () => {
    test('continuation admission compares migrated lineage JSON semantically', async () => {
      ...
      await seedTask(database, 'task-migrated-lineage')
    ...
    test('one pending intent has one winner; a revoked epoch writes zero domain rows', async () => {
      const database = harness.db
      await seedTask(database, 'task-owner')
      ...
      await expect(
        module.claim({ db: database, intentId: submitted.intentId, now: 12 }),
      ).rejects.toEqual(expect.objectContaining({ code: 'task-execution-owner-conflict' }))
      ...
      expect(
        (await database.select({ value: tasks.errorSummary }).from(tasks).get())?.value,
      ).toBeNull()
  ...
  describe('RFC-328 exact-token runtime registry', () => {
    test('sticky stop closes both stop-first and attach-first claim windows', async () => {
      const database = harness.db
      await seedTask(database, 'task-stop-first')
      await seedTask(database, 'task-attach-first')
    ...
    test('module disposal seals admission, drains permits, and stops exact handles', async () => {
      const database = harness.db
      await seedTask(database, 'task-module-dispose')
    ...
    test('provider-session pause is reversible while terminal disposal stays sealed', async () => {
      const database = harness.db
      await seedTask(database, 'task-module-pause')
  ...
  describe('RFC-328 logical effect, fence, watermark and unknown closure', () => {
    test('local effect generations follow their own retained family, not the task continuation count', async () => {
      const database = harness.db
      await seedTask(database, 'task-local-generation')
    ...
    test('a stop between probe authorization and resend releases the owner without losing the retry', async () => {
      const database = harness.db
      await seedTask(database, 'task-probe-stop-window')
    ...
    test('record-before-act holds every resource and settles projection + watermark atomically', async () => {
      const database = harness.db
      await seedTask(database, 'task-effect')
    ...
    test('resource collision has one acting winner and task-wide closure retains actor replay', async () => {
      const database = harness.db
      await seedTask(database, 'task-unknown')
    ...
    test('same-task agent and script effects stay parallel unless they share a real resource', async () => {
      const database = harness.db
      await seedTask(database, 'task-parallel-effects')
  ...
  describe('RFC-328 successor-daemon effect recovery', () => {
    test('releases only the interrupted claim and preserves its pending gate successor', async () => {
      ...
      await seedTask(database, taskId)
    ...
    test('the orphan barrier preserves auto-resume for known process launches and non-launches', async () => {
      ...
      for (const input of [
        ...
        await seedTask(database, input.taskId)
      ...
      await seedTask(database, 'task-remote-unknown')
      ...
      await database.update(tasks).set({ status: 'interrupted' }).run()
      await database.update(nodeRuns).set({ status: 'interrupted', finishedAt: 202 }).run()
      ...
      expect(
        (
          await database
            ...
            .from(taskExecutionEffects)
            .where(
              eq(taskExecutionEffects.id, processEffects.get('task-process-activated')!.effectId),
            )
            .get()
        )?.state,
      ).toBe('succeeded')
      expect(
        (
          await database
            ...
            .where(
              eq(
                taskExecutionEffects.id,
                processEffects.get('task-process-not-activated')!.effectId,
              ),
            )
            .get()
        )?.state,
    ...
    test('boot probes adopt known code-host success and preserve a same-generation retry', async () => {
      ...
      for (const spec of [
        ...
        await seedTask(database, spec.taskId)
        await database
          .update(tasks)
          .set({ status: 'running' })
          .where(eq(tasks.id, spec.taskId))
          .run()
  ...
  describe('RFC-328 retained aggregation and terminal maintenance', () => {
    test('later success preserves capability while later failure cannot erase ambiguity', () => {
      ...
      expect(canonicalResourceKeySet(['process:b', 'process:a'])).toEqual([
        'process:a',
        'process:b',
      ])
    })
    ...
    test('maintenance claim precedes IO and blocks new continuation admission', async () => {
      const database = harness.db
      await seedTask(database, 'task-maintenance', 'done')
      ...
      expect(await database.select().from(taskExecutionOwners).all()).toHaveLength(0)
      expect(await database.select().from(taskExecutionIntents).all()).toHaveLength(0)
    })

    test('boot recovery resumes the exact delete claim after the IO/DB barrier', async () => {
      const database = harness.db
      await seedTask(database, 'task-delete-recovery', 'done')
      const terminalMaintenance = new DrizzleTerminalMaintenancePersistence(database)
      ...
      expect(await database.select().from(tasks).all()).toHaveLength(0)
      expect(
        (
          await database
    ...
    test('archive exports all six execution ledgers plus the exact claim manifest', async () => {
      const database = harness.db
      await seedTask(database, 'task-archive-ledger')
