...
import * as reviewMutationCoordinator from '../src/services/reviewMutationCoordinator'
...
describeEachProvider('RFC-359 W25 review decision providers', (harness) => {
  interface Harness {
  ...
  async function holdReviewWrites(h: Harness): Promise<() => Promise<void>> {
    if (harness.capabilities.isolation === 'exclusive') {
      const release = await acquireWriterLease(h.db)
    ...
    const transaction = harness.session.transaction(async (tx) => {
      // Decisions write these rows; comment insertion also references the
      // document. Ordinary scope SELECTs can finish while these writes wait.
      await harness.session.engine.lockAggregateRoot(tx, tasks, tasks.id, h.taskId)
      await harness.session.engine.lockAggregateRoot(tx, docVersions, docVersions.id, h.dvId)
      acquired.release()
  ...
  async function waitForMutationQueue(
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        const inspect = (): void => {
          if (reviewMutationCoordinator.__hasTaskReviewMutationQueueForTesting(taskId)) {
            void callbackEntered.then(resolve, reject)
          } else immediate = setImmediate(inspect)
        }
        // Match the existing multi-document fixture: observe queue entry,
        // with a bounded fixture failure rather than a scheduling sleep.
        timer = setTimeout(() => reject(new Error('review winner did not enter its queue')), 2_000)
        void winner.then(
          () => reject(new Error('review winner settled before the barrier was released')),
          reject,
        )
        inspect()
      })
  ...
  async function runConcurrentReviewMutations<Winner, Contender>(
    h: Harness,
    winner: () => Promise<Winner>,
    contender: () => Promise<Contender>,
    release: () => Promise<void>,
    callbackEntered: Promise<void> = Promise.resolve(),
  ) {
    const started: Promise<unknown>[] = []
    try {
      expect(reviewMutationCoordinator.__hasTaskReviewMutationQueueForTesting(h.taskId)).toBe(false)
      ...
      await waitForMutationQueue(h.taskId, first, callbackEntered)
      const second = contender()
      started.push(second)
      const results = Promise.allSettled([first, second])
      expect(reviewMutationCoordinator.__hasTaskReviewMutationQueueForTesting(h.taskId)).toBe(true)
  ...
  async function concurrentReviewMutations<Winner, Contender>(
    ...
    return await runConcurrentReviewMutations(h, winner, contender, await holdReviewWrites(h))
  }

  async function concurrentReviewMutationsAtCallback<Winner, Contender>(
    h: Harness,
    winner: () => Promise<Winner>,
    contender: () => Promise<Contender>,
  ) {
    // The stale iteration fails before any write. Delay only its original
    // callback inside the real queue, then execute the unchanged callback.
    const entered = gate()
    const release = gate()
    const original = reviewMutationCoordinator.withReviewNodeMutationLock
    let firstCallback = true
    function delayedFirstCallback<T>(
      source: Parameters<typeof original>[0],
      nodeRunId: string,
      callback: () => Promise<T>,
    ): Promise<T> {
      const pause = firstCallback && source === h.db && nodeRunId === h.reviewRunId
      if (!pause) return original(source, nodeRunId, callback)
      firstCallback = false
      return original(source, nodeRunId, async () => {
        entered.release()
        await release.promise
        return await callback()
      })
    }
    const spy = spyOn(reviewMutationCoordinator, 'withReviewNodeMutationLock').mockImplementation(
      delayedFirstCallback,
    )
    try {
      return await runConcurrentReviewMutations(
        h,
        winner,
        contender,
  ...
  describe('review decision concurrency — one complete winner, zero loser side effects', () => {
    ...
    test('concurrent approve then reject commits only the approval fact set', async () => {
      ...
      const reviewRun = (
        await h.db.select().from(nodeRuns).where(eq(nodeRuns.id, h.reviewRunId))
      )[0]!
    ...
    test('a failing lock holder releases the queue so the next valid decision completes', async () => {
      ...
      const results = await concurrentReviewMutationsAtCallback(
        h,
        () =>
          submitReviewDecision({
            db: h.db,
    ...
    test('reject cascade and sibling approve serialize at task scope', async () => {
      ...
      expectRejectedWithCode(results[1]!, 'review-not-awaiting')
      const siblingRun = (
        await h.db.select().from(nodeRuns).where(eq(nodeRuns.id, siblingRunId))
      )[0]!
