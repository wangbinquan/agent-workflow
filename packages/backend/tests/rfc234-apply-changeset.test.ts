 ...
 *  在 PostgreSQL 上交出的是没人 await 的 Promise，草稿行在 apply 读它时还没落库。 */
...
// RFC-359（AC-6）—— 改成双引擎。这是 intent apply 最大的一份端到端用例（含 P2-2 崩溃矩阵、
// 幂等重放、stale fence 整包零落库、call-ref 可见性围栏）。此前钉死在 `createInMemoryDb` 上
// 不是因为判据与引擎有关，而是因为生产签名当时只收 `DbClient`；§5ea 合一之后限制没了。
// §5ea 照出的「名字域 dangle 容忍」缺陷正是这份用例里的 call-ref 那一组先红的。
describeEachProvider('RFC-234 intent apply 端到端（双引擎）', (harness) => {
  beforeEach(async () => {
  ...
  describe('applyIntentChangeset', () => {
    test('happy bundle: six resources land atomically with wiring + provenance + epoch close', async () => {
      ...
      const journal = await db.select().from(intentApplyJournal).get()
    ...
    test('RFC-302 create persists confirmed draft geometry exactly and replay never re-layouts', async () => {
      ...
      const { session } = await createIntentSession(db, actor, {
        message: 'keep reviewed geometry',
      })
      ...
      const readGeometry = async () => {
        const row = await db.select().from(workflows).where(eq(workflows.id, workflowId)).get()
      ...
      expect(await readGeometry()).toEqual({ position, size })
    ...
    test('stale agent fence: bundle lands NOTHING and staged skill is compensated', async () => {
      ...
      const agentRow = await db.select().from(agents).where(eq(agents.id, existing.id)).get()
      expect(agentRow?.description).toBe('existing')
      expect((await db.select().from(intentApplyJournal).get())?.state).toBe('failed')
    })
    ...
    test('a compensation failure leaves the journal retryable until convergence succeeds', async () => {
      ...
      const pending = (await db.select().from(intentApplyJournal).get())!
    ...
    test('post-commit crash: resources visible, convergence replays roll-forward', async () => {
      ...
      expect((await db.select().from(intentApplyJournal).get())?.state).toBe('committed')
    ...
    test('foreign-owner update: modify forbidden, copy lands a new resource', async () => {
      ...
      const draft = await installDraft(session.id, changeset, [
        {
          handle: 'res#agent#1',
          resourceType: 'agent',
      ...
      const untouched = await db.select().from(agents).where(eq(agents.id, foreignId)).get()
      ...
      expect(
        (await db.select().from(agents).where(eq(agents.id, foreignId)).get())?.description,
      ).toBe('someone else owns this')
    })
  ...
  describe('intent create path enforces call-ref visibility (RFC-243 §5.3)', () => {
    ...
    async function applyCall(node: Record<string, unknown>): Promise<unknown> {
      ...
      const draft = await installDraft(session.id, callBundle(node), [])
  ...
  describe('call-ref fence is order-independent (RFC-243 §5.3)', () => {
    ...
    async function applyPair(callerFirst: boolean): Promise<unknown> {
      ...
      const draft = await installDraft(session.id, pairBundle(callerFirst), [])
      return applyIntentChangeset(deps(), {
        sessionId: session.id,
        clientMutationId: ulid(),
        ...draft,
  ...
  describe('call-ref fence: workgroup half, grants and admin bypass', () => {
    ...
    async function applyWith(a: Actor, node: Record<string, unknown>): Promise<unknown> {
      ...
      const draft = await installDraft(session.id, callerWith(node), [])
      ...
      test(`a bundle-created workgroup is referenceable with caller ${callerFirst ? 'BEFORE' : 'AFTER'} it`, async () => {
        ...
        const receipt = (await applyIntentChangeset(deps(), {
          ...
          decisions: [
            {
              opId: squad.opId,
              slots: [{ slotId: `human:${squad.opId}:someone`, value: APPROVER }],
            },
          ],
        })) as { applied: unknown[] }
        expect(receipt.applied.length).toBe(2)
