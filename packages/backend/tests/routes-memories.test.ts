...
describeEachProviderHttpApplication(
  'routes-memories — permission gates',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
    tempPrefix: 'aw-memories-',
  },
  (scope) => {
    let h: Harness
    beforeEach(async () => {
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'routes-memories — happy paths via daemon token (admin)',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
...
// RFC-359 AC-6：两个引擎各跑一遍。
describeEachProviderHttpApplication(
  'RFC-285 B7 — candidate 读面收紧（Q4/E12）',
  {
    token: DAEMON_TOKEN,
    opencodeVersion: '1.14.25',
    dbVersion: 1,
  ...
  (scope) => {
    ...
    test('普通用户：list 滤掉 candidate（两读法）、detail 404 同形；admin 全见', async () => {
      ...
      expect(norm(await invisible.text(), created.id)).toBe(
        norm(await missing.text(), 'mem_no_such'),
      )
