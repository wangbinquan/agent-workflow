...
//    把它抹成重算结果（'{}'），立刻翻红。
//
// 顺序保证：up → wrapper 的显式排序边（s04 同款手法——buildScopeUpstreams
// 保留 wrapper 为 target 的普通边；运行时 wrapper 不解析输入，端口名无意义）
// 确保 run1 在 wrapper fresh-mint 前 up 已 done，consumed 一定绑到 up 的行。
// wrapperExternalUpstreamSources 把「target=wrapper 自身」的边也纳入源集，
// 正是为覆盖这种排序边（survey 明示）。
//
  ...
  test('loop wrapper：第二次调度铸全新 wrapper 代，consumed 绑到上游新行', async () => {
    ...
    const def: WorkflowDefinition = {
      ...
      edges: [
        ...
        // 排序边：保证 run1 中 up 在 loop fresh-mint 之前 done。
        {
          id: 'e2',
          source: { nodeId: 'up', portName: 'doc' },
          target: { nodeId: 'loop', portName: 'dep' },
        },
      ],
    }
