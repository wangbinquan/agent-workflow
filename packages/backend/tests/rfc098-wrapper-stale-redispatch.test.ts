...
describe('RFC-098 B3 (S-7) — 上游 rerun 后 loop/git wrapper 判 stale 重派', () => {
  ...
  test('loop wrapper：第二次调度铸全新 wrapper 代，consumed 绑到上游新行', async () => {
    ...
    const upAgentId = await seedReadonlyAgent(h.db, 'up-agent', ['doc'])
    const auditorId = await seedReadonlyAgent(h.db, 'auditor', ['doc', 'findings'])
    const def: WorkflowDefinition = {
      $schema_version: 1,
      inputs: [],
      nodes: [
        { id: 'up', kind: 'agent-single', agentId: upAgentId, agentName: 'up-agent' },
        { id: 'audit', kind: 'agent-single', agentId: auditorId, agentName: 'auditor' },
        {
          id: 'loop',
          kind: 'wrapper-loop',
          nodeIds: ['audit'],
  ...
  test('git wrapper：第二次调度铸全新 wrapper 代并重抓 baseline（新 HEAD）', async () => {
    ...
    const upAgentId = await seedReadonlyAgent(h.db, 'up-agent', ['doc'])
    const fixerId = await seedReadonlyAgent(h.db, 'fixer', ['doc', 'summary'])
    const def: WorkflowDefinition = {
      $schema_version: 1,
      inputs: [],
      nodes: [
        { id: 'up', kind: 'agent-single', agentId: upAgentId, agentName: 'up-agent' },
        { id: 'fix', kind: 'agent-single', agentId: fixerId, agentName: 'fixer' },
...
describe('RFC-098 B3 修订#6 — resume 不覆盖 wrapper 行的 consumed', () => {
  ...
  test('loop wrapper：interrupted 复活续跑后 consumed 原封不动（且不铸新行）', async () => {
    ...
    const auditorId = await seedReadonlyAgent(h.db, 'auditor', ['findings'])
    const def: WorkflowDefinition = {
      $schema_version: 1,
      inputs: [],
      nodes: [
  ...
  test('git wrapper：interrupted 复活续跑后 consumed 原封不动（baseline 沿用 progress）', async () => {
    ...
    const fixerId = await seedReadonlyAgent(h.db, 'fixer', ['summary'])
    const def: WorkflowDefinition = {
      $schema_version: 1,
      inputs: [],
      nodes: [
