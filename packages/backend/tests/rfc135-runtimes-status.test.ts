...
/**
 * RFC-359 AC-6：库与应用都取自双引擎作用域。`tmp` 只剩「放桩二进制」一个用途——它必须
 * 留在作用域之外，因为 `scope.open()` 会在每次打开时重建（并在用例结束时删掉）app home，
 * 而两个协议默认路径要在**装配之前**就写进 config（`open({ config })` 就是这个口子）。
 */
async function makeHarness(scope: ProviderHttpApplicationScope): Promise<Harness> {
  const tmp = mkdtempSync(join(tmpdir(), 'aw-rfc135-bin-'))
  ...
  const opened = await scope.open({
    config: { opencodePath: opencodeBin, claudeCodePath: claudeBin },
  })
  return {
    app: opened.app,
    db,
...
describeEachProviderHttpApplication(
  'RFC-135 GET /api/runtimes/status',
  {
    token: TOKEN,
    opencodeVersion: null,
    dbVersion: 1,
    tempPrefix: 'aw-rfc135-',
  ...
  (scope) => {
    ...
    test.skipIf(process.platform === 'win32')(
      ...
      async () => {
        ...
        // RFC-284 T26: 注入走 deps（env 通道已删）。RFC-359 AC-6：改成**按用例**重开同一个
        // 作用域的应用——库还是本用例那一个（内建运行时已 seed 过），只有注入值不同。
        const slow = {
          ...h,
          app: (
            await scope.open({
              config: { opencodePath: h.opencodeBin, claudeCodePath: h.claudeBin },
              runtimeDiagnosticTestDependencies: {
                ...FIXTURE_RUNTIME_DIAGNOSTICS,
                probeTimeoutMsForTest: 2000,
              },
            })
          ).app,
        }
        try {
          ...
          // `slow.tmp` 与 `h.tmp` 是同一个目录（只有注入值不同），交给 afterEach 统一删。
        }
      },
    )
    ...
    test.skipIf(process.platform === 'win32')(
      ...
      async () => {
        ...
        const res = await req(h.app)
        expect(res.status).toBe(200)
        expect((await bodyOf(res)).runtimes.find((r) => r.name === 'forks-and-dies')!.ok).toBe(
          false,
        )
