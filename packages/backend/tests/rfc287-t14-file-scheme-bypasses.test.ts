...
describeEachProvider(
  'RFC-287 G5 —— 存量 file:// 镜像不可运行（design §10.7 定音口径）',
  (harness) => {
    ...
    async function seedFileMirror(db: ProviderNeutralDatabase): Promise<string> {
      const id = ulid()
      const now = Date.now()
...
describeEachProvider('RFC-287 G5 绕过③ — 后台保鲜对 url_redacted=NULL 的存量行', (harness) => {
