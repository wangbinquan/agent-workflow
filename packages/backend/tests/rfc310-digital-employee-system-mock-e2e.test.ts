...
//
// RFC-359 AC-6 —— 这条 E2E 走的整条链路（webhook ingress / event-center / employee OS /
// platform work items / development adapters）在 RFC-359 之后是「一份实现两个 provider
// 共用」，所以它在两个引擎上各跑一遍，库由 `describeEachProvider` 给。
//
// 双跑要过的坎不在库上，在**库以外**：harness 只 TRUNCATE 数据库，出进程的 system mock
// 套件与磁盘夹具都不复位。第一遍跑完，code-host mock 里 `rfc310/digital-employee-os-review`
// 已被 seed（再 seed 直接抛 `already seeded`）、审批 mock 的请求序号已经走到 APP-00001、
// 请求流水也攒了一堆，而 `root` 下的 baseline 仓库已经有了提交（`git commit` 第二遍
// 会以「nothing to commit」失败）。因此每次运行**开局先 `suite.client.reset()`**（控制面
...
/** 每次运行自己的磁盘夹具根：baseline 仓库带提交，复用会让第二遍的 `git commit` 空提交失败。 */
const roots: string[] = []

/** 开局复位「库以外」的世界：出进程的 system mock 不随 harness 的 TRUNCATE 回到零态。 */
async function freshExternalWorld(): Promise<string> {
  await suite.client.reset()
  const created = mkdtempSync(join(tmpdir(), 'rfc310-os-system-mock-'))
  roots.push(created)
  return created
}
...
describeEachProvider('RFC-310 Digital Employee OS System Mock E2E（双引擎）', (harness) => {
  describe('RFC-310 Digital Employee OS system mock E2E', () => {
    test('signed ISSUE -> Event Center -> MR -> red gates -> repair -> ready -> merged', async () => {
      const db = harness.db
      const root = await freshExternalWorld()
            ...
            const groups = [
              ...
              ...(() => {
                ...
                return refs.length === 0
                  ? []
                  : [{ type: 'other-pipeline-failure', checkRefs: refs }]
              })(),
            ]
            outputJson = directOutput({
              outcome: 'completed',
      ...
      for (let attempt = 0; attempt < 100; attempt += 1) {
        ...
        if ((JSON.parse(await runtime.queries.listCases()) as unknown[]).length > 0 && accepted)
          break
        await Bun.sleep(1)
      }
      ...
      const driveUntilIdle = async (budget = 300): Promise<void> => {
        ...
        const pendingOutbox = (await db.select().from(employeeOsOutbox).all())
      ...
      expect(
        projection.contexts.find((context) => context.typeId === 'development.merge-request')
          ?.state,
      ).toMatchObject({
        headSha: repairedParentMrHead,
      ...
      expect(
        deliveryOnlyProjection.attention.map((attention) => attention.eventTypeRef.id),
