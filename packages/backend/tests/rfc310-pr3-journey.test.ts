...
//
// RFC-359 AC-6 —— 这条 journey 锁的是「一份实现两个 provider 共用」之后的接线，
// 所以整组判据在两个引擎上各跑一遍：库由双引擎 HTTP 作用域给（每个用例一份干净库
// + 一个现装的真应用 + 专属 app home），`automation` 与应用共用同一个 app home
// ——用例要 unlink 应用刚落盘的 evidence blob，两者的 evidence 根必须是同一个。
...
// baseline git 仓只被读（facts collector 只跑 rev-parse / 目录扫描），与 provider
// 和用例都无关，因此整文件建一次；应用的 app home 由作用域按用例现建。
let repoHome = ''
let repoPath = ''
...
beforeAll(async () => {
  ...
  repoHome = mkdtempSync(resolve(tmpdir(), 'rfc310-journey-repo-'))
  repoPath = resolve(repoHome, 'repo-src')
...
async function seedFixture(scope: ProviderHttpApplicationScope): Promise<void> {
  db = scope.harness.db
  // 每个用例都把 REQ-7 复位到 r1：source-refresh 用例会把 mock 推进到 r2，而 mock
  // 是整文件一份（`--randomize` 下用例顺序不定，跨引擎也共用同一个 mock 进程）。
...
describeEachProviderHttpApplication(
  'RFC-310 PR-3 收口 journey —— HTTP 面 + composition 接线（双引擎）',
  {
    tempPrefix: 'rfc310-journey-home-',
    token: DAEMON_TOKEN,
  ...
  (scope) => {
    ...
    describe('rfc310 pr3 journey — direct body-only', () => {
      ...
      test('HTTP launch → stash → manifest → file streaming faces → honest facts-collector block', async () => {
        ...
        const launched = (await res.json()) as {
          missionId: string
          status: string
          created: boolean
        }
        ...
        // 已 409，能走到这里即配对成立）。
        expect((await fx.store.getMission(missionId))!.sourceContentDigest).toMatch(
          /^[0-9a-f]{64}$/,
        )
      ...
      test('manifest 404 while nothing has materialized (admission-blocked mission)', async () => {
        ...
        const manifestRes = await reqAs(
          `/api/code/missions/${launched.missionId}/requirement-manifest`,
        )
    ...
    describe('rfc310 pr3 journey — external reference + source refresh', () => {
      ...
      test('external three-file bundle → facts block → refresh preview/apply; non-external 422', async () => {
        const res = await reqAs('/api/code/missions', {
          method: 'POST',
          body: launchBody('journey-external-1', {
        ...
        const samePreview = (await same.json()) as {
          changed: boolean
        ...
        const changedPreview = (await changed.json()) as {
          changed: boolean
        ...
        expect((await applied.json()) as object).toMatchObject({
          changed: true,
          sourceRevision: 'r2',
        })
        ...
        const missing = await reqAs(
          '/api/code/missions/01ARZ3NDEKTSV4RRFFQ69G5FAV/source-refresh',
          {
            method: 'POST',
          },
        )
