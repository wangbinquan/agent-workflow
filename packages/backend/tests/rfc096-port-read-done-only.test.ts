...
// primitive itself, against the real provider persistence, with the very row
...
describeEachProvider(
  'RFC-096 §3.3 — readPortRowAtFrame reads the newest DONE row only',
  (harness) => {
    test('a younger pending row in the same frame never shadows the done value', async () => {
      const db = harness.db
      const taskId = await seedTask(db)
      const { doneId } = await seedDonePlusYoungerPending(
        db,
        taskId,
        'src',
        'signal',
    ...
    test('the read is frame-scoped: a done row of another frame is invisible', async () => {
      ...
      expect(
        (await readPortRowAtFrame(persistence, taskId, 'src', 'signal', TOP_FRAME)).runId,
      ).toBe(null)
      expect(
        (
