...
describeEachProvider('RFC-358 意图图校验与反馈（双引擎）', (harness) => {
  beforeEach(async () => {
  ...
  describe('RFC-358 — apply-time gate and copy backfill', () => {
    ...
    // RFC-359（AC-6）—— 写从 `.run()` 改成 `await`。`.run()` 是 bun:sqlite 的同步执行面；
    // 在 PostgreSQL 上它交出一个没人 await 的 Promise，草稿行在 apply 读它时还没落库。
    async function installDraft(sessionId: string, changeset: IntentChangeset) {
    ...
    test('AC-6: a workflow that fails graph validation is refused at commit with zero rows written', async () => {
      ...
      const before = (await db.select().from(workflowsTable)).length
      await expect(
      ...
      expect((await db.select().from(workflowsTable)).length).toBe(before)
    })
