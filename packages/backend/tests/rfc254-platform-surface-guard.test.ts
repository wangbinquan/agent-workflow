...
const ALLOWANCES: readonly Allowance[] = [
  ...
  },
  {
    ...
    match: "'/usr/bin:/bin'",
    count: 1,
    why: 'the controlled server PATH; win32 form (including the git directory the design gate found missing) is RFC-254 T12',
    closedBy: 'RFC-254-T12',
  },
  {
    rule: 'posix-path-list',
    ...
    match: "'/usr/bin:/bin'",
    count: 1,
    why: 'model-list probe PATH; migrates with the win32 controlled-PATH table',
    closedBy: 'RFC-254-T12',
  },
  {
    rule: 'posix-path-list',
    file: 'services/runtime/mcpTestExecutionMaterial.ts',
    match: "'/usr/bin:/bin'",
    count: 1,
    why: 'MCP test-plan PATH; migrates with the win32 controlled-PATH table',
    closedBy: 'RFC-254-T12',
  },
  {
    rule: 'posix-path-list',
