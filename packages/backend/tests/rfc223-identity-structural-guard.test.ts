...
describe('RFC-223 T15 structural identity guard', () => {
  test('production source matches the exact reviewed fingerprint multiset', () => {
    ...
    // RFC-264 landed and closed the 141st occurrence exactly as e5892959
    ...
    // RFC-271 T26 显式改判：134 → 136。导入预检**必须**按名字找本地候选——包里
    // 没有 id（决策 4/12：不带任何权属与本地标识），名字是导入方唯一能用的匹配面。
    //   · `eq(table.name, name)` —— 候选查询；
