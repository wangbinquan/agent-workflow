...
async function seedSkill(db: ProviderNeutralDatabase): Promise<{
...
describeEachProvider('RFC-271 skill version transaction phases', (harness) => {
  describe('① 四段各自可用，组合起来等于一次完整提交', () => {
    test('stage → commitInTx（外部事务）→ publish：live 内容与版本行都到位', async () => {
      const { db, appHome, skillId, initial } = await seedSkill(harness.db)
    ...
    test("`source:'import'` 是新增的来源枚举 —— 包导入不冒充编辑器编辑", async () => {
      const { db, appHome, skillId } = await seedSkill(harness.db)
      const staged = await stageSkillVersion(
        db,
        { appHome },
        skillId,
