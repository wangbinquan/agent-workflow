      ...
      edges: [
        {
          id: 'e-typo',
          source: { nodeId: 'up', portName: 'files' },
          target: { nodeId: 'fan', portName: 'docz' },
        },
      ] as WorkflowDefinition['edges'],
    })
    const issues = validateWorkflowDef(def, {
      ...
      edges: [
        {
          ...
          target: { nodeId: 'fan', portName: '__done__' },
          boundary: 'wrapper-output',
        },
      ] as WorkflowDefinition['edges'],
    })
    const codes2 = validateWorkflowDef(def2, {
      agents: [agent('worker'), agent('lister')],
      skills: EMPTY_SKILLS,
    }).issues.filter((i) => i.code === 'edge-target-port-missing')
