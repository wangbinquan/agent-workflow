...
//
// RFC-359 AC-6 —— 整条链路（workspace / event-center / employee OS / platform work
// items / repository transport）在 RFC-359 之后是「一份实现两个 provider 共用」，
// 所以这条 E2E 在两个引擎上各跑一遍。库由 `describeEachProvider` 给；code-host mock
// 套件仍是整文件一份，但**每次运行现开一个自己的 project**——stateful store 的
...
/** 每次运行一个全新的 mock project：stateful code-host 的 seed 对同名 project 抛错。 */
...
describeEachProvider('RFC-310 Digital Employee 冲突收口 System Mock E2E（双引擎）', (harness) => {
  describe('RFC-310 Digital Employee conflict System Mock E2E', () => {
    test('real target advance -> conflict Attention -> bounded repair -> CAS merge commit', async () => {
      ...
      const projectPath = `rfc310/digital-employee-conflict-${++projectOrdinal}`
      ...
      try {
            ...
            async inspect({ implementation }) {
              const ref =
                implementation.kind === 'agent'
                  ? implementation.agentRef
                  : implementation.workflowRef
              return {
        ...
        const driveUntilIdle = async (budget = 300): Promise<void> => {
          ...
          const pendingOutbox = (await db.select().from(employeeOsOutbox)).filter(
